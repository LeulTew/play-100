...
beforeAll(async () => {
  if (!['127.0.0.1', 'localhost'].includes(host ?? '') || !/^(127[.]0[.]0[.]1|localhost):[0-9]+$/.test(authAddress))
    throw new Error('Shelf SDK tests only run against explicitly local emulators.');
  setLogLevel('silent');
...
async function client(anonymous = false) {
  ...
  const store = new FriendShelfStore(db);
  const friends = new FriendStore(db);
  if (anonymous) return { uid: '', db, store, friends, auth };
  const user = (
    await createUserWithEmailAndPassword(
      auth,
      `shelf-${crypto.randomUUID()}@example.test`,
  ...
  await friends.initialize(user.uid);
  await friends.saveIdentity(user.uid, { displayName: 'Shelf test nickname', avatar }, 0);
...
async function select(a: Client, entries: FriendShelfEntry[]) {
  ...
  return a.store.saveConfig(
    a.uid,
    { enabled: true, selectedIds: entries.map((item) => item.id), consentSyncEpoch: 1 },
    config,
  );
}
async function publish(a: Client, entries = [entry]) {
  const config = await select(a, entries);
...
async function stage(a: Client, config: FriendShelfConfig, count: number, sourceInput = source) {
  ...
  await runTransaction(a.db, async (tx) => {
    ...
    tx.set(doc(a.db, 'friendShelves', a.uid, 'generations', id), {
      ...
      uploaded: 0,
      ids: [],
      status: count ? 'staging' : 'ready',
      createdAt: serverTimestamp(),
    });
  });
  return id;
}
async function rawChunk(
  a: Client,
  id: string,
  entries: unknown[],
  ids: string[],
  ...
  const gen = doc(a.db, 'friendShelves', a.uid, 'generations', id);
...
function worstEntries(sourceName: FriendShelfEntry['source']): FriendShelfEntry[] {
  return Array.from({ length: 200 }, (_, i) => {
    ...
    return {
      id: sourceName === 'collection' ? sourceId : `${sourceName}:${sourceId}`,
      sourceId,
      source: sourceName,
      title: 'T'.repeat(200),
      year: 2100,
...
describe('selected shelf SDK authorization and strict full-size chunks', () => {
  it.each(['collection', 'wikidata', 'steam', 'freetogame', 'manual'] as const)(
    'publishes and reads all 200 worst-case %s entries through 100 immutable chunks',
    async (sourceName) => {
      ...
      if (sourceName === 'collection')
        await seed('catalog/author', {
          records: Object.fromEntries(entries.map((item) => [item.id, { title: item.title, year: item.year }])),
        });
      ...
      await assertFails(
        setDoc(doc(a.db, 'friendShelves', a.uid, 'generations', id, 'chunks', '0'), {
          index: 0,
          entries: entries.slice(0, 2),
          ids: entries.slice(0, 2).map((item) => item.id),
        }),
      );
      if (sourceName === 'manual' || sourceName === 'freetogame') {
        await a.store.saveConfig(
          a.uid,
          { enabled: false, selectedIds: [], consentSyncEpoch: null },
          (await a.store.config(a.uid))!,
        );
  ...
  it('uses the actual public 100 canonical facts without modifying public/ranking consent', async () => {
    ...
    const entries: FriendShelfEntry[] = games.map((game) => ({
      id: game.slug,
      title: game.title,
      year: game.year,
      source: 'collection',
  ...
  it('starts old/new shelf accounts off and denies strangers, anonymous and all private data', async () => {
    ...
    const anonymous = await client(true);
    expect(await a.store.config(a.uid)).toBeNull();
    const config = await a.store.initialize(a.uid);
    expect(config.enabled).toBe(false);
    expect(config.selectedIds).toEqual([]);
    await assertFails(
      setDoc(doc(b.db, 'friendShelfSettings', b.uid), {
        format: 1,
        enabled: true,
        deleted: false,
        consentSyncEpoch: 1,
    ...
    for (const path of [
      ['members', a.uid],
      ['syncHeads', a.uid],
      ['accounts', a.uid, 'chunks', 'private'],
      ['creatorRanks', a.uid],
      ['friendShelfSettings', a.uid],
      ['friendShelfRegistry', a.uid],
    ]) {
  ...
  it('rejects unknown fields, rank forgeries, invalid sources, canonical mismatches, and arbitrary image URLs on the server', async () => {
    ...
    const bad: Record<string, unknown>[] = [
      ...
      { completed: false },
      { queue: [] },
      { imageUrl: 'https://example.test/art.svg' },
    ...
    const canonical: FriendShelfEntry = {
      ...entry,
      id: 'known-canonical',
      source: 'collection',
      sourceId: 'known-canonical',
    };
    await seed('catalog/author', { records: { 'known-canonical': { title: 'Real canonical title', year: 2020 } } });
    const next = await select(a, [canonical]);
    const canonicalId = await stage(a, next, 1);
    await assertFails(rawChunk(a, canonicalId, [canonical], [canonical.id]));
    await assertSucceeds(
      rawChunk(a, canonicalId, [{ ...canonical, title: 'Real canonical title', year: 2020 }], [canonical.id]),
    );
  });
  it.each([
    ...
    const config = await a.store.saveConfig(
      a.uid,
      { enabled: true, selectedIds: [patch.id], consentSyncEpoch: 1 },
      await a.store.initialize(a.uid),
    );
    const id = await stage(a, config, 1);
  ...
  it('denies malformed selection/control/generation mutations and incomplete publication', async () => {
    ...
    for (const patch of [
      { selection: `${entry.id}|${entry.id}` },
      { selection: Array.from({ length: 201 }, (_, i) => `manual:x${i}`).join('|') },
    ...
    batch.set(doc(a.db, 'friendShelfHeads', a.uid), {
      format: 1,
...
describe('shelf revocation, source CAS and bounded recovery', () => {
  it('revokes disabled/old-generation content but preserves independently authorized identity', async () => {
    ...
    await assertFails(
      getDocFromServer(doc(b.db, 'friendShelves', a.uid, 'generations', first.head.current!.generation, 'chunks', '0')),
    );
    ...
    await expect(
      a.store.publish(a.uid, [entry], config, source, second.head.revision, () => true),
  ...
  it('revokes old-client online deletion and re-enable until fresh epoch-bound consent; ordinary pause keeps the last shelf', async () => {
    ...
    await assertFails(
      getDocFromServer(
        doc(b.db, 'friendShelves', a.uid, 'generations', shared.head.current!.generation, 'chunks', '0'),
      ),
    );
    expect((await b.friends.identity(a.uid))?.displayName).toBe('Shelf test nickname');
    ...
    await expect(
      a.store.publish(a.uid, [entry], previousConsent, newSource, shared.head.revision, () => true),
    ).rejects.toMatchObject({ name: 'FriendShelfConsentError' });
    const reviewed = await a.store.saveConfig(
      a.uid,
      { enabled: true, selectedIds: [entry.id], consentSyncEpoch: resumed.epoch },
      previousConsent,
    );
    await a.store.publish(a.uid, [entry], reviewed, newSource, (await a.store.head(a.uid))!.revision, () => true);
    expect((await b.store.shelf(a.uid)).entries).toEqual([entry]);
  });
  it.each(['remove', 'block', 'legacy-delete', 'lifecycle-cancel', 'shelf-delete'] as const)(
    'denies current head/chunk reads after %s',
    async (action) => {
      const a = await client();
      const b = await client();
      const pair = await connect(a, b);
      const shared = await publish(a);
      const path = doc(b.db, 'friendShelves', a.uid, 'generations', shared.head.current!.generation, 'chunks', '0');
      ...
      await assertFails(b.store.head(a.uid));
      await assertFails(getDocFromServer(path));
      if (action === 'block') {
  ...
  it('denies writes from stale private source/selection revisions and cancels an in-flight generation synchronously', async () => {
    const a = await client();
    const entries = worstEntries('manual').slice(0, 4);
    const config = await select(a, entries);
    ...
    let checks = 0;
    await expect(
      a.store.publish(a.uid, entries, config, { syncEpoch: 1, remoteRevision: 1 }, 0, () => ++checks < 8),
  ...
  it('reserves irreversible deletion even after legacy deletion, then cleans <=3 generations resumably', async () => {
    ...
    await assertSucceeds(a.store.revokeForDeletion(a.uid));
    ...
    const tombstone = (await a.store.config(a.uid))!;
    expect(tombstone.deleted).toBe(true);
    await assertFails(
      setDoc(doc(a.db, 'friendShelfSettings', a.uid), {
        format: 1,
        enabled: true,
        deleted: false,
        consentSyncEpoch: 1,
  ...
  it('recovers an acknowledged publication readback failure without replaying any write', async () => {
    ...
    await expect(a.store.publish(a.uid, [entry], config, source, 0, () => true)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'publish-shelf', uid: a.uid, revision: 1 },
      phase: 'refresh',
    });
  ...
  it('retains acknowledged head generations during cleanup-only retries, even after consent changes', async () => {
    ...
    await expect(a.store.publish(a.uid, [entry], config, source, 0, () => true)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'publish-shelf' },
      phase: 'cleanup',
    });
    cleanup.mockRestore();
    const head = (await a.store.head(a.uid))!;
    ...
    expect((await getDocFromServer(doc(a.db, 'friendShelfRegistry', a.uid))).data()?.ids).toContain(
      head.current!.generation,
    );
    expect(await a.store.cleanupSharing(a.uid)).toBe(1);
  });
  it('receives a pair-target revocation on block and denies fresh shelf reads independently of head listener timing', async () => {
    ...
    const ready = new Promise<void>((resolve, reject) => {
      releaseHead = b.store.watchHead(
        a.uid,
        (head) => {
          if (head?.current) resolve();
        },
        reject,
      );
    });
    const removed = new Promise<void>((resolve, reject) => {
      releasePair = b.friends.watchPair(
        b.uid,
        a.uid,
        (pair) => {
    ...
    try {
      await ready;
      await pairReady;
      await b.friends.block(b.uid, a.uid);
      await removed;
      await assertFails(b.store.shelf(a.uid));
  ...
  it('pulses the head atomically on Stop so an open shelf subscription is actually revoked', async () => {
    ...
    const denied = new Promise<Error>((resolve) => {
      release = b.store.watchHead(
        a.uid,
        (head) => {
          if (head?.current) allowed();
        },
        resolve,
      );
    });
