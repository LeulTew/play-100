...
describe('unused registration cancellation without weakening content verification', () => {
  it('lets only the matching signed-in unverified registration create a content-free cancellation marker', async () => {
    const user = environment
      .authenticatedContext('typo-user', { email: 'typo@example.test', email_verified: false })
      .firestore();
  ...
  it('denies unused-registration cancellation if any online activity was already reserved', async () => {
    await environment.withSecurityRulesDisabled(async (context) => {
      await context.firestore().doc('accountLifecycle/existing-user').set({ state: 'active' });
    });
    const user = environment
      .authenticatedContext('existing-user', { email: 'existing@example.test', email_verified: false })
      .firestore();
  ...
  it('serializes cancellation versus a verified writer and never allows both reservations', async () => {
    const unverified = environment
      .authenticatedContext('racing-user', { email: 'racing@example.test', email_verified: false })
      .firestore();
    const verified = environment
      .authenticatedContext('racing-user', { email: 'racing@example.test', email_verified: true })
      .firestore();
