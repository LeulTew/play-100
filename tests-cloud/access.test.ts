...
const member = (uid: string) => ({
  uid, displayName: uid, avatar: { version: 1, seed: '1'.repeat(32), palette: 'lime' },
  createdAt: new Date(), updatedAt: new Date(), consentVersion: 1, rankCount: 0, gameCount: 0,
});
beforeAll(async () => {
...
describe('managed verified identity and protected creator role', () => {
  it('denies anonymous and unverified access to private profiles and state', async () => {
    for (const db of [environment.unauthenticatedContext().firestore(), environment.authenticatedContext('alice', { email: 'alice@example.test', email_verified: false }).firestore()]) {
  ...
  it('authorizes the creator from protected configuration, not a forged client admin flag', async () => {
    const creator = environment.authenticatedContext('creator-uid', { email: 'owner@example.test', email_verified: true }).firestore();
  ...
  it('rejects malformed profile fields and avatar descriptors without allowing email/role storage', async () => {
    ...
    await assertFails(alice.doc('members/alice').update({ avatar: { version: 1, url: 'https://evil.invalid/avatar.svg' } }));
