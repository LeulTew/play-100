...
beforeAll(async () => {
  if (
    !['127.0.0.1', 'localhost'].includes(firestoreHost ?? '') ||
    !/^(127[.]0[.]0[.]1|localhost):[0-9]+$/.test(authAddress)
  )
    throw new Error('Friendship tests require explicitly local emulator endpoints.');
  setLogLevel('silent');
...
async function seed(path: string, value: Record<string, unknown>) {
  await environment.withSecurityRulesDisabled(async (context) => {
    await context.firestore().doc(path).set(value);
  });
}
...
async function share(owner: Client, entries = [entry]) {
  const control = await owner.store.saveSettings(
    owner.uid,
    { enabled: true, selectedIds: entries.map((item) => item.id) },
...
describe('canonical friendship requests and private relationship metadata', () => {
  ...
  it.each(['revoked', 'consumed', 'replaced', 'recipient-retired'] as const)(
    'does not accept a token %s after preview and leaves no partial pair or quota write',
    async (change) => {
      ...
      expect(
        (await getDocFromServer(doc(recipient.db, 'accountQuotas', recipient.uid, 'limits', 'pairs'))).exists(),
      ).toBe(false);
  ...
  it('starts sharing off and rejects stranger identities, private data, forged edges and half-edges', async () => {
    ...
    await assertFails(
      setDoc(doc(stranger.db, 'friendPairs', friendPairId(a.uid, b.uid)), pairData(a.uid, b.uid, stranger.uid)),
    );
  });
  it('makes duplicate and opposite-direction requests explicit and accepts once under contention', async () => {
    ...
    const sender = pending.from === a.uid ? a : b;
    const recipient = pending.from === a.uid ? b : a;
  ...
  it('can request a published legacy target before they initialize Friends, without writing their private settings', async () => {
    ...
    const recipient = await client(false, false);
  ...
  it('requires participant-constrained, capped queries and supports a 20-row actual page', async () => {
    ...
    await assertFails(
      getDocsFromServer(
        query(collection(a.db, 'friendPairs'), where('participants', 'array-contains', a.uid), limit(21)),
      ),
    );
    ...
    await environment.withSecurityRulesDisabled(async (context) => {
      ...
      for (let i = 0; i < 22; i += 1) {
        const other = `synthetic-${i}`;
        const data = pairData(a.uid, other, a.uid);
        batch.set(context.firestore().doc(`friendPairs/${friendPairId(a.uid, other)}`), {
          ...data,
          createdAt: now,
          updatedAt: now,
        });
  ...
  it('reports acknowledged request, acceptance and removal separately from post-commit readback failure', async () => {
    ...
    await expect(a.store.sendRequest(a.uid, b.uid)).rejects.toMatchObject({
      code: 'committed-refresh-failed',
      committed: true,
      receipt: { operation: 'send-request', epoch: 1 },
    });
    ...
    await expect(b.store.respond(b.uid, a.uid, 'accept', 1)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'respond', epoch: 2 },
    });
    ...
    await expect(a.store.respond(a.uid, b.uid, 'remove', 2)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'respond', epoch: 3 },
    });
  ...
  it('preserves a successful sharing-stop acknowledgement when its settings readback fails', async () => {
    ...
    await failNextReadback(disconnected);
    await expect(
      a.store.saveSettings(a.uid, { enabled: true, selectedIds: [entry.id] }, control),
    ).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'save-settings', uid: a.uid },
    });
    ...
    expect(await new FriendStore(a.db).settings(a.uid)).toMatchObject({
      enabled: false,
      selectedIds: [],
  ...
  it('watches only the selected pair and returns an explicit unsubscribe', async () => {
    ...
    const observed = new Promise<FriendPair>((resolve, reject) => {
      unsubscribe = b.store.watchPair(
        b.uid,
        a.uid,
        (value) => {
          if (value?.state === 'pending') resolve(value);
        },
        reject,
      );
    });
...
describe('single-use, fixed-slot invitation capabilities', () => {
  ...
  it.each(['unavailable', 'deadline-exceeded', 'unauthenticated'] as const)(
    'does not retry an invitation %s error through a transaction',
    async (code) => {
  ...
  it('offers minimal anonymous exact-token preview, forbids listing, self-use and standalone consumption', async () => {
    ...
    expect(Object.keys(preview).sort()).toEqual(
      ['avatar', 'createdAt', 'displayName', 'expiresAt', 'lifetimeDays', 'ownerUid', 'singleUse'].sort(),
    );
    ...
    batch.update(doc(b.db, 'friendInvites', invite.token), { state: 'consumed', acceptedBy: b.uid });
  ...
  it('allows exactly one concurrent recipient, rejects replay, and never reveals who consumed the link', async () => {
    ...
    const results = await Promise.allSettled([
      b.store.acceptInvite(b.uid, invite.token),
      c.store.acceptInvite(c.uid, invite.token),
    ]);
  ...
  it('allocates distinct vacant slots under concurrent invitation creators', async () => {
    ...
    for (const invite of invites)
      expect(await a.store.previewInvite(invite.token)).toMatchObject({ ownerUid: a.uid, singleUse: true });
  });
  it('retains one actual invitation when its committed timestamp readback fails', async () => {
    ...
    await expect(a.store.createInvite(a.uid)).rejects.toMatchObject({
      code: 'committed-refresh-failed',
      committed: true,
      receipt: { operation: 'create-invite', uid: a.uid },
    });
  ...
  it('enforces server expiry, revoke, blocked acceptance and lifecycle revocation', async () => {
    ...
    await environment.withSecurityRulesDisabled(async (context) => {
      await context
        .firestore()
        .doc(`friendInvites/${expired.token}`)
        .update({ createdAt: Timestamp.fromMillis(Date.now() - 8 * 86400000) });
    });
  ...
  it('caps allocation at 20 slots and deletes a prior token before reusing its slot', async () => {
    ...
    await environment.withSecurityRulesDisabled(async (context) => {
      await context
        .firestore()
        .doc(`friendInvites/${expiredToken}`)
        .update({ createdAt: Timestamp.fromMillis(Date.now() - 8 * 86400000) });
    });
...
describe('bounded strict friends-only ranking generations', () => {
  it('publishes 200 games as 100 immutable two-entry chunks within rule budgets and preserves zero/null', async () => {
    ...
    await assertFails(
      getDocsFromServer(
        query(
          collection(b.db, 'friendShares', a.uid, 'generations', result.head.current!.generation, 'chunks'),
          limit(101),
        ),
      ),
    );
    ...
    await expect(
      a.store.publishRanking(a.uid, entries, await settings(a), source, result.head.revision),
    ).resolves.toMatchObject({ changed: false });
    const rewrite = writeBatch(a.db);
    rewrite.update(doc(a.db, 'friendShares', a.uid, 'generations', result.head.current!.generation, 'chunks', '99'), {
      entries: entries.slice(198).map((row) => ({ ...row, score: 7 })),
    });
    ...
    expect(
      (
        await getDocsFromServer(
          query(
            collection(a.db, 'friendShares', a.uid, 'generations', result.head.current!.generation, 'chunks'),
            limit(100),
          ),
        )
      ).size,
    ...
    const tail = await getDocFromServer(
      doc(a.db, 'friendShares', a.uid, 'generations', oddShare.head.current!.generation, 'chunks', '99'),
    );
  ...
  it('publishes 100 canonical plus 100 Wikidata games through catalog-checked chunks and a bounded friend query', async () => {
    ...
    await seed('catalog/author', {
      records: Object.fromEntries(games.map((game) => [game.slug, { title: game.title, year: game.year }])),
    });
    const a = await client();
    const b = await client();
    await connect(a, b);
    const entries: PublicEntry[] = [
      ...games.map((game, index): PublicEntry => ({
        position: index + 1,
        id: game.slug,
        title: game.title,
        year: game.year,
      ...
      ...games.map((_, index): PublicEntry => ({
        ...entry,
        position: games.length + index + 1,
    ...
    const chunks = await getDocsFromServer(
      query(
        collection(b.db, 'friendShares', a.uid, 'generations', manifest.generation, 'chunks'),
        orderBy('index'),
        limit(100),
      ),
    );
    ...
    chunks.docs.forEach((chunk, index) => {
      ...
      expect(chunk.data()).toEqual({
        index,
        entries: entries.slice(index * 2, index * 2 + 2),
        ids: entries.slice(index * 2, index * 2 + 2).map((row) => row.id),
      });
    });
    expect((await b.store.ranking(a.uid)).entries).toEqual(entries);
  }, 60000);
  it('supports empty/removal projections and promptly retires beyond current/previous without waiting five minutes', async () => {
    ...
    for (let score = 1; score <= 4; score += 1) {
      head = (await a.store.publishRanking(a.uid, [{ ...entry, score }], await settings(a), source, head.revision))
        .head;
    }
  ...
  it('retries cleanup alone after a published ACK and preserves both head generations even after consent stops', async () => {
    const a = await client();
    let head = (await share(a)).head;
    head = (await a.store.publishRanking(a.uid, [{ ...entry, score: 1 }], await settings(a), source, head.revision))
      .head;
    const originalCleanup = a.store.cleanupSharing.bind(a.store);
    vi.spyOn(a.store, 'cleanupSharing')
      .mockImplementationOnce(originalCleanup)
      .mockRejectedValueOnce(new Error('Cleanup interrupted after publication.'));
    await expect(
      a.store.publishRanking(a.uid, [{ ...entry, score: 2 }], await settings(a), source, head.revision),
    ).rejects.toMatchObject({
      committed: true,
      phase: 'cleanup',
      receipt: { operation: 'publish-ranking', revision: head.revision + 1 },
    });
    ...
    for (const id of [published.current.generation, published.previous.generation]) {
      expect((await getDocFromServer(doc(a.db, 'friendShares', a.uid, 'generations', id))).data()?.status).toBe(
        'published',
      );
    }
    const registry = await getDocFromServer(doc(a.db, 'friendShareRegistry', a.uid));
  ...
  it('serializes a racing head publication versus pruning an old staging generation without deleting the winning head', async () => {
    ...
    await environment.withSecurityRulesDisabled(async (context) => {
      await context
        .firestore()
        .doc(candidateRef.path)
        .update({ createdAt: Timestamp.fromMillis(Date.now() - 360000) });
    });
    ...
    const publish = runTransaction(a.db, async (tx) => {
      ...
      if (!generation.exists() || generation.data().status !== 'ready')
        throw new Error('Pruning won the generation race.');
    ...
    expect(winner.current.generation).toBe(
      published.status === 'fulfilled' ? candidate : first.head.current!.generation,
    );
    expect(
      (await getDocFromServer(doc(a.db, 'friendShares', a.uid, 'generations', winner.current.generation))).data()
        ?.status,
    ).toBe('published');
  ...
  it('strictly validates all five source branches, private fields, numeric bounds and forged progress', async () => {
    ...
    const rows: PublicEntry[] = [
      ...
      },
      {
        position: 4,
        id: 'freetogame:10',
        title: 'FreeToGame fixture',
        year: null,
        source: 'freetogame',
        sourceId: '10',
        sourceUrl: 'https://www.freetogame.com/example-game',
        score: null,
      },
      {
        position: 5,
        id: 'manual:exact-fixture',
        title: 'Manual fixture',
        year: 2020,
        source: 'manual',
        sourceId: 'exact-fixture',
        sourceUrl: null,
        score: 0,
      },
    ];
    ...
    const control = await a.store.saveSettings(
      a.uid,
      { enabled: true, selectedIds: rows.map((row) => row.id) },
      await settings(a),
    );
    const generation = crypto.randomUUID();
    const ref = doc(a.db, 'friendShares', a.uid, 'generations', generation);
    ...
    const invalidChanges: Array<{ index: number; change: Record<string, unknown> }> = [
      { index: 0, change: { title: 'Forged canonical title' } },
      { index: 0, change: { year: canonical.year + 1 } },
      { index: 0, change: { sourceUrl: 'https://example.test/' } },
      { index: 1, change: { sourceUrl: 'https://example.test/' } },
      { index: 1, change: { sourceId: 'Q0' } },
      { index: 2, change: { sourceUrl: 'https://example.test/' } },
      { index: 3, change: { sourceUrl: 'https://www.freetogame.com.evil.invalid/game' } },
      { index: 4, change: { source: 'unknown' } },
      { index: 4, change: { sourceUrl: 'https://example.test/' } },
      { index: 4, change: { sourceId: 'different-identity' } },
      { index: 4, change: { position: '5' } },
      { index: 4, change: { position: 5.5 } },
      { index: 4, change: { id: 'manual:bad/id' } },
      { index: 4, change: { title: '' } },
      { index: 4, change: { title: 'x'.repeat(201) } },
      { index: 4, change: { year: 1899 } },
      { index: 4, change: { score: -1 } },
      { index: 4, change: { score: 11 } },
      { index: 4, change: { score: '0' } },
      { index: 4, change: { notes: 'private' } },
      { index: 4, change: { email: 'private@example.test' } },
    ];
    for (let chunkIndex = 0; chunkIndex < 3; chunkIndex += 1) {
      ...
      const progressData = {
        ...
        ids: rows.slice(0, start + 2).map((row) => row.id),
        status: chunkIndex === 2 ? 'ready' : 'staging',
      };
      for (const { index, change } of invalidChanges.filter((item) => item.index >= start && item.index < start + 2)) {
        const batch = writeBatch(a.db);
        batch.set(doc(ref, 'chunks', String(chunkIndex)), {
          index: chunkIndex,
          entries: chunkRows.map((row, i) => (i + start === index ? { ...row, ...change } : row)),
          ids: chunkRows.map((row) => row.id),
        });
        batch.update(ref, progressData);
        await assertFails(batch.commit());
      }
      const progress = writeBatch(a.db);
      progress.update(ref, progressData);
      ...
      valid.set(doc(ref, 'chunks', String(chunkIndex)), {
        index: chunkIndex,
        entries: chunkRows,
        ids: chunkRows.map((row) => row.id),
      });
  ...
  it.each([0, 1])(
    'enforces new selected-ranking source URL boundaries in packed position%s through direct SDK writes',
    async (offset) => {
      const owner = await client();
      ...
      const rows: PublicEntry[] = [
        { ...entry, position: 1 },
        {
          ...
          id: 'wikidata:Q124',
          sourceId: 'Q124',
          sourceUrl: 'https://www.wikidata.org/wiki/Q124',
        },
      ];
      const accepted: PublicEntry = {
        ...entry,
        position: offset + 1,
        id: 'freetogame:10',
        source: 'freetogame',
      ...
      await assertFails(
        put(
          rows.map((row, index) =>
            index === offset ? { ...row, sourceUrl: prefix + 'a'.repeat(2049 - prefix.length) } : row,
          ),
        ),
      );
  ...
  it('keeps historical oversized selected-ranking links readable by an accepted friend without allowing republishing', async () => {
    ...
    const digest = Array.from(
      new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify([old])))),
      (value) => value.toString(16).padStart(2, '0'),
    ).join('');
    await environment.withSecurityRulesDisabled(async (context) => {
      await context
        .firestore()
        .doc(`friendShares/${owner.uid}/generations/${generation}/chunks/0`)
        .set({ index: 0, entries: [old], ids: [old.id] });
    ...
    await expect(
      owner.store.publishRanking(owner.uid, [old], await settings(owner), source, published.head.revision),
    ).rejects.toThrow(/source link.*2048.*private/i);
    expect(await owner.store.shareHead(owner.uid)).toEqual(before);
    expect((await friend.store.ranking(owner.uid)).entries).toEqual([old]);
    await assertFails(
      setDoc(doc(owner.db, 'friendShares', owner.uid, 'generations', generation, 'chunks', '0'), {
        index: 0,
        entries: [allowed],
        ids: [allowed.id],
      }),
  ...
  it('rejects duplicate/unselected IDs across chunks and source/settings stale publication', async () => {
    ...
    await expect(
      a.store.publishRanking(a.uid, [{ ...entry, score: 1 }], control, source, published.head.revision),
    ).rejects.toThrow(/copy changed/);
    await expect(
  ...
  it('validates both packed entries, rejects incomplete or mutable chunks and prevents cross-chunk duplicate IDs', async () => {
    ...
    invalid.set(doc(ref, 'chunks', '0'), {
      index: 0,
      entries: entries.map((row, i) => (i === 1 ? { ...row, notes: 'private second entry' } : row)),
      ids: entries.map((row) => row.id),
    });
    ...
    incomplete.set(doc(ref, 'chunks', '0'), {
      index: 0,
      entries: entries.slice(0, 1),
      ids: entries.slice(0, 1).map((row) => row.id),
    });
    ...
    mutate.update(doc(ref, 'chunks', '0'), {
      entries: entries.map((row, i) => (i === 0 ? { ...row, score: 7 } : row)),
    });
    await assertFails(mutate.commit());
    const prematureHead = writeBatch(a.db);
    prematureHead.update(ref, { status: 'published' });
    prematureHead.set(doc(a.db, 'friendShareHeads', a.uid), {
      format: 1,
      epoch: control.epoch,
      settingsRevision: control.revision,
      source,
  ...
  it('revokes reads on remove/unshare/full deletion but not merely on private-saving pause', async () => {
    ...
    await assertFails(
      getDocFromServer(
        doc(b.db, 'friendShares', a.uid, 'generations', published.head.current!.generation, 'chunks', '0'),
      ),
    );
    await a.store.revokeForDeletion(a.uid);
    await assertFails(b.store.identity(a.uid));
...
describe('private groups, export and resumable account deletion', () => {
  it('reconciles a repeated create intent at its retained UUID without duplicating or rewriting the server group', async () => {
    ...
    await expect(
      a.store.saveGroup(a.uid, { ...input, participantUids: [...input.participantUids].reverse() }, 0),
  ...
  it('enforces owner-only groups, 2-6 unique members, pagination and edit/delete conflicts', async () => {
    ...
    await expect(
      a.store.saveGroup(a.uid, { id: group.id, name: 'Stale', participantUids: [a.uid, b.uid] }, 0),
    ).rejects.toThrow(/changed/);
    const invalid = {
      format: 1,
      name: 'Invalid',
      participantUids: [a.uid, a.uid],
      revision: 1,
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
    };
    ...
    await assertFails(
      setDoc(doc(a.db, 'friendGroups', a.uid, 'items', crypto.randomUUID()), { ...invalid, participantUids: [a.uid] }),
    );
    await assertFails(
      setDoc(doc(a.db, 'friendGroups', a.uid, 'items', 'a'.repeat(36)), {
        ...invalid,
        participantUids: [a.uid, b.uid],
      }),
    );
    await assertFails(
      setDoc(doc(a.db, 'friendGroups', a.uid, 'items', crypto.randomUUID()), {
        ...invalid,
        participantUids: Array.from({ length: 7 }, (_, i) => `person${i}`),
      }),
    );
    await assertFails(
      setDoc(doc(a.db, 'friendGroups', a.uid, 'items', crypto.randomUUID()), {
    ...
    group = await a.store.saveGroup(
      a.uid,
      { id: group.id, name: 'Renamed', participantUids: ['different-person', b.uid] },
      group.revision,
    );
  ...
  it('exports no active capabilities or copied friend scores and clears all owned content after a first irreversible reservation', async () => {
    ...
    const invite = await a.store.createInvite(a.uid);
    const oldSettings = await settings(a);
    ...
    await assertFails(b.store.ranking(a.uid));
    await expect(
      a.store.saveSettings(a.uid, { enabled: true, selectedIds: [entry.id] }, oldSettings),
    ...
    await assertFails(
      setDoc(doc(a.db, 'friendSettings', a.uid), {
        format: 1,
        enabled: false,
        deleted: false,
        selection: '',
        epoch: oldSettings.epoch + 2,
        revision: oldSettings.revision + 2,
        updatedAt: serverTimestamp(),
      }),
    );
  });
