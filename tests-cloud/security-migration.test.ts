...
} from '../src/cloud/account-lifecycle';
...
const publication = (handle: string) => ({
  handle,
  displayName: 'Published migration name',
  avatar,
  title: 'Migration ranking',
  listed: false,
  creator: false,
  entries: [entry],
});
...
for (const policy of ['live-270f', 'candidate'] as const)
  describe(`real-client S3 migration under ${policy} rules`, () => {
    ...
    beforeAll(async () => {
      environment = await initializeTestEnvironment({
        ...
        firestore: firestore(policy === 'live-270f' ? live270fRules() : candidateRules()),
      });
    });
    beforeEach(async () => {
      ...
      try {
        ...
        if (policy === 'live-270f') {
          const restored = await initializeTestEnvironment({
            projectId: endpoints.projectId,
            firestore: firestore(candidateRules()),
          });
    ...
    it('publishes, renames and reports through the new client, selecting legacy allocation only on registry permission-denied', async () => {
      ...
      if (policy === 'live-270f')
        await expect(getDocFromServer(registry)).rejects.toMatchObject({ code: 'permission-denied' });
      ...
      expect(
        (await getDocFromServer(doc(reporter.db, 'reports', `${owner.uid}_${reporter.uid}`))).data(),
      ).toMatchObject({
        reporterUid: reporter.uid,
        targetUid: owner.uid,
        reason: 'Synthetic migration report',
        status: 'open',
      });
      const fallback = info.mock.calls.filter(
        ([message]) =>
          message ===
          'Publication quota controls are not yet available; using the legacy client-first publication path.',
      );
      if (policy === 'live-270f') {
    ...
    it('uses a real outgoing manager feed to select the published name, not the separately readable private identity', async () => {
      ...
      if (policy === 'live-270f')
        expect((await sender.friends.identity(recipient.uid))?.displayName).toBe('Private friend-only name');
      ...
      try {
        ...
        await vi.waitFor(
          () => {
            ...
            expect(feed.getSnapshot().identities[recipient.uid]).toMatchObject({
              status: 'ready',
              value: { uid: recipient.uid, displayName: 'Published migration name', avatar },
            });
          },
          { timeout: 10000 },
        );
        ...
        expect(
          visibleFriendPairs(snapshot.pairs, snapshot.identities, sender.uid, {
            view: 'sent',
            name: 'Published migration',
            order: 'name',
          }),
        ).toEqual([pending]);
      } finally {
    ...
    it('cleans all four generation stores with one legacy-order fallback per retired generation only on old rules', async () => {
      ...
      await expect(
        owner.cloud.upload(state, head, async () => {
      ...
      await expect(
        owner.social.publish(owner.uid, publication('migration_cleanup'), initialControl, async () => {
      ...
      await owner.friends.publishRanking(
        owner.uid,
        [entry],
        selected,
        { syncEpoch: head.epoch, remoteRevision: head.revision },
        0,
      );
      ...
      const selectedShelf = await shelf.saveConfig(
        owner.uid,
        {
          enabled: true,
          selectedIds: [entry.id],
          consentSyncEpoch: head.epoch,
        },
      ...
      expect(
        info.mock.calls.filter(
          ([message]) =>
            message === 'Payload release counters are not yet available; using the legacy cleanup order once.',
        ),
      ).toHaveLength(policy === 'live-270f' ? 4 : 0);
    }, 60000);
    ...
    it('requires deletion-mode LIST permission before deleting any private payload, never falling back to registry-only account removal', async () => {
      ...
      if (policy === 'live-270f') {
        ...
        expect((await getDocFromServer(doc(owner.db, 'accounts', owner.uid, 'metadata', 'registry'))).exists()).toBe(
          false,
        );
        expect(await owner.cloud.probeDeletedCopy()).toBe('unknown');
      }
    ...
    it('uses counted All sharing when supported and tries the old begin only once per attempt on old rules', async () => {
      ...
      const first = await all.publish(
        owner.uid,
        'games',
        rows,
        control,
        { syncEpoch: 1, remoteRevision: 0 },
        () => true,
      );
      ...
      await all.publish(
        owner.uid,
        'games',
        rows.map((row) => ({ ...row, title: 'Changed shared title' })),
        control,
        { syncEpoch: 1, remoteRevision: 0 },
        () => true,
      );
      expect(
        info.mock.calls.filter(
          ([message]) => message === 'Counted friend sharing is not available yet; trying the legacy begin once.',
        ),
    ...
    it('revokes invitation links with a bounded legacy fallback and no unavailable-token existence distinction', async () => {
      ...
      for (const token of [invite.token, 'f'.repeat(64)]) {
        await expect(guest.friends.previewInvite(token)).rejects.toMatchObject({
    ...
    it('creates and removes groups and blocks with the declared old-rule compatibility path', async () => {
      ...
      await owner.friends.initialize(owner.uid);
      const group = await owner.friends.saveGroup(
        owner.uid,
        { name: 'Bounded group', participantUids: [owner.uid, target.uid] },
        0,
      );
    ...
    it('removes group and block quota records at full social deletion without breaking the old-rule path', async () => {
      ...
      const target = await actor();
      await owner.friends.initialize(owner.uid);
      await owner.friends.saveGroup(
        owner.uid,
        { name: 'Removed on deletion', participantUids: [owner.uid, target.uid] },
        0,
      );
      await owner.friends.block(owner.uid, target.uid);
    ...
    it('keeps request/accept/cancel compatible with the declared policy and charges only new counted pairs', async () => {
      const a = await peer('request_peer_a');
      const b = await peer('request_peer_b');
    ...
    if (policy === 'candidate') {
      it('allows new twenty-MiB private and sixteen-MiB ranking manifests, but denies a larger ranking allocation atomically', async () => {
        ...
        const manifest = (id: string, bytes: number) => ({
          ...
          bytes,
          digest: 'a'.repeat(64),
          chunks: Array.from({ length: Math.ceil(bytes / CHUNK_BYTES) }, (_, index) =>
        ...
        begin.set(doc(owner.db, 'accounts', owner.uid, 'generations', first), {
          private: manifest(first, MAX_SNAPSHOT_BYTES),
          ranking: manifest(first, MAX_RANKING_SNAPSHOT_BYTES),
          status: 'staging',
          epoch: head.epoch,
        ...
        invalid.set(doc(owner.db, 'accounts', owner.uid, 'generations', overflow), {
          private: manifest(overflow, MAX_SNAPSHOT_BYTES),
          ranking: manifest(overflow, MAX_RANKING_SNAPSHOT_BYTES + 1),
          status: 'staging',
          epoch: head.epoch,
          createdAt: serverTimestamp(),
        });
        ...
        expect((await getDocFromServer(doc(owner.db, 'accounts', owner.uid, 'generations', overflow))).exists()).toBe(
          false,
        );
      });

      it.each([MAX_RANKING_SNAPSHOT_BYTES + 1, MAX_RANKING_CHUNKS * CHUNK_BYTES + 1])(
        'denies an oversized READY creator ranking manifest with %s bytes while permitting the bounded control',
        async (bytes) => {
          const owner = await actor();
          const head = await owner.cloud.enable(null);
          ...
          const manifest = (size: number) => ({
            ...
            bytes: size,
            digest: 'b'.repeat(64),
            chunks: Array.from({ length: Math.ceil(size / CHUNK_BYTES) }, (_, index) =>
              index.toString(16).padStart(64, '0'),
            ),
          });
      ...
      it('releases at most twenty rollback orphan IDs per kind and continues without an error message', async () => {
        ...
        await seed({
          [`accountQuotas/${owner.uid}/limits/groups`]: { ids: [crypto.randomUUID()], revision: 1 },
          [`accountQuotas/${owner.uid}/limits/blocks`]: {
            ids: Array.from({ length: 45 }, (_, index) => `Orphan-${index}`),
            revision: 1,
          },
        });
      ...
      it('keeps completion blocked when an item excluded by its missing index field still exists', async () => {
        ...
        await seed({
          ...
          [`friendGroups/${owner.uid}/items/${id}`]: {
            format: 1,
            name: 'Malformed retained fixture',
            participantUids: [owner.uid, 'KnownPeer'],
            revision: 1,
            createdAt: aged(),
          },
        });
      ...
      it('preserves every legacy pair lifecycle in place without counting, converting or releasing its quota', async () => {
        const a = await peer('legacy_pair_a');
        const b = await peer('legacy_pair_b');
        const c = await peer('legacy_pair_c');
        ...
        forbiddenConversion.update(doc(b.db, 'friendPairs', id), {
          creatorUid: b.uid,
          state: 'accepted',
          epoch: 2,
          updatedAt: serverTimestamp(),
        ...
        await expect(a.friends.sendRequest(a.uid, b.uid)).rejects.toThrow(
          "You can't send this person a request right now.",
        );
      ...
      it('denies legacy acceptance when blocked or when the peer was retired, without changing either count', async () => {
        const a = await peer('legacy_block_a');
        const b = await peer('legacy_block_b');
        const [left, right] = [a.uid, b.uid].sort();
        const id = friendPairId(a.uid, b.uid);
        await seed({
          [`friendPairs/${id}`]: {
      ...
      it('retains a declined counted slot for thirty days and requires an exact cross-account delete before release', async () => {
        const a = await peer('counted_pair_a');
        const b = await peer('counted_pair_b');
        ...
        await seed({
          [`friendPairs/${id}`]: { ...recent, updatedAt: Timestamp.fromMillis(Date.now() - 29 * 86400000) },
        });
        await expect(a.friends.sendRequest(a.uid, b.uid)).rejects.toThrow(
          "You can't send this person a request right now.",
        );
        await assertFails(
          updateDoc(doc(b.db, 'friendPairs', id), { epoch: pair.epoch + 1, updatedAt: serverTimestamp() }),
        );
        await assertFails(a.friends.releasePair(a.uid, b.uid));
        await assertFails(b.friends.releasePair(b.uid, a.uid));
        const before = await stored(`friendPairs/${id}`);
        await seed({
          [`friendPairs/${id}`]: { ...before, updatedAt: Timestamp.fromMillis(Date.now() - 31 * 86400000) },
        });
      ...
      it('denies creation at the attributed pair cap, then frees one eligible slot and retries only after a real cancellation', async () => {
        ...
        for (let start = 0; start < ids.length; start += 400) {
          await seed(
            Object.fromEntries(
              ids.slice(start, start + 400).map((other) => {
                const [left, right] = [a.uid, other].sort();
                return [
                  `friendPairs/${friendPairId(a.uid, other)}`,
                  {
                    format: 2,
                    creatorUid: a.uid,
                    a: left,
        ...
        await seed({
          [quota.path]: { count: ACCOUNT_LIMITS.pairs, revision: 1, lastPair: friendPairId(a.uid, b.uid) },
        });
        const id = friendPairId(a.uid, c.uid);
        const [left, right] = [a.uid, c.uid].sort();
        const beyond = writeBatch(a.db);
        beyond.set(doc(a.db, 'friendPairs', id), {
          format: 2,
          creatorUid: a.uid,
          a: left,
          b: right,
      ...
      it.each(['cancel', 'remove'] as const)(
        'releases a freshly %s pair without waiting for the decline cooldown',
        async (action) => {
          const a = await peer(`eligible_${action}_a`);
          const b = await peer(`eligible_${action}_b`);
          let pair = await a.friends.sendRequest(a.uid, b.uid);
      ...
      it.each([1, 2] as const)(
        'accepts a mutually intended invite over a declined format%s pair without re-attributing or increasing its count',
        async (format) => {
          const a = await peer(`mutual_invite_a${format}`);
          const b = await peer(`mutual_invite_b${format}`);
          const c = await peer(`mutual_invite_c${format}`);
          ...
          if (format === 1) {
            ...
            await seed({
              [`friendPairs/${id}`]: { ...legacy, format: 1 },
      ...
      it('charges a newly invite-created pair to the accepter and releases a counterparty-created pair during account deletion', async () => {
        const a = await peer('invite_owner_a');
        const b = await peer('invite_owner_b');
        const c = await peer('invite_owner_c');
        await b.friends.sendRequest(b.uid, c.uid);
        const invite = await a.friends.createInvite(a.uid);
      ...
      it.each(['groups', 'blocks'] as const)(
        'enforces the %s registry cap and a visible product cap including frozen legacy records',
        async (kind) => {
          const owner = await actor();
          await owner.friends.initialize(owner.uid);
          const cap = ACCOUNT_LIMITS[kind];
          const ids = Array.from({ length: cap }, (_, index) =>
            kind === 'groups' ? crypto.randomUUID() : `Blocked-${index}`,
          );
          const path = (id: string) =>
            `${kind === 'groups' ? 'friendGroups' : 'friendBlocks'}/${owner.uid}/items/${id}`;
          const value = (createdAt: Timestamp | ReturnType<typeof serverTimestamp>) =>
            kind === 'groups'
              ? {
                  format: 1,
                  name: 'Saved group',
          ...
          if (kind === 'groups') {
            await updateDoc(legacyRef, {
              name: 'Legacy edit stays unenrolled',
              revision: 2,
              updatedAt: serverTimestamp(),
            });
            expect((await getDocFromServer(quota)).data()?.ids).not.toContain(legacyId);
            await expect(
              owner.friends.saveGroup(
                owner.uid,
                { name: 'Beyond product cap', participantUids: [owner.uid, 'KnownPeer'] },
                0,
              ),
            ).rejects.toBeInstanceOf(AccountQuotaFull);
            await owner.friends.deleteGroup(owner.uid, legacyId, 2);
            const added = await owner.friends.saveGroup(
              owner.uid,
              { name: 'One free slot', participantUids: [owner.uid, 'KnownPeer'] },
              0,
            );
            expect(
              (
                await owner.friends.saveGroup(
                  owner.uid,
                  { id: added.id, name: 'Enrolled edit', participantUids: added.participantUids },
                  added.revision,
                )
              ).name,
            ).toBe('Enrolled edit');
      ...
      it('bounds reports and releases a creator-resolved slot only with the exact counted report deletion', async () => {
        ...
        await seed({
          ...Object.fromEntries(
            reports.map((id, index) => [
              `reports/${id}`,
              {
                reporterUid: owner.uid,
                targetUid: `Target${index}`,
                reason: 'Synthetic registered report',
                status: 'open',
                counted: true,
                createdAt: aged(),
              },
        ...
        await seed({
          [`reports/${legacyId}`]: {
            reporterUid: owner.uid,
            targetUid: 'LegacyTarget',
            reason: 'Legacy report',
            status: 'open',
            createdAt: aged(),
          },
        });
        await expect(owner.social.report(owner.uid, 'NewReportTarget', 'Wait for review')).rejects.toBeInstanceOf(
          AccountQuotaFull,
        );
        ...
        await seed({
          ...Object.fromEntries(
            Array.from({ length: 100 }, (_, index) => [
              `reports/Resolved${index}_${owner.uid}`,
              {
                reporterUid: owner.uid,
                targetUid: `Resolved${index}`,
      ...
      it('keeps legacy All rows readable to their owner but frozen, and counts physical format3 rows across epochs', async () => {
        ...
        expect(
          (await all.publish(owner.uid, 'games', rows, resumed, { syncEpoch: 1, remoteRevision: 0 }, () => true)).count,
        ).toBe(1);
      });
      ...
      it('cleans ten old public entries in three-position steps while another generation remains live and published', async () => {
        ...
        const first = await owner.social.publish(
          owner.uid,
          { ...publication('live_cleanup'), entries: rows },
          initialControl,
        );
        const current = await owner.social.publish(
          owner.uid,
          publication('live_cleanup'),
          await owner.social.control(owner.uid),
        );
        ...
        expect(
          await owner.social.cleanup(owner.uid, true, async () => {
            const uploaded: unknown = (await getDocFromServer(oldRef)).data()?.uploaded;
            if (typeof uploaded !== 'number')
              throw new Error('The old generation lost its countdown before metadata removal.');
      ...
      it('allows only deletion-mode owner chunk lists of at most twenty, never peers, guests or collection-group scans', async () => {
        ...
        for (const kind of ['accounts', 'creatorRanks']) {
          await expect(
            getDocsFromServer(query(collection(owner.db, kind, owner.uid, 'chunks'), limit(20))),
          ).rejects.toMatchObject({ code: 'permission-denied' });
        }
        const deleting = await owner.cloud.revoke(head, true);
        for (const kind of ['accounts', 'creatorRanks']) {
          expect(
            (await getDocsFromServer(query(collection(owner.db, kind, owner.uid, 'chunks'), limit(20)))).empty,
          ).toBe(true);
          for (const client of [other, guest]) {
            await expect(
              getDocsFromServer(query(collection(client.db, kind, owner.uid, 'chunks'), limit(20))),
            ).rejects.toMatchObject({ code: 'permission-denied' });
          }
          await expect(
            getDocsFromServer(query(collection(owner.db, kind, owner.uid, 'chunks'), limit(21))),
          ).rejects.toMatchObject({ code: 'permission-denied' });
          await expect(getDocsFromServer(collection(owner.db, kind, owner.uid, 'chunks'))).rejects.toMatchObject({
            code: 'permission-denied',
          });
        }
        await expect(getDocsFromServer(query(collectionGroup(owner.db, 'chunks'), limit(20)))).rejects.toMatchObject({
          code: 'permission-denied',
        });
        await seed({
          [`syncHeads/${owner.uid}`]: {
      ...
      it('pauses on a real network interruption after confirmed deletion and resumes orphan discovery with a fresh signed-in client', async () => {
        ...
        for (let index = 0; index < 41; index += 1) {
          ...
          entries[`accounts/${owner.uid}/chunks/${chunk.digest}`] = {
            ...chunk,
            holders: [orphanGeneration],
            holder: orphanGeneration,
            createdAt: aged(),
          };
          if (index < 3)
            entries[`creatorRanks/${owner.uid}/chunks/${chunk.digest}`] = {
              ...chunk,
              holders: [orphanGeneration],
              holder: orphanGeneration,
              createdAt: aged(),
        ...
        await expect(
          owner.cloud.cleanup(true, {
            expectedDeletionEpoch: deleting.epoch,
            onProgress: async ({ confirmed }) => {
              if (confirmed === 20) await disableNetwork(owner.db);
            },
          }),
        ...
        expect((await getDocFromServer(doc(fresh.db, 'accounts', owner.uid, 'metadata', 'registry'))).exists()).toBe(
          false,
        );
        for (const kind of ['accounts', 'creatorRanks']) {
          expect(
            (await getDocsFromServer(query(collection(fresh.db, kind, owner.uid, 'chunks'), limit(20)))).empty,
          ).toBe(true);
        }
      ...
      it('releases all 107+107 maximum transport chunks with shared holders, then purges the retained copy in account-deletion mode', async () => {
        ...
        const maximum = async (label: string) => {
          ...
          const payload = Array.from({ length: MAX_CHUNKS }, (_, index) => {
            ...
            .join('')
            .slice(0, MAX_SNAPSHOT_BYTES - overhead);
        ...
        const privateCopy = await maximum('P');
        const ranking = await maximum('R');
        const retired = crypto.randomUUID();
        const kept = crypto.randomUUID();
        const copyFor = (id: string) => ({
          private: { ...privateCopy.manifest, generation: id },
          ranking: { ...ranking.manifest, generation: id },
          epoch: base.epoch,
          createdAt: aged(),
        });
        await seed({
        ...
        for (const [kind, chunks] of [
          ['accounts', privateCopy.chunks],
          ['creatorRanks', ranking.chunks],
        ] as const) {
            ...
            await seed(
              Object.fromEntries(
                chunks
                  .slice(index, index + 8)
                  .map((chunk) => [
                    `${kind}/${owner.uid}/chunks/${chunk.digest}`,
                    { ...chunk, holders: [retired, kept], holder: kept, createdAt: aged() },
                  ]),
              ),
            );
        }
        ...
        for (const [kind, chunks] of [
          ...
          for (const chunk of [chunks[0]!, chunks[106]!]) {
            expect(
              (await getDocFromServer(doc(owner.db, kind, owner.uid, 'chunks', chunk.digest))).data()?.holders,
            ).toEqual([kept]);
          }
        }
        ...
        await environment.withSecurityRulesDisabled(async (context) => {
          for (const kind of ['accounts', 'creatorRanks']) {
            expect((await context.firestore().collection(`${kind}/${owner.uid}/chunks`).limit(1).get()).empty).toBe(
              true,
            );
          }
        });
      ...
      it.each(['ranking', 'shelf'] as const)(
        'cleans a full 100-chunk %s generation with active retained pointers within the release access budget',
        async (kind) => {
          const owner = await actor();
          const base = await owner.cloud.enable(null);
          await owner.friends.initialize(owner.uid);
          ...
          const control =
            kind === 'ranking'
              ...
              : await shelf.saveConfig(
                  owner.uid,
                  { enabled: true, selectedIds, consentSyncEpoch: base.epoch },
                  await shelf.initialize(owner.uid),
                );
          const names =
            kind === 'ranking'
          ...
          const common = {
            ...
            source: { syncEpoch: base.epoch, remoteRevision: base.revision },
          };
          const sharedRows =
            kind === 'ranking'
              ? rows
          ...
          const entries: Record<string, object> = {
            ...
            [`${names.head}/${owner.uid}`]: {
              ...
              previous: { generation: previous, count: 200, digest },
              updatedAt: aged(),
            },
          ...
          for (let start = 94; start < 100; start += 3) {
            ...
            for (let index = start; index < start + 3; index += 1)
              stalePayloadOnly.delete(doc(retiredRef, 'chunks', String(index)));
            await stalePayloadOnly.commit();
          }
          expect((await getDocFromServer(retiredRef)).data()?.uploaded).toBe(100);
          expect(
            kind === 'ranking' ? await owner.friends.cleanupSharing(owner.uid) : await shelf.cleanupSharing(owner.uid),
          ).toBe(1);
          expect((await getDocFromServer(doc(owner.db, names.registry, owner.uid))).data()?.ids).toEqual([
            current,
            previous,
          ]);
          expect(
            (
              await getDocsFromServer(
                query(collection(owner.db, names.collection, owner.uid, 'generations', retired, 'chunks'), limit(100)),
              )
            ).empty,
          ).toBe(true);
          for (const id of [current, previous]) {
            expect(
              (
                await getDocsFromServer(
                  query(collection(owner.db, names.collection, owner.uid, 'generations', id, 'chunks'), limit(100)),
                )
              ).size,
            ).toBe(100);
          }
      ...
      it('locks private manifests and holder membership, including additions to a deleting generation', async () => {
        ...
        await assertFails(
          setDoc(doc(owner.db, 'creatorRanks', owner.uid, 'chunks', ranking.digest), {
            ...ranking,
            holders: [id],
            holder: id,
            createdAt: serverTimestamp(),
      ...
      it('recovers all eight slots left deleting by a stale client whose payload deletions completed but parent deletion was denied', async () => {
        ...
        const entries: Record<string, object> = {
          [`accounts/${owner.uid}/metadata/registry`]: {
            ids: snapshots.map((value) => value.manifest.generation),
            revision: 8,
          },
        };
        for (const value of snapshots)
          entries[`accounts/${owner.uid}/generations/${value.manifest.generation}`] = {
            private: value.manifest,
            ranking: value.manifest,
            epoch: head.epoch,
            status: 'deleting',
            createdAt: aged(),
          };
        ...
        staleFinish.update(registry, {
          ids: snapshots.slice(1).map((value) => value.manifest.generation),
          revision: 9,
        });
      ...
      it('advances both private release counters during account deletion even when their ranking chunk is deduplicated', async () => {
        ...
        expect(
          (await getDocFromServer(doc(owner.db, 'creatorRanks', owner.uid, 'chunks', summary.digest))).data()?.holders,
        ).toHaveLength(2);
        ...
        for (const manifest of [first.current, second.current]) {
          ...
          for (const digest of manifest.chunks)
            expect(await stored(`accounts/${owner.uid}/chunks/${digest}`)).toBeUndefined();
        }
      });

      it.each(['ranking', 'shelf'] as const)(
        'blocks %s metadata-only slot reuse and forbids regaining released payload',
        async (kind) => {
          const owner = await actor();
          const head = await owner.cloud.enable(null);
          await owner.friends.initialize(owner.uid);
          ...
          const generation = {
            epoch: control.epoch,
            settingsRevision: control.revision,
            source: { syncEpoch: head.epoch, remoteRevision: head.revision },
            count: 1,
            digest: 'a'.repeat(64),
            uploaded: 0,
            ids: [],
          ...
          await upload.commit();
          await seed({
            [`${collection}/${owner.uid}/generations/${id}`]: {
              ...generation,
              uploaded: 1,
              ids: [row.id],
              status: 'ready',
      ...
      it('shrinks a valid twelve-generation legacy registry, then really uploads and downloads with retained rollback data', async () => {
        ...
        const entries: Record<string, object> = {
          [`syncHeads/${owner.uid}`]: {
            ...base,
            revision: 12,
            current: first.privateCopy.manifest,
            previous: previous.privateCopy.manifest,
            updatedAt: oldTime,
          },
          [`creatorRanks/${owner.uid}`]: {
            format: 1,
            epoch: base.epoch,
            revision: 12,
            current: first.ranking.manifest,
            previous: previous.ranking.manifest,
            updatedAt: oldTime,
          },
          [`accounts/${owner.uid}/metadata/registry`]: {
            ids: snapshots.map((value) => value.privateCopy.manifest.generation),
            revision: 12,
          },
        };
        for (const value of snapshots) {
          ...
          entries[`accounts/${owner.uid}/generations/${id}`] = {
            private: value.privateCopy.manifest,
            ranking: value.ranking.manifest,
            epoch: base.epoch,
            status: 'ready',
            createdAt: oldTime,
          };
          for (const [collection, chunks] of [
            ['accounts', value.privateCopy.chunks],
            ['creatorRanks', value.ranking.chunks],
          ] as const) {
            for (const chunk of chunks)
              entries[`${collection}/${owner.uid}/chunks/${chunk.digest}`] = {
                ...chunk,
                holders: [id],
                holder: id,
                createdAt: oldTime,
      ...
      it('cleans an unregistered legacy public generation and enrolls the next real publication', async () => {
        ...
        const published = await owner.social.publish(
          owner.uid,
          publication('migration_enrolled'),
          await owner.social.control(owner.uid),
        );
        expect(
          (await getDocFromServer(doc(owner.db, 'publicProfiles', owner.uid, 'metadata', 'registry'))).data()?.ids,
      ...
      it.each(['leul_tew', 'play100_official', 'support_team'])(
        'lets legacy %s unpublish and rename, never republish the reserved handle',
        async (handle) => {
          const owner = await actor();
          const generation = crypto.randomUUID();
          const legacy: PublicProfile = {
            uid: owner.uid,
            handle,
            displayName: 'Legacy published name',
            avatar,
          ...
          await seed({
            ...
            [`publicProfiles/${owner.uid}/generations/${generation}`]: {
              epoch: 0,
              count: 1,
              uploaded: 1,
              status: 'ready',
      ...
      it('does not use the legacy allocation fallback for an actual offline registry read', async () => {
        ...
        try {
          await expect(
            owner.social.publish(owner.uid, publication('migration_offline'), initialControl),
          ).rejects.toMatchObject({ code: 'unavailable' });
        } finally {
          await enableNetwork(owner.db);
        }
        expect(
          info.mock.calls.some(([message]) => typeof message === 'string' && message.includes('legacy client-first')),
        ).toBe(false);
        expect(await stored(`publicProfiles/${owner.uid}/metadata/registry`)).toBeUndefined();
      ...
      it('blocks the private metadata-only orphan loop and permits slot reuse only after honest chunk release', async () => {
        ...
        for (let index = 0; index < 2; index += 1) {
          ...
          if (!Array.isArray(ids) || ids.length !== 1 || typeof ids[0] !== 'string')
            throw new Error('Expected one bounded staging generation.');
          ...
          for (const [collection, value] of [
            ['accounts', data.private],
            ['creatorRanks', data.ranking],
          ] as const) {
      ...
      it('blocks the public metadata-only orphan loop and permits slot reuse only after honest entry countdown', async () => {
        ...
        for (let index = 0; index < 2; index += 1) {
          await expect(
            owner.social.publish(owner.uid, publication('migration_orphans'), initialControl, async () => {
          ...
          const ids: unknown = current.data()?.ids;
          if (!Array.isArray(ids) || ids.length !== 1 || typeof ids[0] !== 'string')
            throw new Error('Expected one bounded public generation.');
