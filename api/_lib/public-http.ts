...
export function requirePublicUrl(value: URL | string, hosts: readonly string[]): URL {
  ...
  try {
    ...
    throw new CatalogError('The public source URL is invalid.', 502, 'invalid');
  }
  if (
    url.protocol !== 'https:' ||
    ...
    throw new CatalogError('The public source URL is not allowed.', 502, 'invalid');
  return url;
}
...
async function cancelUpstreamBody(body: { cancel: () => Promise<void> } | null, status: number): Promise<void> {
  try {
    ...
    console.warn('Catalog upstream response cleanup failed.', { status });
  }
}
...
export async function publicBytes(
  ...
  const timeout = setTimeout(
    () =>
      controller.abort(new CatalogError('The public source took too long to reply. Try again later.', 504, 'timeout')),
  ...
  const read = async () => {
    ...
    if (!response.ok) {
      await cancelUpstreamBody(response.body, response.status);
      throw new CatalogError(
        `The source catalog is unavailable (${response.status}). Try again later or add a game manually.`,
        503,
      );
    }
    ...
    cancelRead = () => {
      void cancelUpstreamBody(reader, response.status);
    };
...
export async function upstreamJson(
  ...
  try {
    ...
    throw new CatalogError('The source returned something other than readable catalog data.');
  }
  if (
    payload &&
