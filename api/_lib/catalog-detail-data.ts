...
function claimDate(value: unknown): string | null {
  ...
  if (
    ...
    typeof time.precision !== 'number' ||
    time.precision < 9 ||
    time.calendarmodel !== 'http://www.wikidata.org/entity/Q1985727'
  )
    return null;
...
function references(statement: JsonObject) {
  ...
  const grouped = refs.flatMap((ref) => {
    ...
    if (!snaks) return [];
    const urls = Array.isArray(snaks.P854)
      ? snaks.P854.map(snakValue).flatMap((value) => publicHttpsUrl(value) ?? [])
      : [];
    const dates = [
      ...new Set(Array.isArray(snaks.P813) ? snaks.P813.map(snakValue).flatMap((value) => claimDate(value) ?? []) : []),
    ];
    return [{ url: urls[0] ?? null, date: dates.length === 1 ? dates[0]! : null }];
  });
  return (
    grouped.find((ref) => ref.url !== null) ?? grouped.find((ref) => ref.date !== null) ?? { url: null, date: null }
  );
}
...
export function reviewLabelIds(entity: JsonObject): string[] {
  return [
    ...new Set(
      statements(entity, 'P444')
        .slice(0, ENRICHMENT_LIMITS.ratings)
        .flatMap((statement) => ['P447', 'P400', 'P459'].flatMap((property) => entityIds(values(statement, property)))),
    ),
  ].slice(0, 50);
}
...
export function wikidataRatings(entity: JsonObject, entities: unknown, fetchedAt: string): CatalogExternalRating[] {
  ...
  const result = statements(entity, 'P444').flatMap((statement, index): CatalogExternalRating[] => {
    ...
    if (!score || publishers.length !== 1 || platforms.length > ENRICHMENT_LIMITS.platforms || methods.length > 1)
      return [];
    ...
    return [
      {
        id: `wikidata:${statementId ?? `${gameId}-${index}`}`,
        source: 'wikidata',
        kind: 'review-score',
        publisher: entityLabel(labels, publisherId),
        publisherId,
        score,
        platforms: platforms.map((id) => entityLabel(labels, id)),
        method: methods[0] ? entityLabel(labels, methods[0]) : null,
        count: countValues.length === 1 ? countValues[0]! : null,
        asOf: dates.length === 1 ? dates[0]! : null,
        referenceDate: reference.date,
        retrievedAt: fetchedAt,
        sourceUrl: `https://www.wikidata.org/wiki/${gameId}#P444`,
...
export function exactSteamApp(entity: JsonObject): { id: string | null; ambiguous: boolean } {
  ...
  const ids = [
    ...new Set(
      (preferred.length ? preferred : all).flatMap((statement) => {
...
export function steamRating(payload: unknown, appId: string, fetchedAt: string): CatalogExternalRating | null {
  ...
  if (
    ...
    typeof total !== 'number' ||
    ![positive, negative, total].every(
      (value) => Number.isSafeInteger(value) && value >= 0 && value <= 1_000_000_000,
    ) ||
    positive + negative !== total
  )
    throw new Error('Steam returned inconsistent review counts.');
  ...
  return {
    ...
    publisher: 'Steam',
    publisherId: null,
    score: { text: `${value}%`, value, scale: 100, unit: 'percent' },
    platforms: ['Steam'],
