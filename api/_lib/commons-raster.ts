...
function reject(message: string): never {
  throw new CatalogError(message, 502, 'unsupported');
}
...
export function commonsRasterPermission(payload: unknown, expectedFile?: string): CommonsRasterPermission {
  ...
  if (!license || !artist || (restrictions && restrictions !== 'trademarked'))
    return reject('This image needs a clear reusable license and complete attribution.');
  ...
  if (!licenseUrl || (license !== 'Public domain' && declaredUrl?.replace(/\/$/, '') !== licenseUrl.replace(/\/$/, '')))
    return reject('This image license is not approved for reuse here.');
  ...
  const original =
    ...
    RASTER_TYPES.includes(info.mime) &&
    /\.(?:png|jpe?g|webp)$/i.test(originalUrl.pathname) &&
    info.width <= 640 &&
    info.height <= 640 &&
    typeof info.size === 'number' &&
    Number.isSafeInteger(info.size) &&
    info.size > 0 &&
    info.size <= INPUT_BYTES;
  if (!original && !text(info.thumburl))
    return reject('Commons did not supply a bounded original or permitted thumbnail.');
  ...
  if (
    !sourceUrl.pathname.startsWith('/wiki/File:') ||
    !originalUrl.pathname.startsWith('/wikipedia/commons/') ||
    !downloadUrl.pathname.startsWith('/wikipedia/commons/') ||
    !/\.(?:png|jpe?g|webp)$/i.test(downloadUrl.pathname) ||
    typeof mime !== 'string' ||
    !RASTER_TYPES.includes(mime) ||
    typeof width !== 'number' ||
    ...
    return reject('Commons did not supply a bounded supported raster image.');
  const parts = [
    artist,
    ...[field('Credit'), field('Attribution')].filter((part): part is string => Boolean(part)).map(plainCredit),
  ].filter(Boolean);
  ...
  if (credit.length > ENRICHMENT_LIMITS.creditLength)
    return reject('The complete image credit exceeds the supported length; the image was not copied.');
  return {
    sourceUrl: sourceUrl.href,
    originalUrl: originalUrl.href,
    downloadUrl: downloadUrl.href,
    selection: original ? 'original' : 'thumbnail',
    width,
    height,
    mime,
    bytes,
...
export function patchedDecoder(version: string): boolean {
  ...
  return Boolean(
    match && (Number(match[1]) > 0 || Number(match[2]) > 35 || (Number(match[2]) === 35 && Number(match[3]) >= 4)),
  );
}

export function verifiedRaster(input: Buffer, contentType: string): boolean {
  if (contentType === 'image/jpeg')
    return input[0] === 0xff && input[1] === 0xd8 && input[2] === 0xff && !input.includes(Buffer.from('MPF\0'));
  const png =
    contentType === 'image/png' && input.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]));
  const webp =
  ...
  if (
    ...
    (input.length < 33 ||
      ...
      input.readUInt32BE(16) < 1 ||
      input.readUInt32BE(16) > 640 ||
      input.readUInt32BE(20) < 1 ||
      input.readUInt32BE(20) > 640)
  )
    return false;
  ...
  while (offset + (png ? 12 : 8) <= input.length) {
    ...
    if (
      kind === 'VP8X' &&
      (size !== 10 ||
        offset + 18 > input.length ||
        input.readUIntLE(offset + 12, 3) + 1 > 640 ||
        input.readUIntLE(offset + 15, 3) + 1 > 640)
    )
      return false;
    if (kind === 'acTL' || kind === 'ANIM' || kind === 'ANMF' || (kind === 'VP8X' && (input[offset + 8]! & 2) !== 0))
      return false;
...
export async function fetchCommonsRaster(
  permission: CommonsRasterPermission,
  signal: AbortSignal,
  fetchedAt: string,
): Promise<ExternalCatalogArtwork> {
  const { default: sharp } = await import('sharp');
  if (!patchedDecoder(sharp.versions.sharp))
    throw new CatalogError(
      'A patched image decoder is required before new artwork can be loaded. Existing artwork is unchanged.',
      503,
      'unsupported',
    );
  const { bytes, contentType } = await publicBytes(permission.downloadUrl, signal, {
    hosts: RASTER_HOSTS,
    maxBytes: permission.bytes ?? INPUT_BYTES,
    timeoutMs: 3500,
    contentTypes: [permission.mime],
  });
  signal.throwIfAborted();
  const input = Buffer.from(bytes);
  if (permission.bytes !== null && input.length !== permission.bytes)
    throw new CatalogError('The original image byte length did not match its verified metadata.', 502, 'invalid');
  if (!verifiedRaster(input, contentType))
    throw new CatalogError(
      'The image was not a supported single-frame raster matching its declared type.',
      502,
      'invalid',
    );
  const decoder = sharp(input, { limitInputPixels: 640 * 640, failOn: 'warning', animated: false }).timeout({
    seconds: 2,
  });
  ...
  const stop = () => {
    decoder.destroy();
    fail(signal.reason ?? new CatalogError('Image processing was cancelled.', 504, 'timeout'));
  };
  ...
  const encode = async (): Promise<ExternalCatalogArtwork> => {
    ...
    if (
      ...
      metadata.height > 640 ||
      (metadata.pages ?? 1) !== 1 ||
      metadata.width !== permission.width ||
      metadata.height !== permission.height
    ) {
    ...
    signal.throwIfAborted();
    if (data.length > ENRICHMENT_LIMITS.imageBytes)
      throw new CatalogError('The reusable image exceeded the detail image budget.', 502, 'invalid');
    ...
    return {
      ...
      width: info.width,
      height: info.height,
      alt: 'Licensed game artwork from Wikimedia Commons',
  ...
  try {
    return await Promise.race([encode(), aborted]);
  } finally {
