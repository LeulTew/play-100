...
import {
  ...
  writeArtworkPresence,
} from './generate-discovery-artwork-presence.ts';
import {
  DISCOVERY_LIMITS,
  parseDiscoveryCatalog,
  parseDiscoveryCatalogJson,
  type CatalogArtwork,
...
const FTG_ART_REASON =
  'FreeToGame API metadata is permitted; thumbnail display/copy rights are not explicit. Image excluded.';
...
export function validateFetchUrl(value: string, image: boolean): URL {
  ...
  const allowed = image
    ...
    : (url.origin === 'https://www.freetogame.com' && url.pathname === '/api/games' && !url.search) ||
      (['https://www.wikidata.org', 'https://commons.wikimedia.org'].includes(url.origin) &&
        url.pathname === '/w/api.php' &&
        ['query', 'wbgetentities'].includes(url.searchParams.get('action') ?? ''));
  if (!allowed || /%(?:2f|5c|00)/i.test(url.pathname))
    throw new Error(`Upstream endpoint not allowlisted: ${url.origin}${url.pathname}`);
  return url;
}
...
export class BoundedClient {
    ...
    try {
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (++this.requests > MAX_REQUESTS)
          throw new Error(`Request budget exceeded (${MAX_REQUESTS}); last good snapshot preserved.`);
        this.nextRequest = this.clock() + 550;
        const signal = AbortSignal.any([this.signal, AbortSignal.timeout(30_000)]);
        const response = await this.fetcher(url, {
          redirect: 'error',
          signal,
          headers: { 'User-Agent': USER_AGENT, Accept: image ? 'image/png,image/jpeg,image/webp' : 'application/json' },
        });
        if ([429, 502, 503, 504].includes(response.status) && attempt < 2) {
          ...
          if (!Number.isFinite(delay) || delay > 60_000)
            throw new UpstreamError(
              'Provider requests a longer wait. Rerun later; snapshot unchanged.',
              response.status,
            );
  ...
  async json(url: string): Promise<unknown> {
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (error)
        throw new Error(`Provider API error: ${string(error.code) ?? 'unknown'}. Rerun later; snapshot unchanged.`);
      return result;
    }
...
function claims(entity: JsonObject, property: string): unknown[] {
  ...
  return values
    .filter((value) => object(value)?.rank !== 'deprecated')
    .flatMap((value) => {
...
function metadataProvenance(
  ...
  retrievedAt: string,
  reason: string,
): DiscoveryItem['provenance'] {
  return {
    retrievedAt,
    metadataLicense: source === 'wikidata' ? 'CC0-1.0' : 'FreeToGame API terms',
    metadataLicenseUrl: source === 'wikidata' ? CC0 : FTG_TERMS,
    artworkMissingReason: reason,
  };
}
...
export function parseFreeToGame(payload: unknown, retrievedAt: string): DiscoveryItem[] {
  if (!Array.isArray(payload) || !payload.length || payload.length > 1000)
    throw new Error('Invalid FreeToGame catalog size.');
  ...
  return payload.map((value) => {
    ...
    return {
      record: {
        id: `freetogame:${sourceId}`,
        source: 'freetogame',
        sourceId,
        sourceUrl: requiredText(row.freetogame_profile_url, 'FreeToGame profile URL', 2000),
        title: requiredText(row.title, 'FreeToGame title'),
        year: year >= 1900 && year <= 2100 ? year : null,
        studio: optionalText(row.developer),
        genre: optionalText(row.genre),
        collectionRank: null,
      },
      aliases: [],
      artwork: null,
      provenance: metadataProvenance('freetogame', retrievedAt, FTG_ART_REASON),
    };
  });
}
...
export function parseWikiEntity(entity: JsonObject, names: JsonObject, retrievedAt: string): DiscoveryItem | null {
  ...
  return {
    record: {
      id: `wikidata:${id}`,
      title: requiredText(title, 'Wikidata title'),
    ...
    aliases: [...new Set(allAliases)]
      ...
      .sort()
      .slice(0, DISCOVERY_LIMITS.aliases),
    artwork: null,
    provenance: metadataProvenance(
      'wikidata',
      retrievedAt,
      'No reusable Commons image or logo is referenced by the source.',
    ),
  };
}
...
function plainCredit(value: string): string {
  return value
    .replace(/<a\b[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi, (_, href: string, title: string) => {
    ...
    .replace(
      ...
      (_, name: string) => ({ amp: '&', quot: '"', apos: "'", lt: '<', gt: '>', nbsp: ' ' })[name]!,
    )
    .replace(/\s+/g, ' ')
    .trim();
}
...
export function commonsPermission(info: JsonObject): CommonsPermission | string {
  ...
  if (restrictions && restrictions !== 'trademarked')
    return `Commons additional restrictions need manual review: ${restrictions}`.slice(0, 300);
  const cc = /^CC (BY|BY-SA) (2\.0|2\.5|3\.0|4\.0)$/.exec(license);
  const expected = cc
    ? `https://creativecommons.org/licenses/${cc[1]!.toLowerCase()}/${cc[2]}/`
    : license === 'CC0'
      ? CC0
      : license === 'Public domain' && field('Copyrighted') === 'False'
  ...
  const credit = [artist, field('Credit'), field('Attribution')]
  ...
  try {
    ...
    if (
      ...
      original.origin !== 'https://upload.wikimedia.org' ||
      !original.pathname.startsWith('/wikipedia/commons/')
    ) {
      return 'Commons supplied a non-allowlisted provenance URL.';
    }
  } catch {
    return 'Commons has no supported, allowlisted raster derivative.';
  }
...
export async function makeArtwork(
  ...
  if (
    ...
    throw new Error('Asset did not decode as a single supported raster image.');
  ...
  return {
    bytes,
    artwork: {
      src: `/images/discovery/${sha256}.webp`,
      width: result.width,
      ...
      licenseUrl: permission.licenseUrl,
      retrievedAt,
      sha256,
      bytes: bytes.length,
    },
...
async function entities(client: BoundedClient, parameters: Record<string, string>): Promise<JsonObject> {
  const response = requiredObject(
    await client.json(apiUrl(WIKI, { action: 'wbgetentities', ...parameters })),
    'Wikidata response',
  );
...
export async function collect(
...
): Promise<{ catalog: DiscoveryCatalog; assets: Map<string, Buffer>; report: JsonObject }> {
  ...
  for (let offset = 0; offset < FEATURED_WIKI_TITLES.length; offset += 40) {
    Object.assign(
      wikiEntities,
      await entities(client, {
        sites: 'enwiki',
        titles: FEATURED_WIKI_TITLES.slice(offset, offset + 40).join('|'),
  ...
  const response = requiredObject(
    await client.json(
      apiUrl(WIKI, {
        action: 'query',
        list: 'search',
        srsearch: 'haswbstatement:P31=Q7889',
        srsort: 'incoming_links_desc',
        srlimit: String(WIKI_SEARCH_COUNT),
        srprop: '',
      }),
    ),
    'Wikidata search',
  ...
  if (!Array.isArray(hits) || hits.length < 100)
    throw new Error('Wikidata selection was unexpectedly small; snapshot unchanged.');
  const ids = [...new Set(hits.map((hit) => requiredText(object(hit)?.title, 'Wikidata search identity')))];
  for (let offset = 0; offset < ids.length; offset += 20) {
    Object.assign(
      wikiEntities,
      await entities(client, {
        ids: ids.slice(offset, offset + 20).join('|'),
        props: 'labels|aliases|claims',
        languages: 'en|mul',
      }),
    );
  }
  const validEntities = Object.values(wikiEntities)
    .map((value) => requiredObject(value, 'Wikidata entity'))
    .filter((entity) => related(entity, 'P31').includes('Q7889') && label(entity));
  const relatedIds = [
    ...new Set(validEntities.flatMap((entity) => [...related(entity, 'P178'), ...related(entity, 'P136')])),
  ];
  ...
  for (let offset = 0; offset < relatedIds.length; offset += 50) {
    Object.assign(
      names,
      await entities(client, {
        ids: relatedIds.slice(offset, offset + 50).join('|'),
        props: 'labels',
        languages: 'en|mul',
      }),
    );
  ...
  const wiki = validEntities.flatMap((entity) => {
    ...
    if (
      item &&
      [item.record.title, ...item.aliases].some((title) => freeTitles.has(title.normalize('NFKC').toLowerCase().trim()))
    ) {
  ...
  console.log(
    `Wikidata: ${wiki.length} classified records after selection exclusions; checking per-file Commons licenses.`,
  );
  const assets = new Map<string, Buffer>();
  const artCache = new Map<string, { artwork: CatalogArtwork; bytes: Buffer } | string>();
  for (const { item, entity } of wiki) {
    const files = [
      ...new Set([...claims(entity, 'P154'), ...claims(entity, 'P18')].flatMap((value) => string(value) ?? [])),
    ].slice(0, 2);
    for (const file of files) {
      const cached = artCache.get(file);
      if (cached) {
        if (typeof cached === 'string') {
          item.provenance.artworkMissingReason = cached;
          continue;
        }
      ...
      if (artCache.size >= MAX_ART_FILES) {
        item.provenance.artworkMissingReason =
          'The bounded Commons file-review budget was reached; no image license assumed.';
        continue;
      }
      ...
      const permission = info
        ? commonsPermission(info)
        : 'Referenced Commons file is missing or has no image information.';
      if (typeof permission === 'string') {
        artCache.set(file, permission);
        item.provenance.artworkMissingReason = permission;
        continue;
      }
      try {
        const converted = await makeArtwork(
          await client.bytes(permission.downloadUrl, true),
          permission,
          item.record.title,
        ...
        item.provenance.artworkMissingReason = null;
        if (assets.size % 25 === 0)
          console.log(`Commons: ${assets.size} licensed raster assets decoded; ${client.requests} requests total.`);
        break;
      } catch (error) {
        ...
        artCache.set(file, reason);
        item.provenance.artworkMissingReason = reason;
      }
    }
  }
  ...
  if (items.length < 500)
    throw new Error(`Only ${items.length} records obtained; expected >=500. Last good snapshot preserved.`);
  if (!items.some(({ record }) => record.id === 'wikidata:Q15408545'))
    throw new Error('Required Kingdom Come source entity missing.');
  ...
  return {
    ...
    report: {
      ...
      omittedTitleCollisions,
      reviewedCommonsFiles: artCache.size,
      maximumReviewedCommonsFiles: MAX_ART_FILES,
      featuredLookupTitles: FEATURED_WIKI_TITLES.length,
      classifiedWikidataRecords: wiki.length,
    },
  };
}
...
export async function verifySnapshot(root = ROOT): Promise<JsonObject> {
  ...
  for (const [src, artwork] of assets) {
    ...
    if (
      checksum(bytes) !== artwork.sha256 ||
      bytes.length !== artwork.bytes ||
      image.format !== 'webp' ||
  ...
  if (allLocalBytes > DISCOVERY_LIMITS.totalImageBytes)
    throw new Error('All local images, including retained older assets, exceed 35 MiB.');
  const sourceCounts = Object.fromEntries(
    ['wikidata', 'freetogame'].map((source) => [
      source,
      catalog.items.filter(({ record }) => record.source === source).length,
    ]),
  );
  const licenses = Object.fromEntries(
    [...new Set([...assets.values()].map((asset) => asset.license))]
      .sort()
      .map((license) => [license, [...assets.values()].filter((asset) => asset.license === license).length]),
  );
  return {
    generatedAt: catalog.generatedAt,
    records: catalog.items.length,
    sourceCounts,
...
export async function writeSnapshot(
  ...
  const referenced = new Map(
    validated.items.flatMap(({ artwork }) => (artwork ? [[artwork.src, artwork] as const] : [])),
  );
  ...
  for (const [src, artwork] of referenced) {
    ...
    if (!bytes || bytes.length !== artwork.bytes || checksum(bytes) !== artwork.sha256)
      throw new Error(`Missing or mismatched asset ${src}.`);
    const image = await sharp(bytes, { failOn: 'warning' }).metadata();
    await sharp(bytes, { failOn: 'warning' }).raw().toBuffer();
    if (
      image.format !== 'webp' ||
  ...
  for (const name of existing)
    if (/^[a-f0-9]{64}\.webp$/.test(name)) diskBytes += (await stat(path.join(directory, name))).size;
  for (const [src, bytes] of assets) if (!existing.has(path.basename(src))) diskBytes += bytes.length;
  if (diskBytes > DISCOVERY_LIMITS.totalImageBytes)
    throw new Error('Retained and new assets exceed 35 MiB; review unused old assets before rerunning.');
  ...
  try {
    ...
    for (const [src, bytes] of assets) {
      ...
      if (existing.has(path.basename(src))) {
        if (checksum(await readFile(destination)) !== checksum(bytes))
          throw new Error(`Corrupt existing immutable asset: ${src}`);
      } else {
...
async function main(): Promise<void> {
  ...
  if (args[0] === '--verify') {
    console.log(JSON.stringify(await verifySnapshot(), null, 2));
    return;
  }
      ...
      JSON.stringify(
        {
          ...
          ...(args[0] === '--dry-run'
            ? {
                dryRun: true,
                records: result.catalog.items.length,
                images: result.assets.size,
