...
const FONT_FILES = [
  ...read(new URL('../src/main.tsx', import.meta.url)).matchAll(/^import '(@fontsource[\w/-]*\/[\w-]+\.css)';$/gm),
]
  .flatMap(([, specifier]) => [
    ...read(new URL(`../node_modules/${specifier}`, import.meta.url)).matchAll(
      /url\((['"]?)\.\/files\/([\w-]+)\.(woff2?)\1\)/g,
    ),
  ])
  .map(([, , name, extension]) => `assets/${name}-Hash_1-2.${extension}`);
const EMITTED = [
  ...new Set(FONT_FILES),
  'assets/index-Co7EAvP7.js',
  'assets/index-F6C_fDKP.css',
  'assets/CatalogDetail-BC-82rWH.css',
];
...
describe('landing font preloads', () => {
  ...
  it('fails the build when a landing font file is missing or ambiguous', () => {
    expect(() =>
      landingFontFiles(EMITTED.filter((name) => !name.startsWith('assets/barlow-condensed-latin-700-normal-'))),
    ).toThrow(
      'exactly one landing font file matching /^assets\\/barlow-condensed-latin-700-normal-[\\w-]+\\.woff2$/, not 0.',
    );
    expect(() => landingFontFiles([...EMITTED, 'assets/hanken-grotesk-latin-wght-normal-Other123.woff2'])).toThrow(
      ', not 2.',
    );
  });
});
