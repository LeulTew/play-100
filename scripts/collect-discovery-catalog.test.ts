...
import {
  BoundedClient, checksum, collect, commonsPermission, makeArtwork, parseFreeToGame,
  parseWikiEntity, validateFetchUrl, verifySnapshot, writeSnapshot,
...
const entity = {
  id: 'Q15408545', labels: { en: { value: 'Kingdom Come: Deliverance' } },
...
const permission: CommonsPermission = {
  ...
  credit: 'Fixture creator | converted to WebP', sourceUrl: 'https://commons.wikimedia.org/wiki/File:Fixture.png',
...
const commonsInfo = {
  url: permission.originalUrl, thumburl: permission.downloadUrl, descriptionurl: permission.sourceUrl,
  width: 320, height: 180, extmetadata: {
    Artist: { value: '<a href="/wiki/User:Artist">Artist</a>' },
    Credit: { value: 'Own work' }, Attribution: { value: 'Required attribution' },
    LicenseShortName: { value: 'CC BY-SA 4.0' }, LicenseUrl: { value: permission.licenseUrl },
    Copyrighted: { value: 'True' },
  },
};
const freeRow = (id = 1) => ({
  id, title: `Fixture free game ${id}`, developer: 'Fixture developer', genre: 'RPG',
  release_date: '2020-01-01', freetogame_profile_url: `https://www.freetogame.com/fixture-${id}`,
  short_description: 'Never copied', thumbnail: 'https://www.freetogame.com/g/1/thumbnail.jpg',
});
...
async function fixture() {
  const input = await sharp({ create: { width: 16, height: 9, channels: 4, background: '#789abc' } }).png().toBuffer();
...
describe('bounded request transport', () => {
  ...
  it.each([
    ...
    'https://evil.example/a.png', 'https://upload.wikimedia.org/arbitrary/a.png',
  ...
  it('identifies requests, forbids redirects, and rate-limits starts globally', async () => {
    ...
    const client = new BoundedClient(new AbortController().signal, fetcher, async (ms) => { clock += ms; }, () => clock);
  ...
  it('obeys Retry-After and fails rather than ignoring a long provider pause', async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(json({}, 429, { 'Retry-After': '3' })).mockResolvedValueOnce(json({ ok: true }));
    const delays: number[] = [];
    const client = new BoundedClient(new AbortController().signal, fetcher, async (ms) => { delays.push(ms); });
    expect(await client.json('https://www.freetogame.com/api/games')).toEqual({ ok: true });
    expect(delays).toContain(3000);
    await expect(clientWith(vi.fn<typeof fetch>().mockResolvedValue(json({}, 429, { 'Retry-After': '120' })))
      .bytes('https://www.freetogame.com/api/games')).rejects.toThrow(/longer wait/);
  });
  it('caps retries and surfaces source errors instead of empty success', async () => {
    ...
    await expect(clientWith(vi.fn<typeof fetch>().mockImplementation(async () => json({ error: { code: 'maxlag' } })))
      .json('https://www.freetogame.com/api/games')).rejects.toThrow(/maxlag/);
  });
  it('rejects HTML, misleading sizes, malformed JSON and oversized streaming bodies', async () => {
    ...
    ]) await expect(clientWith(vi.fn<typeof fetch>().mockResolvedValue(response)).json('https://www.freetogame.com/api/games')).rejects.toThrow();
  });
  it('honors cancellation without making requests', async () => {
    const controller = new AbortController();
    controller.abort(new Error('Cancelled'));
    const fetcher = vi.fn<typeof fetch>();
    await expect(new BoundedClient(controller.signal, fetcher).bytes('https://www.freetogame.com/api/games')).rejects.toThrow(/Cancelled/);
    expect(fetcher).not.toHaveBeenCalled();
  });
  ...
  it('recovers from maxlag only within its bounded retry allowance', async () => {
    const delays: number[] = [];
    const fetcher = vi.fn<typeof fetch>().mockResolvedValueOnce(json({ error: { code: 'maxlag' } })).mockResolvedValueOnce(json({ ok: true }));
    const client = new BoundedClient(new AbortController().signal, fetcher, async (ms) => { delays.push(ms); });
...
describe('provider facts and per-file rights', () => {
  ...
  it('requires video-game classification and preserves actual aliases/ambiguous years', () => {
    ...
    const dated = { ...entity, claims: {
      ...entity.claims,
      P577: [claim({ time: '+2018-01-01T00:00:00Z', precision: 11 }), claim({ time: '+2019-01-01T00:00:00Z', precision: 11 })],
    } };
  ...
  it.each([
    ...
    expect(typeof commonsPermission({ ...commonsInfo, extmetadata: { ...commonsInfo.extmetadata, ...patch } })).toBe('string');
  });
  it('accepts a declared public-domain file without mislabelling it CC0', () => {
    expect(commonsPermission({ ...commonsInfo, extmetadata: {
      ...commonsInfo.extmetadata, LicenseShortName: { value: 'Public domain' }, Copyrighted: { value: 'False' },
...
describe('raster and atomic snapshot output', () => {
  ...
  it('preserves a last-good manifest on failed assets, schema or byte budgets', async () => {
    ...
    Object.assign(badCatalog.items[0]!.artwork!, { src: `/images/discovery/${checksum(bad)}.webp`, sha256: checksum(bad), bytes: bad.length });
    await expect(writeSnapshot(badCatalog, new Map([[badCatalog.items[0]!.artwork!.src, bad]]), root)).rejects.toThrow();
    expect(await readFile(manifestPath)).toEqual(before);
  ...
  it('rejects HTML-as-image and bounds transformed output against original source dimensions', async () => {
    ...
    const input = await sharp({ create: { width: 100, height: 100, channels: 4, background: '#000' } }).png().toBuffer();
  ...
  it('runs the full collector with a dry offline provider fixture and no public side effects', async () => {
    ...
    const wikiEntities = Object.fromEntries(wikiIds.map((id) => [id, {
      id, labels: { en: { value: `Fixture wiki ${id}` } }, claims: entity.claims,
    }]));
    const fetcher: typeof fetch = async (url) => {
      const parsed = new URL(String(url));
      if (parsed.hostname === 'www.freetogame.com') return json(Array.from({ length: 500 }, (_, index) => freeRow(index + 1)));
      if (parsed.hostname === 'commons.wikimedia.org') return json({ query: { pages: { 1: { imageinfo: [commonsInfo] } } } });
      if (parsed.hostname === 'thumb.wikimedia.org') return new Response(new Uint8Array(input), { headers: { 'Content-Type': 'image/png' } });
      if (parsed.searchParams.get('action') === 'query') return json({ query: { search: wikiIds.map((title) => ({ title })) } });
      if (parsed.searchParams.has('sites')) return json({ entities: {
        Q15408545: { ...entity, claims: { ...entity.claims, P154: [claim('Fixture.png')] } },
      } });
      return json({ entities: Object.fromEntries(parsed.searchParams.get('ids')!.split('|').map((id) => [id, wikiEntities[id]])) });
    };
