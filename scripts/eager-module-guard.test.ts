...
function chunk(fileName: string, modules: Record<string, { renderedLength: number }> = {}, imports: string[] = [], isEntry = false) {
...
describe('build-only exact eager module guard', () => {
  it.each(DEFERRED_SOURCE_MODULES)('rejects rendered %s inside an entry or shared static chunk', module => {
    ...
    expect(() => assertDeferredBundleModules(root, bundle, [])).toThrow(`Deferred module is eager: ${module} in shared.js.`);
  });
  ...
  it('allows lazy code, erased imports, cycles and external source modules without exposing any inventory', () => {
    ...
    const bundle = {
      ...
      'shared.js': chunk('shared.js', { [path.join(root, 'node_modules', 'react', 'index.js')]: { renderedLength: 100 } }, ['entry.js']),
  ...
  it('fails closed when a static or HTML eager JS chunk has no bundle origin record', () => {
    expect(() => assertDeferredBundleModules(process.cwd(), { 'entry.js': chunk('entry.js', {}, ['missing.js'], true) }, [])).toThrow('Missing built eager chunk: missing.js.');
    expect(() => assertDeferredBundleModules(process.cwd(), {}, ['preload.js'])).toThrow('Missing built eager chunk: preload.js.');
  });
});
