...
function indexField(value: unknown): IndexField {
  ...
  const fieldPath = text(row.fieldPath);
  ...
  return {
    indexes: list(row.indexes).map((value) => {
      ...
      if (new Set(fields.map((field) => field.fieldPath)).size !== fields.length)
        throw new Error('Repeated index field.');
    ...
    fieldOverrides: list(row.fieldOverrides).map((value) => {
      ...
      const indexes: FieldOverride['indexes'] = list(override.indexes).map((value) => {
        const index = record(value);
        const mode = indexField({ ...index, fieldPath: '_' });
        return {
          queryScope: scope(index.queryScope),
          ...(mode.order ? { order: mode.order } : { arrayConfig: mode.arrayConfig }),
        };
      });
...
function queryShape(value: unknown): QueryShape {
  const row = record(value);
  return {
    id: `operator:${text(row.id)}`,
    collectionGroup: text(row.collectionGroup),
    scope: scope(row.scope),
    filters: list(row.filters).map((value) => {
      const filter = record(value);
      const operator = text(filter.operator);
      if (
        !['==', '!=', '<', '<=', '>', '>=', 'in', 'not-in', 'array-contains', 'array-contains-any'].includes(operator)
      )
        throw new Error('Unknown operator query predicate.');
    ...
    orders: list(row.orders).map((value) => {
      ...
      if (order.direction !== 'ASCENDING' && order.direction !== 'DESCENDING')
        throw new Error('Unknown operator order.');
...
const baseline = configuration(
  JSON.parse(verifiedFixture(manifest.indexBaseline.path, manifest.indexBaseline.blob).toString('utf8')),
);
const currentFiles = auditedRoots
  .flatMap((directory) => sourceFiles(path.join(root, directory)))
  .filter((file) => !auditFiles.has(file));
...
const operatorBlock =
  /<!-- firestore-operator-queries:start -->\s*```json\s*([\s\S]*?)```\s*<!-- firestore-operator-queries:end -->/g;
const blocks = [...runbook.matchAll(operatorBlock)];
if (blocks.length !== 1)
  throw new Error('The runbook must contain exactly one authoritative operator-query inventory.');
...
describe('STORAGE-02 exact query/index contract', () => {
  ...
  it('preserves every accepted composite and override and adds only thirteen exact exemptions', () => {
    ...
    expect(config.fieldOverrides.slice(baseline.fieldOverrides.length)).toEqual(
      STORAGE_02_EXEMPTIONS.map((value) => ({ ...value, indexes: [] })),
    );
    expect(new Set(config.fieldOverrides.map((value) => `${value.collectionGroup}.${value.fieldPath}`)).size).toBe(
      config.fieldOverrides.length,
    );
    expect(
      config.fieldOverrides.some((value) => value.fieldPath.includes('*') || value.collectionGroup.includes('*')),
    ).toBe(false);
  });
  ...
  it('enumerates all current and immutable rollback query constructors before checking availability', () => {
    const oldOwners = [
      ...
      ...['page', 'inventory', 'prepareEpoch', 'pruneInactive', 'cleanupPage'].map(
        (name) => `src/cloud/friend-all-store.ts#FriendAllStore.${name}`,
      ),
      'src/cloud/friend-shelf-store.ts#FriendShelfStore.shelf',
      ...['relationsQuery', 'listBlocks', 'listInvites', 'ranking', 'listGroups'].map(
        (name) => `src/cloud/friend-store.ts#FriendStore.${name}`,
      ),
      ...['entries', 'directory', 'members', 'reports', 'cleanup', 'deleteProfile'].map(
        (name) => `src/cloud/social-store.ts#SocialStore.${name}`,
      ),
    ];
  ...
  it('proves all four shared chunks namespaces retain the only field order they query', () => {
    const namespaces = (rules: string) =>
      [...rules.matchAll(/match\s+(\/[^\s]*\/chunks\/\{[^}]+\})/g)]
        .map((value) => value[1]!.replace(/\{[^}]+\}/g, '{}'))
        .sort();
    const expected = [
      '/accounts/{}/chunks/{}',
      '/creatorRanks/{}/chunks/{}',
      '/friendShares/{}/generations/{}/chunks/{}',
      '/friendShelves/{}/generations/{}/chunks/{}',
    ].sort();
    expect(namespaces(readFileSync(path.join(root, 'firestore.rules'), 'utf8'))).toEqual(expected);
    expect(
      namespaces(readFileSync(path.join(root, 'tests-cloud', 'fixtures', 'live-270f4c7', 'firestore.rules'), 'utf8')),
    ).toEqual(expected);
    ...
    ).toEqual(new Set(['index:ASCENDING']));
  });
  ...
  it('audits the complete formal runbook inventory, including creator and participant recounts', () => {
    ...
    expect(operatorQueries.find((query) => query.id === 'operator:creator-pairs')?.filters).toEqual([
      { field: 'creatorUid', operator: '==' },
    ]);
    expect(operatorQueries.find((query) => query.id === 'operator:legacy-participant-pairs')?.filters).toEqual([
      { field: 'participants', operator: 'array-contains' },
    ]);
  ...
  it.each(STORAGE_02_EXEMPTIONS)('rejects a query of proposed exemption $collectionGroup.$fieldPath', (exemption) => {
    const query: QueryShape = {
      id: 'negative control',
      collectionGroup: exemption.collectionGroup,
      scope: 'COLLECTION',
      filters: [{ field: exemption.fieldPath, operator: '==' }],
      orders: [],
    };
    expect(() => assertQueryIndexes(config, [query])).toThrow(/proposed exemption is queried/);
    expect(() =>
      assertQueryIndexes(config, [
        { ...query, filters: [{ field: `${exemption.fieldPath}.nested`, operator: 'array-contains' }] },
      ]),
  ...
  it('rejects missing required ASC/DESC/CONTAINS support and required composites', () => {
    for (const [collectionGroup, fieldPath, queries] of [
      ['chunks', 'index', currentQueries],
      ['generations', 'createdAt', oldQueries],
      ['reports', 'createdAt', currentQueries],
      ['friendPairs', 'participants', operatorQueries],
      ['friendPairs', 'creatorUid', operatorQueries],
      ['reports', 'reporterUid', operatorQueries],
      ['handles', 'uid', operatorQueries],
    ] as const) {
    ...
    const changed = structuredClone(config);
    changed.indexes = changed.indexes.filter(
      (index) => !(index.collectionGroup === 'entries' && index.fields[0]?.fieldPath === 'format'),
    );
    expect(() => assertQueryIndexes(changed, currentQueries)).toThrow(/missing .* composite/);
    const withoutPairs = structuredClone(config);
    withoutPairs.indexes = withoutPairs.indexes.filter(
      (index) => !index.fields.some((field) => field.fieldPath === 'creatorUid'),
    );
  ...
  it('handles aliases and conditional fields, and fails closed on hidden or dynamic query factories', () => {
    ...
    expect(() => extractQueries('probe.ts', `${prelude} const hidden = w; hidden('token', '==', 1);`)).toThrow(
      /aliased\/passed/,
    );
    expect(() =>
      extractQueries('probe.ts', `${prelude} function f(db, field) { return q(c(db, 'entries'), w(field, '==', 1)); }`),
    ).toThrow(/resolve query binding/);
    expect(() => extractQueries('probe.ts', `${prelude} function f() { return w('token', '==', 1); }`)).toThrow(
      /not covered/,
    );
  });
  ...
  it.each(auditedRoots)(
    'rejects direct RPC and structured query syntax in %s until an extractor is reviewed',
    (directory) => {
      for (const source of [
        ...
        expect(() => extractQueries(`${directory}/probe.ts`, source)).toThrow(
          /runQuery\/structuredQuery requires a reviewed extractor/,
        );
      }
    },
  );
  ...
  it.each(auditedRoots)('rejects REST ordering in %s without rejecting bound SDK ordering', (directory) => {
    for (const source of [
      ...
      expect(() => extractQueries(`${directory}/probe.ts`, source)).toThrow(
        /REST orderBy requires a reviewed extractor/,
      );
    }
    expect(
      extractQueries(
      ...
      function f(db) { return query(collection(db, 'chunks'), orderBy('index')); }`,
      ),
    ).toHaveLength(1);
  });
  ...
  it('does not accept source-ordered multi-inequality fields as the SDK implicit order', () => {
    const wrongOrder: IndexConfiguration = {
      fieldOverrides: [],
      indexes: [
        {
  ...
  it('fails closed when explicit document-ID-only ordering omits an inequality field', () => {
    ...
    expect(() => assertQueryIndexes({ indexes: [], fieldOverrides: [] }, queries)).toThrow(
      /implicit inequality ordering/,
    );
  });
});
