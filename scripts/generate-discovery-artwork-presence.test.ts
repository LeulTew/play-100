...
it('derives artwork presence across sources without provider assumptions or copying credits', () => {
  expect(discoveryArtworkPresenceJson({ schemaVersion: 1, items: [
...
it('rejects invalid shapes, IDs, duplicate records and ambiguous missing artwork rather than generating an incomplete hint', () => {
  for (const value of [null, {}, { schemaVersion: 2, items: [] }, { schemaVersion: 1, items: [
    row('wikidata:Q1'), row('wikidata:Q1', {}),
  ] }, ...[
    null, row('manual:private'), row('wikidata:Q0'), row('freetogame:01'), { record: { id: 'wikidata:Q1' } },
