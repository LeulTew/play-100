...
function ownerName(owner: Owner): string {
  ...
  return ts.isMethodDeclaration(owner) && ts.isClassDeclaration(owner.parent)
    ? `${owner.parent.name?.text ?? '<anonymous>'}.${name}`
    : name;
}
...
export function extractQueries(file: string, text: string): QueryShape[] {
  ...
  for (const statement of source.statements) {
    ...
    if (imports && ts.isNamedImports(imports))
      imports.elements.forEach((item) => bindings.set(item.name.text, item.propertyName?.text ?? item.name.text));
  ...
  const strings = (expression: ts.Expression, owner: Owner, depth = 0): string[] => {
    ...
    if (
      ...
      return strings(expression.expression, owner, depth + 1);
    if (ts.isConditionalExpression(expression))
      return [
        ...new Set([
          ...strings(expression.whenTrue, owner, depth + 1),
          ...strings(expression.whenFalse, owner, depth + 1),
        ]),
      ];
  ...
  const clauses = (expression: ts.Expression, owner: Owner, depth = 0): Clause[][] => {
    ...
    if (
      ...
      return clauses(expression.expression, owner, depth + 1);
    if (ts.isSpreadElement(expression)) return clauses(expression.expression, owner, depth + 1);
    if (ts.isArrayLiteralExpression(expression))
      return expression.elements.reduce<Clause[][]>(
        (result, item) => combine(result, clauses(item, owner, depth + 1)),
        [[]],
      );
    if (ts.isConditionalExpression(expression))
      return [...clauses(expression.whenTrue, owner, depth + 1), ...clauses(expression.whenFalse, owner, depth + 1)];
    if (ts.isIdentifier(expression)) {
      ...
      walk(owner, (node) => {
        ...
        if (node.expression.name.text !== 'push')
          fail(node, 'Only audited push-based query construction is supported.');
        ...
        while (ancestor && ancestor !== owner) {
          ...
          if (
            ...
            fail(ancestor, 'Loop-built query constraints require a reviewed extractor.');
    ...
    if (name === 'where') {
      ...
      return fields.flatMap((value) =>
        values.map<Clause[]>((operation) => [{ kind: 'filter', value: { field: value, operator: operation } }]),
      );
    }
    if (name === 'orderBy') {
      const directions = expression.arguments[1] ? strings(expression.arguments[1], owner) : ['asc'];
      if (directions.some((value) => value !== 'asc' && value !== 'desc'))
        return fail(expression, 'Invalid orderBy direction.');
      consumed.add(expression);
      return fields.flatMap((value) =>
        directions.map<Clause[]>((direction) => [
          { kind: 'order', value: { field: value, direction: direction === 'asc' ? 'ASCENDING' : 'DESCENDING' } },
        ]),
      );
    }
    return fail(expression, 'Unknown query constraint factory; no index assumption was made.');
  ...
  const collectionOf = (expression: ts.Expression, owner: Owner, depth = 0): { group: string; scope: QueryScope } => {
    ...
    if (name === 'collection' || name === 'collectionGroup') {
      ...
      if (groups.length !== 1 || !groups[0])
        return fail(expression, 'A dynamic collection group requires an explicit reviewed extractor.');
      if (groups[0].includes('/'))
        return fail(expression, 'A slash-joined collection path requires an explicit reviewed extractor.');
    ...
    if (
      ...
      const method =
        parent && ts.isClassDeclaration(parent)
          ? parent.members.find(
      ...
      if (!method || returned?.length !== 1 || !returned[0]?.expression)
        return fail(expression, 'Unresolved collection-reference method.');
  ...
  walk(source, (node) => {
    ...
    const rawName = ts.isIdentifier(node.expression)
      ? node.expression.text
      : ts.isPropertyAccessExpression(node.expression)
        ? node.expression.name.text
        : undefined;
    if (!name && rawName && ['where', 'orderBy', 'collectionGroup'].includes(rawName))
      fail(node, 'Unbound query primitive requires review.');
    ...
    for (const variant of variants)
      queries.push({
        id: `${file}#${ownerName(owner)}`,
        collectionGroup: location.group,
  ...
  for (const call of primitiveCalls)
    if (!consumed.has(call)) fail(call, 'A where/orderBy call is not covered by a query construction.');
...
function singleAvailable(config: IndexConfiguration, query: QueryShape, field: string, mode: IndexMode): boolean {
  ...
  const override = config.fieldOverrides
    .filter(
      (value) =>
        value.collectionGroup === query.collectionGroup &&
        (field === value.fieldPath || field.startsWith(`${value.fieldPath}.`)),
    )
    .sort((a, b) => b.fieldPath.length - a.fieldPath.length)[0];
  if (!override) return query.scope === 'COLLECTION';
  return override.indexes.some(
    (index) => index.queryScope === query.scope && (index.order ?? index.arrayConfig) === mode,
  );
}

export function assertQueryIndexes(
  config: IndexConfiguration,
  queries: readonly QueryShape[],
  forbidden: readonly Exemption[] = STORAGE_02_EXEMPTIONS,
): void {
  ...
  for (const query of queries) {
    ...
    const ranges = [
      ...new Set(
        filters
          .filter((value) => !equality.has(value.operator) && !arrays.has(value.operator))
    ...
    const effectiveOrders = orders.length
      ? orders
      : ranges.map((field) => ({ field, direction: 'ASCENDING' as const }));
    ...
    const equalFields = new Set(
      filters.filter((value) => equality.has(value.operator) && !ordered.has(value.field)).map((value) => value.field),
    );
    const allFields = new Set([...filters.map((value) => value.field), ...orders.map((value) => value.field)]);
    const needsComposite =
      (arrayFields.size > 0 && allFields.size > 1) ||
      (effectiveOrders.length > 0 && [...equalFields].some((field) => !ordered.has(field))) ||
    ...
    const matches = config.indexes.some((index) => {
      ...
      if (new Set(index.fields.slice(0, prefix.size).map((field) => field.fieldPath)).size !== prefix.size)
        return false;
      if (
        !index.fields
          .slice(0, prefix.size)
          .every((field) => prefix.get(field.fieldPath) === (field.order ?? field.arrayConfig))
      )
        return false;
    ...
    if (needsComposite) {
      if (!matches)
        throw new Error(
          `${query.id}: missing ${query.scope} composite for ${JSON.stringify({ filters, orders: effectiveOrders })}`,
        );
      continue;
    }
    if (matches) continue;
    for (const filter of filters) {
      const mode = arrays.has(filter.operator)
        ? 'CONTAINS'
        : (effectiveOrders.find((value) => value.field === filter.field)?.direction ?? 'ASCENDING');
      if (!singleAvailable(config, query, filter.field, mode))
        throw new Error(`${query.id}: missing ${mode} index for ${filter.field}`);
    }
    for (const order of effectiveOrders) {
      if (!singleAvailable(config, query, order.field, order.direction))
        throw new Error(`${query.id}: missing ${order.direction} index for ${order.field}`);
    }
  }
}
