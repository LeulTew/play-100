...
export const PWA_BUDGET = {
  coreFiles: 48, coreBytes: 2 * 1024 * 1024, coreFileBytes: 1024 * 1024,
  imageFiles: 48, imageBytes: 4 * 1024 * 1024, imageFileBytes: 192 * 1024,
...
export function isPublicPwaFile(path: string): boolean {
  return publicFiles.has(path) || /^\/assets\/[A-Za-z0-9_.-]+\.(?:js|css|woff2)$/.test(path) ||
...
export function isPwaShellNavigation(url: URL, origin: string): boolean {
  return url.origin === origin && !url.username && !url.password &&
    shellRoutes.has(url.pathname) && url.search.length <= 2048 &&
...
export function validatePwaManifest(manifest: PwaBuildManifest): void {
  ...
  for (const [kind, assets] of [['core', manifest.core], ['images', manifest.images]] as const) {
    for (const asset of assets) {
      if (!isPublicPwaFile(asset.url) || asset.url.length > 256 || urls.has(asset.url) || !/^[a-f0-9]{64}$/.test(asset.sha256) ||
  ...
  if (manifest.core.reduce((sum, asset) => sum + asset.bytes, 0) > PWA_BUDGET.coreBytes - PWA_BUDGET.metadataBytes ||
    !['/index.html', '/pwa/offline.html', '/data/collection.json', '/data/discovery/catalog.v1.json']
      .every(url => manifest.core.some(asset => asset.url === url))) {
...
function contentTypeMatches(asset: PwaAsset, contentType: string): boolean {
  ...
  const types: Record<PwaAsset['type'], readonly string[]> = {
    html: ['text/html'], script: ['text/javascript', 'application/javascript'],
    style: ['text/css'], font: ['font/woff2', 'application/font-woff2'],
    json: ['application/json'], manifest: ['application/manifest+json', 'application/json'],
...
export async function verifiedPwaResponse(
  ...
  if (response.status !== 200 || response.redirected ||
    ...
    !contentTypeMatches(asset, response.headers.get('content-type') ?? '') || !response.body) {
...
export function installPwaWorker(scope: PwaWorkerHost, manifest: PwaBuildManifest) {
  ...
  const request = (url: string) => new Request(new URL(url, origin), {
  ...
  const clientBindings = async (): Promise<Record<string, ClientBinding>> => {
    ...
    for (const [id, value] of Object.entries(input)) {
      const binding: unknown = typeof value === 'string'
        ? { version: value, observed: false, reservedAt: Date.now() } : value;
      if (!/^[A-Za-z0-9-]{1,128}$/.test(id) || !binding || typeof binding !== 'object' ||
        !('version' in binding) || typeof binding.version !== 'string' ||
        binding.version !== 'network' && !/^[a-f0-9]{64}$/.test(binding.version) ||
        !('observed' in binding) || typeof binding.observed !== 'boolean' ||
        !('reservedAt' in binding) || typeof binding.reservedAt !== 'number' ||
        !Number.isSafeInteger(binding.reservedAt) || binding.reservedAt < 0) {
        throw new Error('Offline page-version ownership could not be confirmed.');
      }
  ...
  const bindClients = (ids: readonly string[], version: string): Promise<void> => {
    return serializeClientWrite(async () => {
      if (!ids.length || ids.some(id => !/^[A-Za-z0-9-]{1,128}$/.test(id))) throw new Error('Offline page identity is unavailable.');
      const alive = new Set((await scope.clients.matchAll({ type: 'window', includeUncontrolled: true })).map(client => client.id));
      const now = Date.now();
      const current: Record<string, ClientBinding> = Object.fromEntries(Object.entries(await clientBindings())
        .filter(([id, binding]) => alive.has(id) || activeNavigations.has(id) ||
      ...
      for (const id of ids) Object.defineProperty(current, id, {
        value: { version, observed: alive.has(id), reservedAt: now }, enumerable: true, configurable: true, writable: true,
      });
      if (Object.keys(current).length > PWA_BUDGET.clients) throw new Error('Close extra Play 100 windows before preparing offline pages.');
  ...
  const renewNavigationGrace = (id: string): Promise<void> => {
    return serializeClientWrite(async () => {
      ...
      if (Date.now() - reservedAt >= reservationLifetimeMs) throw new Error('The navigation grace could not be saved in time.');
    });
  };
  ...
  scope.addEventListener('install', event => {
    event.waitUntil((async () => {
      ...
      try {
        ...
        await scope.caches.delete(coreName);
        console.error('Offline preparation failed; the previous version remains available.', cause instanceof Error ? cause.message : 'Cache failure.');
  ...
  scope.addEventListener('activate', event => {
    event.waitUntil((async () => {
      ...
      const previousVersion = own && typeof own === 'object' && 'previousVersion' in own &&
        typeof own.previousVersion === 'string' && /^[a-f0-9]{64}$/.test(own.previousVersion) ? own.previousVersion : null;
  ...
  scope.addEventListener('fetch', event => {
    ...
    if (input.method !== 'GET' || url.origin !== origin || url.username || url.password ||
      input.headers.has('authorization') || input.headers.has('range') ||
      /^\/(?:api|__|data-use)(?:\/|$)/.test(url.pathname)) return;
    if (input.mode === 'navigate') {
      const id = event.resultingClientId ?? event.clientId ?? '';
      if (!/^[A-Za-z0-9-]{1,128}$/.test(id) || activeNavigations.has(id) || activeNavigations.size >= PWA_BUDGET.clients) {
        event.waitUntil(tell('error', 'This offline navigation could not be assigned a safe page identity. Retry after other pages finish opening.'));
      ...
      event.respondWith(response.then(async ready => {
        if (ready.ok) {
          try { await renewNavigationGrace(id); }
          catch {
            await tell('error', 'This offline page could not retain its safe version while opening. Retry when storage is available.');
            return fallback();
          }
        }
        return ready;
      }).finally(() => { activeNavigations.delete(id); }));
      return;
    }
    ...
    if (asset) {
      event.respondWith((async () => {
        if (asset.type === 'json' || asset.type === 'html') {
          ...
          try { version = await documentVersion(event.clientId ?? ''); }
          catch {
            await tell('error', 'Offline page ownership could not be confirmed. Your page was not assigned newer data.');
        ...
        try {
          ...
          catch { await tell('error', 'Offline storage is unavailable. The online page remains usable.'); }
          return response;
        } catch {
      ...
      if (image) event.respondWith(imageResponse(image).catch(async () => {
  ...
  scope.addEventListener('message', event => {
    ...
    if (!source || source.type !== 'window' || new URL(source.url).origin !== origin ||
      !data || typeof data !== 'object' || !('channel' in data) || data.channel !== channel ||
      !('type' in data) || !event.ports[0]) return;
    const port = event.ports[0];
    event.waitUntil((async () => {
      const coreReady = await ready();
      if (data.type === 'STATUS') {
        port.postMessage({ channel, version: manifest.version, ready: coreReady, clientVersion: await documentVersion(source.id) ?? null });
      } else if (data.type === 'ACTIVATE' && 'version' in data && data.version === manifest.version && coreReady &&
        'previousVersion' in data && typeof data.previousVersion === 'string' && /^[a-f0-9]{64}$/.test(data.previousVersion)) {
