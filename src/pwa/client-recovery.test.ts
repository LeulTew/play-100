...
vi.mock('./apply-update', () => { imports.count++; throw new Error('Synthetic dependency failure'); });
...
it('retains a waiting worker and protects edits before a connected recovery reload', async () => {
  ...
  const registration = Object.assign(new EventTarget(), {
    scope: 'https://play.test/', active: null, waiting, installing: null,
  });
  const serviceWorker = Object.assign(new EventTarget(), {
    controller: null, getRegistration: vi.fn(async () => registration),
  });
  vi.stubGlobal('window', Object.assign(new EventTarget(), {
    isSecureContext: true, matchMedia: () => Object.assign(new EventTarget(), { matches: false }),
  }));
  const browser = { serviceWorker, onLine: true, userAgent: 'Fixture desktop', maxTouchPoints: 0 };
  vi.stubGlobal('navigator', browser);
  const replace = vi.fn();
  vi.stubGlobal('location', { origin: 'https://play.test', pathname: '/', href: 'https://play.test/?info=settings', replace });
