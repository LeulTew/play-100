...
export async function executePwaUpdate(
  ...
  try {
    ...
    if (!status.ready || (reloadOnly ? status.version !== access.requestedVersion() || navigator.serviceWorker.controller !== worker
      : access.waiting() !== worker) || !guard.isCurrent() || !guard.canReload() || !access.isCurrent()) {
      throw new Error('The waiting update or current edit changed. Review it again.');
    }
    if (reloadOnly) { location.reload(); return true; }
    ...
    if (!prior.ready || !prior.clientVersion || navigator.serviceWorker.controller !== priorController ||
      access.waiting() !== worker || !access.isCurrent() || !guard.isCurrent() || !guard.canReload()) {
    ...
    if (!result.accepted || result.version !== status.version) {
      throw new Error(result.reason === 'other-tabs'
    ...
    if (!active || !trustedPwaWorker(active, location.origin)) throw new Error('The new controller could not be verified.');
    const activated = await sendPwaRequest(active, 'STATUS');
    if (activated.version !== status.version || !activated.ready) throw new Error('The active update version did not match.');
    if (!access.isCurrent() || !guard.isCurrent() || !guard.canReload()) {
      access.publish({ updateState: 'reload-required', message: 'The update is ready, but this page changed. Finish or save your edit before reloading.' });
      return false;
    }
    location.reload();
    return true;
  } catch (cause) {
    access.publish({ updateState: access.requestedVersion() ? 'reload-required' : access.waiting() ? 'waiting' : 'none' });
    access.report(cause instanceof Error ? cause.message : 'The requested update failed. Your page was not reloaded.', cause);
    return false;
  } finally { stopWaiting(); }
}
