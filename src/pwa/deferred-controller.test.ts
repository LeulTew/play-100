...
describe('after-load PWA controller', () => {
  ...
  it.each(['/data-use', '/data-use/', '/api/catalog', '/__/auth/handler'])(
    'does not capture, import or register on excluded %s',
    (pathname) => {
      const env = fixture();
      vi.stubGlobal('location', { origin: 'https://play.test', pathname });
  ...
  it.each([
    ...
    'keeps initial install state for %s before any module loads',
    (userAgent, maxTouchPoints, standalone, expected) => {
      const env = fixture();
  ...
  it.each(['startup', 'offline event'])(
    'connects without waiting for load or idle when offline at %s',
    async (when) => {
      const env = fixture();
  ...
  it('makes a client chunk failure terminal and guards explicit reload without registering or prompting', async () => {
    ...
    try {
      ...
      env.runIdle();
      await vi.waitFor(() =>
        expect(controller.getSnapshot()).toMatchObject({ moduleError: true, error: "Offline controls didn't load." }),
      );
      ...
      for (const cause of [new Error(editMessage), new Error(''), 'prepare failed']) {
        ...
        expect(controller.getSnapshot().message).toBe(
          cause instanceof Error && cause.message ? editMessage : genericMessage,
        );
