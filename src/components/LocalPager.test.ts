...
describe('LocalPager availability', () => {
  it.each([0, 1, 25])('omits inactive navigation for %i results', total => {
    expect(renderToStaticMarkup(createElement(LocalPager, {
      total, pageSize: 25, offset: 475, onOffsetChange: vi.fn(),
    }))).toBe('');
  });

  it('keeps the full navigation and truthful clamped range for multiple pages', () => {
    const onOffsetChange = vi.fn();
    const html = renderToStaticMarkup(createElement(LocalPager, {
      total: 26, pageSize: 25, offset: 475, onOffsetChange,
      label: 'Library pages', itemLabel: 'matching games',
    }));
