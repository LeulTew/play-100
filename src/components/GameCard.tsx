    ...
    >
      ...
      >
        ...
        <div className="game-copy">
          ...
          >
            ...
            {game.authorRating && <span> / 10</span>}
          </p>
          ...
          {state?.completed && (
            <span className="completed-marker">
              <Icon name="check" width="15" height="15" /> Completed
            </span>
          )}
        </div>
      </a>
      <div className="card-played">
        <PlayedToggle
          id={game.slug}
          title={game.title}
          played={Boolean(state?.played)}
          completed={state?.completed}
          busy={busy}
          compact
          onChange={(value) => onPlayed(game.slug, value)}
        />
        <CompletedToggle
          title={game.title}
          completed={Boolean(state?.completed)}
          busy={busy}
          onChange={(value) => onCompleted(game.slug, value)}
        />
        {compareActions && <div className="card-compare-actions">{compareActions}</div>}
      </div>
      {savedCopies}
      {selecting && (
        <label className="select-control card-selection">
          <input
            type="checkbox"
