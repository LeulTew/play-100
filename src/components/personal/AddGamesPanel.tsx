...
export default function AddGamesPanel({ records, ownedRecords, existingIds, onAdd, onDiscover, busy, kind = 'ranking' }: {
  records: LibraryRecord[]; ownedRecords: Record<string, LibraryRecord>; existingIds: ReadonlySet<string>; onAdd: (records: LibraryRecord[]) => Promise<boolean>;
  ...
  const choices = useMemo(() => {
    ...
    return candidates.filter(choice => choice.titles.some(title => searchText(title).includes(term))).slice(0, query ? 20 : 6);
  }, [candidates, query]);
  return (
      ...
      {expanded && <div className="game-picker">
        ...
        <ul className="picker-results">
          {choices.map(({ record }) => {
            ...
            return <li key={record.id} data-picker-id={record.id}><div><strong>{record.title}</strong><span>{SOURCE_LABELS[record.source]}{record.year ? ` · ${record.year}` : ''}{existing ? kind === 'ranking' ? ' · Already ranked' : ' · Already in library' : ''}</span></div><button className="icon-button" aria-label={existing ? `Already in ${kind}: ${record.title}` : `Add ${record.title} to ${kind}`} disabled={busy || existing} onClick={() => { void onAdd([record]); }}><Icon name={existing ? 'check' : 'plus'} width="18" height="18" /></button></li>;
          })}
        </ul>
