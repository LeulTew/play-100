...
export default function LibraryPage({ state, filters, busy, animate, onFilters, onAction, onOpen, onDiscover, onBrowse }: LibraryPageProps) {
  ...
  return (
    <section className="app-page" aria-labelledby="library-title">
      <div className="page-heading"><div><h1 id="library-title" tabIndex={-1} data-page-heading>My library</h1></div><button className="button button-dark" onClick={onDiscover}><Icon name="plus" width="18" height="18" />Find more games</button></div>
      ...
      {tab === 'later' && <p className="queue-instructions">{canReorder ? 'Drag or use arrows to reorder.' : 'Clear search and exit selection to reorder.'}</p>}
