...
export default function LibraryPage({ state, filters, busy, animate, onFilters, onAction, onOpen, onDiscover, onBrowse, onPresentationChange, embedded = false, active = true, workspaceView, completedOnly = false, progressFilter, onPin, onUnpin, pinnedIds, renderDragHandle }: LibraryPageProps) {
  ...
  const renderRecord = (record: LibraryRecord) => <CompareDragSource record={record} disabled={!active}>{binding => <div ref={binding.sourceRef} {...binding.surfaceProps} className="library-row-content">
    ...
    <div className="record-actions">
      {onPin && <button className="icon-button" disabled={pinnedIds?.has(record.id) && !onUnpin} aria-pressed={onUnpin ? pinnedIds?.has(record.id) ?? false : undefined} aria-label={`${pinnedIds?.has(record.id) && !onUnpin ? 'Pinned' : 'Pin'} for comparison: ${record.title}`} title={`${pinnedIds?.has(record.id) && !onUnpin ? 'Pinned' : 'Pin'} for comparison`} onClick={() => { if (pinnedIds?.has(record.id)) onUnpin?.(record.id); else onPin(record); }}><Icon name="stack" width="19" height="19" fill={pinnedIds?.has(record.id) ? 'currentColor' : 'none'} /></button>}
      ...
      <button className="icon-button" disabled={busy} aria-pressed={Boolean(state.progress[record.id]?.later)} aria-label={`Play later: ${record.title}`} title="Play later" onClick={() => { void onAction({ type: 'toggle-progress', record, key: 'later' }); }}><Icon name="bookmark" width="19" height="19" fill={state.progress[record.id]?.later ? 'currentColor' : 'none'} /></button>
      ...
      <button className="icon-button remove-library-action" disabled={busy} aria-label={`Remove ${record.title} from my library`} onClick={event => requestRemoval([record], event.currentTarget)}><Icon name="trash" width="19" height="19" /></button>
    </div>
  ...
  return (
    <section className={embedded ? 'my-games-editor' : 'app-page'} aria-labelledby="library-title">
      ...
      {!embedded && <div className="personal-tabs" role="group" aria-label="Personal library views">
        <button aria-label={`Play later, ${state.queueOrder.length}`} aria-pressed={tab === 'later'} onClick={() => onFilters({ list: 'later', q: '' })}>Play later<span>{state.queueOrder.length}</span></button>
        <button aria-label={`Completed, ${completedCount}`} aria-pressed={tab === 'completed'} onClick={() => onFilters({ list: 'completed', q: '' })}>Completed<span>{completedCount}</span></button>
        <button aria-label={`All my games, ${Object.keys(state.records).length}`} aria-pressed={tab === 'all'} onClick={() => onFilters({ list: 'all', q: '' })}>All my games<span>{Object.keys(state.records).length}</span></button>
      </div>}
      {!firstRunEmpty && <div className="personal-tools"><div className="search-field"><Icon name="search" /><label className="sr-only" htmlFor="library-search">{tab === 'later' ? 'Search your queue' : 'Search your library'}</label><input id="library-search" type="search" placeholder={tab === 'later' ? 'Find a game in your queue' : 'Find a game in your library'} value={query} maxLength={160} onChange={(event) => setQuery(event.target.value)} /></div><button className="button button-outline" onClick={() => { setSelecting((value) => !value); setSelected(new Set()); }}><Icon name="select" width="18" height="18" />{selecting ? 'Exit selection' : 'Select games'}</button></div>}
      ...
      {tab === 'later' && records.length > 0 && <p className="queue-instructions">{canReorder ? 'Drag or use arrows to reorder. Completed games can stay here for a replay.' : 'Clear search, progress filters and selection to reorder.'}</p>}
      ...
      {tab !== 'later' && <div ref={pageBoundary} className="library-results-boundary">
        ...
        <p className={page.pageCount > 1 ? 'sr-only' : 'library-results-count'} role="status" aria-atomic="true">Showing {page.start}–{page.end} of {records.length} matching games</p>
