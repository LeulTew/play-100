...
export function RemoveGamesDialog({
  records,
  state,
  busy,
  onClose,
    ...
    >
      ...
      {mode.scope !== 'guest' && (
        <p className="section-help">
          This account-library removal will sync while online saving is enabled. Any separately published snapshot stays
          unchanged until you update or unpublish it.
        </p>
      )}
      {remaining.length > 0 && (
        <>
          <ul className="removal-games">
            {remaining.slice(0, 6).map((record) => (
      ...
      {failed && (
        <p className="inline-error" role="alert">
          Nothing was removed. Your saved data is unchanged. Check the storage warning and try again.
        </p>
      )}
      <div className="button-row">
        ...
        {remaining.length > 0 && (
          ...
          >
            ...
            {removing ? 'Removing…' : `Remove ${remaining.length} ${remaining.length === 1 ? 'game' : 'games'}`}
          </button>
        )}
      </div>
    </Dialog>
