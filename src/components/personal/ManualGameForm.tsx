...
export default function ManualGameForm({ onAdd, busy, actionLabel = 'Add game' }: { onAdd: (record: LibraryRecord) => Promise<boolean>; busy: boolean; actionLabel?: string }) {
  const prefix = useId();
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!trimmed) { setError('Enter a game title.'); return; }
    ...
    const record: LibraryRecord = {
      id: `manual:${id}`, title: trimmed, year: enteredYear, studio: null, genre: null,
