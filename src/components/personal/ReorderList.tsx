    ...
    <DndContext
      ...
      accessibility={{
        screenReaderInstructions: {
          draggable:
            'Press Space to pick up a game. Use arrow keys to move it. Press Space again to drop, or Escape to cancel. Move up and down buttons are also available.',
        },
        announcements: {
          onDragStart: ({ active }) =>
...
function ReorderRow({
  ...
  const { attributes, listeners, setNodeRef, setActivatorNodeRef, transform, transition, isDragging } = useSortable({
    id,
    disabled,
    transition: animate ? { duration: 180, easing: 'cubic-bezier(.16,1,.3,1)' } : null,
  });
  return (
    <li
      ref={setNodeRef}
    ...
    >
      <div className="record-order">
        ...
        {position !== null && (
          <span className="personal-position">
