...
export default function CatalogDetail({ record, saved, progress, rankingPosition, rating, busy, artwork, motionOrigin, publicLookup, onClose, onAction, onRankings }: CatalogDetailProps) {
    ...
    <Dialog open titleId="catalog-game-title" onClose={onClose} className="info-dialog catalog-detail-dialog" motion={{ preset: 'dialog', continuity: { target: artRef, lease: motionOrigin } }}>
      ...
      <div className="catalog-detail-visual">
        <div className="catalog-detail-sleeve" ref={artRef}>
          {artwork ? <GameArtwork record={record} artwork={artwork} className="catalog-detail-artwork" /> : externalArtwork ? <ExternalCatalogArtwork artwork={externalArtwork} /> : <span className="catalog-detail-artwork catalog-detail-artwork-empty" aria-hidden="true"><Icon name="stack" width="28" height="28" /></span>}
        </div>
      ...
      <div className="detail-actions"><button className={`button ${progress?.later ? 'button-lime' : 'button-dark'}`} disabled={busy} aria-pressed={Boolean(progress?.later)} onClick={() => { void onAction({ type: 'toggle-progress', record, key: 'later' }); }}><Icon name="bookmark" fill={progress?.later ? 'currentColor' : 'none'} />Play later</button><button className={`button ${progress?.completed ? 'button-lime' : 'button-outline'}`} disabled={busy} aria-pressed={Boolean(progress?.completed)}       onClick={() => { void onAction({ type: 'set-progress', records: [record], key: 'completed', value: !progress?.completed }); }}><Icon name={progress?.completed ? 'check' : 'plus'} />Completed</button></div>
      <div className="personal-detail-actions"><PlayedToggle id={record.id} title={record.title} played={Boolean(progress?.played)} completed={progress?.completed} busy={busy}       onChange={value => { void onAction({ type: 'set-progress', records: [record], key: 'played', value }); }} />{rankingPosition === null ? <button className="text-button" disabled={busy} onClick={() => { void onAction({ type: 'add-ranking', records: [record] }); }}><Icon name="rank" width="18" height="18" />Add to my ranking</button> : <button className="text-button" onClick={onRankings}>Your rank: #{rankingPosition}<Icon name="arrow" width="17" height="17" /></button>}</div>
      <div className="catalog-detail-rating"><PersonalRatingInput key={record.id} title={record.title} value={rating} busy={busy} onCommit={(score) => onAction({ type: 'rate-game', record, score })} /><p>Rating adds this game to Ranking in My games. It does not mark it played or change a fixed position.</p></div>
      <p className="device-note">{saved ? 'Saved in My games.' : 'Preview only. Add to My games from Discover, or rate or mark progress here to keep this game.'} The 100 stays unchanged.</p>
    </Dialog>
  );
}
