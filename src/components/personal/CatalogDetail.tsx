...
export default function CatalogDetail({ record, saved, progress, rankingPosition, rating, busy, onClose, onAction, onRankings }: {
  ...
  return (
    <Dialog open titleId="catalog-game-title" onClose={onClose} className="info-dialog">
      ...
      <div className="catalog-detail-rating"><PersonalRatingInput title={record.title} value={rating} busy={busy} onCommit={(score) => onAction({ type: 'rate-game', record, score })} /><p>Rating saves this game to your private ranking. It does not mark it played or change a manual position.</p></div>
