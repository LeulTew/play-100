...
export default function CatalogDetail({ record, saved, progress, rankingPosition, rating, busy, onClose, onAction, onRankings }: {
  ...
  return (
    <Dialog open titleId="catalog-game-title" onClose={onClose} className="info-dialog">
      ...
      <p className="device-note">{saved ? 'Saved in your active library.' : 'Preview only. Save or rate this game to keep it after closing the page.'} Guest copies stay on this device; connected account copies sync only while enabled. Public sharing is separate.</p>
    </Dialog>
  );
}
