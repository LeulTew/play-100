...
export default function CatalogDetail({ record, saved, progress, rankingPosition, rating, busy, artwork, motionOrigin, publicLookup, onClose, onAction, onRankings }: CatalogDetailProps) {
  ...
  return (
    <Dialog open titleId="catalog-game-title" onClose={onClose} className="info-dialog catalog-detail-dialog" motion={{ preset: 'dialog', continuity: { target: artRef, lease: motionOrigin } }}>
      ...
      <div className="detail-actions"><button className={`button ${progress?.later ? 'button-lime' : 'button-dark'}`} disabled={busy} aria-pressed={Boolean(progress?.later)} onClick={() => { void onAction({ type: 'toggle-progress', record, key: 'later' }); }}><Icon name="bookmark" />{progress?.later ? 'Saved for later' : 'Play later'}</button><button className="button button-outline" disabled={busy} aria-pressed={Boolean(progress?.completed)}       onClick={() => { void onAction({ type: 'set-progress', records: [record], key: 'completed', value: !progress?.completed }); }}><Icon name="check" />{progress?.completed ? 'Completed' : 'Mark completed'}</button></div>
