...
export function MenuDialog({
  ...
  const activate = async (commit: () => void) => {
    ...
    try {
      const saved = await flushPendingEdits((target) => {
        ...
        recovery.current = { target, isCurrent };
      });
      if (!active.current || !isCurrent()) return;
      if (saved) commit();
      else
        setError(
          visibleFocusTarget(blockedTarget)
  ...
  const getReturnFocus = () => {
    ...
    return (
      [...document.querySelectorAll<HTMLElement>('[data-page-heading], #collection-title')].find(visibleFocusTarget) ??
      null
    );
  };
  ...
  const link = (label: string, next: AppPage, patch: Partial<Filters> = {}, current = page === next) => {
    ...
    return (
      <li>
        <a
          href={`${destination.path}${destination.search}`}
        ...
        >
          <span>{label}</span>
          {current && <small aria-hidden="true">Current</small>}
        </a>
      </li>
    );
  };
  ...
  return (
    ...
    >
      <h2 id="menu-title" data-autofocus tabIndex={-1}>
        Menu
      </h2>
      <div className="menu-feedback">
        <p role="status" className={!saving && status && statusError && !moduleRecovery ? 'inline-error' : undefined}>
          {saving ? 'Saving your open edit…' : moduleRecovery ? '' : status}
        </p>
        {moduleRecovery}
        {error && (
          <div role="alert">
      ...
      <nav className="menu-scroll" aria-label="All navigation" aria-busy={saving}>
        <div className="menu-groups">
          ...
          {onlineAvailable && (
            <section aria-labelledby="menu-people">
