...
export function MobileNav({ page, personalPage, gamesView, onlineAvailable, menuOpen, pageHref, onNavigateLink, onBrowseLink, onMenu }: MobileNavProps) {
  return <nav className="mobile-nav" aria-label="Mobile navigation">
    <a href={pageHref('collection')} aria-current={page === 'collection' ? 'page' : undefined} onClick={onBrowseLink}><Icon name="grid" width="20" height="20" /><span>The 100</span></a>
    <a href={pageHref('discover')} aria-current={page === 'discover' ? 'page' : undefined} onClick={event => onNavigateLink(event, 'discover')}><Icon name="search" width="20" height="20" /><span>Discover</span></a>
