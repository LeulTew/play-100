...
export function AppHeader({ page, onlineAvailable, libraryScope, libraryLabel, syncStatus, headerIdentity, savedCount, animate, menuOpen, pageHref, onNavigateLink, onQueue, onMenu, onAccount, onIntent }: AppHeaderProps) {
  ...
  return <header className={`site-header ${onlineAvailable ? 'site-header-online' : ''}`}>
    ...
    <div className="header-actions">
      <button className="saved-nav" aria-label={`Play later, ${savedCount} games in your queue`} onClick={onQueue}><Icon name="bookmark" width="19" height="19" /><span className="saved-nav-label">Play later</span><CountUp key={libraryScope} to={savedCount} animate={animate} className="saved-count" /><span className="sr-only"> games in your queue</span></button>
