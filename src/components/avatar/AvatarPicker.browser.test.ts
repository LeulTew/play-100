...
describe('AvatarPicker in a real browser', () => {
  ...
  it('supports native arrow selection with visible focus, without changing persisted input', async () => {
    ...
    const outline = await page
      .locator('.avatar-picker__candidate')
      .nth(1)
  ...
  it('keeps palette changes and Shuffle local, and Cancel never calls Save', async () => {
    ...
    expect(await faces().evaluateAll((inputs) => inputs.map((input) => input.getAttribute('value')))).not.toEqual(
      oldSeeds,
    );
  ...
  it('saves a snapshot exactly once, disables pending controls and does not call Cancel on success', async () => {
    ...
    expect(
      await page
        .locator('.avatar-picker input, .avatar-picker button')
        .evaluateAll((controls) => controls.every((control) => control.matches(':disabled'))),
    ).toBe(true);
  ...
  it.each(['failure', 'throw'] as const)(
    'shows %s inline, preserves the draft and permits an explicit retry',
    async (mode) => {
  ...
  it('asks once to try again when the save fails without an Error, keeping a real cause otherwise', async () => {
    ...
    await browserExpect(page.getByRole('alert')).toHaveText(
      'Could not save avatar. Your choice is still here. Try saving again.',
    );
    await page.evaluate(() => window.avatarTest.mode('failure'));
    await page.getByRole('button', { name: 'Save avatar', exact: true }).click();
    await browserExpect(page.getByRole('alert')).toHaveText(
      'Could not save avatar. Synthetic save failure. Your choice is still here. Try saving again.',
    );
  ...
  it.each(['resolve', 'reject'] as const)(
    'ignores a late %s from the old identity while the new identity is saving',
    async (settle) => {
      await page.evaluate(() => window.avatarTest.mode('deferred'));
  ...
  it('ignores late rejection after unmount and does not poison a reopened picker', async () => {
    ...
    await page.evaluate(() => {
      window.avatarTest.reject(0);
      window.avatarTest.mount();
    });
