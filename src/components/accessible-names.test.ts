...
describe('composite control accessible names', () => {
  it('includes the whole visible rating label before the game context', () => {
    const html = renderToStaticMarkup(
      h(PersonalRatingInput, {
        title: discoveryFixture.record.title,
        value: null,
        busy: false,
  ...
  it.each([false, true])(
    'keeps the completion name stable with a distinct non-color cue for pressed=%s',
    (completed) => {
      const html = renderToStaticMarkup(
        h(CompletedToggle, {
          title: discoveryFixture.record.title,
  ...
  it.each([false, true])('names discovery Pin as an add-only action (pinned=%s)', (pinned) => {
    ...
    expect(pin).toContain(
      '<path d="m3 7 9-4 9 4-9 4-9-4Z"></path><path d="M3 12l9 4 9-4M3 17l9 4 9-4" fill="none"></path>',
    );
  });
  ...
  it.each([false, true])('keeps the discovery queue name stable with pressed=%s', (selected) => {
    ...
    const html = renderToStaticMarkup(
      h(DiscoveryCard, {
        ...
        state: {
          ...
          progress: { [record.id]: { later: selected, played: false, completed: false } },
        },
        busy: false,
        pinned: selected,
        onPin: vi.fn(),
    ...
    const queue = html
      .match(/<button\b[^>]*>[\s\S]*?<\/button>/g)
      ?.find((button) => button.endsWith('</svg>Play later</button>'));
    ...
    const html = renderToStaticMarkup(
      h(CatalogSourceStatus, {
        sources: emptySources().map((source) => ({
          ...
          notices: ['Provider-specific coverage.'],
        })),
        onRetry: vi.fn(),
        onMore: vi.fn(),
      }),
  ...
  it.each([false, true])(
    'starts saved-copy names with the visible action, including its source when shown (multiple=%s)',
    (multiple) => {
      const record = discoveryFixture.record;
      const copies = multiple
        ? [record, { ...record, id: 'manual:copy', source: 'manual' as const, sourceId: 'copy' }]
        : [record];
  ...
  it('names each film opener from its visible content so the visible label leads the name', () => {
    ...
    collectionFilms.forEach((film, index) => {
      ...
      expect(openers[index]).toContain(
        `<span class="film-summary"><strong>${escapeHtml(film.title)}</strong> <span>${escapeHtml(film.description)}</span> <small>${filmDuration(film.durationSeconds)} · Watch film</small></span></button>`,
      );
    });
  });
  ...
  it('speaks missing table scores through sr-only text instead of a label on a generic span', () => {
    ...
    expect(html).toContain(
      '<td class="numeric-score table-author-rating"><span aria-hidden="true">—</span><span class="sr-only">Original author rating unavailable</span></td>',
    );
    expect(html).toContain(
      '<td class="numeric-score"><span aria-hidden="true">—</span><span class="sr-only">Unavailable</span></td>',
    );
  ...
  it('keeps the visible position digits and speaks the position through sr-only text', () => {
    ...
    expect(html).toContain(
      '<span class="personal-position"><span aria-hidden="true">03</span><span class="sr-only">Position 3</span></span>',
    );
