...
describe('About data and privacy summary', () => {
  it('uses the same plain storage, limit and deletion wording as the data-use page', () => {
    ...
    for (const term of ['IndexedDB', 'eviction', 'local cache', 'Free cloud quotas', 'revocation markers', 'stale tabs']) expect(privacy).not.toContain(term);
  });

  it('keeps the consent, creator-view and operator-access disclosures unchanged', () => {
    expect(privacy).toContain('Before online saving, you agree that the creator can view your account profile and ranking summary.');
    expect(privacy).toContain('a database operator can technically access stored data.');
    expect(privacy).toContain('Signing in does not automatically upload the guest library; online saving requires a separate choice and consent.');
  });
});
