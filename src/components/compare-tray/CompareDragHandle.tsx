...
export function CompareDragHandle({ record, compact = false }: { record: LibraryRecord; compact?: boolean }) {
  ...
  return <button ref={sourceRef} {...source.surfaceProps} data-compare-drag-grip="" type="button" className={`compare-drag-handle ${compact ? 'icon-button' : 'text-button'}`}
    hidden={coarsePointer} aria-hidden={compact || coarsePointer || undefined} tabIndex={compact || coarsePointer ? -1 : undefined}
    aria-label={compact ? undefined : `${coarsePointer ? 'Pin to tray' : 'Drag to tray'}: ${record.title}${coarsePointer ? '' : ', or click to pin for comparison'}`} title={compact ? 'Drag to tray' : undefined}
