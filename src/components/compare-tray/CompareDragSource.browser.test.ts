...
beforeAll(async () => {
  ...
  server = (
    await createFetchSafeViteServer(() =>
      createServer({
        configFile: false,
        root: process.cwd(),
        cacheDir: 'node_modules/.vite-compare-tests',
        logLevel: 'error',
        appType: 'custom',
  ...
  console.info(
    `Compare fixture ${origin}; runner PID ${process.pid}; Chrome PID ${browserServer.process().pid}; Chrome ${browser.version()}`,
  );
  resourceReceipt = {
    origin,
...
async function openFixture(touch = false, width = 1280) {
  ...
  context = await browser.newContext({
    viewport: { width, height: touch ? 851 : 960 },
    hasTouch: touch,
    isMobile: touch,
...
async function nativeTouchTap(cdp: CDPSession, target: Locator, padding = false) {
  ...
  expect(
    await target.evaluate((node, point) => node.contains(document.elementFromPoint(point.x, point.y)), point),
  ).toBe(true);
...
async function observePinInput() {
  await page.evaluate(() => {
    ...
    new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          if (
            ...
            (node.matches('.compare-drag-ghost') || node.querySelector('.compare-drag-ghost'))
          )
            receipt.ghosts += 1;
        }
        if (
          mutation.attributeName === 'data-compare-dragging' &&
          mutation.target instanceof Element &&
          mutation.target.hasAttribute('data-compare-dragging')
        ) {
      ...
      childList: true,
      attributes: true,
      attributeFilter: ['data-compare-dragging'],
    });
  });
}
...
describe('Compare source browser contract', () => {
  ...
  it('uses actual native title drag and only the opaque MIME, without opening or copying private UI', async () => {
    ...
    expect(
      await page
        .locator('.compare-drag-ghost input,.compare-drag-ghost textarea,.compare-drag-ghost img,[data-compare-token]')
        .count(),
    ).toBe(0);
    await nativeDrop();
    await browserExpect
  ...
  it('removes native feedback on hide, source removal, and unmount even with reduced optional motion', async () => {
    for (const action of ['hidden', 'source', 'unmount'] as const) {
      ...
      await page.mouse.up();
      expect(
        await page.evaluate(() => Object.keys(localStorage).filter((key) => key.startsWith('play100:compare-tray:'))),
      ).toEqual([]);
    }
  });
    ...
    try {
      ...
      for (let step = 1; step <= 5; step++) {
        await cdp.send('Input.dispatchTouchEvent', {
          type: 'touchMove',
          touchPoints: [{ x, y: Math.max(8, y - step * 20) }],
        });
        await touchInputFrame();
      }
      await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
  ...
  for (const width of [393, 320]) {
    it.each(['icon center', 'button padding'] as const)(
      `uses the native touch %s as Pin only and preserves the next title tap at ${width}px`,
      async (target) => {
        await context.close();
        ...
        try {
          ...
          expect(
            receipt.events.some((event) => event.type === 'click' && event.trusted && event.pointerType === 'touch'),
          ).toBe(true);
          expect(
            receipt.events.filter((event) => event.type.startsWith('pointer')).every((event) => !event.prevented),
          ).toBe(true);
    ...
    it(`keeps a native pan from the dedicated Pin back to page top and the next title tap at ${width}px`, async () => {
      ...
      try {
        ...
        for (let step = 1; step <= 5; step++) {
          await cdp.send('Input.dispatchTouchEvent', {
            type: 'touchMove',
            touchPoints: [{ x: start.x, y: start.y + step * 35 }],
          });
          await touchInputFrame();
        }
        await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
  ...
  it.each(['touch', 'pen'] as const)(
    'never arms the grip for actual %s input on a fine-pointer layout',
    async (pointerType) => {
      ...
      try {
        ...
        expect(
          receipt.events.some(
            (event) =>
              event.type === 'pointerdown' && event.pointerType === pointerType && event.trusted && event.isPrimary,
          ),
        ).toBe(true);
      } finally {
        await cdp.detach();
  ...
  it.each([393, 320])(
    'keeps the existing broad-touch hold, token drop and direct fresh native tap at %ipx',
    async (width) => {
      await context.close();
      await openFixture(true, width);
      ...
      try {
        ...
        const endX = dock.x + dock.width / 2,
          endY = dock.y + dock.height / 2;
        for (let step = 1; step <= 8; step++) {
          await cdp.send('Input.dispatchTouchEvent', {
            type: 'touchMove',
            touchPoints: [{ x: x + ((endX - x) * step) / 8, y: y + ((endY - y) * step) / 8 }],
          });
          await touchInputFrame();
        }
        await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
        ...
        expect(
          await page
            ...
            .evaluate((node, point) => node.contains(document.elementFromPoint(point.x, point.y)), point),
        ).toBe(true);
        await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [point] });
        await touchInputFrame();
        await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
