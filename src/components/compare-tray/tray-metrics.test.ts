 ...
 */
function page(
  heights: { header?: number; navigation?: number; toast?: number; tray?: number; trayMarkTop?: number },
  ...
  const targets: TrayMetricTargets = {
    style,
    header: element('header', 0),
    navigation: element('navigation', 780),
    toast: element('toast', 600),
    tray,
  };
  return {
    targets,
...
describe('compare tray metrics', () => {
  it('reads every height before it writes, so measuring forces no style recalculation', () => {
    ...
    expect(current.log).toEqual([
      ...
      'read navigation',
      'read toast',
      'set --site-header-height 73px',
      'set --mobile-nav-height 66px',
      'set --toast-height 48px',
      'set --compare-tray-height 99px',
    ]);
    expect(current.values()).toEqual({
      '--site-header-height': '73px',
      '--mobile-nav-height': '66px',
      '--toast-height': '48px',
      '--compare-tray-height': '99px',
    });
  });
