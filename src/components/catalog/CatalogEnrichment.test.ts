...
const state = () => ({ key: 'public', status: 'ready' as const, data: enrichmentFixture(), cached: false, error: null, connected: true, retry: vi.fn() });
...
describe('separate public review provenance', () => {
  ...
  it('shows issuer, literal scale, platform, method and old score/reference dates separately', () => {
    ...
    for (const text of ['83/100', 'Example publication', 'PC', 'Critic average', 'via Wikidata', '2024-04-20', '2024-04-21', '2026-09-22']) expect(html).toContain(text);
  ...
  it('offers an explicit online enable action and keeps cached dates visible', () => {
    const html = renderToStaticMarkup(createElement(CatalogEnrichment, { enrichment: { ...state(), cached: true, status: 'disabled' }, lookup: { ...lookup, online: false } }));
  ...
  it('surfaces partial source failures outside the disclosure without deleting successful ratings', () => {
    ...
    enrichment.data.sources[1] = { source: 'steam', status: 'error', code: 'timeout', retryAfter: 0, message: 'Steam took too long.' };
  ...
  it('retains licensed image attribution, original link and transformation/retrieval details', () => {
    const artwork: Artwork = {
      kind: 'commons-raster', src: 'data:image/webp;base64,UklGRg==', width: 1, height: 1, alt: 'Test art',
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:Example.png', originalUrl: 'https://upload.wikimedia.org/wikipedia/commons/a/aa/Example.png',
      license: 'CC BY-SA 4.0', licenseUrl: 'https://creativecommons.org/licenses/by-sa/4.0/',
      credit: 'Original artist; resized and converted to WebP; trademark caveat.', retrievedAt: '2026-09-22T12:00:00.000Z',
    };
    const html = renderToStaticMarkup(createElement(ExternalCatalogArtworkCredit, { artwork }));
    for (const text of [artwork.credit, artwork.sourceUrl, artwork.originalUrl, artwork.licenseUrl, '2026-09-22']) expect(html).toContain(text);
