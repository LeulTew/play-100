...
export function DiscoveryCard({
  ...
  const compare = useCompareDragSource({
    record: onPin ? actionRecord : undefined,
    sourceRef: cardRef,
  ...
  return (
    <li
      ref={cardRef}
      {...compare.surfaceProps}
      className={`discovery-card${selected ? ' is-selected' : ''}`}
    ...
    >
      ...
      <div className="discovery-card-body">
        <h3>
          {onPreview ? (
            <button {...compare.titleProps} type="button" onClick={preview}>
              {record.title}
            </button>
          ) : (
            record.title
        ...
        {game && (
          <p className="discovery-canonical">
            From The 100 · #{game.rank}
            <span>
              {author.shortName}'s rating{' '}
              <strong title={game.authorRating?.rawValue}>
                {authorRatingText(game.authorRating)}
                {game.authorRating ? ' / 10' : ''}
              </strong>
            </span>
          </p>
        )}
        ...
        <details className="discovery-card-details">
          ...
          <div className="discovery-card-secondary">
            <PlayedToggle
              ...
              busy={busy}
              onChange={(value) => {
                void onAction({ type: 'set-progress', records: [actionRecord], key: 'played', value });
              }}
            />
            <CompletedToggle
              title={record.title}
              ...
              busy={busy}
              onChange={(value) => {
                void onAction({ type: 'set-progress', records: [actionRecord], key: 'completed', value });
              }}
            />
            <button
              className="button button-outline"
            ...
            <PersonalRatingInput
              ...
              title={record.title}
              value={ranking?.score ?? null}
              busy={busy}
              onCommit={(score) => onAction({ type: 'rate-game', record: actionRecord, score })}
            />
          </div>
          <div className="discovery-card-source">
            ...
            {record.sourceUrl ? (
              ...
              <p>{SOURCE_LABELS[record.source]}</p>
            )}
            {game ? (
              <p>Original collection metadata and workbook artwork.</p>
            ) : (
              artwork && (
                <>
                  <p>{artwork.credit}</p>
