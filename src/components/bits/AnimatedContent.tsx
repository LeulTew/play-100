...
export default function AnimatedContent({ children, animate, className = '' }: { children: ReactNode; animate: boolean; className?: string }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      ...
      animation = element.animate(
        [{ clipPath: 'inset(0 0 3% 0)', transform: 'translateY(8px)' }, { clipPath: 'inset(0)', transform: 'translateY(0)' }],
        { duration: 450, easing: 'cubic-bezier(.16,1,.3,1)' },
      );
      observer.disconnect();
    }, { threshold: 0.08 });
    observer.observe(element);
    const onVisibility = () => { if (document.hidden) animation?.finish(); };
