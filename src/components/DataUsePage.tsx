...
export default function DataUsePage() {
  ...
  return <>
    ...
    <main className="app-page data-use-page" id="data-use">
      ...
      <p>Friends ranking sharing is off until you preview a selection and agree. Up to 200 selected ranked games, their order and optional scores can update automatically after your saved edits. Newly added games are not selected automatically. Notes, email, queue and play history are excluded.</p>
      <p>Shared games is a separate, optional shelf of up to 200 selected saved games, including unranked additions. It shares only game identity and display metadata, never scores, notes, queue or play history. Removing then re-adding a game does not reselect it. Restarting online saving requires a fresh shelf review.</p>
      <p>Removing a friendship, blocking a person, stopping that sharing option or deleting the account stops new friends-only reads. Previously copied information cannot be recalled. Stopping private online saving pauses automatic updates but can keep the last shared ranking and shelf visible until their sharing is also stopped.</p>
