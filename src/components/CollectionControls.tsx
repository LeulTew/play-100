...
export function CollectionControls({
  ...
  const years = useMemo(
    () =>
      [
        ...new Set(
          [...games, ...extraRecords].map((game) => game.year).filter((year): year is number => year !== null),
        ),
      ].sort((a, b) => b - a),
    [games, extraRecords],
  );
  ...
  const secondaryCount = [
    ...
    filters.list !== 'all',
    progress !== 'all',
    filters.sort !== 'rank' || filters.direction === 'desc',
  ...
  return (
    <>
      ...
      <div className="collection-search">
        <label className="field-label" htmlFor="game-search">
          Search games, studios or genres
        </label>
        <div className="search-field">
          ...
          <input
            ...
            onBlur={() => {
              searchSession.current = false;
            }}
          />
          {filters.q ? (
            <button className="icon-button" aria-label="Clear search" onClick={() => onChange({ q: '' })}>
      ...
      <BrowseFilters activeCount={secondaryCount} label="Filters & sort" className="collection-filters">
        <div className="collection-tabs" role="group" aria-label="Your collection views">
          <button
            className={filters.list === 'all' ? 'is-active' : ''}
            aria-label={`All games, ${games.length + addedCount}`}
            aria-pressed={filters.list === 'all'}
            onClick={() => onChange({ list: 'all', progress: 'all' })}
          >
            All games <span>{games.length + addedCount}</span>
          </button>
          <button
            className={filters.list === 'later' ? 'is-active' : ''}
            aria-label={`Play later, ${savedCount}`}
            aria-pressed={filters.list === 'later'}
            onClick={() => onChange({ list: 'later' })}
          >
            Play later <span>{savedCount}</span>
          </button>
          <button
            className={filters.list === 'completed' ? 'is-active' : ''}
            aria-label={`Completed, ${completedCount}`}
            aria-pressed={filters.list === 'completed'}
            onClick={() => onChange({ list: 'completed', progress: 'all' })}
          >
        ...
        <div className="search-and-filters">
          ...
          >
            ...
            {filters.year && !years.includes(Number(filters.year)) && (
              <option value={filters.year}>{filters.year} (not loaded)</option>
            )}
            {years.map((year) => (
          ...
          <SelectField
            ...
            value={filters.tier}
            onChange={(tier) =>
              onChange({ tier: tier === 'core' ? 'core' : tier === 'essential' ? 'essential' : 'all' })
            }
          >
          ...
          >
            <option value="rank">Collection rank</option>
            <option value="title">Title</option>
            <option value="newest">Newest first</option>
            <option value="oldest">Oldest first</option>
            <option value="score">Critic snapshot average</option>
            <option value="author-rating">{author.shortName}'s original rating</option>
            {criticColumns.map(({ key, label }) => (
        ...
        <div className="search-scope">
          <label className="check-control">
            <input
              ...
              disabled={!onlineScope}
              aria-describedby="catalog-search-help"
              onChange={(event) => onChange({ catalogs: event.target.checked ? 'on' : 'off' })}
            />
      ...
      <div className="collection-utilities">
        <div className="result-summary">
          <p role="status" aria-live="polite" aria-atomic="true">
            <strong>{count - unrankedCount}</strong> in The 100
            {unrankedCount > 0 ? (
            ...
            {searching && <span className="result-breakdown">Searching public catalogs…</span>}
          </p>
          {activeFilters && (
            <button
              className="text-button clear-filters"
              onClick={() =>
                onChange({ ...defaultFilters, catalogs: filters.catalogs, sort: filters.sort, view: filters.view })
              }
            >
              Reset filters
