...
describe('data use explanation', () => {
  ...
  it('keeps internal storage and protocol terms out of the explanation', () => {
    for (const term of ['guest scope', 'account cache', 'Dirty copies', 'safe choice', "Firebase's free quotas", 'Quota exhaustion',
      'absent new settings', 'atomically', 'safety signal', 'Private count records']) expect(text).not.toContain(term);
  });
});
