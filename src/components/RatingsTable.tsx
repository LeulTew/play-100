        ...
        <table className="ratings-table">
          ...
          <tbody>{games.map((game) => {
            ...
            <tr key={game.slug} data-game={game.slug} className={selected.has(game.slug) ? 'row-selected' : ''}>
              ...
              <td>{game.year}</td>
              <td className="numeric-score table-author-rating" title={game.authorRating?.rawValue}>{game.authorRating ? authorRatingText(game.authorRating) : <><span aria-hidden="true">—</span><span className="sr-only">Original author rating unavailable</span></>}</td>
              {criticColumns.map(({ key }) => <td key={key} className="numeric-score">{game.critics[key] === null ? <><span aria-hidden="true">—</span><span className="sr-only">Unavailable</span></> : game.critics[key]}</td>)}
