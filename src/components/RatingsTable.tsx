...
export default function RatingsTable({
  ...
  const sortedHeader = (label: string, sort: SortOrder, scale?: string) => {
    ...
    return (
      <th
        scope="col"
        className={sort === 'rank' ? 'table-rank' : sort === 'title' ? 'table-game' : undefined}
        aria-sort={active ? (direction === 'asc' ? 'ascending' : 'descending') : undefined}
      >
        <button
          className="table-sort"
          aria-label={scale ? `${label} ${scale}` : label}
          onClick={() => {
            const nextDirection = active
              ? direction === 'asc'
                ? 'desc'
                : 'asc'
              : sortDirection({ sort, direction: 'auto' });
            onSort(
              sort === 'newest'
                ? { sort: nextDirection === 'asc' ? 'oldest' : 'newest', direction: 'auto' }
        ...
        >
          ...
          <Icon name={active && direction === 'asc' ? 'up' : 'down'} width="13" height="13" />
        </button>
      </th>
    );
  };
            ...
            <tr>
              {selecting && (
                <th className="selection-column" scope="col">
                  <span className="sr-only">Select games</span>
                </th>
              )}
              ...
              {criticColumns.map(({ key, label, scale }) => (
                <TableHeading
                  ...
                  direction={direction}
                  onClick={() =>
                    onSort({ sort: key, direction: filters.sort === key && direction === 'desc' ? 'asc' : 'desc' })
                  }
                />
              ))}
            ...
            {games.map((game) => {
              ...
              return (
                <tr key={game.slug} data-game={game.slug} className={selected.has(game.slug) ? 'row-selected' : ''}>
                  ...
                  <th scope="row" className="table-game">
                    <RatingsGameLink game={game} filters={filters} onOpen={onOpen} compareRecord={compareRecord} />
                    <span>
                  ...
                  <td>{game.year}</td>
                  <td className="numeric-score table-author-rating" title={game.authorRating?.rawValue}>
                    {game.authorRating ? (
                      authorRatingText(game.authorRating)
                    ) : (
                      <>
                        <span aria-hidden="true">—</span>
                        <span className="sr-only">Original author rating unavailable</span>
                      </>
                    )}
                  </td>
                  {criticColumns.map(({ key }) => (
                    <td key={key} className="numeric-score">
                      {game.critics[key] === null ? (
                        <>
                          <span aria-hidden="true">—</span>
                  ...
                  <td>
                    <div className="table-progress">
                      <PlayedToggle
                        id={game.slug}
                        title={game.title}
                        played={Boolean(progress[game.slug]?.played)}
                        completed={progress[game.slug]?.completed}
                        busy={busy}
                        compact
                        onChange={(value) => onToggle(game.slug, 'played', value)}
                      />
                      <CompletedToggle
                        title={game.title}
                        completed={Boolean(progress[game.slug]?.completed)}
                        busy={busy}
                        onChange={(value) => onToggle(game.slug, 'completed', value)}
                      />
                      <button
...
function TableHeading({
  ...
  return (
    <th scope="col" aria-sort={active ? (direction === 'asc' ? 'ascending' : 'descending') : undefined}>
      <button className="table-sort" aria-label={`${label} / ${scale}`} onClick={onClick}>
        <span>
          {label}
          <small>/ {scale}</small>
        </span>
