...
export function useFriendAll(
    ...
    const work = new SyncWorkQueue(
      async () => {
        ...
        const publicationCurrent = () =>
          ...
          current.current.snapshot.sync.epoch === local.sync.epoch &&
        ...
        const saved = await store.publish(
          uid,
          'games',
          projectAllFriendGames(local.state, current.current.games),
          policy,
          source,
          publicationCurrent,
          progress,
        );
        const ranking = await store.publish(
          uid,
          'ranking',
          projectAllFriendRankings(local.state, current.current.games),
          policy,
          source,
          publicationCurrent,
          progress,
      ...
      (cause) => {
        ...
        const failure =
          cause instanceof FriendAllCommittedError
            ? syncFailure(cause.cause)
        ...
        setState((old) =>
          old?.key === key
            ? {
                ...old,
                status: failure === 'quota' ? 'quota' : failure === 'transient' ? 'retrying' : 'error',
                error:
                  failure === 'quota'
                    ? FRIEND_ALL_QUOTA_MESSAGE
                    : cause instanceof FriendAllCommittedError
  ...
  useEffect(() => {
    ...
    let alive = true;
    void Promise.all([store.progress(uid, 'games', policy.epoch), store.progress(uid, 'ranking', policy.epoch)])
      .then(([games, ranking]) => {
        if (alive && owns()) setProgress({ key, games, ranking });
      })
      .catch((cause) => {
        if (alive && owns())
          setState((old) => (old?.key === key ? { ...old, status: 'error', error: onlineError(cause) } : old));
      });
    return () => {
      alive = false;
    };
  ...
  const suspend = useCallback(() => {
    ...
    return () => {
      if (restart && generation.current === lease && owns()) setResumed((value) => value + 1);
    };
  }, [owns]);
  ...
  const upToDate =
    state?.key === key &&
    policy &&
    [state.games, state.ranking].every(
      (head) =>
        head?.status === 'ready' &&
        head.epoch === policy.epoch &&
        head.policyRevision === policy.revision &&
        head.source.syncEpoch === snapshot?.sync.epoch &&
        head.source.remoteRevision === snapshot?.sync.baseRemoteRevision,
    );
  const status: FriendAllStatus = error
    ? (state?.status ?? 'error')
    : choiceKind === 'checking'
      ? 'checking'
      : choiceKind === 'paused' || choiceKind === 'revoked'
        ? 'paused'
        : choiceKind !== 'all'
          ? 'off'
          : snapshot?.sync.dirty || pending
            ? 'pending'
            : upToDate
              ? 'saved'
              : state?.status === 'saving'
  ...
  return {
    ...
    status,
    error,
    ready: state?.key === key && choiceKind !== 'checking',
    gamesCount: state?.key === key ? (state.games?.count ?? null) : null,
    rankingCount: state?.key === key ? (state.ranking?.count ?? null) : null,
