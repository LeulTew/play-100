export function onlineError(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    'auth/user-mismatch':
      'Choose the Google identity already linked to this account. Nothing has been deleted or copied.',
    'auth/account-exists-with-different-credential':
      'Sign in with your existing email method first, then link Google from Account. Matching an email alone does not grant access.',
    'auth/credential-already-in-use':
      'That Google identity is already connected to another account. Sign in to that account instead.',
    'auth/requires-recent-login':
      'For this sensitive action, sign in again and retry. Your remaining data has not been silently deleted.',
    ...
    'failed-precondition':
      'Online storage needs attention before this action can finish. Your local data is safe; try again later.',
  };
  return (
    messages[code] ??
    (error instanceof Error ? error.message : 'The online action could not finish. Your local data is retained.')
  );
}
