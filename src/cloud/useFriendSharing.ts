...
export type FriendSharingStatus = 'off' | 'paused' | 'checking' | 'pending' | 'saving' | 'saved' | 'retrying' | 'quota' | 'error';
...
export function useFriendSharing(uid: string | undefined, scope: LibraryScope | null, snapshot: ScopedLibrary | null, verified: boolean, games: Game[], visibleTools: boolean, authGeneration = 0) {
  ...
  const acceptSettings = useCallback((next: FriendSettings | null, owner: string, explicitThroughRevision?: number) => {
    ...
    setValue({ uid: owner, settings: next }); setFailure(null);
    const target = current.current.scope;
    if (next && target && current.current.snapshot) void updateFriendSelectionCache(target, next.revision, next.selectedIds, explicitThroughRevision, current.current.snapshot.state.revision).catch((cause) => {
      if (current.current.uid === owner) { setFailure({ uid: owner, message: onlineError(cause) }); setStatus('error'); queue.current?.failed('blocked'); }
    });
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const failed = (cause: unknown) => {
      if (alive && cloudAuth.currentUser?.uid === uid) {
        setFailure({ uid, message: onlineError(cause) }); setStatus('error');
        unsubscribe?.(); unsubscribe = undefined;
        queue.current?.failed(sharingFailure(cause));
      }
    };
    const next = (data: FriendSettings | null) => { if (alive) acceptSettings(data, uid); };
    const attach = (force = false) => {
      if (!alive) return;
      if (document.hidden || !navigator.onLine) { unsubscribe?.(); unsubscribe = undefined; return; }
    ...
    const refresh = () => {
      attach();
      if (alive && !document.hidden && navigator.onLine && !unsubscribe && !queue.current?.reason) void store.settings(uid).then(next).catch(failed);
    };
    const unavailable = () => attach();
    reconnectSettings.current = () => attach(true);
    refresh();
    window.addEventListener('online', refresh); window.addEventListener('offline', unavailable);
    window.addEventListener('focus', refresh); document.addEventListener('visibilitychange', refresh);
    return () => {
      alive = false; unsubscribe?.(); reconnectSettings.current = () => {};
      window.removeEventListener('online', refresh); window.removeEventListener('offline', unavailable);
  ...
  useEffect(() => {
    ...
    const isCurrent = () => alive && cancellationGeneration.current(generation) && current.current.uid === owner && cloudAuth.currentUser?.uid === owner &&
      current.current.authGeneration === authGeneration && current.current.verified && current.current.snapshot?.sync.enabled === true && current.current.snapshot.sync.epoch === expectedEpoch;
    const retry = new SyncWorkQueue(async () => {
      if (!isCurrent() || document.hidden || !navigator.onLine || hasPendingEdits() || !current.current.games.length) return;
      if (committedRefresh) {
        ...
        if (acknowledged.receipt.operation === 'publish-ranking' && acknowledged.receipt.generation && head?.current?.generation === acknowledged.receipt.generation) fingerprint = attemptedSignature;
      ...
      if (!isCurrent() || !control?.enabled || control.deleted || local.sync.dirty || !local.sync.enabled || local.sync.epoch !== expectedEpoch) return;
      let projected = projectFriendRanking(local.state, control.selectedIds.filter((id) => !removed.has(id)), current.current.games);
      const signature = `${control.epoch}:${control.revision}:${JSON.stringify(projected.entries)}`;
      if (fingerprint === signature && !retry.reason) { setStatus('saved'); return; }
      reconnectSettings.current();
      const fresh = await store.settings(owner);
      if (!isCurrent()) return;
      if (!fresh?.enabled || fresh.deleted) { acceptSettings(fresh, owner); return; }
      control = fresh;
      acceptSettings(control, owner);
      projected = projectFriendRanking(local.state, control.selectedIds.filter((id) => !removed.has(id)), current.current.games);
      if (fingerprint === `${control.epoch}:${control.revision}:${JSON.stringify(projected.entries)}`) {
        retry.succeeded(true); setFailure(null); setStatus('saved'); return;
      }
      setStatus('saving');
      ...
      await store.publishRanking(owner, projected.entries, reviewed,
        { syncEpoch: local.sync.epoch, remoteRevision: local.sync.baseRemoteRevision }, head?.revision ?? 0, isCurrent);
      if (!isCurrent()) return;
      fingerprint = `${reviewed.epoch}:${reviewed.revision}:${JSON.stringify(projected.entries)}`;
      retry.succeeded(); setFailure(null); setStatus('saved');
    }, (cause) => {
      if (!isCurrent()) return;
      ...
      setFailure({ uid: owner, message: `${onlineError(cause)}${kind === 'blocked' ? '' : ' Friends sharing will retry automatically.'}` });
    });
    queue.current = retry;
    const wake = () => {
    ...
    return () => {
      alive = false; retry.dispose(); if (queue.current === retry) queue.current = null;
  ...
  const visibleStatus: FriendSharingStatus = !verified ? 'paused' : !ready ? 'checking' : !settings?.enabled ? 'off' : !snapshot?.sync.enabled ? 'paused' : !games.length ? 'checking' : snapshot.sync.dirty || pendingEdits ? 'pending' : status;
