...
export function CreatorPage({
  ...
  const openTarget = async (uid: string, known?: Member) => {
    ...
    setSelected({
      uid,
      displayName: known?.displayName ?? 'Opening profile…',
    ...
    setError('');
    try {
      const [member, publication, permissions] = await Promise.all([
        known ?? social.member(uid),
        social.ownProfile(uid),
        social.control(uid),
      ]);
      if (request !== selectedRequest.current) return;
      setSelected({
        uid,
        displayName: member?.displayName ?? publication?.displayName ?? 'Removed profile',
        avatar: member?.avatar ?? publication?.avatar ?? null,
        publicOnly: false,
      });
      ...
      const rows =
        publicOnly && publication
          ? (await social.entries(publication)).map(({ id, title, position, score }) => ({
              id,
              title,
              position,
              score,
      ...
      if (request === selectedRequest.current) setError(onlineError(cause));
    } finally {
      if (request === selectedRequest.current) setBusy(false);
    }
  };
  if (!allowed)
    return (
      <section className="app-page empty-state">
        ...
        <p>
          {verified
            ? 'This verified account is not authorized for the creator desk. Private member information is protected by server rules.'
            : 'Sign in and verify the owner account before accessing member information.'}
        </p>
  ...
  return (
    <section className="app-page creator-page" aria-labelledby="creator-title">
      <div className="page-heading">
        <div>
          <h1 id="creator-title" data-page-heading tabIndex={-1}>
            Creator desk
          </h1>
        </div>
      </div>
      <div className="personal-tabs">
        <button aria-pressed={tab === 'members'} onClick={() => setTab('members')}>
          Members
        </button>
      ...
      {tab === 'members' ? (
            ...
            {members.map((member) => (
              <li key={member.uid}>
                ...
                <button
                  ...
                  onClick={() => {
                    void openTarget(member.uid, member);
                  }}
                >
                  View ranking
            ...
            <li key={report.id}>
              ...
              <div className="button-row">
                <button
                  className="text-button"
                  onClick={() => {
                    void openTarget(report.targetUid);
                  }}
                >
                  Inspect profile
                </button>
                      ...
                      .then((removed) =>
                        setReports((previous) =>
                          removed
                            ...
                            : previous.map((item) => (item.id === report.id ? { ...item, status: 'resolved' } : item)),
                        ),
                      )
                ...
                >
                  {report.status === 'resolved' ? 'Remove resolved report' : 'Resolve and remove'}
                </button>
              </div>
            </li>
          ))}
        ...
        !error && (
          <div className="empty-state">
            ...
            <p>Members can keep one report per public profile open. Reporting alone never hides it.</p>
          </div>
        )
      )}
      {cursor && (
        <button
          className="button button-outline public-more"
          disabled={busy}
          onClick={() => {
            setBusy(true);
            void (
              tab === 'members'
                ? social.members(cursor).then((page) => {
                    setMembers((previous) => [...previous, ...page.members]);
                    setCursor(page.cursor);
                  })
                : social.reports(cursor).then((page) => {
                    setReports((previous) => [...previous, ...page.reports]);
                    setCursor(page.cursor);
                  })
            )
              .catch((cause) => setError(onlineError(cause)))
      ...
      {selected && (
        <Dialog
          open
          titleId="member-ranking-title"
          className="info-dialog creator-ranking-dialog"
          onClose={() => {
        ...
        >
          ...
          <ol className="publication-preview-list">
            {ranking.slice(0, limit).map((entry) => (
              <li key={entry.id}>
                <span>
          ...
          {!busy && !ranking.length && (
            <p>No current ranking snapshot is available. Moderation still applies to this account.</p>
          )}
          {limit < ranking.length && (
            <button className="text-button" onClick={() => setLimit((value) => value + 30)}>
              Show more ranked games
            </button>
          )}
          {control && (
            <div className="moderation-panel">
              <h3>Public-content moderation</h3>
              <p>
                {control.hidden
                  ? 'Publishing is paused for this account. Restoring permission does not republish a list automatically.'
                  : 'Hiding removes the public list from new reads and prevents this account from republishing until you restore permission.'}
              </p>
              {confirmHide ? (
                <div className="button-row">
                  <button className="button button-outline" disabled={busy} onClick={() => setConfirmHide(false)}>
                    Cancel moderation
                  </button>
                  <button
                    ...
                    onClick={() => {
                      setBusy(true);
                      void social
                        .moderate(selected.uid, !control.hidden)
                        .then(() => {
                          setControl({ ...control, hidden: !control.hidden, epoch: control.epoch + 1 });
                          setConfirmHide(false);
                        })
                        .catch((cause) => setError(onlineError(cause)))
                        .finally(() => setBusy(false));
                  ...
                  >
                    {control.hidden ? 'Restore publishing permission' : 'Hide and pause publishing'}
                  </button>
                </div>
              ) : (
                <button className="text-button danger-text" disabled={busy} onClick={() => setConfirmHide(true)}>
                  {control.hidden ? 'Restore publishing permission' : 'Hide public profile'}
                </button>
              )}
            </div>
          )}
