...
export function CreatorPage({ social, allowed, verified, onAccount }: { social: SocialStore; allowed: boolean; verified: boolean; onAccount: () => void }) {
  ...
  return (
    <section className="app-page creator-page" aria-labelledby="creator-title">
      <div className="page-heading"><div><h1 id="creator-title" data-page-heading tabIndex={-1}>Creator desk.</h1><p>Consenting member profiles, ranking summaries and public reports. Private notes and queues are not loaded in this view.</p></div></div>
      ...
      {tab === 'members' ? members.length ? <ul className="creator-members">{members.map((member) => <li key={member.uid}><Avatar descriptor={member.avatar} size={48} /><div><h2>{member.displayName}</h2><p>{member.rankCount} privately synced ranks · Updated {new Intl.DateTimeFormat(undefined, { dateStyle: 'medium' }).format(member.updatedAt)}</p></div><button className="text-button" onClick={() => { void openTarget(member.uid, member); }}>View ranking<Icon name="arrow" width="17" height="17" /><span className="sr-only">for {member.displayName}</span></button></li>)}</ul> : !busy && !error && <div className="empty-state"><h2>No member profiles yet.</h2><p>Only accounts that choose to save a profile or ranking appear here. There are no seeded or invented people.</p></div> :
