            ...
            {identity.verificationPending ? (
              ...
              <form
                onSubmit={(event) => {
                  ...
                  if (validName() && validChoice && remoteReady && cache && !busy)
                    void onConnect(selected, name.trim());
                }}
              >
                {head && (!head.enabled || head.deleted) && (
                  <p className="account-notice">
                    {head.deleted ? (
                      <>
                        Online saving is off because you deleted your online copy. Turning it on starts a new online
                        copy.{deletionState !== 'complete' && " It doesn't finish the earlier deletion."}
                      </>
                    ) : (
            ...
            <details className="account-danger">
              ...
              {identity.verified && (
                <button
                  ...
                  disabled={busy}
                  onClick={() => setConfirmation('delete-copy')}
                >
                  Delete online copy
                </button>
              )}
              <button
                className="text-button danger-text"
                disabled={busy}
                onClick={() => setConfirmation('delete-account')}
              >
      ...
      {confirmation && (
        ...
        >
          ...
          {cancelledRegistration && confirmation === 'delete-account' && (
            <p>
              This removes the cancelled sign-in and its account copy on this device. You can then register again with
              the same email. Your guest library stays here.
            </p>
          )}
