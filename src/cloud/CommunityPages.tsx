...
export function CommunityPage({ social, onOpen, onPublish }: { social: SocialStore; onOpen: (handle: string) => void; onPublish: () => void }) {
  ...
  const load = async (nextTerm: string, more = false) => {
    ...
    try {
      ...
      setResults((previous) => more ? [...new Map([...previous, ...page.profiles].map((profile) => [profile.uid, profile])).values()] : page.profiles);
      setCursor(page.cursor); setTerm(nextTerm);
    } catch (cause) { if (current === generation.current) setError(onlineError(cause)); }
    finally { if (current === generation.current) setBusy(false); }
  };
  useEffect(() => {
    let disposed = false;
    const current = ++generation.current;
    void social.directory(initial).then((page) => {
      if (!disposed && current === generation.current) { setResults(page.profiles); setCursor(page.cursor); setBusy(false); }
    }).catch((cause) => { if (!disposed) { setError(onlineError(cause)); setBusy(false); } });
...
export function PublicProfilePage({ social, handle, games, library, identity, onOpenRecord, onShare, onAccount, onFriend }: {
  ...
  useEffect(() => {
    let canceled = false;
    setBusy(true); setError(''); setSelected(new Set()); setVisible(30); setEntries([]); setProfile(null);
    void (async () => {
      const value = await social.profile(handle);
      const ranking = value ? await social.entries(value) : [];
      if (!canceled) { setProfile(value); setEntries(ranking); }
  ...
  const save = async (values: PublicEntry[]) => {
    try {
      ...
      if (await library.perform({ type: 'set-progress', records, key: 'later', value: true })) {
        setMessage(`${records.length} ${records.length === 1 ? 'game added' : 'games added'} to your play queue. The publisher's scores were not copied.`);
        setSelected(new Set());
  ...
  return (
    <section className="app-page public-profile-page" aria-labelledby="public-ranking-title">
      <div className="public-profile-heading"><Avatar descriptor={profile.avatar} size={80} /><div><p>{profile.displayName} <span>@{profile.handle}</span>{profile.creator && <strong className="creator-badge">Collection creator</strong>}</p><h1 id="public-ranking-title" data-page-heading tabIndex={-1}>{profile.title}</h1><p className="section-help">Personal preferences, not an official ranking. Published {new Intl.DateTimeFormat(undefined, { dateStyle: 'medium' }).format(profile.updatedAt)}.</p></div><button className="button button-outline" onClick={() => onShare(`${profile.title} by ${profile.displayName}`, `${location.origin}/u/${profile.handle}`)}><Icon name="share" width="18" height="18" />Share</button></div>
      <div className="public-ranking-tools"><p>{entries.length} games in this published snapshot. Saving adds games to your library, not their ratings or play history.</p><div className="button-row"><button className="text-button" onClick={() => setSelected(selected.size === entries.length ? new Set() : new Set(entries.map((entry) => entry.id)))}>{selected.size === entries.length ? 'Clear selection' : `Select all ${entries.length}`}</button><button className="button button-dark" disabled={!selected.size || library.busy} onClick={() => { void save(entries.filter((entry) => selected.has(entry.id))); }}>Save {selected.size || 'selected'} for later<Icon name="bookmark" width="18" height="18" /></button></div></div>
      {error && <p className="inline-error" role="alert">{error}</p>}{message && <p className="account-notice" role="status">{message}</p>}
      <ol className="public-ranking-list">{entries.slice(0, visible).map((entry) => <li key={entry.id}>
      ...
      {reporting && <Dialog open titleId="report-profile-title" onClose={() => setReporting(false)} className="info-dialog"><h2 id="report-profile-title" data-autofocus tabIndex={-1}>Report this profile</h2><p>The creator can review one report per account and profile. A report does not automatically hide anyone.</p><form onSubmit={(event) => {
        ...
        setBusy(true);
        void social.report(identity.uid, profile.uid, reason).then(() => { setReporting(false); setMessage('Your report was submitted for the creator to review.'); }).catch((cause) => setError(onlineError(cause))).finally(() => setBusy(false));
      }}><label htmlFor="profile-report-reason">What needs attention?</label><textarea id="profile-report-reason" name="report-reason" autoComplete="off" rows={4} required maxLength={400} value={reason} onChange={(event) => setReason(event.target.value)} /><p className="section-help">Do not include private contact details or sensitive information.</p>{error && <p className="inline-error" role="alert">{error}</p>}<button className="button button-dark" disabled={busy}>Submit report</button></form></Dialog>}
    </section>
  );
}
