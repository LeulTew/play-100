...
export function FriendShelfCards({ entries, status, error: operationError, onSave, onPin, onOpen, savedIds, renderArtwork }: FriendShelfCardsProps) {
  ...
  return <section className="friend-shelf-cards" aria-labelledby={`${id}-title`}>
    <div className="friend-shelf-heading"><h2 id={`${id}-title`}>Shared games</h2>{status === 'ready' && <span>{entries.length} games</span>}</div>
    {status === 'loading' ? <p role="status">Loading shared games...</p> : status === 'unavailable' ? <p>Shared games are unavailable.</p> : !entries.length ? <p>No games are selected for this shelf.</p> : <>
      <ul className="friend-shelf-grid">{entries.slice(0, count).map((entry) => <li key={entry.id}>
        {renderArtwork && <div className="friend-shelf-art">{renderArtwork(entry)}</div>}
      ...
      {count < entries.length && <button className="text-button" onClick={() => setCount((old) => old + 24)}>Show 24 more shared games</button>}
    </>}
