...
export function PublishPage({ social, identity, member, avatar, state, games, existing, isCreator, onAccount, onPublished }: {
  ...
  return (
    <section className="app-page publish-page" aria-labelledby="publish-title">
      <div className="page-heading"><div><h1 id="publish-title" data-page-heading tabIndex={-1}>{existing?.published ? 'Update your snapshot.' : 'Share your taste.'}</h1><p>Choose exactly what leaves your private library. Notes, email, queue and play history never appear in this snapshot.</p></div><Avatar descriptor={avatar} size={80} /></div>
      ...
      {!rows.length ? <div className="empty-state"><h2>A ranking comes first.</h2><p>Add games and your optional scores on My rankings. Nothing has been published.</p><a className="button button-dark" href="/my-rankings">Open My rankings</a></div> : <>
        ...
        <label className="check-control directory-consent"><input type="checkbox" checked={listed} onChange={(event) => setListed(event.target.checked)} disabled={busy} />Show in Community</label><p className="section-help">Unchecked means link-only, not private. Anyone who has the published link can read it.</p>
      ...
      {preview && <Dialog open titleId="publication-preview-title" className="info-dialog publish-preview-dialog" onClose={() => { if (!busy) setPreview(null); }}>
        <h2 id="publication-preview-title" data-autofocus tabIndex={-1}>Exactly what becomes public.</h2><div className="publication-identity"><Avatar descriptor={preview.avatar} size={64} /><div><strong>{preview.displayName}</strong><p>@{preview.handle}</p><h3>{preview.title}</h3></div></div><p>Anyone with the link can read or copy this ranking. Your notes, email and play queue stay out of it.</p>
