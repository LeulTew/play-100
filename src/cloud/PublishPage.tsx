...
function PublishDraft({ social, identity, member: incomingMember, avatar, state, games, existing: incomingProfile, isCreator, onAccount, onPublished }: PublishPageProps) {
  ...
  let handleIssue = '';
  if (handle) {
    try { normalizeHandle(handle); }
    catch (cause) { handleIssue = onlineError(cause); }
  }
  useEffect(() => {
  ...
  const rows = useMemo(() => state.ranking.flatMap((entry) => {
  ...
  const buildPreview = () => {
    try {
      ...
      if (!name.trim() || name.trim().length > 60 || !title.trim() || title.trim().length > 80) throw new Error('Choose a public name up to 60 characters and a ranking title up to 80 characters.');
      setPreview({ displayName: name.trim(), handle: normalizeHandle(handle), title: title.trim(), listed, avatar, entries: projectPublicRanking(state, selected, games), control });
  ...
  const publish = async () => {
    ...
    try {
      await social.saveMember(identity.uid, member?.displayName || preview.displayName, member?.avatar ?? preview.avatar);
      const result = await social.publish(identity.uid, { ...preview, creator: isCreator }, preview.control);
      onPublished(result);
    } catch (cause) { setError(onlineError(cause)); }
    finally { setBusy(false); }
  };
  if (!identity.verified) return <section className="app-page empty-state"><h1 data-page-heading tabIndex={-1}>Verify before publishing.</h1><p>Your current ranking is still private. Verify your sign-in email before making a public profile.</p><button className="button button-dark" onClick={onAccount}>Open Account</button></section>;
  return (
    <section className="app-page publish-page" aria-labelledby="publish-title">
      <div className="page-heading"><div><h1 id="publish-title" data-page-heading tabIndex={-1}>{existing?.published ? 'Update public ranking' : 'Publish ranking'}</h1></div><Avatar descriptor={avatar} size={80} /></div>
      ...
      {!rows.length ? <div className="empty-state"><h2>A ranking comes first.</h2><p>Add games and your optional scores on My rankings. Nothing has been published.</p><a className="button button-dark" href="/my-rankings">Open My rankings</a></div> : <>
        ...
        <button className="button button-dark" disabled={!selected.size || busy || Boolean(handleIssue) || !control || control.hidden || control.deleted} onClick={buildPreview}>Preview public snapshot<Icon name="arrow" width="18" height="18" /></button>
      </>}
      ...
      {preview && <Dialog open titleId="publication-preview-title" className="info-dialog publish-preview-dialog" onClose={() => { if (!busy) setPreview(null); }}>
        <h2 id="publication-preview-title" data-autofocus tabIndex={-1}>Public preview</h2><div className="publication-identity"><Avatar descriptor={preview.avatar} size={64} /><div><strong>{preview.displayName}</strong><p>@{preview.handle}</p><h3>{preview.title}</h3></div></div><p>Anyone with the link can view or copy these games and scores.</p>
        <ol className="publication-preview-list">{preview.entries.map((row) => <li key={row.id}><span>{row.position}. {row.title}<small className="preview-source">{row.year ?? 'Year not supplied'} · {row.source}{row.sourceUrl && <a href={row.sourceUrl} target="_blank" rel="noreferrer">Source<Icon name="up-right" width="13" height="13" /></a>}</small></span><strong>{row.score ?? '—'}</strong></li>)}</ol>
        <p className="section-help">{preview.listed ? 'This profile will also appear in Community.' : 'This is link-only and will not be listed in Community.'} This preview is frozen; later private edits are not added to it automatically.</p>
        <label className="check-control sync-consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} disabled={busy} /><span>I want this selected snapshot to be public.</span></label>
        {error && <p className="inline-error" role="alert">{error}</p>}<div className="button-row"><button className="button button-outline" disabled={busy} onClick={() => setPreview(null)}>Keep editing</button><button className="button button-dark" disabled={!consent || busy} onClick={() => { void publish(); }}>{busy ? 'Publishing snapshot…' : existing?.published ? 'Update published ranking' : 'Publish this ranking'}</button></div>
      </Dialog>}
      {confirmUnpublish && <Dialog open titleId="unpublish-title" onClose={() => { if (!busy) setConfirmUnpublish(false); }} className="info-dialog"><h2 id="unpublish-title">Unpublish this ranking?</h2><p>New server reads stop, including the Community listing. This cannot recall screenshots or games others already saved.</p><div className="button-row"><button data-autofocus className="button button-outline" disabled={busy} onClick={() => setConfirmUnpublish(false)}>Keep published</button><button className="button button-danger" disabled={busy || !control} onClick={() => {
        if (!control) return;
        setBusy(true);
        void social.unpublish(identity.uid, control).then(() => onAccount()).catch((cause) => setError(onlineError(cause))).finally(() => setBusy(false));
      }}>Unpublish now</button></div>{error && <p className="inline-error" role="alert">{error}</p>}</Dialog>}
    </section>
  );
}
