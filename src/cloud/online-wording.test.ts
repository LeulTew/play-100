...
describe('online service and saved-copy wording', () => {
  it('names a reached service limit the same way in errors, friend outcomes and the saving status', () => {
    expect(onlineError({ code: 'resource-exhausted' })).toBe(
    ...
    expect(friendMutationError({ code: 'resource-exhausted' })).toBe(
      'The online service has reached a limit. Wait, then refresh to check whether the change was saved.',
    );
    expect(SYNC_LABELS.quota).toBe('Waiting for the online service…');
    for (const text of [
      onlineError({ code: 'resource-exhausted' }),
      friendMutationError({ code: 'resource-exhausted' }),
      SYNC_LABELS.quota,
    ]) {
  ...
  it('describes the account copy and the online copy without storage protocol nouns', () => {
    expect(onlineError({ code: 'auth/user-token-expired' })).toBe(
