    ...
    super(
      {
        ...
        pairs:
          "You've reached 1,000 connections and requests. Cancel a request, remove a connection, or try again later.",
      }[kind],
    );
...
export async function readQuotaSlots(
  tx: Transaction,
...
export function occupyQuotaSlot(
  tx: Transaction,
  ref: DocumentReference<DocumentData>,
  slots: QuotaSlots,
  id: string,
...
export function releaseQuotaSlot(tx: Transaction, ref: DocumentReference<DocumentData>, slots: QuotaSlots, id: string) {
  if (slots.ids.includes(id))
    tx.update(ref, { ids: slots.ids.filter((value) => value !== id), revision: slots.revision + 1 });
}

export async function requireVisibleCapacity<Cursor>(
  kind: AccountQuotaKind,
  read: (cursor?: Cursor) => Promise<{ items: readonly unknown[]; scanned?: number; cursor: Cursor | undefined }>,
) {
  let count = 0;
