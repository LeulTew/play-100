...
export interface GoogleReturn {
  attempted: boolean;
  intent: GoogleRedirectIntent | null;
  uid: string | null;
  completed: boolean;
  error: string;
  message: string;
...
export async function startGoogleRedirect(auth: Auth, request: GoogleRequest): Promise<never> {
  ...
  if ((auth.currentUser?.uid ?? null) !== request.uid)
    throw new Error('The signed-in account changed. Start the Google action again from Account.');
...
export function finishGoogleRedirect(auth: Auth): Promise<GoogleReturn> {
  ...
  const task = (async (): Promise<GoogleReturn> => {
    ...
    const outcome: GoogleReturn = {
      attempted: stored.raw !== null,
    ...
    try {
      ...
      if (result) {
        ...
        validateGoogleReturn(
          stored.intent,
          { uid: result.user.uid, providerId: result.providerId, operationType: result.operationType },
          auth.currentUser?.uid ?? null,
        );
        outcome.uid = result.user.uid;
        outcome.completed = true;
        outcome.error = '';
      } else if (stored.intent) {
        outcome.message =
          stored.intent.kind === 'reauthenticate'
            ? 'Google confirmation was not completed. Nothing has been deleted.'
            : stored.intent.kind === 'link'
              ? 'Google linking was not completed. Your existing account is unchanged.'
    ...
    try {
      ...
      outcome.error = onlineError(cause);
