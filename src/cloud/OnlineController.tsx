...
export default function OnlineController({ page, publicHandle, invitation, showSheet, signInPurpose, guest, games, onBridge, onCloseSheet, getSignInReturnFocus, onNavigate, onProfile, onOpenRecord, onShare, onPinRecord, artwork }: {
  ...
  useEffect(() => {
    ...
    if (intent.kind === 'sign-in') {
      ...
      if (!['publish', 'creator', 'friends', 'friend', 'invite', 'compare', 'friend-sharing', 'friend-shelf'].includes(navigation.current.page)) navigation.current.onNavigate('account');
  ...
  const afterSignIn = async (user: User) => {
    ...
    if (!['publish', 'creator', 'friends', 'friend', 'invite', 'compare', 'friend-sharing', 'friend-shelf'].includes(page)) onNavigate('account');
  };
      ...
      {cloudPage && (restoring ? <div className="page-loading" role="status"><h1>Restoring account…</h1></div> :
        ...
        page === 'friend-shelf' && friendIdentity ? <section className="app-page shared-games-page"><div className="page-heading"><h1 data-page-heading tabIndex={-1}>Shared games</h1><button className="text-button" onClick={() => onNavigate('account')}>Account</button></div>
          <FriendShelfEditor key={`${scope}:${authSessionEpoch.current}`} state={account.snapshot?.state ?? emptyPersonalLibrary()} games={games} config={shelf.config} identity={friendIdentity}
            connected={Boolean(identity.verified && account.snapshot?.sync.enabled)} status={shelf.status} error={shelf.error}
            onPrepare={async () => {
              const session = authSessionEpoch.current;
              const settings = await prepareFriendIdentity(friends.store, friendIdentity);
              if (cloudAuth.currentUser?.uid !== friendIdentity.uid || authSessionEpoch.current !== session) throw new Error('The account changed. Preview these games again.');
              friends.acceptSettings(settings);
              const config = await shelf.store.initialize(friendIdentity.uid);
              if (cloudAuth.currentUser?.uid !== friendIdentity.uid || authSessionEpoch.current !== session) throw new Error('The account changed. Preview these games again.');
              return config;
            }}
            onSave={shelf.saveSelection} onStop={shelf.stopSharing} onRetry={shelf.retry}
            renderArtwork={(entry) => <GameArtwork record={recordFromFriendShelf(entry, games)} artwork={artwork?.get(entry.id)} />} />
          <ShelfArtworkCredits records={Object.values(account.snapshot?.state.records ?? {})} artwork={artwork} />
        </section> :
