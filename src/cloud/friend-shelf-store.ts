...
import {
  FriendStoreError, friendUid, parseFriendGeneration, parseFriendRegistry, parseFriendSource, retainsFriendGeneration,
...
function active(value: FriendShelfConfig | null): FriendShelfConfig {
  ...
  if (value.deleted) throw new FriendStoreError('deleted', 'This account is being deleted. Shared games cannot be enabled.');
  return value;
}
...
function errorValue(cause: unknown): Error { return cause instanceof Error ? cause : new Error('Shared games could not be refreshed.'); }

export class FriendShelfStore {
  constructor(readonly db: Firestore) {}
  private ref(name: string, uid: string): DocumentReference<DocumentData> { return doc(this.db, name, friendUid(uid)); }
  ...
  private async confirmed<T>(ref: DocumentReference<DocumentData>, parse: (data: DocumentData) => T): Promise<T | null> {
    return runTransaction(this.db, async (tx) => {
      const snapshot = await tx.get(ref);
      return snapshot.exists() ? parse(snapshot.data()) : null;
    }, { maxAttempts: 3 });
  }
  private async afterCommit<T>(receipt: FriendShelfReceipt, operation: () => Promise<T>, phase: 'refresh' | 'cleanup' = 'refresh'): Promise<T> {
    try { return await operation(); }
    catch (cause) { throw new FriendShelfCommittedError(receipt, cause, phase); }
  }
  private watch<T>(ref: DocumentReference<DocumentData>, parse: (data: DocumentData) => T, next: (value: T | null) => void, error: (cause: Error) => void): () => void {
    return onSnapshot(ref, { includeMetadataChanges: true }, (snapshot) => {
      ...
      catch (cause) { error(errorValue(cause)); }
    }, error);
  }
  config(uid: string): Promise<FriendShelfConfig | null> { return this.read(this.ref('friendShelfSettings', uid), parseFriendShelfConfig); }
  ...
  async initialize(uid: string): Promise<FriendShelfConfig> {
    online(); await ensureAccountActivity(this.db, friendUid(uid));
  ...
  async shelf(uid: string): Promise<FriendShelf> {
    ...
    const entries: FriendShelfEntry[] = []; const manifest = head.current;
    if (manifest.count) {
      const chunks = await getDocsFromServer(query(collection(this.db, 'friendShelves', uid, 'generations', manifest.generation, 'chunks'), orderBy('index'), limit(FRIEND_SHELF_CHUNK_LIMIT)));
      if (chunks.size !== Math.ceil(manifest.count / FRIEND_SHELF_CHUNK_SIZE)) throw new FriendStoreError('unavailable', 'The shared shelf is incomplete. Refresh it.');
      chunks.docs.forEach((chunk, index) => {
        if (chunk.id !== String(index)) throw new FriendStoreError('invalid', 'Shared games have inconsistent chunk identities.');
        entries.push(...parseFriendShelfChunk(chunk.data(), index, manifest.count));
      });
    }
    if (new Set(entries.map((entry) => entry.id)).size !== entries.length || await friendShelfDigest(entries) !== manifest.digest) throw new FriendStoreError('invalid', 'Shared games failed their integrity check.');
    const latest = await this.head(uid);
    if (latest?.revision !== head.revision || latest.current?.generation !== manifest.generation) conflict('Shared games changed while loading. Refresh them.');
    return { head, entries };
  }
  async publish(uid: string, input: FriendShelfEntry[], expected: FriendShelfConfig, sourceInput: FriendSourceRevision, expectedHeadRevision: number, isCurrent: () => boolean): Promise<{ changed: boolean; head: FriendShelfHead }> {
    ...
    const checkSource = (data: DocumentData | undefined) => {
      ...
      if (!sync?.enabled || sync.deleted || sync.epoch !== source.syncEpoch || sync.revision !== source.remoteRevision) conflict('Wait for the current private online copy to finish saving.');
    };
    guard(); await this.config(uid); guard();
    const digest = await friendShelfDigest(entries);
    const configRef = this.ref('friendShelfSettings', uid); const headRef = this.ref('friendShelfHeads', uid); const syncRef = this.ref('syncHeads', uid);
    const prior = await runTransaction(this.db, async (tx) => {
      ...
      expectedConfig(config.exists() ? parseFriendShelfConfig(config.data()) : null, expected); checkSource(sync.data());
    ...
    await this.prune(uid); guard();
    const id = crypto.randomUUID(); const generationRef = doc(this.db, 'friendShelves', uid, 'generations', id); const registryRef = this.ref('friendShelfRegistry', uid);
    await runTransaction(this.db, async (tx) => {
      guard();
      ...
      if (ids.length >= 3) throw new FriendStoreError('limit', 'A shelf update is still pending. Shared games will retry after the upload expires.');
      guard();
      tx.set(registryRef, { ids: [...ids, id], revision: registry.exists() ? registry.data().revision + 1 : 1 });
      tx.set(generationRef, { epoch: expected.epoch, settingsRevision: expected.revision, source, count: entries.length, digest, uploaded: 0, ids: [], status: entries.length ? 'staging' : 'ready', createdAt: serverTimestamp() });
    });
    for (let index = 0; index < Math.ceil(entries.length / FRIEND_SHELF_CHUNK_SIZE); index += 1) {
      ...
      batch.update(generationRef, { uploaded: index + 1, ids: entries.slice(0, (index + 1) * FRIEND_SHELF_CHUNK_SIZE).map((entry) => entry.id), status: (index + 1) * FRIEND_SHELF_CHUNK_SIZE >= entries.length ? 'ready' : 'staging' });
      await batch.commit();
    }
    await runTransaction(this.db, async (tx) => {
      guard();
      const [config, head, generation, sync] = await Promise.all([tx.get(configRef), tx.get(headRef), tx.get(generationRef), tx.get(syncRef)]);
      expectedConfig(config.exists() ? parseFriendShelfConfig(config.data()) : null, expected); checkSource(sync.data());
      const current = head.exists() ? parseFriendShelfHead(head.data()) : null;
      ...
      tx.set(headRef, { format: 1, epoch: expected.epoch, settingsRevision: expected.revision, source, revision: expectedHeadRevision + 1, current: { generation: id, digest, count: entries.length }, previous: current?.current ?? null, updatedAt: serverTimestamp() });
    });
  ...
  cleanupSharing(uid: string): Promise<number> { return this.cleanupGenerations(uid, false); }
  private async cleanupGenerations(uid: string, preserveHead: boolean): Promise<number> {
    ...
    const ids = parseFriendRegistry(snapshot.data()); let deleted = 0;
    for (const id of ids) {
      ...
      const removable = await runTransaction(this.db, async (tx) => {
        ...
        if (control?.enabled && !control.deleted && gen.epoch === control.epoch && gen.settingsRevision === control.revision && gen.status !== 'deleting' && gen.status !== 'published' && gen.createdAt + 300000 > Date.now()) return false;
      ...
      await runTransaction(this.db, async (tx) => {
        ...
        tx.delete(ref); tx.update(registryRef, { ids: current.filter((value) => value !== id), revision: registry.data().revision + 1 });
      });
      deleted += 1;
    }
    return deleted;
  ...
  async exportOwn(uid: string): Promise<{ format: 1; config: FriendShelfConfig | null; shelf: FriendShelf | null }> {
    const config = await this.config(uid); const head = await this.head(uid);
    const shelf = head?.current && config?.enabled && !config.deleted && config.epoch === head.epoch && config.revision === head.settingsRevision ? await this.shelf(uid) : null;
