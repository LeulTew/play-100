...
import {
  ...
  FRIEND_ALL_LIMIT,
  FRIEND_ALL_PAGE_SIZE,
  friendAllEntrySignature,
  planFriendAllChanges,
} from '../lib/friend-all';
import {
  FRIEND_ALL_OWNER_PAGE,
  FRIEND_ALL_TRACKED_WRITE_GROUP,
  FRIEND_ALL_WRITE_GROUP,
  friendAllId,
  parseFriendAllEntry,
  parseFriendAllHead,
  parseFriendAllJob,
  parseFriendAllPolicy,
  parseFriendAllRow,
} from '../lib/friend-all-transport';
import type { FriendAllHead, FriendAllJob } from '../lib/friend-all-transport';
import { FriendStoreError, friendUid, parseFriendSettings, parseFriendSource } from '../lib/friend-types';
...
export interface FriendAllProgress {
  kind: FriendAllKind;
  applied: number;
  total: number;
  targetCount: number;
...
export class FriendAllCommittedError extends FriendStoreError {
  ...
  constructor(
    readonly uid: string,
    readonly operation: 'policy' | 'publish',
    readonly cause: unknown,
    readonly kind?: FriendAllKind,
  ) {
    super(
      'committed-refresh-failed',
      'The sharing change was acknowledged. Refresh its status instead of repeating the change.',
    );
    this.name = 'FriendAllCommittedError';
  }
}
function conflict(message = 'The account or sharing controls changed. Refresh before continuing.'): never {
  throw new FriendStoreError('conflict', message);
}
function online() {
  if (typeof navigator !== 'undefined' && navigator.onLine === false)
    throw new FriendStoreError('offline', 'Reconnect before changing automatic friend sharing.');
}
...
function policyMatches(policy: FriendAllPolicy, controls: FriendAllControls): boolean {
  return (
    ...
    controls.policy.revision === policy.revision &&
    controls.policy.enabled &&
    !controls.policy.deleted &&
    controls.ranking?.enabled === true &&
    !controls.ranking.deleted &&
    !controls.ranking.selectedIds.length &&
    controls.ranking.epoch === policy.ranking.epoch &&
    controls.ranking.revision === policy.ranking.revision &&
    controls.shelf?.enabled === true &&
    !controls.shelf.deleted &&
    !controls.shelf.selectedIds.length &&
    controls.shelf.epoch === policy.shelf.epoch &&
    controls.shelf.revision === policy.shelf.revision &&
    controls.shelf.consentSyncEpoch === policy.syncEpoch
  );
}
...
async function digest(entries: readonly FriendAllEntry[]): Promise<string> {
  ...
  return Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', encoded)), (byte) =>
...
export class FriendAllStore {
  ...
  private policyRef(uid: string) {
    return doc(this.db, 'friendAllPolicies', friendUid(uid));
  }
  private headRef(uid: string, kind: FriendAllKind) {
    return doc(this.db, 'friendAllHeads', friendUid(uid), 'views', kind);
  }
  private jobRef(uid: string, kind: FriendAllKind) {
    return doc(this.db, 'friendAllJobs', friendUid(uid), 'views', kind);
  }
  private rows(uid: string, kind: FriendAllKind) {
    return collection(this.db, kind === 'games' ? 'friendAllGames' : 'friendAllRankings', friendUid(uid), 'entries');
  }
  ...
  private async contended<T>(
    uid: string,
    kind: FriendAllKind,
    seen: () => FriendAllMark | null,
    write: () => Promise<T>,
  ): Promise<T> {
    try {
      return await write();
      ...
      const before = seen();
      if (!before || !cause || typeof cause !== 'object' || !('code' in cause) || cause.code !== 'permission-denied')
        throw cause;
      const after = await Promise.all([
        this.head(uid, kind),
        this.read(this.jobRef(uid, kind), parseFriendAllJob),
      ]).then(
        ([head, job]) => markOf(head, job),
        () => null,
      );
      if (
        after &&
        (after.revision !== before.revision || after.token !== before.token || after.applied !== before.applied)
      )
        conflict('Another tab advanced the sharing update.');
      throw cause;
    }
  }
  private watch<T>(
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        ...
        try {
          ...
          error(cause instanceof Error ? cause : new Error('Automatic sharing could not be read.'));
        }
      },
      error,
    );
  ...
  policy(uid: string) {
    return this.read(this.policyRef(uid), (value) => parseFriendAllPolicy(value, uid));
  }
  ...
  head(uid: string, kind: FriendAllKind) {
    return this.read(this.headRef(uid, kind), parseFriendAllHead);
  }
  async progress(uid: string, kind: FriendAllKind, epoch?: number): Promise<FriendAllProgress | null> {
    ...
    return job && (epoch === undefined || epoch === job.epoch)
      ? {
          kind,
          applied: job.applied,
          total: job.total,
          ...
          ready:
            head?.status === 'ready' &&
            head.epoch === job.epoch &&
            head.digest === job.digest &&
            head.source.remoteRevision === job.source.remoteRevision,
        }
      : null;
  }
  watchHead(
    uid: string,
    kind: FriendAllKind,
    next: (value: FriendAllHead | null) => void,
    error: (cause: Error) => void,
  ) {
  ...
  async controls(uid: string): Promise<FriendAllControls> {
    const [policy, ranking, shelf] = await Promise.all([
      this.policy(uid),
      this.read(doc(this.db, 'friendSettings', uid), parseFriendSettings),
  ...
  async setPolicy(
    ...
    isCurrent: () => boolean,
  ): Promise<FriendAllPolicy | null> {
    if (origin === 'default' && !enabled)
      throw new FriendStoreError('invalid', 'Default sharing can only initialize an eligible new setup.');
    const guard = () => {
      online();
      if (!isCurrent()) conflict();
    };
    guard();
    await this.policy(uid);
    guard();
    const ref = this.policyRef(uid);
    const rankingRef = doc(this.db, 'friendSettings', uid);
    const shelfRef = doc(this.db, 'friendShelfSettings', uid);
    const changed = await runTransaction(this.db, async (tx) => {
      ...
      const [policySnap, rankingSnap, shelfSnap, syncSnap, shelfHead] = await Promise.all([
        ...
        tx.get(rankingRef),
        tx.get(shelfRef),
        tx.get(doc(this.db, 'syncHeads', uid)),
        tx.get(doc(this.db, 'friendShelfHeads', uid)),
      ]);
      ...
      tx.set(shelfRef, {
        ...
        deleted: false,
        selection: '',
        consentSyncEpoch: next ? epoch : null,
        ...shelf,
        updatedAt: serverTimestamp(),
      });
      if (shelfHead.exists())
        tx.update(shelfHead.ref, {
          revision: parseFriendShelfHead(shelfHead.data()).revision + 1,
          updatedAt: serverTimestamp(),
        });
      tx.set(ref, {
        format: 2,
        ...
        deleted: false,
        origin,
        epoch: (old.policy?.epoch ?? 0) + 1,
        revision: (old.policy?.revision ?? 0) + 1,
        syncEpoch: epoch,
        ranking,
        shelf,
        updatedAt: serverTimestamp(),
    ...
    try {
      guard();
      return await this.confirmed(ref, (value) => parseFriendAllPolicy(value, uid));
    } catch (cause) {
  ...
  ): Promise<FriendAllPage> {
    const head = await this.head(uid, kind);
    if (!head || head.status !== 'ready')
      throw new FriendStoreError('unavailable', 'This shared account view is still updating or unavailable.');
    if (expectedRevision !== undefined && head.revision !== expectedRevision)
      conflict('The shared list changed. Refresh its first page.');
    const result = await getDocsFromServer(
      query(
        this.rows(uid, kind),
        ...(head.format === 3 ? [where('format', '==', 3)] : []),
        where('epoch', '==', head.epoch),
        where('active', '==', true),
        orderBy(kind === 'games' ? 'entry.title' : 'entry.position'),
        orderBy(documentId()),
        ...(cursor ? [startAfter(cursor)] : []),
        limit(FRIEND_ALL_PAGE_SIZE),
      ),
    );
    const entries = result.docs.map((row) => {
      const value = parseFriendAllRow(row.data(), row.id, kind);
      if (!value.active || !value.entry || value.epoch !== head.epoch || value.format !== head.format)
        throw new FriendStoreError('invalid', 'The shared page contains an invalid entry.');
      return value.entry;
    });
  ...
  async exact(
  ...
  ): Promise<{ head: FriendAllHead; entries: FriendAllEntry[]; resolvedIds: string[] }> {
    if (!input.length || input.length > FRIEND_ALL_EXACT_LIMIT || new Set(input).size !== input.length)
      throw new FriendStoreError('invalid', 'Choose one to six distinct game identities.');
    ...
    const entries = rows.flatMap((row) => {
      ...
      if (!value.entry || value.epoch !== head.epoch || value.format !== head.format || !ids.includes(value.entry.id))
        throw new FriendStoreError('invalid', 'The shared lookup contains an invalid entry.');
      return [value.entry];
    });
    await this.requireSameHead(uid, kind, head);
  ...
  private async requireSameHead(uid: string, kind: FriendAllKind, expected: FriendAllHead) {
    ...
    if (
      ...
      latest.digest !== expected.digest
    )
      conflict('The shared account view changed while reading. Refresh it.');
  }
  private async inventory(
    uid: string,
    kind: FriendAllKind,
    epoch: number,
  ): Promise<{ job: FriendAllJob | null; entries: FriendAllEntry[] }> {
    const before = await this.read(this.jobRef(uid, kind), parseFriendAllJob);
    const entries: FriendAllEntry[] = [];
    ...
    do {
      const page = await getDocsFromServer(
        query(
          ...
          orderBy(documentId()),
          ...(cursor ? [startAfter(cursor)] : []),
          limit(FRIEND_ALL_OWNER_PAGE),
        ),
      );
      for (const item of page.docs) {
        const row = parseFriendAllRow(item.data(), item.id, kind);
        if (before?.format === 3 && row.format === 2) continue;
        if (!row.entry || !row.active || row.epoch !== epoch)
          throw new FriendStoreError('invalid', 'The sharing inventory is inconsistent.');
        entries.push(row.entry);
      }
      if (entries.length > FRIEND_ALL_LIMIT)
        throw new FriendStoreError('limit', 'The shared account inventory exceeds its supported limit.');
    ...
    if (entries.length !== (after?.epoch === epoch ? after.count : 0))
      throw new FriendStoreError('invalid', 'The sharing count does not match its stored inventory.');
  ...
  private async releaseRows(uid: string, kind: FriendAllKind, rows: readonly QueryDocumentSnapshot<DocumentData>[]) {
    ...
    for (const known of rows.filter((row) => row.data().format !== 2))
      await runTransaction(this.db, async (tx) => {
        ...
        if (!job || job.format !== 3 || job.count < 1)
  ...
  private async prepareEpoch(uid: string, kind: FriendAllKind, epoch: number, guard: () => void) {
    ...
    for (;;) {
      guard();
      const stale = await getDocsFromServer(
        query(this.rows(uid, kind), where('epoch', '<', epoch), limit(FRIEND_ALL_OWNER_PAGE)),
      );
  ...
  private async pruneInactive(uid: string, kind: FriendAllKind, guard: () => void) {
    for (;;) {
      guard();
      const inactive = await getDocsFromServer(
        query(this.rows(uid, kind), where('active', '==', false), limit(FRIEND_ALL_OWNER_PAGE)),
      );
  ...
  private async pruneLegacy(uid: string, kind: FriendAllKind, guard: () => void) {
    for (let page = 0; page < 200; page += 1) {
      ...
      const legacy = await getDocsFromServer(
        query(this.rows(uid, kind), where('format', '==', 2), limit(FRIEND_ALL_OWNER_PAGE)),
      );
  ...
  async publish(
    uid: string,
    kind: FriendAllKind,
    input: readonly FriendAllEntry[],
    policy: FriendAllPolicy,
    sourceInput: FriendSourceRevision,
    isCurrent: () => boolean,
    progress?: (value: FriendAllProgress) => void,
  ): Promise<FriendAllHead> {
    const guard = () => {
      online();
      if (!isCurrent()) conflict();
    };
    ...
    const entries = input
      .map((value) => parseFriendAllEntry(value, kind))
    ...
    if (entries.length > FRIEND_ALL_LIMIT || new Set(entries.map((entry) => entry.id)).size !== entries.length)
      throw new FriendStoreError(
        'limit',
        'Automatic sharing supports 10,000 distinct account games without truncation.',
      );
    ...
    const cacheKey = `${uid}:${kind}`;
    const cached = this.cache.get(cacheKey);
    const policyRef = this.policyRef(uid);
    const headRef = this.headRef(uid, kind);
    const jobRef = this.jobRef(uid, kind);
    const syncRef = doc(this.db, 'syncHeads', uid);
    const checkSource = (data: DocumentData | undefined) => {
      const head = data ? parseHead(data) : null;
      if (!head?.enabled || head.deleted || head.epoch !== source.syncEpoch || head.revision !== source.remoteRevision)
        conflict('Wait for the current account copy to finish saving before sharing.');
    };
    if (
      publicHead?.format === 3 &&
      publicHead.status === 'ready' &&
      publicHead.epoch === policy.epoch &&
      publicHead.policyRevision === policy.revision &&
      publicHead.digest === targetDigest &&
      sameShelfSource(publicHead.source, source)
    ) {
      return runTransaction(this.db, async (tx) => {
        guard();
        const [head, sync, control, ranking, shelf] = await Promise.all([
          tx.get(headRef),
          tx.get(syncRef),
          tx.get(policyRef),
          tx.get(doc(this.db, 'friendSettings', uid)),
          tx.get(doc(this.db, 'friendShelfSettings', uid)),
        ]);
        ...
        const confirmedControls = {
          ...
          ranking: ranking.exists() ? parseFriendSettings(ranking.data()) : null,
          shelf: shelf.exists() ? parseFriendShelfConfig(shelf.data()) : null,
        };
        if (
          !policyMatches(policy, confirmedControls) ||
          current?.status !== 'ready' ||
          current.revision !== publicHead.revision
        )
          conflict();
    ...
    const initial =
      cached?.epoch === policy.epoch &&
      observedJob?.epoch === policy.epoch &&
      observedJob.applied === observedJob.total &&
      cached.digest === observedJob.digest
        ? { entries: cached.entries, job: observedJob }
        : await this.inventory(uid, kind, policy.epoch);
    guard();
    const operationsFor = (format: 2 | 3) => {
      const changes = planFriendAllChanges(format === 3 && initial.job?.format !== 3 ? [] : initial.entries, entries);
      return [
        ...changes.removals.map((id) => ({ id, entry: null })),
        ...changes.upserts.map((entry) => ({ id: entry.id, entry })),
      ];
    };
    let seen: FriendAllMark | null = null;
    let beganAt = 0;
    const begin = (format: 2 | 3) =>
      this.contended(
        uid,
        kind,
        () => seen,
        () =>
          runTransaction(this.db, async (tx) => {
            seen = null;
            guard();
            const [control, currentHead, currentJob, sync] = await Promise.all([
              tx.get(policyRef),
              tx.get(headRef),
              tx.get(jobRef),
              tx.get(syncRef),
            ...
            if (
              head?.format === format &&
              head.status === 'updating' &&
              old?.format === format &&
              old.epoch === policy.epoch &&
              old.policyRevision === policy.revision &&
              old.digest === targetDigest &&
              sameShelfSource(old.source, source) &&
              old.total - old.applied === operations.length
            ) {
              beganAt = head.revision;
              return old;
            }
            const revision = (head?.revision ?? 0) + 1;
            const next: FriendAllJob = {
              format,
              epoch: policy.epoch,
              ...
              count: format === 3 ? (old?.format === 3 ? old.count : 0) : initial.entries.length,
    ...
    if (
      job.format === 3 &&
      (initial.job?.format !== 3 || (publicHead?.status === 'updating' && initial.job.applied === 0))
    ) {
    ...
    const present = new Set(
      job.format === 3 && initial.job?.format !== 3 ? [] : initial.entries.map((entry) => entry.id),
    );
    ...
    for (let index = 0; index < operations.length; index += writeGroup) {
      ...
      for (const [offset, operation] of group.entries()) {
        if (operation.entry && !present.has(operation.id)) {
          count += 1;
          present.add(operation.id);
        }
        if (!operation.entry && present.has(operation.id)) {
          count -= 1;
          present.delete(operation.id);
        }
        ...
        if (job.format === 3 && !operation.entry) batch.delete(ref);
          ...
          batch.set(ref, {
            ...
            epoch: policy.epoch,
            token: job.token,
            step: job.applied + offset + 1,
            active: operation.entry !== null,
            entry: operation.entry,
          });
      }
      const applied = job.applied + group.length;
      const last = group.map((operation) => operation.id);
      ...
      if (applied % 50 === 0 || applied === job.total)
        progress?.({ kind, applied, total: job.total, targetCount: job.targetCount, ready: false });
    }
    guard();
      ...
      () =>
        runTransaction(this.db, async (tx) => {
          ...
          if (
            ...
            latest.revision !== policy.revision ||
            !complete ||
            complete.token !== job.token ||
            complete.applied !== complete.total ||
            complete.count !== entries.length ||
            !head
          )
            conflict();
          if (
            head.status === 'ready' &&
            head.digest === targetDigest &&
            head.epoch === policy.epoch &&
            sameShelfSource(head.source, source)
          )
            return;
    ...
    try {
      ...
      if (
        ...
        committed.digest !== targetDigest ||
        committed.epoch !== policy.epoch
      )
        conflict();
      this.cache.set(cacheKey, { epoch: policy.epoch, digest: targetDigest, entries });
      ...
      throw new FriendAllCommittedError(uid, 'publish', cause, kind);
    }
  }
  async cleanupPage(uid: string, kind: FriendAllKind): Promise<{ deleted: number; done: boolean }> {
    ...
    const rows = await getDocsFromServer(
      query(this.rows(uid, kind), orderBy(documentId()), limit(FRIEND_ALL_OWNER_PAGE)),
    );
    if (!rows.empty) {
  ...
  async exportOwn(uid: string) {
    ...
    const views = await Promise.all(
      (['games', 'ranking'] as const).map(async (kind) => {
        const head = await this.head(uid, kind);
  ...
  async revokeForDeletion(uid: string) {
    ...
    await runTransaction(this.db, async (tx) => {
      ...
      if (rankControl && !rankControl.deleted)
        tx.set(ranking.ref, {
          format: 1,
          enabled: false,
          deleted: true,
          selection: '',
          epoch: rankControl.epoch + 1,
          revision: rankControl.revision + 1,
          updatedAt: serverTimestamp(),
        });
      if (shelfControl && !shelfControl.deleted) {
        tx.set(shelf.ref, {
          ...
          epoch: shelfControl.epoch + 1,
          revision: shelfControl.revision + 1,
          updatedAt: serverTimestamp(),
        });
        if (shelfHead.exists())
          tx.update(shelfHead.ref, {
