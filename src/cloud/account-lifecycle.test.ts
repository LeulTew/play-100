...
const calls = vi.hoisted(() => ({
  read: vi.fn(), transaction: vi.fn(), deleteDevice: vi.fn(async () => {}),
}));
vi.mock('firebase/firestore', async original => ({
  ...await original<typeof import('firebase/firestore')>(),
  getDocFromServer: calls.read, runTransaction: calls.transaction,
}));
...
const token: IdTokenResult = {
  authTime: '', issuedAtTime: '', expirationTime: '', signInProvider: 'password', signInSecondFactor: null,
  token: 'synthetic-unit-token', claims: { email_verified: true },
};
...
describe('cancelled verified sign-in recovery', () => {
  ...
  it('keeps active registrations on the normal cleanup path and denies stale scope or unverified removal', async () => {
    const identity = user(); const scope = accountScope(identity.uid, 'demo-play100');
  ...
  it('does not delete when lifecycle reading fails or the identity changes during it', async () => {
    ...
    calls.read.mockImplementationOnce(async () => {
      current = false; return { exists: () => true, data: () => ({ state: 'cancelled' }) };
    });
