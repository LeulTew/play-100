...
export function FriendSharedGames({
  ...
  onOpen: (record: LibraryRecord, authority?: PreviewAuthority) => void;
  onPin?: (record: LibraryRecord) => boolean;
  artwork?: ReadonlyMap<string, CatalogArtwork>;
}) {
  const view = useFriendSharedView(
    friends,
    store,
    uid,
    peer,
    'games',
    verified && games.length > 0,
    null,
    authGeneration,
  );
  const preview = useMemo(
    () => createShelfPreviewAuthority(accountScope(uid, store.db.app.options.projectId), peer, authGeneration),
    [uid, peer, store, authGeneration],
  );
  useLayoutEffect(() => {
    preview.update(view.status === 'ready' ? view.entries.map((entry) => entry.id) : []);
  ...
  const currentRecord = (id: string) => {
    ...
    if (!live.current || cloudAuth.currentUser?.uid !== uid || latest.current.status !== 'ready' || !entry) {
      ...
      throw new Error(
        latest.current.status === 'unavailable'
    ...
    <>
      <FriendShelfCards
        ...
        onSave={async (entry) => {
          ...
          if (!(await library.perform({ type: 'add-records', records: [record] })))
            throw new Error('This game could not be saved. Your existing library is unchanged.');
        }}
        onPin={(entry) => {
          if (!onPin?.(currentRecord(entry.id)))
            throw new Error('This game was not pinned. Check the Compare tray limit or account status.');
        }}
        onOpen={(entry) => onOpen(currentRecord(entry.id), preview.authority)}
        renderArtwork={(entry) => (
          <GameArtwork record={recordFromFriendShelf(entry, games)} artwork={artwork?.get(entry.id)} />
        )}
      />
      ...
      {view.status === 'ready' && (
        <p className="section-help">
          {view.entries.length} loaded / {view.total} shared games{view.complete ? '' : ' · more available'}
        </p>
      )}
      {view.status === 'ready' && !view.complete && (
        ...
        >
          {view.loadingMore ? 'Loading games…' : 'Load next 25 shared games'}
        </button>
      )}
    </>
  );
