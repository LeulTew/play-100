...
interface Recovery {
  key: string;
  cause: unknown;
  cleanup: 'prune' | 'stop' | null;
  selection?: {
    ids: string[];
    through: number;
    expectedRevision: number;
    expectedEpoch: number;
    consentSyncEpoch: number;
  };
}
export function useFriendShelf(
  ...
  const owns = useCallback(() => {
    ...
    return Boolean(
      ...
      cloudAuth.app === cloudDb.app &&
      scope === accountScope(uid, cloudDb.app.options.projectId) &&
      state.scope === scope &&
      state.snapshot?.scope === scope,
    );
  }, [uid, scope, key]);
  const acceptConfig = useCallback(
    async (next: FriendShelfConfig | null, explicitThroughRevision?: number) => {
  ...
  const recover = useCallback(async () => {
    ...
    const confirmedReview =
      selected &&
      fresh?.enabled &&
      fresh.revision === selected.expectedRevision &&
      fresh.epoch === selected.expectedEpoch &&
      fresh.consentSyncEpoch === selected.consentSyncEpoch &&
      fresh.selectedIds.join('|') === selected.ids.join('|')
        ? selected.through
        : undefined;
  ...
  useEffect(() => {
    ...
    const next = (data: FriendShelfConfig | null) => {
      if (alive && owns()) void acceptConfig(data).catch(failed);
    };
    const refresh = () => {
      if (!alive || !owns()) return;
  ...
  useEffect(() => {
    if (automaticMode || !uid || !scope || !verified || !config?.enabled || config.deleted || !snapshot?.sync.enabled)
      return;
    const ticket = generation.next();
    const syncEpoch = snapshot.sync.epoch;
    let alive = true;
    ...
    const isCurrent = () =>
      ...
      current.current.snapshot.sync.epoch === syncEpoch &&
      controlNow.current?.enabled === true &&
      !controlNow.current.deleted;
    const work = new SyncWorkQueue(
      async () => {
        ...
        if (projected.selectedIds.length !== fresh.selectedIds.length) {
          reviewed = await store.saveConfig(
            uid,
            { enabled: true, selectedIds: projected.selectedIds, consentSyncEpoch: fresh.consentSyncEpoch },
            fresh,
            isCurrent,
          );
          if (!isCurrent()) return;
        ...
        const publicationCurrent = () =>
          isCurrent() &&
          controlNow.current?.epoch === reviewed.epoch &&
          controlNow.current.revision === reviewed.revision &&
          current.current.snapshot?.state.revision === local.state.revision &&
          !current.current.snapshot.sync.dirty &&
        ...
        await store.publish(
          ...
          projected.entries,
          reviewed,
          { syncEpoch, remoteRevision: local.sync.baseRemoteRevision },
          head?.revision ?? 0,
          publicationCurrent,
        );
        if (!isCurrent()) return;
      ...
      (cause) => {
        ...
        if (acknowledged)
          recovery.current = { key, cause, cleanup: cause.receipt.operation === 'publish-shelf' ? 'prune' : null };
    ...
    const wake = () => {
      ...
      work.setAvailable(available);
      if (available) work.wake();
    };
    wake();
  ...
  useEffect(() => {
    if (config?.enabled && snapshot?.sync.enabled && !snapshot.sync.dirty && !pendingEdits)
  ...
  const saveSelection = async (ids: string[], expected: FriendShelfConfig, reviewedStateRevision: number) => {
    if (!owns() || !uid || !scope || recovery.current?.key === key || hasPendingEdits())
      throw new Error('Refresh shared games before saving this selection.');
    const local = await loadScopedLibrary(scope);
    if (!owns() || local.state.revision !== reviewedStateRevision || !local.sync.enabled || local.sync.dirty)
      throw new Error('Your library changed or is still saving. Preview the saved games again.');
    const projected = projectFriendShelf(local.state, shelfSelection(ids), current.current.games);
    if (projected.selectedIds.length !== ids.length)
      throw new Error('A selected game was removed. Preview the selection again.');
    stop();
    const ticket = mutations.next();
    const operation = Symbol('shelf-selection');
    mutationActive.current = operation;
    const mutationCurrent = () => owns() && mutations.current(ticket);
    try {
      const next = await store.saveConfig(
        uid,
        { enabled: true, selectedIds: projected.selectedIds, consentSyncEpoch: local.sync.epoch },
        expected,
        mutationCurrent,
      );
      ...
      if (mutationCurrent()) {
        const changed =
          !expected.enabled ||
          expected.consentSyncEpoch !== local.sync.epoch ||
          expected.selectedIds.join('|') !== projected.selectedIds.join('|');
        recovery.current = {
          key,
          cause,
          cleanup: null,
          selection: {
            ...
            through: reviewedStateRevision,
            expectedRevision: expected.revision + Number(changed),
            expectedEpoch: expected.epoch + Number(changed),
            consentSyncEpoch: local.sync.epoch,
          },
        };
        setFailure({
          key,
          message:
            cause instanceof FriendShelfCommittedError
              ? cause.message
      ...
      if (mutationActive.current === operation) mutationActive.current = null;
    }
  };
  const stopSharing = async () => {
    ...
    const ticket = mutations.next();
    const operation = Symbol('shelf-stop');
    mutationActive.current = operation;
    const mutationCurrent = () => owns() && mutations.current(ticket);
    try {
      const fresh = await store.config(uid);
      ...
      if (mutationCurrent()) {
        recovery.current = { key, cause, cleanup: 'stop' };
        setFailure({ key, message: 'Sharing stop needs confirmation. Refresh to check its current status.' });
      }
      throw cause;
    } finally {
      if (mutationActive.current === operation) mutationActive.current = null;
  ...
  const retry = async () => {
    ...
    try {
      ...
      if (owns()) setFailure({ key, message: onlineError(cause) });
      throw cause;
    }
  };
  const ready = value?.key === key;
  const visibleStatus: FriendShelfStatus = !verified
    ? 'paused'
    : !ready
      ? failure?.key === key
        ? 'error'
