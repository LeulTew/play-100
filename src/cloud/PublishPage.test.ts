...
const record: LibraryRecord = {
  id: 'manual:publication-test',
  title: 'Publication fixture',
  year: 2020,
  genre: '',
  studio: '',
  source: 'manual',
  sourceId: 'publication-test',
  sourceUrl: null,
  collectionRank: null,
};
...
const member: Member = {
  uid: 'alpha',
  displayName: 'Private member name',
  avatar,
  createdAt: 1,
  updatedAt: 1,
  consentVersion: 1,
...
const profile: PublicProfile = {
  uid: 'alpha',
  displayName: 'Published identity',
...
function props(patch: Partial<PublishPageProps> = {}): PublishPageProps {
  return {
    social: {
      control: vi.fn(async () => ({ epoch: 0, hidden: false, deleted: false })),
      saveMember: vi.fn(async () => {}),
      publish: vi.fn(async () => profile),
...
describe('publication field initialization', () => {
  ...
  it('does not change the existing selection or unverified publication gate', () => {
    ...
    const html = renderToStaticMarkup(
      createElement(PublishPage, { ...verified, identity: { ...verified.identity, verified: false } }),
    );
