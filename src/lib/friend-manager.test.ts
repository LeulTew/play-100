...
const profile = (uid: string, displayName: string): FriendIdentityState => ({
  status: 'ready',
  value: { format: 1, uid, displayName, avatar, revision: 1, updatedAt: 1 },
});
const invite: FriendInvitation = {
  ownerUid: 'owner',
  displayName: 'Owner',
  avatar,
  token: 'b'.repeat(64),
...
describe('loaded-only friends manager', () => {
  it('round-trips all views, escaped name filters and ordering without private selections', () => {
    ...
    expect(parseFriendsView('?view=unknown&name=' + 'x'.repeat(90) + '&order=bad')).toEqual({
      view: 'friends',
      name: 'x'.repeat(60),
      order: 'recent',
    });
  ...
  it('separates incoming from sent without treating one scanned page as the full inbox', () => {
    ...
    expect(
      visibleFriendPairs([...sent, incoming], {}, 'owner', { view: 'incoming', name: '', order: 'recent' }),
    ).toEqual([incoming]);
    expect(
      visibleFriendPairs([...sent, incoming], {}, 'owner', { view: 'sent', name: '', order: 'recent' }),
    ).toHaveLength(20);
  });
  it('filters and sorts loaded names with unavailable names last and stable ties', () => {
    const rows = [
      pair('z', 'owner', 'accepted', 30),
      pair('a', 'owner', 'accepted', 10),
      pair('missing', 'owner', 'accepted', 50),
    ];
    const names = { a: profile('a', 'ALPHA'), z: profile('z', 'Zulu') };
    expect(
      visibleFriendPairs(rows, names, 'owner', { view: 'friends', name: '', order: 'name' }).map((row) =>
        row.a === 'owner' ? row.b : row.a,
      ),
    ).toEqual(['a', 'z', 'missing']);
    expect(visibleFriendPairs(rows, names, 'owner', { view: 'friends', name: 'alpha', order: 'recent' })).toEqual([
      rows[1],
    ]);
    expect(rows[0]?.updatedAt).toBe(30);
  });
  it('deduplicates page boundaries without regressing a newer relationship epoch', () => {
    const older = pair('peer');
    const newer = { ...older, epoch: 2, updatedAt: 20 };
