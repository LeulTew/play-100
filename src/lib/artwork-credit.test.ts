...
describe('contextual artwork-credit disclosure', () => {
  const artwork = { ...artworkFixture, credit: 'Original artist; resized and converted to WebP. Logos remain trademarks of their owners.' };
  it('keeps the complete original credit and both links behind an explicitly labelled native disclosure', () => {
    const html = renderToStaticMarkup(createElement(GameArtworkCredit, { artwork, disclosureLabel: 'Artwork credits for a pinned game' }));
  ...
  it('does not collapse unrelated surfaces or invent missing attribution', () => {
    ...
    expect(renderToStaticMarkup(createElement(GameArtworkCredit, { artwork: null, disclosureLabel: 'Artwork credits' }))).toBe('');
  });
  it('still escapes source text and keeps unsafe credit links inactive', () => {
    const html = renderToStaticMarkup(createElement(GameArtworkCredit, { artwork: { ...artwork, credit: '<script>not markup</script>', sourceUrl: 'javascript:alert(1)', licenseUrl: 'http://example.com/license' }, disclosureLabel: 'Artwork credits' }));
