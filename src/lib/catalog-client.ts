...
export async function fetchCatalogPage(source: CatalogSource, query: string, offset: number, signal: AbortSignal): Promise<CatalogPage> {
  if (!['wikidata', 'freetogame'].includes(source) || query.length > 80 || [...query].some((character) => character.charCodeAt(0) < 32 || character.charCodeAt(0) === 127) ||
  ...
  const payload = await fetchCatalogJson(`/api/catalog?${new URLSearchParams({ source, q: query.trim(), offset: String(offset) })}`, signal, 256 * 1024);
