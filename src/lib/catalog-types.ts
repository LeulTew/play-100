...
export function parseCatalogPage(value: unknown): CatalogPage {
  ...
  if (
    ...
    (row.nextOffset !== null &&
      (typeof row.nextOffset !== 'number' ||
        ...
        row.nextOffset <= row.offset ||
        row.nextOffset > 10_000)) ||
    !Array.isArray(row.notices) ||
    row.notices.length > 10 ||
  ...
  for (const item of row.items) {
    if (
      ...
      Object.hasOwn(recordMap, item.id)
    )
      throw new Error('The catalog returned invalid or duplicate games.');
  ...
  return {
    source: row.source,
    query: row.query,
    items: Object.values(parsed.records),
    total: row.total,
    offset: row.offset,
