...
import {
  CHUNK_BYTES, creatorRanks, MAX_RANKING_CHUNKS, MAX_RANKING_JSON_BYTES, MAX_RANKING_SNAPSHOT_BYTES, MAX_SNAPSHOT_BYTES,
} from './cloud-types';
...
describe('parser-derived creator ranking envelope', () => {
  it('derives the accepted sixteen-MiB allocation cap from the real library parser limits and pins the rules', () => {
    ...
    expect(rules).toContain(`manifest(value) && value.bytes <= ${MAX_RANKING_SNAPSHOT_BYTES} && value.chunks.size() <= ${MAX_RANKING_CHUNKS}`);
  });
  ...
  it('serializes a representable ten-thousand-record maximum with genuine control and lone-surrogate escaping', async () => {
    ...
    for (let index = 1; index <= MAX_LIBRARY_RECORDS; index += 1) {
      ...
      input.records[id] = { id, title, source: 'manual', sourceId: 'local', sourceUrl: null, year: null, studio: null, genre: null, collectionRank: null };
    ...
    console.info('Synthetic maximum ranking envelope', {
      records: state.ranking.length, privateBytes: privateCopy.manifest.bytes, rankingBytes: ranking.manifest.bytes,
      rankingChunks: ranking.chunks.length, base64Characters: ranking.chunks.reduce((sum, chunk) => sum + chunk.data.length, 0),
    });
  }, 60000);

  it.each([100, 1000])('records deterministic representative sizes for %s games without claiming a user average', async count => {
    const input = emptyPersonalLibrary();
    for (let index = 1; index <= count; index += 1) {
      const id = `manual:example-${index}`;
      input.records[id] = {
        id, title: `Example game ${index}`, source: 'manual', sourceId: `example-${index}`, sourceUrl: null,
        year: 2020, studio: 'Example studio', genre: 'Adventure', collectionRank: null,
      };
      input.progress[id] = { later: index % 4 === 0, played: index % 3 === 0, completed: false };
      if (input.progress[id]!.later) input.queueOrder.push(id);
      input.ranking.push({ id, score: index % 5 ? 8 : null, note: index % 10 ? '' : 'A short note about this game.', manualPosition: null });
    }
    const state = parsePersonalLibrary(input);
    const privateCopy = await packLibrary(state);
    const ranking = await packSnapshot(creatorRanks(state));
    ...
    console.info('Synthetic representative library', { records: count, privateBytes: privateCopy.manifest.bytes, rankingBytes: ranking.manifest.bytes,
