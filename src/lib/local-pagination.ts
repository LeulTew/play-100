...
export function getLocalPage(total: number, pageSize: number, offset: number): LocalPage {
  if (!Number.isSafeInteger(total) || total < 0 || !Number.isSafeInteger(pageSize) || pageSize < 1 || !Number.isSafeInteger(offset) || offset < 0) {
  ...
  return { offset: boundedOffset, page, pageCount, start: total ? boundedOffset + 1 : 0, end: Math.min(boundedOffset + pageSize, total) };
}
