...
describe('reviewed public identities, not fuzzy title matching', () => {
  ...
  it.each([
    ['Q112231148', 'resident-evil-4'],
    ['Q1757876', 'tomb-raider'],
    ['Q29154231', 'star-wars-battlefront-ii'],
    ['Q18345138', 'god-of-war'],
    ['Q18515944', 'overwatch'],
    ['Q275960', 'mass-effect'],
    ...
    const fromProvider = {
      ...
      id: `wikidata:${id}`,
      sourceId: id,
      title: 'Changed upstream label',
      year: null,
    };
    expect(resolveCatalogRecord(fromProvider, games)).toEqual(
      recordFromGame(games.find((game) => game.slug === slug)!),
    );
  });
  it.each([
    'wikidata:Q275950',
    ...
    'wikidata:Q953242',
    'manual:same-title',
  ])('does not map a different edition, franchise or unknown exact-title ID: %s', (id) => {
...
describe('canonical-first local and remote result composition', () => {
  ...
  it.each(games.map((game) => [game.slug, game.title]))(
    'includes %s without either provider or seed',
    (slug, title) => {
  ...
  it.each(['RDR2', 'Red Dead Redemption II', 'Red Dead 2'])(
    'keeps verified alias %s on the original record, artwork and rank',
    (q) => {
  ...
  it('does not lose matches under source/year/genre filters or leak provider metadata into originals', () => {
    ...
    expect(parseDiscoverySearch(createDiscoverySearch({ ...filters, source: 'collection' }))).toEqual({
      ...filters,
      source: 'collection',
    });
  });
  it('deduplicates all local results before paging, not just the visible page, and deduplicates remote IDs', () => {
    ...
    const pages = Array.from({ length: Math.ceil(local.length / 24) }, (_, index) =>
      catalogPageRecords(local, [], index * 24, 24),
    );
    const ids = pages.flat().map((record) => record.id);
    expect(new Set(ids).size).toBe(local.length);
    const firstPage = pages[0]!;
    const laterCanonical = pages
      .slice(1)
      .flat()
      .find((record) => record.source === 'collection')!;
  ...
  it('preserves actual same-title originals and remakes as separate results', () => {
    const records = searchDiscoveryItems(items, { ...defaultDiscoveryFilters, q: 'Resident Evil 4' }).map(
      (item) => item.record.id,
    );
    expect(records).toContain('resident-evil-4');
    expect(records).toContain('wikidata:Q275950');
...
describe('existing private opinions and identities are not migrated', () => {
  const actions = (record: LibraryRecord): PersonalAction[] => [
    ...
    { type: 'set-progress', records: [record], key: 'played', value: true },
    { type: 'set-progress', records: [record], key: 'completed', value: true },
    { type: 'set-progress', records: [record], key: 'completed', value: false },
    { type: 'toggle-progress', record, key: 'later' },
    { type: 'add-ranking', records: [record] },
    { type: 'rate-game', record, score: 8.4 },
  ];
  it('uses canonical IDs for every new mutation without inferring progress from save/rating', () => {
    ...
    expect(state.queueOrder).toEqual([canonical.id]);
    expect(state.ranking[0]?.id).toBe(canonical.id);
  });
  ...
  it('keeps both conflicting opinions and unknown same-title manual entries; canonical controls only edit canonical', () => {
    ...
    state = applyPersonalAction(state, {
      ...
      id: provider.id,
      score: 3.1,
      note: 'Provider-copy note.',
    });
    state = applyPersonalAction(state, {
      type: 'edit-ranking',
      id: canonical.id,
      score: 9.8,
      note: 'Canonical-copy note.',
    });
    ...
    expect(state.ranking.find((entry) => entry.id === provider.id)).toEqual(
  ...
  it('does not transfer an unknown same-title record or private progress into a canonical record', () => {
    const state = applyPersonalAction(initial(), {
      type: 'set-progress',
      records: [unknown],
      key: 'completed',
      value: true,
...
describe('bounded tray identity without migration or opinion fields', () => {
  it.each([
    [provider, canonical],
    [canonical, provider],
  ])('does not spend another slot or rewrite an already pinned equivalent', (first, second) => {
    ...
    expect(store.pin(first)).toBe(true);
    const before = [...data.values()][0];
    expect(store.pin(second)).toBe(true);
    expect(store.getSnapshot().items).toEqual([first]);
    expect(storage.setItem).toHaveBeenCalledTimes(1);
    expect([...data.values()][0]).toBe(before);
  ...
  describe('private ranking picker effective identities', () => {
    it('offers the canonical target once for fresh known duplicates before a consumer applies result limits', () => {
      expect(
        catalogPickerChoices([canonical, provider, canonical, ...games.slice(1).map(recordFromGame)], {}),
      ).toHaveLength(100);
    ...
    it('keeps the unique owned alias as the single already-ranked candidate without changing an opinion', () => {
      ...
      state = applyPersonalAction(state, {
        ...
        id: provider.id,
        score: 8.5,
        note: 'Keep this existing opinion.',
      });
    ...
    it('adds only the existing alias opinion when the owned game is not ranked', () => {
      const before = applyPersonalAction(initial(), {
        type: 'set-progress',
        records: [provider],
        key: 'later',
        value: true,
    ...
    it('preserves both explicitly owned copies, including their own metadata, instead of merging them', () => {
      ...
      for (const choice of choices)
        expect(choice.titles).toEqual(expect.arrayContaining([canonical.title, legacy.title]));
      expect(owned[legacy.id]).toBe(legacy);
    });
    it('keeps unknown same-title manual records and genuine distinct editions separate', () => {
      expect(catalogPickerChoices([canonical], { [unknown.id]: unknown }).map((choice) => choice.record.id)).toEqual([
        unknown.id,
        canonical.id,
      ]);
      ...
      expect(catalogPickerChoices([remake], { [original.id]: original }).map((choice) => choice.record.id)).toEqual([
        original.id,
        remake.id,
      ]);
    });
