...
export class FriendCommittedError extends FriendStoreError {
  ...
  constructor(
    readonly receipt: FriendMutationReceipt,
    readonly cause: Error,
    readonly phase: 'refresh' | 'cleanup' = 'refresh',
  ) {
    super(
      'committed-refresh-failed',
      'The change was saved, but its latest details could not be refreshed. Refresh instead of repeating the action.',
    );
...
export interface FriendManifest {
  generation: string;
  digest: string;
  count: number;
}
export interface FriendSourceRevision {
  syncEpoch: number;
...
export interface FriendShareHead {
  ...
  current: FriendManifest | null;
  previous: FriendManifest | null;
  updatedAt: number;
}
export interface FriendRanking {
  head: FriendShareHead;
  entries: PublicEntry[];
}
export function retainsFriendGeneration(
  id: string,
  ...
  return (
    ...
    Boolean(
      settings?.enabled &&
      !settings.deleted &&
      head?.epoch === settings.epoch &&
      head.settingsRevision === settings.revision,
    )
  );
}
...
export interface FriendExport {
  ...
  settings: FriendSettings | null;
  relations: FriendPair[];
  groups: FriendGroup[];
  blocks: FriendBlock[];
}
export interface FriendCleanupResult {
  deleted: number;
  done: boolean;
  message?: string;
}
export interface FriendGeneration {
  ...
  source: FriendSourceRevision;
  ids: string[];
  status: 'staging' | 'ready' | 'published' | 'deleting';
  createdAt: number;
}
...
export function friendUid(value: unknown): string {
  if (typeof value !== 'string' || !/^[A-Za-z0-9_-]{1,128}$/.test(value))
    invalid('This account identity is unsupported.');
  return value;
}
...
export function friendToken(value: unknown): string {
  if (typeof value !== 'string' || !/^[a-f0-9]{64}$/.test(value))
    invalid('This invitation link is malformed. Ask for a new link.');
  return value;
}
export function friendName(value: unknown, max = 60): string {
  if (typeof value !== 'string' || !value.trim() || value.trim().length > max || hasAsciiControl(value))
    invalid(`Choose a name between 1 and ${max} characters.`);
  return value.trim();
}
...
export function friendParticipants(value: unknown): string[] {
  if (!Array.isArray(value) || value.length < 2 || value.length > 6 || new Set(value).size !== value.length)
    invalid('Choose between two and six different people.');
...
export function parseFriendIdentity(value: unknown): FriendIdentity {
  ...
  return {
    format: version(row.format),
    uid: friendUid(row.uid),
    displayName: friendName(row.displayName),
...
export function parseFriendPair(value: unknown): FriendPair {
  if (!value || typeof value !== 'object' || !('format' in value) || (value.format !== 1 && value.format !== 2))
    invalid();
  const row = object(
    value,
    value.format === 2
  ...
  const a = friendUid(row.a);
  const b = friendUid(row.b);
  const from = friendUid(row.from);
  if (
    a >= b ||
    !Array.isArray(row.participants) ||
    row.participants.length !== 2 ||
    row.participants[0] !== a ||
    row.participants[1] !== b ||
    ![a, b].includes(from) ||
    typeof row.state !== 'string' ||
    !['pending', 'accepted', 'declined', 'cancelled', 'removed'].includes(row.state)
  )
    invalid();
  const pair: FriendPairBase = {
    ...
    participants: [a, b],
    from,
    state: row.state as FriendPairState,
    epoch: integer(row.epoch, 1),
    inviteSlot: row.inviteSlot === null ? null : integer(row.inviteSlot, 0, 19),
...
export function parseFriendHead(value: unknown): FriendShareHead {
  ...
  return {
    format: version(row.format),
    epoch: integer(row.epoch, 1),
    settingsRevision: integer(row.settingsRevision, 1),
    revision: integer(row.revision, 1),
    source: parseFriendSource(row.source),
    current: row.current === null ? null : parseFriendManifest(row.current),
    previous: row.previous === null ? null : parseFriendManifest(row.previous),
    updatedAt: time(row.updatedAt),
  };
}
export function parseFriendGroup(id: string, value: unknown): FriendGroup {
  ...
  return {
    format: version(row.format),
    id: friendUuid(id),
    name: friendName(row.name, 80),
    participantUids: friendParticipants(row.participantUids),
    revision: integer(row.revision, 1),
    createdAt: time(row.createdAt),
    updatedAt: time(row.updatedAt),
  };
...
export function parseFriendInvite(token: string, value: unknown): FriendInvitation {
  ...
  if (
    row.format !== 1 ||
    typeof row.state !== 'string' ||
    !['active', 'consumed', 'revoked'].includes(row.state) ||
    (row.state === 'consumed' ? typeof row.acceptedBy !== 'string' : row.acceptedBy !== null)
  )
    invalid();
  if (row.acceptedBy !== null) friendUid(row.acceptedBy);
  const createdAt = time(row.createdAt);
  return {
    token: friendToken(token),
    ownerUid: friendUid(row.ownerUid),
    slot: integer(row.slot, 0, 19),
    displayName: friendName(row.displayName),
    avatar: parseAvatar(row.avatar),
    state: row.state as FriendInvitation['state'],
    createdAt,
    expiresAt: createdAt + FRIEND_INVITE_LIFETIME_MS,
    lifetimeDays: 7,
    singleUse: true,
  };
}
export function parseFriendGeneration(value: unknown): FriendGeneration {
  ...
  const count = integer(row.count, 0, FRIEND_SELECTION_LIMIT);
  const uploaded = integer(row.uploaded, 0, FRIEND_CHUNK_LIMIT);
  const ids = friendSelection(row.ids);
  if (
    uploaded > Math.ceil(count / FRIEND_CHUNK_SIZE) ||
    ids.length > count ||
    (row.status === 'deleting'
      ? uploaded > Math.ceil(ids.length / FRIEND_CHUNK_SIZE) ||
        (ids.length !== count && ids.length % FRIEND_CHUNK_SIZE !== 0)
      : ids.length !== Math.min(uploaded * FRIEND_CHUNK_SIZE, count)) ||
    ((row.status === 'ready' || row.status === 'published') && ids.length !== count)
  )
    invalid();
  return {
    epoch: integer(row.epoch, 1),
    ...
    uploaded,
    ids,
    status: row.status as FriendGeneration['status'],
    createdAt: time(row.createdAt),
  };
}
...
export function parseFriendChunk(value: unknown, index: number, count: number): PublicEntry[] {
  ...
  integer(index, 0, FRIEND_CHUNK_LIMIT - 1);
  integer(count, 1, FRIEND_SELECTION_LIMIT);
  if (
    row.index !== index ||
    !Array.isArray(row.entries) ||
    row.entries.length !== Math.min(FRIEND_CHUNK_SIZE, count - index * FRIEND_CHUNK_SIZE) ||
    row.entries.length < 1
  )
    invalid();
  const entries = row.entries.map(parsePublicEntry);
  const ids = friendSelection(row.ids);
  if (
    entries.some(
      (entry, offset) => entry.position !== index * FRIEND_CHUNK_SIZE + offset + 1 || entry.id !== ids[offset],
    ) ||
...
export function validateFriendEntries(value: readonly PublicEntry[], selectedIds: readonly string[]): PublicEntry[] {
  const selected = new Set(friendSelection(selectedIds));
  const entries = value.map(parsePublicationEntry);
  if (
    entries.length > 200 ||
    new Set(entries.map((entry) => entry.id)).size !== entries.length ||
    entries.some((entry, index) => entry.position !== index + 1 || !selected.has(entry.id))
  )
    invalid('The selected ranking changed. Review its games and order before sharing.');
  return entries;
}
export function projectFriendRanking(
  state: PersonalLibraryState,
  selectedIds: readonly string[],
  games: Game[],
): { entries: PublicEntry[]; selectedIds: string[] } {
  ...
  return {
    entries: remaining.length ? projectPublicRanking(state, new Set(remaining), games) : [],
