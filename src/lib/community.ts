...
function sourceUrl(record: Pick<LibraryRecord, 'source' | 'sourceId' | 'sourceUrl'>): string | null {
  ...
  if (
    ...
    record.sourceUrl &&
    /^https:\/\/www\.freetogame\.com\/[a-z0-9/-]+$/.test(record.sourceUrl)
  )
...
export function projectPublicRanking(
  state: PersonalLibraryState,
  selected: ReadonlySet<string>,
  games: Game[],
): PublicEntry[] {
  if (!selected.size || selected.size > PUBLIC_LIMIT)
    throw new Error('Choose between 1 and 200 ranked games. Nothing is automatically left out.');
  const canonical = new Map(games.map((game) => [game.slug, game]));
  const entries = state.ranking
    .filter((entry) => selected.has(entry.id))
    .map((entry, index): PublicEntry => {
      const saved = state.records[entry.id];
      if (!saved) throw new Error('A selected game is missing its metadata.');
      const trusted = canonical.get(saved.id);
      if (saved.source === 'collection' && !trusted)
        throw new Error(
          'A selected entry claims an unknown original collection game. Correct that record before publishing.',
        );
      const record = trusted ? recordFromGame(trusted) : saved;
      return {
        position: index + 1,
  ...
  if (entries.length !== selected.size)
    throw new Error('Your ranking changed while selecting games. Review the selection before publishing.');
...
export function parsePublicEntry(value: unknown): PublicEntry {
  ...
  const candidate = {
    ...
    collectionRank: row.source === 'collection' ? 1 : null,
  };
  ...
  if (
    !record ||
    (record.source !== 'collection' && record.id !== `${record.source}:${record.sourceId}`) ||
    ...
    record.sourceUrl !== sourceUrl(record)
  )
    throw new Error('A published game has an unsupported identity or source link.');
  return {
...
export function recordFromPublic(value: PublicEntry, games: Game[]): LibraryRecord {
  ...
  if (entry.source === 'collection')
    throw new Error('This list references an unknown original game. Reload the collection before saving it.');
  return {
    id: entry.id,
    source: entry.source,
