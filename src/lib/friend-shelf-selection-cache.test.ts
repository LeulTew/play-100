...
import { acknowledgeScopedUpload, commitScopedAction, connectScopedLibrary, deleteScopedLibrary, loadScopedLibrary, parseScopedLibrary, restoreScopedLibrary } from './scoped-library';
...
const game: LibraryRecord = { id: 'manual:shelf-game', source: 'manual', sourceId: 'shelf-game', sourceUrl: null, title: 'Synthetic unranked game', year: null, studio: null, genre: null, collectionRank: null };
beforeEach(() => {
...
describe('atomic independently selected library sharing', () => {
  it.each([true, false])('requires explicit review when removal precedes first remote config (online=%s)', async (online) => {
    ...
    const initial = await connectScopedLibrary(a, saved.state, head, 'Synthetic player', true, { localRevision: saved.state.revision, epoch: 0, enabled: false });
    ...
    await expect(friendShelfJournal.update(a, 5, [game.id], undefined, readded.state.revision)).rejects.toThrow(/review/);
    await expect(friendShelfJournal.pending(a, readded.state.revision)).rejects.toThrow(/review/);
    await expect(friendShelfJournal.update(a, 6, [game.id], initial.state.revision, readded.state.revision)).rejects.toThrow(/review/);
  ...
  it('metadata-only upload ACK leaves the state-revision journal valid and unchanged', async () => {
    ...
    const head: SyncHead = { format: 1, epoch: 1, revision: 1, enabled: true, deleted: false, current: null, previous: null, updatedAt: 1 };
    const connected = await connectScopedLibrary(a, state, head, 'Synthetic player', false, { localRevision: initial.state.revision, epoch: 0, enabled: false });
  ...
  it('corrupt optional shelf state blocks only shelf updates and can be repaired by explicit review', async () => {
    ...
    await friendShelfSelectionStorageTransaction(a, (_, store) => store.put({ invalid: true }, friendShelfSelectionKey(a)));
  ...
  it('requires explicit review after an old writer skipped removal journaling even when the game was re-added', async () => {
    const before = await commitScopedAction(a, { type: 'add-records', records: [game] });
    await friendShelfJournal.update(a, 1, [game.id], undefined, before.state.revision);
    const actions: PersonalAction[] = [{ type: 'remove-records', ids: [game.id] }, { type: 'add-records', records: [game] }];
    for (const action of actions) await accountStorageTransaction(a, (value, store) => {
      const current = parseScopedLibrary(value, a);
      store.put({ ...current, state: applyPersonalAction(current.state, action), sync: { ...current.sync, dirty: true, dataRevision: current.sync.dataRevision + 1 } }, a);
    });
