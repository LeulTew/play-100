...
describe('optional App Check boundary', () => {
  it('stays off unless the build flag is exactly "true"', () => {
    for (const flag of [undefined, '', 'false', '1', 'TRUE', ' true']) {
      expect(readAppCheckConfiguration({ VITE_APP_CHECK_ENABLED: flag, VITE_APP_CHECK_SITE_KEY: siteKey })).toEqual({
        config: null,
        error: null,
      });
    }
  ...
  it('accepts only a public reCAPTCHA v3 site key once enabled', () => {
    expect(
      readAppCheckConfiguration({ VITE_APP_CHECK_ENABLED: 'true', VITE_APP_CHECK_SITE_KEY: ` ${siteKey} ` }),
  ...
  it('is not loaded by default and never runs against the emulator', () => {
    ...
    const staticDependencies = [
      ...sourceNodes(source, ts.isImportDeclaration),
      ...sourceNodes(source, ts.isExportDeclaration),
    ];
    expect(
      staticDependencies.some(
        (declaration) =>
          declaration.moduleSpecifier &&
          ts.isStringLiteral(declaration.moduleSpecifier) &&
          declaration.moduleSpecifier.text === 'firebase/app-check',
      ),
    ).toBe(false);
    expect(
      sourceNodes(source, ts.isIfStatement).map((statement) => sourceTokens(statement.expression.getText(source))),
    ...
    const flag = sourceNodes(viteSource, ts.isPropertyAssignment).find(
      (property) =>
        ts.isStringLiteral(property.name) && property.name.text === 'import.meta.env.VITE_APP_CHECK_ENABLED',
    );
    expect(sourceTokens(flag?.initializer.getText(viteSource) ?? '')).toBe(
      sourceTokens("JSON.stringify(appCheck.config ? 'true' : 'false')"),
    );
  });
});
