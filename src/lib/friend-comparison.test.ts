...
import {
  compareFriendRankings, comparisonGameKey, FRIEND_COMPARISON_LIMITS,
  FriendComparisonValidationError, getComparisonPage,
} from './friend-comparison';
import type {
  ComparisonAvailability, ComparisonEntry, ComparisonPageOptions, ComparisonParticipant,
  FriendComparison,
} from './friend-comparison';

function game(sourceId: string, position = 1, score: number | null = null, fields: Partial<ComparisonEntry> = {}): ComparisonEntry {
  return {
    id: sourceId, title: `Game ${sourceId}`, year: 2020, source: 'collection',
    sourceId, sourceUrl: null, position, score, ...fields,
  };
}

function sourced(source: ComparisonEntry['source'], sourceId: string, position = 1, score: number | null = null): ComparisonEntry {
  return game(sourceId, position, score, {
    source, id: source === 'collection' ? sourceId : `${source}:${sourceId}`, title: 'The same title',
  });
}

function ready(id: string, entries: readonly ComparisonEntry[] = [], kind: 'self' | 'friend' = 'friend'): ComparisonParticipant {
...
it('uses bounded indexed participant and entry arrays rather than overridden iterators', () => {
  ...
  entries[Symbol.iterator] = function* () { for (let index = 0; index < 250; index += 1) yield game(`fake-${index}`, index + 1); return undefined; };
  const input = [ready('one', entries), ready('two', [game('a')])];
  input[Symbol.iterator] = function* () { for (let index = 0; index < 7; index += 1) yield ready(`fake-${index}`); return undefined; };
...
function reject(input: unknown) {
  expect(() => compareFriendRankings(input as readonly ComparisonParticipant[])).toThrow(FriendComparisonValidationError);
}
...
describe('exact friend ranking comparisons', () => {
  it('distinguishes zero, unrated, and absent entries without rounding scores', () => {
    ...
    expect(row(comparison, 'collection:zero')).toMatchObject({
      coverage: 2, raterCount: 2, meanScore: 0, scoreSpread: 0, scoreDifference: 0,
      cells: [{ status: 'ranked', score: 0, position: 1 }, { status: 'ranked', score: 0, position: 2 }],
    });
    expect(row(comparison, 'collection:unrated')).toMatchObject({
      coverage: 2, raterCount: 1, meanScore: 0, scoreSpread: null, scoreDifference: null,
      cells: [{ status: 'ranked', score: null }, { status: 'ranked', score: 0 }],
    });
    expect(row(comparison, 'collection:solo')).toMatchObject({
      coverage: 1, raterCount: 1, meanScore: 7.123456789, scoreSpread: null, scoreDifference: null,
      cells: [{ status: 'ranked', score: 7.123456789 }, { status: 'absent', score: null, position: null }],
    });
    expect(comparison.summary).toEqual({
      availableGameCount: 3, sharedGameCount: 2, jointlyRatedCount: 1,
      pairs: [{
        participantIds: ['a', 'b'], incomplete: false, hasStaleData: false,
        sharedGameCount: 2, jointlyRatedCount: 1, meanAbsoluteScoreGap: 0,
      }],
    });
  });
  ...
  it('keeps entirely unrated common games ranked, but leaves score aggregates unknown', () => {
    ...
    expect(row(comparison, 'collection:unrated')).toMatchObject({
      coverage: 2, raterCount: 0, meanScore: null, scoreSpread: null, scoreDifference: null,
    });
    expect(comparison.summary).toMatchObject({
      sharedGameCount: 1, jointlyRatedCount: 0,
  ...
  it('reports a signed first-minus-second difference and an unsigned pair gap', () => {
    ...
    expect(row(compareFriendRankings([a, b]), 'collection:same')).toMatchObject({
      scoreDifference: -10, scoreSpread: 10, meanScore: 5,
    });
  ...
  it.each([
    ...
    expect(comparison.summary).toMatchObject({
      availableGameCount: union, sharedGameCount: 0, jointlyRatedCount: 0,
  ...
  it('compares identical lists including null and zero with an optional, not required, self', () => {
    ...
    expect(comparison.summary).toMatchObject({
      sharedGameCount: 3, jointlyRatedCount: 2,
  ...
  it('calculates each group pair using its own actually commonly rated subset', () => {
    ...
    expect(comparison.summary.pairs).toEqual([
      {
        participantIds: ['a', 'b'], incomplete: false, hasStaleData: false,
        sharedGameCount: 3, jointlyRatedCount: 2, meanAbsoluteScoreGap: 3,
      },
      {
        participantIds: ['a', 'c'], incomplete: false, hasStaleData: false,
        sharedGameCount: 2, jointlyRatedCount: 2, meanAbsoluteScoreGap: 6.5,
      },
      {
        participantIds: ['b', 'c'], incomplete: false, hasStaleData: false,
        sharedGameCount: 2, jointlyRatedCount: 2, meanAbsoluteScoreGap: 4.5,
      },
    ]);
    expect(row(comparison, 'collection:x')).toMatchObject({
      coverage: 3, raterCount: 3, meanScore: 4, scoreSpread: 10, scoreDifference: null,
    });
  });
  ...
  it('matches an imported exact shared manual identity, not a newly created same-title record', () => {
    ...
    expect(comparison.summary.pairs[0]).toMatchObject({ sharedGameCount: 1, jointlyRatedCount: 1, meanAbsoluteScoreGap: 9 });
  ...
  it('separates providers, catalog identities, and editions even with the same title', () => {
    const identities = [
      sourced('collection', '42'), sourced('steam', '42'), sourced('freetogame', '42'),
      sourced('wikidata', 'Q42'), sourced('manual', '42'), sourced('steam', '43'), sourced('wikidata', 'Q43'),
    ];
    expect(new Set(identities.map(comparisonGameKey)).size).toBe(7);
    const comparison = compareFriendRankings([
      ready('a', identities.slice(0, 4).map((entry, index) => ({ ...entry, position: index + 1 }))),
      ready('b', identities.slice(4).map((entry, index) => ({ ...entry, position: index + 1 }))),
    ]);
  ...
  it('preserves sparse private/public positions and uses positions rather than array order or score', () => {
    ...
    expect(getComparisonPage(comparison, { sort: { by: 'position', participantId: 'a' } }).rows.map(({ key }) => key))
  ...
  it('has deterministic row and floating-point accumulation order independent of entry array order', () => {
    ...
    expect(compareFriendRankings([ready('a', a), ready('b', b)]))
      .toEqual(compareFriendRankings([ready('a', [...a].reverse()), ready('b', [...b].reverse())]));
  });
...
describe('cohort completeness and freshness', () => {
  it.each(['loading', 'unshared', 'unavailable', 'error'] as const)('keeps a %s participant distinct from a ready empty list', (status) => {
    const comparison = compareFriendRankings([ready('a', [game('a', 1, 0)]), unready('b', status)]);
    expect(row(comparison, 'collection:a').cells[1]).toEqual({ participantId: 'b', status, position: null, score: null });
    expect(comparison.participants[1]?.availability).toBe(status);
    expect(comparison.cohort).toMatchObject({
      participantIds: ['a', 'b'], participantCount: 2,
      availableParticipantIds: ['a'], availableParticipantCount: 1,
      unavailableParticipantIds: ['b'], incomplete: true,
    });
    expect(comparison.rows['common-ranked']).toEqual([]);
    expect(comparison.summary).toMatchObject({
      sharedGameCount: null, jointlyRatedCount: null,
  ...
  it('does not label an available pair as the complete selected group', () => {
    const comparison = compareFriendRankings([
      ready('a', [game('a', 1, 0)]), ready('b', [game('a', 1, 4)]), unready('c', 'loading'),
    ]);
    expect(comparison.summary.pairs).toMatchObject([
      { participantIds: ['a', 'b'], incomplete: false, sharedGameCount: 1, jointlyRatedCount: 1, meanAbsoluteScoreGap: 4 },
    ...
    expect(row(comparison, 'collection:a')).toMatchObject({
      coverage: 2, raterCount: 2, meanScore: 2, scoreSpread: 4, scoreDifference: null,
    });
  ...
  it('preserves ready stale snapshots and explicit unknown freshness without pretending data is current', () => {
    ...
    expect(comparison.participants).toMatchObject([
      { id: 'a', freshness: 'stale', updatedAt: 0 }, { id: 'b', freshness: 'unknown', updatedAt: null },
    ]);
...
describe('strict comparison input validation', () => {
  it.each([
    { input: null }, { input: {} }, { input: [] }, { input: [ready('a')] },
  ...
  it('rejects oversized lists and positions using each participant kind limit', () => {
    reject([ready('a', Array.from({ length: 201 }, (_, index) => game(`a-${index}`, index + 1))), ready('b')]);
    reject([ready('a', Array.from({ length: 10_001 }, (_, index) => sourced('manual', String(index), index + 1)), 'self'), ready('b')]);
    reject([ready('a', [game('a', 10_001)], 'self'), ready('b')]);
    expect(FRIEND_COMPARISON_LIMITS).toEqual({
      minParticipants: 2, maxParticipants: 6, maxFriendEntries: 200, maxSelfEntries: 10_000, maxPageSize: 25,
    });
  });
  ...
  it.each([0, -1, 1.5, 201, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1, '1', null])('rejects invalid friend position %s', (position) => {
  ...
  it.each([
    { title: '' }, { title: '   ' }, { title: 'x'.repeat(201) }, { title: null },
    { sourceUrl: 'javascript:alert(1)' }, { sourceUrl: 'http://example.test/' },
    { sourceUrl: 'https://user:password@example.test/' }, { sourceUrl: 'https://example.test/a b' },
    { sourceUrl: 42 }, { sourceUrl: 'https://' }, { note: 'Not comparison metadata' },
  ...
  it.each([
    { id: '' }, { id: ' a ' }, { displayName: ' ' }, { kind: 'stranger' },
    { availability: 'complete' }, { freshness: 'maybe' },
    { updatedAt: -1 }, { updatedAt: NaN }, { updatedAt: Infinity }, { updatedAt: 1.5 },
  ...
  it('rejects not-ready participants carrying entries, missing fields, sparse arrays, and accessors', () => {
    ...
    let reads = 0;
    const accessor = { ...game('a'), get score() { reads++; return 5; } };
...
describe('bounded deterministic comparison pages', () => {
  const fixture = () => compareFriendRankings([
    ready('a', [
      game('z', 2, 0, { title: 'Same' }), game('a', 3, null, { title: 'Same' }), game('b', 1, 8, { title: 'alpha' }),
    ]),
    ready('b', [
      game('z', 2, 0, { title: 'Same' }), game('b', 1, 2, { title: 'Another name' }), game('c', 3, 10),
    ]),
  ]);

  it('defaults to 25 common-ranked rows and supports hard-bounded pagination of all available rows', () => {
    const entries = Array.from({ length: 60 }, (_, index) => game(`game-${String(index + 1).padStart(2, '0')}`, index + 1));
    ...
    expect(pages.flatMap(({ rows }) => rows.map(({ key }) => key))).toEqual(comparison.rows['all-shared'].map(({ key }) => key));
  ...
  it('filters by case-insensitive query, shared coverage and actual rater count before paging', () => {
    ...
    expect(getComparisonPage(comparison, { mode: 'all-shared', query: ' SAME ' }).rows.map(({ key }) => key))
      .toEqual(['collection:a', 'collection:z']);
    expect(getComparisonPage(comparison, { mode: 'all-shared', query: 'another name' }).rows.map(({ key }) => key))
      .toEqual(['collection:b']);
    expect(getComparisonPage(comparison, { mode: 'all-shared', query: 'collection:c' }).rows.map(({ key }) => key))
    ...
    expect(getComparisonPage(comparison, { mode: 'all-shared', minRaters: 2, pageSize: 1, page: 2 }).rows[0]?.key)
      .toBe('collection:z');
  });

  it('sorts with stable title/identity ties and keeps nulls after real zeros in either direction', () => {
    const comparison = fixture();
    const keys = (options: ComparisonPageOptions) => getComparisonPage(comparison, { mode: 'all-shared', ...options }).rows.map(({ key }) => key);
    expect(keys({})).toEqual(['collection:b', 'collection:c', 'collection:a', 'collection:z']);
    expect(keys({ sort: { by: 'title', direction: 'desc' } })).toEqual(['collection:z', 'collection:a', 'collection:c', 'collection:b']);
    expect(keys({ sort: { by: 'mean-score' } })).toEqual(['collection:z', 'collection:b', 'collection:c', 'collection:a']);
    expect(keys({ sort: { by: 'mean-score', direction: 'desc' } })).toEqual(['collection:c', 'collection:b', 'collection:z', 'collection:a']);
    expect(keys({ sort: { by: 'position', participantId: 'a' } })).toEqual(['collection:b', 'collection:z', 'collection:a', 'collection:c']);
    expect(keys({ sort: { by: 'position', participantId: 'a', direction: 'desc' } })).toEqual(['collection:a', 'collection:z', 'collection:b', 'collection:c']);
    expect(keys({ sort: { by: 'score', participantId: 'a' } })).toEqual(['collection:z', 'collection:b', 'collection:c', 'collection:a']);
    expect(keys({ sort: { by: 'score-spread', direction: 'desc' } })).toEqual(['collection:b', 'collection:z', 'collection:c', 'collection:a']);
    expect(keys({ sort: { by: 'score-difference', direction: 'desc' } })).toEqual(['collection:b', 'collection:z', 'collection:c', 'collection:a']);
    expect(keys({ sort: { by: 'coverage', direction: 'desc' } })).toEqual(['collection:b', 'collection:z', 'collection:c', 'collection:a']);
    expect(keys({ sort: { by: 'rater-count' } })).toEqual(['collection:a', 'collection:c', 'collection:b', 'collection:z']);
  });

  it.each([
    { page: 0 }, { page: -1 }, { page: 1.5 }, { page: NaN }, { page: Infinity },
    { pageSize: 0 }, { pageSize: 26 }, { pageSize: 1.5 }, { pageSize: null }, { pageSize: NaN },
    { minCoverage: 0 }, { minCoverage: 3 }, { minRaters: -1 }, { minRaters: 3 },
    { minRaters: NaN }, { mode: 'all' }, { query: null },
    { sort: { by: 'random' } }, { sort: { by: 'title', direction: 'up' } },
    { sort: { by: 'position' } }, { sort: { by: 'score', participantId: 'missing' } },
    { sort: { by: 'title', participantId: 'a' } }, { sort: {} }, { sort: null },
  ])('rejects invalid page, filter or sort options %#', (options) => {
    expect(() => getComparisonPage(fixture(), options as ComparisonPageOptions)).toThrow(FriendComparisonValidationError);
  });
});
...
describe('private-list scale with bounded friend intersections', () => {
  it('accepts 10,000 local rows and five 200-row friends, producing 15 bounded pair summaries', () => {
    const self = Array.from({ length: 10_000 }, (_, index) => sourced('manual', String(index + 1), index + 1, index % 11));
    const friends = Array.from({ length: 5 }, (_, person) => ready(`friend-${person}`,
    ...
    expect(comparison.summary).toMatchObject({ availableGameCount: 10_000, sharedGameCount: 200, jointlyRatedCount: 200 });
    ...
    expect(row(comparison, 'manual:10000').cells.map(({ position }) => position)).toEqual([10_000, 200, 200, 200, 200, 200]);
