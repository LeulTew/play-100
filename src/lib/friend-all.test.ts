...
const policy: FriendAllPolicy = {
  ...
  revision: 1,
  syncEpoch: 4,
  ranking: { epoch: 2, revision: 2 },
  shelf: { epoch: 2, revision: 2 },
  updatedAt: 1,
};
function facts(patch: Partial<FriendAllFacts> = {}): FriendAllFacts {
  return {
    uid: 'account-a',
    scope: 'account:demo-play100:account-a',
    projectId: 'demo-play100',
    verified: true,
    cacheReady: true,
    confirmed: true,
    source: { enabled: true, deleted: false, epoch: 4 },
    ranking: null,
    shelf: null,
    policy: null,
    ...patch,
...
function library(count: number): PersonalLibraryState {
  ...
  for (let index = 1; index <= count; index += 1) {
    ...
    state.records[id] = {
      ...
      studio: 'Private imported studio',
      genre: 'Private imported genre',
      collectionRank: null,
    };
    state.ranking.push({
      id,
      score: index === 1 ? 0 : null,
      note: 'Never publish this private note',
...
describe('versioned all-account sharing eligibility', () => {
  ...
  it('recognizes current all-mode bindings and never turns a recorded Stop back on', () => {
    ...
    expect(
      friendAllEligibility(facts({ ranking, shelf, policy: { ...policy, enabled: false, origin: 'explicit' } })),
    ).toEqual({ kind: 'off', canEnable: true });
  });
  it('treats a disabled default as a setup waiting for online saving, not as a recorded Stop', () => {
    const waiting: Partial<FriendAllFacts> = {
      ranking: { ...ranking, enabled: false },
      shelf: { ...shelf, enabled: false, consentSyncEpoch: null },
      policy: { ...policy, enabled: false },
    };
    expect(friendAllEligibility(facts(waiting))).toEqual({ kind: 'default', canEnable: true });
    expect(friendAllEligibility(facts({ ...waiting, source: null }))).toEqual({
      kind: 'paused',
      reason: 'saving',
      canEnable: false,
    });
    expect(friendAllEligibility(facts({ ...waiting, source: { enabled: false, deleted: false, epoch: 5 } }))).toEqual({
      kind: 'paused',
      reason: 'saving',
      canEnable: false,
    });
    expect(
      friendAllEligibility(facts({ ...waiting, policy: { ...policy, enabled: false, origin: 'explicit' } })),
    ).toEqual({ kind: 'off', canEnable: true });
    expect(
      friendAllEligibility(facts({ ...waiting, ranking: { ...ranking, enabled: false, epoch: 3, revision: 3 } })),
    ).toEqual({ kind: 'legacy', reason: 'changed-controls', canEnable: true });
  });
  it('treats either old-client Stop or selection change as authoritative', () => {
    for (const enabled of [false, true]) {
      expect(
        friendAllEligibility(facts({ ranking: { ...ranking, enabled, epoch: 3, revision: 3 }, shelf, policy })),
      ).toEqual({ kind: 'legacy', reason: 'changed-controls', canEnable: true });
      expect(
        friendAllEligibility(facts({ ranking, shelf: { ...shelf, enabled, epoch: 3, revision: 3 }, policy })),
      ).toEqual({ kind: 'legacy', reason: 'changed-controls', canEnable: true });
    }
    expect(
      friendAllEligibility(facts({ ranking: { ...ranking, selectedIds: ['wikidata:Q1'] }, shelf, policy })).kind,
    ).toBe('legacy');
  });
  it('keeps pause and saving-epoch restart distinct from active sharing', () => {
    expect(
      friendAllEligibility(facts({ ranking, shelf, policy, source: { enabled: false, deleted: false, epoch: 5 } })),
    ).toEqual({ kind: 'paused', reason: 'saving', canEnable: false });
    expect(
      friendAllEligibility(facts({ ranking, shelf, policy, source: { enabled: true, deleted: false, epoch: 6 } })),
    ).toEqual({ kind: 'paused', reason: 'saving-restarted', canEnable: true });
  });
  it.each([
  ...
  it('rejects a foreign policy and never adopts the guest scope', () => {
    expect(() => friendAllEligibility(facts({ ranking, shelf, policy: { ...policy, uid: 'account-b' } }))).toThrow(
      'another account',
    );
    expect(friendAllEligibility(facts({ uid: 'account-b', ranking, shelf, policy })).kind).toBe('paused');
    expect(
      friendAllEligibility(facts({ scope: 'account:play100-online-48823b32:account-a', ranking, shelf, policy })).kind,
    ).toBe('paused');
  });
});
...
describe('friends sharing page view', () => {
  const settled: FriendAllEligibility[] = [
    { kind: 'legacy', reason: 'existing-choice', canEnable: true },
    { kind: 'off', canEnable: true },
    { kind: 'paused', reason: 'saving', canEnable: false },
  ...
  it('never renders the selected-sharing editors for a connected account until automatic sharing settles', () => {
    expect(
      friendSharingView({ controlsAll: false, connected: true, ready: false, eligibility: { kind: 'checking' } }),
    ).toBe('checking');
    expect(
      friendSharingView({
        controlsAll: false,
        connected: true,
        ready: true,
        eligibility: { kind: 'default', canEnable: true },
      }),
    ).toBe('checking');
    for (const eligibility of settled)
      expect(friendSharingView({ controlsAll: false, connected: true, ready: true, eligibility })).toBe('selected');
    expect(
      friendSharingView({ controlsAll: true, connected: true, ready: false, eligibility: { kind: 'checking' } }),
    ).toBe('automatic');
    expect(
      friendSharingView({
        controlsAll: true,
        connected: true,
        ready: true,
        eligibility: { kind: 'all', canEnable: false },
      }),
    ).toBe('automatic');
  });
  it('keeps the non-interactive selected pages for accounts that cannot share yet', () => {
    expect(
      friendSharingView({ controlsAll: false, connected: false, ready: false, eligibility: { kind: 'checking' } }),
    ).toBe('selected');
    expect(
      friendSharingView({
        controlsAll: false,
        connected: false,
        ready: true,
        eligibility: { kind: 'paused', reason: 'verification', canEnable: false },
      }),
    ).toBe('selected');
  });
});
...
describe('complete, privacy-limited all-account projection', () => {
  it('projects all10,000 games and rankings without the old200-entry truncation', () => {
    ...
    expect(JSON.stringify({ games, ranks })).not.toMatch(
      /Never publish|Private imported|later|played|completed|queueOrder|manualPosition/,
    );
  });
  ...
  describe('paged comparison knowledge', () => {
    ...
    const self: ComparisonParticipant = {
      id: 'a',
      displayName: 'Self',
      kind: 'self',
      availability: 'ready',
    ...
    const peer: ComparisonParticipant = {
      id: 'b',
      displayName: 'Friend',
      kind: 'friend',
      availability: 'ready',
    ...
    it('marks an exact absent identity known without pretending the whole203-game list was read', () => {
      const compared = compareFriendRankings([
        self,
        { ...peer, coverage: { kind: 'exact', total: 203, ids: [q1.id, q202.id] } },
      ]);
    ...
    it('accepts a real rank beyond200 only with explicit new coverage and refuses malformed coverage', () => {
      expect(
        compareFriendRankings([self, { ...peer, entries: [q202] }]).rows['all-shared'].find(
          (row) => row.game.id === q202.id,
        )?.cells[1],
      ).toMatchObject({ status: 'ranked', position: 202 });
      expect(() =>
        compareFriendRankings([
          self,
          { ...peer, coverage: { kind: 'exact', total: 203, ids: [q1.id] }, entries: [q202] },
        ]),
      ).toThrow('unresolved');
      expect(() => compareFriendRankings([self, { ...peer, coverage: { kind: 'complete', total: 203 } }])).toThrow(
        'coverage',
      );
    });
  });
  ...
  it('handles empty, new, removed and re-added games through current membership without selected-mode suppression', () => {
    ...
    expect(planFriendAllChanges(removed, all)).toEqual({
      count: 202,
      upserts: [all.find((entry) => entry.id === 'wikidata:Q202')],
      removals: [],
    });
    expect(planFriendAllChanges(all, [])).toMatchObject({
      count: 0,
      upserts: [],
      removals: expect.arrayContaining(['wikidata:Q202']),
    });
  });
  it('plans one changed ranking record rather than rewriting the complete projection', () => {
    ...
    expect(planFriendAllChanges(before, after)).toEqual({
      count: 250,
      upserts: [after.find((entry) => entry.id === 'wikidata:Q201')],
      removals: [],
    });
    expect(planFriendAllChanges(after, after)).toEqual({ count: 250, upserts: [], removals: [] });
    const reordered = after.map((entry) => ({
      score: entry.score,
