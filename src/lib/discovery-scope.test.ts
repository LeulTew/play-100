...
describe('Discover outside-The-100 result scope', () => {
  ...
  it('resolves reviewed source aliases before returning recovery links, not duplicate cards', () => {
    const provider = {
      ...discoveryFixture,
      record: { ...discoveryFixture.record, id: 'wikidata:Q27438121', sourceId: 'Q27438121', title: 'RDR2' },
      aliases: ['RDR2'],
    };
  ...
  it('does not merge an unknown same-title edition and applies genre family and exact filters together', () => {
    ...
    const scope = discoveryScope(catalogSearchItems(games, [edition]), {
      ...defaultDiscoveryFilters,
      q: edition.record.title,
  ...
  it('persists include scope without changing legacy genre, detail or unrelated parameters', () => {
    const next = patchDiscoverySearch(
      '?genre=Action+RPG&genreFamily=role-playing&game=wikidata%3AQ15408545&campaign=retained',
      { include100: 'on', offset: 0 },
    );
    expect(parseDiscoverySearch(next)).toMatchObject({
      genre: 'Action RPG',
      genreFamily: 'role-playing',
      include100: 'on',
    });
