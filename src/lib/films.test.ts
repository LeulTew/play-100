...
describe('finished first-party collection films', () => {
  ...
  it('retains exact immutable video, poster, caption and credit bytes', async () => {
    ...
    for (const film of collectionFilms) {
      ...
      expect(await sharp(asset(film.poster.src)).metadata()).toMatchObject({ format: 'jpeg', width: film.poster.width, height: film.poster.height });
  ...
  it('ships fast-start MP4 files with the metadata before the media payload', () => {
    for (const film of collectionFilms) {
      ...
      for (let offset = 0; offset < data.byteLength;) {
        ...
        const size = header === 1 ? Number(data.readBigUInt64BE(offset + 8)) : header === 0 ? data.byteLength - offset : header;
        expect(size).toBeGreaterThanOrEqual(8);
        expect(offset + size).toBeLessThanOrEqual(data.byteLength);
        boxes.push(data.toString('ascii', offset + 4, offset + 8)); offset += size;
      }
  ...
  it('uses self-hosted media caching without weakening the auth-helper policy', () => {
    ...
    expect(auth?.headers.find(header => header.key === 'Content-Security-Policy')?.value).toContain("frame-ancestors 'self'");
  });
  it.each([[0, '0:00'], [22, '0:22'], [59.8, '1:00']])('formats %s seconds without a sixty-second remainder', (seconds, expected) => {
