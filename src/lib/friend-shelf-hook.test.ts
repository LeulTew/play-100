...
const harness = vi.hoisted(() => {
  ...
  return {
    cursor: 0, slots: [] as unknown[],
    effects: new Map<number, { dependencies: readonly unknown[]; cleanup: (() => void) | void }>(),
    pending: [] as Array<() => void>,
    app, config: vi.fn().mockResolvedValue(null), watchConfig: vi.fn(() => vi.fn()),
  };
});
...
vi.mock('react', () => {
  ...
  const memo = <T,>(create: () => T, dependencies: readonly unknown[]): T => {
    ...
    const value = create(); harness.slots[index] = { value, dependencies }; return value;
  };
  return {
    useMemo: memo,
    ...
    useState: <T,>(initial: T | (() => T)) => {
      ...
      return [harness.slots[index] as T, (next: T | ((previous: T) => T)) => {
        harness.slots[index] = typeof next === 'function' ? (next as (previous: T) => T)(harness.slots[index] as T) : next;
      }];
    },
    useEffect: (effect: Effect, dependencies: readonly unknown[]) => {
      const index = harness.cursor++; const previous = harness.effects.get(index);
      if (!previous || !same(previous.dependencies, dependencies)) harness.pending.push(() => {
        previous?.cleanup?.();
...
vi.mock('../cloud/firebase-client', () => ({ cloudAuth: { app: harness.app, currentUser: { uid: 'owner' } }, cloudDb: { app: harness.app } }));
vi.mock('../cloud/friend-shelf-store', () => ({
  FriendShelfStore: class { config = harness.config; watchConfig = harness.watchConfig; },
}));
...
afterEach(() => { for (const effect of harness.effects.values()) effect.cleanup?.(); vi.unstubAllGlobals(); });
function snapshot(): ScopedLibrary {
  return { version: 1, scope, state: emptyPersonalLibrary(), profile: null, recovery: null,
