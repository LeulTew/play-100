  ...
  const read = async () => {
    ...
    if (!response.ok) {
      ...
      const kind: CatalogFailure =
        error.code === 'rate-limited' || response.status === 429
          ? 'rate-limited'
          : error.code === 'timeout' || response.status === 504
            ? 'timeout'
            : 'unavailable';
      const retryAfter = Math.min(60, Math.max(0, Number(response.headers.get('retry-after')) || 0));
      throw new CatalogRequestError(
        typeof error.error === 'string' ? error.error.slice(0, 500) : 'The public catalog is temporarily unavailable.',
        kind,
        retryAfter,
      );
    }
