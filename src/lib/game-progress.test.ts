...
describe('distinct played/completed progress', () => {
  it.each([
    ...
    expect(filterGames(games, filters, progress).map((game) => game.slug)).toEqual(
      indices.map((index) => records[index]!.id),
    );
    const external = records.map((record, index) => ({
      ...record,
      id: `manual:${index}`,
    ...
    expect(
      values.map((value, index) => (matchesProgress(value, view) ? index : -1)).filter((index) => index >= 0),
    ).toEqual([...indices]);
  });
  it('keeps played, completion, queue, scores, notes and manual positions distinct', () => {
    ...
    expect(state.queueOrder).toEqual(queue);
  });
  it('maps every bulk action explicitly; Mark played never falls through to queue or completion', () => {
    expect(selectionOperation('played', records)).toEqual({
      type: 'set-progress',
      records,
      key: 'played',
      value: true,
    });
    expect(selectionOperation('completed', records)).toEqual({
      type: 'set-progress',
      records,
      key: 'completed',
      value: true,
    });
    expect(selectionOperation('uncomplete', records)).toEqual({
      type: 'set-progress',
      records,
      key: 'completed',
      value: false,
    ...
    expect(selectionOperation('remove-later', records)).toEqual({
      type: 'set-progress',
      records,
      key: 'later',
      value: false,
  ...
  it('keeps the existing backup migration and never infers progress from ranking or source notes', () => {
    ...
    const old = {
      ...state,
      version: 2,
      ranking: state.ranking.map((entry) => ({ id: entry.id, score: entry.score, note: entry.note })),
    };
...
describe('Pick for me candidates', () => {
  ...
  const picked = (patch: Partial<Filters>) => {
    ...
    return pickCandidates(filterGames(games, filters, progress).map(recordFromGame), progress, filters).map(
      (record) => record.id,
    );
  };
  it.each([
