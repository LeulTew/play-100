...
it.each([undefined, {}, { onLine: true }, { onLine: false }])('requires an explicit browser offline signal: %j', async (navigator) => {
  ...
  try {
    ...
    if (navigator?.onLine === false) {
      await expect(store.initialize('owner')).rejects.toMatchObject({ code: 'offline' }); expect(activity).not.toHaveBeenCalled();
    } else { await expect(store.initialize('owner')).rejects.toBe(failure); expect(activity).toHaveBeenCalledOnce(); }
...
it('uses the shared jittered queue without letting edits or Retry shorten a quota cooldown', async () => {
  ...
  const task = vi.fn().mockRejectedValueOnce(new FriendStoreError('conflict', 'Source advanced')).mockRejectedValueOnce({ code: 'resource-exhausted' }).mockResolvedValue(undefined);
  const queue = new SyncWorkQueue(task, (cause) => queue.failed(friendShelfFailure(cause)));
  queue.request(); await vi.advanceTimersByTimeAsync(0); expect(task).toHaveBeenCalledTimes(1);
  await vi.advanceTimersByTimeAsync(2000); expect(task).toHaveBeenCalledTimes(2);
  const due = queue.nextAttemptAt;
  queue.request(0, true); await queue.retry(); expect(queue.nextAttemptAt).toBe(due);
  await vi.advanceTimersByTimeAsync(59999); expect(task).toHaveBeenCalledTimes(2);
  await vi.advanceTimersByTimeAsync(1); expect(task).toHaveBeenCalledTimes(3);
  queue.dispose();
});
it('invalidates an in-flight publication lease synchronously rather than waiting for effect cleanup', () => {
  const scope = createFriendWorkGeneration(); const lease = scope.next();
  expect(scope.current(lease)).toBe(true);
  scope.cancel();
  expect(scope.current(lease)).toBe(false);
  const newOwner = scope.next();
  expect(scope.current(newOwner)).toBe(true); expect(scope.current(lease)).toBe(false);
});
