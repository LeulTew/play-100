...
describe('counted All-sharing production query indexes', () => {
  it('pins the only creator-attributed capacity cleanup query to its exact index', () => {
    expect(
      indexes.indexes.filter(
        (index) =>
          index.collectionGroup === 'friendPairs' && index.fields.some((field) => field.fieldPath === 'creatorUid'),
      ),
    ).toEqual([
      {
        collectionGroup: 'friendPairs',
    ...
    expect(sourceTokens(store)).toContain(
      sourceTokens("where('participants', 'array-contains', uid), where('creatorUid', '==', uid)"),
    );
    expect(sourceTokens(store)).toContain(
      sourceTokens("where('state', 'in', ['cancelled', 'removed', 'declined']), orderBy('updatedAt')"),
    );
  });
  it.each(['entry.title', 'entry.position'])(
    'declares the exact format/epoch/active query shape with %s ordering',
    (ordered) => {
  ...
  it('pins the counted one-row step and format-aware query source beside the existing legacy path', () => {
    ...
    expect(
      indexes.indexes.filter((index) => index.collectionGroup === 'entries' && index.fields[0]?.fieldPath === 'format'),
    ).toHaveLength(2);
  });
});
