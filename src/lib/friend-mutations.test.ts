...
describe('acknowledged friendship changes and independent metadata refresh', () => {
  ...
  it('reports a known commit only after acknowledgement, even when its readback fails', async () => {
    ...
    const transaction = vi
      .mocked(firestore.runTransaction)
    ...
    const outcome = expect(result).rejects.toMatchObject({
      ...
      committed: true,
      phase: 'refresh',
      receipt: { operation: 'send-request', uid: 'alice', otherUid: 'bob', epoch: 2 },
      cause: refreshError,
    });
  ...
  it.each(['accept', 'decline', 'cancel', 'remove'] as const)(
    'distinguishes a committed %s from its failed pair refresh',
    async (action) => {
      const store = client();
      vi.spyOn(store, 'settings').mockResolvedValue(settings);
      ...
      await expect(store.respond('alice', 'bob', action, 2)).rejects.toMatchObject({
        committed: true,
        receipt: { operation: 'respond', uid: 'alice', otherUid: 'bob', epoch: 3 },
      });
    },
  );
  it('preserves consumed invitation acknowledgement without putting the capability in its receipt', async () => {
    ...
    const serverReads = mockServerReads(
      new Map<string, firestore.DocumentData | null>([
    ...
    vi.mocked(firestore.runTransaction)
      .mockResolvedValueOnce({ ownerUid: 'bob', epoch: 1 })
      .mockRejectedValueOnce(refreshError);
    await expect(store.acceptInvite('alice', token)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'accept-invite', uid: 'alice', otherUid: 'bob', epoch: 1 },
      cause: refreshError,
    });
    ...
    expect(serverReads.mock.calls.map(([ref]) => ref.path)).toEqual([
      'accountQuotas/alice/limits/pairs',
      `friendInvites/${token}`,
    ]);
  });
  ...
  it('distinguishes initialization and sharing-settings acknowledgement from failed settings reads', async () => {
    ...
    vi.mocked(firestore.runTransaction).mockResolvedValueOnce(undefined).mockRejectedValueOnce(refreshError);
    await expect(store.initialize('alice')).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'initialize', uid: 'alice' },
    });
    vi.mocked(firestore.runTransaction).mockResolvedValueOnce(undefined).mockRejectedValueOnce(refreshError);
    await expect(store.saveSettings('alice', { enabled: false, selectedIds: [] }, settings)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'save-settings', uid: 'alice' },
    });
  });
  it('distinguishes identity/group acknowledgement from failed metadata refreshes', async () => {
    ...
    await expect(store.saveIdentity('alice', { displayName: 'Alice', avatar }, 0)).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'save-identity', uid: 'alice' },
    });
    expect(serverReads).not.toHaveBeenCalled();
    vi.mocked(firestore.runTransaction).mockResolvedValueOnce(undefined).mockRejectedValueOnce(refreshError);
    await expect(
      store.saveGroup('alice', { id, name: 'Friends', participantUids: ['alice', 'bob'] }, 0),
    ).rejects.toMatchObject({
      committed: true,
      receipt: { operation: 'save-group', uid: 'alice', groupId: id, revision: 1 },
    });
    expect(listGroups).toHaveBeenCalledOnce();
    expect(listGroups).toHaveBeenCalledWith('alice', undefined);
    expect(serverReads.mock.calls.map(([ref]) => ref.path)).toEqual([
      'accountQuotas/alice/limits/groups',
      `friendGroups/alice/items/${id}`,
    ]);
  });
  it.each(['refresh', 'cleanup'] as const)(
    'keeps a committed ranking distinct from a later %s failure',
    async (phase) => {
      const store = client();
      ...
      const head: FriendShareHead = {
        format: 1,
        epoch: 1,
        settingsRevision: 1,
        revision: 1,
        source: { syncEpoch: 1, remoteRevision: 0 },
        current: { generation, digest: '0'.repeat(64), count: 0 },
        previous: null,
        updatedAt: 1000,
      };
      ...
      await expect(store.publishRanking('alice', [], settings, head.source, 0)).rejects.toMatchObject({
        committed: true,
        phase,
        receipt: { operation: 'publish-ranking', uid: 'alice', generation, epoch: 1, revision: 1 },
        cause: refreshError,
      });
    },
  );
