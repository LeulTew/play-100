...
export const progressLabels: Record<ProgressFilter, string> = {
  all: 'All progress', 'not-played': 'Not played', unfinished: 'Played (not completed)',
  completed: 'Completed', 'any-played': 'Played, including completed', 'not-completed': 'Not completed',
};
...
export function matchesProgress(value: Partial<PersonalProgress> | undefined, filter: ProgressFilter): boolean {
  ...
  switch (filter) {
    ...
    case 'not-played': return !played;
    case 'unfinished': return played && !completed;
    case 'completed': return completed;
    case 'any-played': return played;
    case 'not-completed': return !completed;
  }
}
...
export function selectionOperation(action: SelectionAction, records: LibraryRecord[]): PersonalAction {
  switch (action) {
    case 'ranking': return { type: 'add-ranking', records };
    case 'played': return { type: 'set-progress', records, key: 'played', value: true };
    case 'completed': return { type: 'set-progress', records, key: 'completed', value: true };
    case 'uncomplete': return { type: 'set-progress', records, key: 'completed', value: false };
    case 'later': return { type: 'set-progress', records, key: 'later', value: true };
    case 'remove-later': return { type: 'set-progress', records, key: 'later', value: false };
  }
}
