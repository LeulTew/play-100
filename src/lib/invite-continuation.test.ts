...
import { captureInviteContinuation, clearInviteContinuation, createInviteUrl, readInviteContinuation, saveInviteContinuation } from './invite-continuation';
...
beforeEach(() => {
  ...
  vi.stubGlobal('location', { pathname: '/invite', hash: `#${synthetic}`, origin: 'https://play-100-collection.vercel.app' });
...
it('expires the temporary resume and clears it on successful consumption', () => {
  ...
  saveInviteContinuation(synthetic); clearInviteContinuation();
  expect(readInviteContinuation()).toBeNull();
});
it('rejects malformed and private-data-shaped intents instead of routing arbitrary URLs', () => {
  vi.stubGlobal('location', { pathname: '/invite', hash: '#https://untrusted.invalid/', origin: 'https://play-100-collection.vercel.app' });
...
it('keeps the opened invitation available in memory with an explicit warning if temporary storage fails', () => {
  vi.stubGlobal('sessionStorage', { setItem: () => { throw new DOMException('Denied', 'SecurityError'); } });
  const result = captureInviteContinuation();
  expect(result.capability).toBe(synthetic);
  expect(result.error).toContain('cannot keep the invitation');
  expect(history.replaceState).toHaveBeenCalledWith(null, '', '/invite');
});
it.each(['normal acknowledgement', 'committed refresh error'])('an old %s cannot clear a newer invitation continuation', () => {
