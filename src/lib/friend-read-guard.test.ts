...
it('a deferred ranking result cannot repopulate data after a relationship permission error', async () => {
  ...
  const read = new Promise<number>((resolve) => {
    ...
    if (access.permits(lease)) visibleScore = score;
  });
  access.revoke();
  visibleScore = null;
  finish?.(9);
...
it.each(['settings', 'head'])(
  'stopping during a deferred %s read invalidates work synchronously, while explicit retry gets a new generation',
  async () => {
