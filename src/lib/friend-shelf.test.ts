...
import {
  ...
  parseFriendShelfChunk, parseFriendShelfConfig, parseFriendShelfEntry, projectFriendShelf, recordFromFriendShelf, validateFriendShelfEntries,
} from './friend-shelf-types';
import {
  applyShelfRemovals, friendShelfSelectionKey, parseShelfSelectionCache, pendingShelfRemovals, rememberShelfSelection,
...
describe('selected saved-library shelf, not a ranking projection', () => {
  it('includes selected unranked manual and catalog additions, never queue/history/score/notes', () => {
    const catalog: LibraryRecord = { ...manual, id: 'wikidata:Q12', source: 'wikidata', sourceId: 'Q12', sourceUrl: 'https://www.wikidata.org/wiki/Q12' };
    let state = applyPersonalAction(emptyPersonalLibrary(), { type: 'add-records', records: [manual, catalog] });
    const before = structuredClone(state);
    expect(projectFriendShelf(state, [manual.id, catalog.id], []).entries).toEqual([entry, { ...entry, id: catalog.id, source: catalog.source, sourceId: catalog.sourceId, sourceUrl: catalog.sourceUrl }]);
    expect(state.ranking).toEqual([]); expect(state).toEqual(before);
  ...
  it('canonicalizes saved original records and refuses unknown canonical identities', () => {
    ...
    const state = applyPersonalAction(emptyPersonalLibrary(), { type: 'add-records', records: [{ ...original, title: 'Forged local title' }] });
  ...
  it('rejects every private/ranking/art field, malformed sources and noncanonical URLs', () => {
    for (const key of ['position', 'score', 'note', 'email', 'queue', 'played', 'completed', 'imageUrl', 'avatar', 'studio', 'genre']) {
      expect(() => parseFriendShelfEntry({ ...entry, [key]: 'not allowed' })).toThrow();
    }
    for (const patch of [{ title: '' }, { title: '  ' }, { title: 'x'.repeat(201) }, { title: 'bad\nname' }, { year: 2101 }, { year: 1.5 }, { id: 'manual:mismatch' }, { sourceId: 'x/y' },
      { sourceUrl: 'data:image/svg+xml,<svg/>' }, { source: 'wikidata', sourceId: 'Q0', id: 'wikidata:Q0', sourceUrl: 'https://www.wikidata.org/wiki/Q0' },
      { source: 'steam', sourceId: '12', id: 'steam:12', sourceUrl: 'https://store.steampowered.com/app/12/?redirect=evil' },
      { source: 'freetogame', sourceId: '12', id: 'freetogame:12', sourceUrl: 'https://www.freetogame.com.evil.test/game' }]) {
  ...
  it('strictly bounds two immutable entries per chunk and all 200 selected games', async () => {
    const entries = Array.from({ length: 200 }, (_, index) => ({ ...entry, id: `manual:game-${index}`, sourceId: `game-${index}` }));
    expect(FRIEND_SHELF_CHUNK_SIZE).toBe(2); expect(FRIEND_SHELF_CHUNK_LIMIT).toBe(100);
    expect(validateFriendShelfEntries(entries, entries.map((item) => item.id))).toEqual(entries);
    for (let i = 0; i < 100; i += 1) {
      const items = entries.slice(i * 2, i * 2 + 2);
      expect(parseFriendShelfChunk({ index: i, entries: items, ids: items.map((item) => item.id) }, i, 200)).toEqual(items);
    }
    expect(() => validateFriendShelfEntries([...entries, entry], [...entries.map((item) => item.id), entry.id])).toThrow();
    expect(() => validateFriendShelfEntries([entry], [])).toThrow();
    expect(() => parseFriendShelfChunk({ index: 0, entries: [entry, entry], ids: [entry.id, entry.id] }, 0, 2)).toThrow();
