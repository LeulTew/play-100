...
export function discoveryScope<T extends CatalogSearchItem>(items: readonly T[], filters: Pick<DiscoveryFilters, 'q' | 'source' | 'genre' | 'genreFamily' | 'year' | 'include100'>) {
  ...
  return {
    ...
    collectionMatches,
    showCollection,
    exactCollectionMatch: Boolean(filters.q.trim() && collectionMatches.some(item => {
