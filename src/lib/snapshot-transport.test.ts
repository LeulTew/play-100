...
describe('bounded immutable snapshot transport', () => {
  it('roundtrips a normal 100-game library with exact private scores, notes and manual positions', async () => {
    const state = applyPersonalAction(emptyPersonalLibrary(), { type: 'add-ranking', records: games.map(recordFromGame) });
    const first = state.ranking[0];
    if (!first) throw new Error('Missing fixture.');
    first.score = 9.7; first.note = 'Private note \u4e16\u754c'; first.manualPosition = 1;
    const packed = await packLibrary(state);
    const chunks = new Map(packed.chunks.map((chunk) => [chunk.digest, chunk]));
    expect(await unpackLibrary(packed.manifest, async (digest) => chunks.get(digest))).toEqual({ ...state, revision: 0, motion: 'auto' });
    ...
    expect((await packLibrary({ ...state, revision: 700, motion: 'full' })).manifest.digest).toBe(packed.manifest.digest);
  });
  ...
  it('roundtrips 10,000 valid records beyond a Firestore document without truncation', async () => {
    const records: LibraryRecord[] = Array.from({ length: 10_000 }, (_, index) => ({
      id: `wikidata:Q${index + 1}`, source: 'wikidata', sourceId: `Q${index + 1}`, sourceUrl: null,
      title: `Game ${index} ${'long title '.repeat(8)}`, year: 2020, genre: null, studio: null, collectionRank: null,
    }));
    ...
    expect(packed.chunks.every((chunk) => chunk.bytes <= CHUNK_BYTES && JSON.stringify(chunk).length < 300_000)).toBe(true);
    const chunks = new Map(packed.chunks.map((chunk) => [chunk.digest, chunk]));
  ...
  it('rejects missing chunks, altered digests and future versions before replacing state', async () => {
    ...
    await expect(unpackLibrary(packed.manifest, async () => ({ ...chunk, data: 'A' + chunk.data.slice(1) }))).rejects.toThrow(/integrity/);
    await expect(unpackLibrary({ ...packed.manifest, digest: 'a'.repeat(64) }, async () => chunk)).rejects.toThrow(/damaged/);
