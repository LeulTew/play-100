...
const records: LibraryRecord[] = ['a', 'b', 'c', 'd'].map((id, index) => ({
  id, title: `Game ${id}`, year: 2020, studio: null, genre: null, source: 'collection',
  sourceId: id, sourceUrl: null, collectionRank: index + 1,
}));
...
const scored = () => apply(ranked(), ...records.map((record, index): PersonalAction => ({
  type: 'edit-ranking', id: record.id, score: 9 - index,
})));

describe('automatic score order with durable manual slots', () => {
  it('sorts new ratings highest first without marking a game played', () => {
    const state = apply(ranked(), { type: 'edit-ranking', id: 'c', score: 8 }, { type: 'edit-ranking', id: 'b', score: 9 });
  ...
  it('keeps ties stable and puts unrated entries after a genuine zero rating', () => {
    const state = apply(ranked(), { type: 'edit-ranking', id: 'b', score: 8 }, { type: 'edit-ranking', id: 'a', score: 8 }, { type: 'edit-ranking', id: 'd', score: 0 });
  ...
  it('keeps a manually moved low-rated game at the exact selected position', () => {
    const state = apply(scored(), { type: 'move-item', list: 'ranking', id: 'd', overId: 'a' }, { type: 'edit-ranking', id: 'd', score: 0 }, { type: 'edit-ranking', id: 'b', score: 10 });
  ...
  it('holds multiple manual slots while sorting the remaining games', () => {
    ...
    expect(state.ranking.filter((entry) => entry.manualPosition !== null).map((entry) => [entry.id, entry.manualPosition])).toEqual([['d', 1], ['c', 3]]);
  });
  it('updates displaced manual slots when the user deliberately drags another item', () => {
    ...
    expect(state.ranking.filter((entry) => entry.manualPosition !== null).map((entry) => [entry.id, entry.manualPosition])).toEqual([['b', 1], ['d', 2], ['c', 4]]);
  });
  ...
  it('can explicitly clear every override and resume score ordering', () => {
    const state = apply(scored(), { type: 'move-item', list: 'ranking', id: 'd', overId: 'a' }, { type: 'use-rating-order' });
    expect(ids(state)).toEqual(['a', 'b', 'c', 'd']);
  ...
  it('preserves earlier version-two ordering instead of guessing which moves were manual', () => {
    const old = { ...scored(), version: 2, ranking: [
  ...
  it('imports version-two backups and round-trips version-three overrides', () => {
    const old = { ...scored(), version: 2, ranking: [{ id: 'b', score: 4, note: 'Old order' }, { id: 'a', score: 9, note: '' }] };
    const migrated = parseLibraryBackup({ app: 'Play 100', formatVersion: 2, exportedAt: '2026-09-14T00:00:00.000Z', library: old });
  ...
  it.each([0, 5, -1, 1.5, '1', NaN])('rejects invalid manual position %s without repairing user data silently', (position) => {
    const state = scored();
    expect(() => parsePersonalLibrary({ ...state, ranking: state.ranking.map((entry, index) => index ? entry : { ...entry, manualPosition: position }) })).toThrow();
  });
  it('rejects duplicate manual positions and unsupported schema versions', () => {
    const state = scored();
    expect(() => parsePersonalLibrary({ ...state, ranking: state.ranking.map((entry) => ({ ...entry, manualPosition: 1 })) })).toThrow();
