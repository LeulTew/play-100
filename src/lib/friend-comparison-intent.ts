...
export function comparisonScope(project: string, uid: string): string {
  if (!/^[a-z0-9-]{1,100}$/.test(project) || !/^[A-Za-z0-9_-]{1,128}$/.test(uid))
    throw new Error('The comparison account is invalid.');
  return `${project}:${uid}`;
}
...
export function parseComparisonView(value: unknown, scope: string): FriendComparisonView | null {
  ...
  if (
    ...
    row.query.length > 160 ||
    typeof row.page !== 'number' ||
    !Number.isSafeInteger(row.page) ||
    row.page < 1 ||
    row.page > 10000 ||
    (row.groupId !== null &&
      (typeof row.groupId !== 'string' || !/^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(row.groupId)))
  )
    return null;
  return {
    version: 1,
...
export function readComparisonView(scope: string): FriendComparisonView | null {
  ...
  try {
    ...
    console.warn('The comparison selection could not be restored in this tab.', cause);
    return null;
  }
}
export function rememberComparisonView(
  value: FriendComparisonView,
  attachToHistory = location.pathname === '/compare',
): void {
  ...
  try {
    sessionStorage.setItem(FRIEND_COMPARISON_KEY, JSON.stringify(value));
  } catch (cause) {
    console.warn(
      'This tab cannot remember comparisons between pages. The current comparison remains in browser history.',
      cause,
    );
  }
}
