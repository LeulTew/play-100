...
import { closePersonalLibrary, commitPersonalAction, loadPersonalLibrary, resetPersonalLibrary, restorePersonalLibrary } from './personal-db';
...
import { clearMotionHint, effectiveMotionPreference, MOTION_HINT_KEY, motionHintKey, parseMotionHint, readMotionHint, rememberMotionHint } from './motion-hint';
...
describe('optional scoped enum-only motion hints', () => {
  ...
  it.each([null, undefined, '', 'FULL', ' lite ', '"auto"', '{}', 1, { motion: 'full' }])('rejects malformed or absent hint %j', value => {
  ...
  it.each(['auto', 'full', 'lite'] as const)('uses hint only during loading and authority always wins for %s', motion => {
  ...
  it('writes guest default/load/save/restore/reset only after successful transactions', async () => {
    ...
    await loadPersonalLibrary([]);
    expect(readMotionHint('guest')).toBe('lite');
    const put = vi.spyOn(IDBObjectStore.prototype, 'put').mockImplementation(() => { throw new Error('Write failed'); });
