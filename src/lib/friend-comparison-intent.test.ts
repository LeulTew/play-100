...
import { clearComparisonView, comparisonScope, FRIEND_COMPARISON_KEY, initialComparison, parseComparisonView, readComparisonView, rememberComparisonView } from './friend-comparison-intent';

const scope = comparisonScope('demo-play100', 'owner');
function browser() {
  const storage = new Map<string, string>();
  const history = { state: {} as Record<string, unknown>, replaceState: vi.fn((value: Record<string, unknown>) => { history.state = value; }) };
  const sessionStorage = {
    getItem: vi.fn((key: string) => storage.get(key) ?? null),
    setItem: vi.fn((key: string, value: string) => { storage.set(key, value); }),
    removeItem: vi.fn((key: string) => { storage.delete(key); }),
  };
  vi.stubGlobal('history', history); vi.stubGlobal('sessionStorage', sessionStorage);
...
describe('private account-bound comparison intent', () => {
  it('includes self with at most five distinct valid peers', () => {
    ...
    expect(() => initialComparison(scope, 'owner', Array.from({ length: 6 }, (_, index) => `peer${index}`))).toThrow();
  ...
  it('rejects scope mismatches, malformed fields, excess participants and unsupported modes', () => {
    ...
    for (const extra of [{ scope: 'demo-play100:other' }, { extra: true }, { query: 'x'.repeat(161) }, { page: 0 }, { page: 1.5 }, { mode: 'mean' }, { groupId: 'bad' }, { selected: ['peer', 'peer'] }, { selected: ['invalid uid'] }]) {
  ...
  it('retains selection and view through tab/history restoration without adding a public query', () => {
    const b = browser();
    const view = { ...initialComparison(scope, 'owner', ['peer']), query: 'Long game', mode: 'all-shared' as const, page: 2 };
  ...
  it('keeps browser-history restoration available when tab storage is denied', () => {
    const b = browser(); const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    b.sessionStorage.setItem.mockImplementation(() => { throw new DOMException('Denied', 'SecurityError'); });
  ...
  it('reports invalid own-tab data and never clears another account preference', () => {
    ...
    expect(readComparisonView(scope)).toBeNull(); expect(warn).toHaveBeenCalledOnce();
