...
export function saveInviteContinuation(capability: string): void {
  ...
  try {
    ...
    throw new Error(
      'This tab cannot keep the invitation through sign-in. Keep the original link and reopen it after signing in.',
    );
  }
}
  ...
  try {
    if (expectedCapability !== undefined) {
      ...
      if (
        ...
        !('capability' in stored) ||
        stored.capability !== expectedCapability
      )
        return;
    }
    sessionStorage.removeItem(KEY);
  } catch {
    console.warn('The invitation could not be cleared from this tab. Close it when finished.');
  }
}
export function readInviteContinuation(): string | null {
  ...
  try {
    ...
    if (
      !data ||
      typeof data !== 'object' ||
      Object.keys(data).sort().join() !== 'capability,expiresAt,version' ||
      !('version' in data) ||
      data.version !== 1 ||
      !('capability' in data) ||
      !isInviteCapability(data.capability) ||
      !('expiresAt' in data) ||
      typeof data.expiresAt !== 'number' ||
      data.expiresAt < Date.now() ||
      data.expiresAt > Date.now() + lifetime
    )
      throw new Error();
...
export function captureInviteContinuation(): { capability: string | null; error: string } {
  ...
  if (!fragment) {
    try {
      return { capability: readInviteContinuation(), error: '' };
    } catch {
      return {
        capability: null,
        error: 'This tab cannot restore the invitation. Reopen the original link after signing in.',
      };
    }
  }
  ...
  try {
    saveInviteContinuation(fragment);
    return { capability: fragment, error: '' };
  } catch (cause) {
    return {
      capability: fragment,
      error: cause instanceof Error ? cause.message : 'Keep the original invitation before signing in.',
    };
  }
}
