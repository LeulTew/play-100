...
export function parseCatalogArtwork(value: unknown): CatalogArtwork {
  ...
  const matchesLicense = expectedLicense
    ...
    : licensePath === '/publicdomain/zero/1.0/'
      ? license === 'CC0'
  ...
  return {
    src,
    width: integer(row.width, DISCOVERY_LIMITS.imageEdge),
    height: integer(row.height, DISCOVERY_LIMITS.imageEdge),
    alt: text(row.alt, 300),
    sourceUrl,
    credit: text(row.credit, 2_000),
    license,
    licenseUrl,
    originalUrl,
    retrievedAt: timestamp(row.retrievedAt),
    sha256,
    bytes: integer(row.bytes, DISCOVERY_LIMITS.imageBytes),
  };
}
...
export function parseDiscoveryCatalog(value: unknown): DiscoveryCatalog {
  ...
  if (
    ...
    !root.items.length ||
    root.items.length > DISCOVERY_LIMITS.items
  )
  ...
  const pending = root.items.map((value: unknown) => {
    ...
    if (
      (record.source !== 'wikidata' && record.source !== 'freetogame') ||
    ...
    const sourceUrl = https(
      record.sourceUrl,
      record.source === 'wikidata' ? ['www.wikidata.org'] : ['www.freetogame.com', 'freetogame.com'],
    );
    ...
    records[id] = record;
    if (!Array.isArray(row.aliases) || row.aliases.length > DISCOVERY_LIMITS.aliases)
      return invalid('too many aliases.');
