...
const dateClaim = (year: number, rank = 'normal') => ({ rank, mainsnak: { snaktype: 'value', datavalue: { value: { time: `+${year}-01-01T00:00:00Z`, precision: 11 } } } });
...
describe('typed, bounded public catalog lookup', () => {
  it('constrains Wikidata to videogames and uses multilingual labels and preferred dates', async () => {
    const fetcher = vi.fn().mockResolvedValueOnce(json({ query: { search: [{ title: 'Q1' }, { title: 'Q2' }], searchinfo: { totalhits: 8 } }, continue: { sroffset: 5 } }))
      .mockResolvedValueOnce(json({ entities: {
        Q1: { labels: { mul: { value: 'A real source title' } }, claims: { P31: [gameClaim], P577: [dateClaim(2018), dateClaim(2020, 'preferred')] } },
    ...
    expect(result.items[0]).toMatchObject({ id: 'wikidata:Q1', title: 'A real source title', year: 2020, sourceUrl: 'https://www.wikidata.org/wiki/Q1', collectionRank: null });
    expect(result.nextOffset).toBe(5);
    expect(result.notices).toContain('Some search hits lacked a usable title or current video-game classification and were not imported.');
  ...
  it('does not guess a year when source dates disagree, and accepts historical games', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(json({ query: { search: [{ title: 'Q3' }, { title: 'Q4' }], searchinfo: { totalhits: 2 } } }))
      .mockResolvedValueOnce(json({ entities: {
        Q3: { labels: { en: { value: 'Ambiguous edition' } }, claims: { P31: [gameClaim], P577: [dateClaim(2000), dateClaim(2003)] } },
        Q4: { labels: { en: { value: 'Historical game' } }, claims: { P31: [gameClaim], P577: [dateClaim(1962)] } },
      } })));
  ...
  it('filters and pages FreeToGame facts without copying artwork or scores', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(json(Array.from({ length: 24 }, (_, index) => ({
      id: index + 1, title: `Game ${String(index).padStart(2, '0')}`, developer: 'A developer', genre: 'RPG',
      release_date: '2020-03-02', freetogame_profile_url: `https://www.freetogame.com/game-${index}`,
      thumbnail: 'https://images.invalid/do-not-copy.jpg', metascore: 99,
    })))));
  ...
  it('rejects malformed source results at the client boundary', () => {
    ...
    expect(() => parseCatalogPage({ source: 'wikidata', query: '', items: [], total: 0, offset: 5, nextOffset: 2, notices: [] })).toThrow();
  });
});
