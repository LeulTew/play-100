...
import { commonsRasterPermission, fetchCommonsRaster, patchedDecoder, verifiedRaster } from '../../api/_lib/commons-raster';

const permissionFixture = () => ({
  query: { pages: { '1': { title: 'File:Example.png', imagerepository: 'local', imageinfo: [{
    ...
    width: 1, height: 1, thumbwidth: 1, thumbheight: 1, mime: 'image/png', thumbmime: 'image/png',
    extmetadata: {
      Artist: { value: 'Fixture artist' }, Credit: { value: 'Original source' }, Attribution: { value: 'Full attribution' },
      LicenseShortName: { value: 'CC BY-SA 4.0' }, LicenseUrl: { value: 'https://creativecommons.org/licenses/by-sa/4.0/' },
      Copyrighted: { value: 'True' }, Restrictions: { value: 'trademarked' },
    },
  }] } } },
});
const png = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a23sAAAAASUVORK5CYII=', 'base64');
function smallOriginalFixture(patch: Record<string, unknown> = {}) {
  ...
  return { query: { pages: { '1': { ...fixture.query.pages['1'], imageinfo: [{
    ...info, width: 483, height: 110, size: 70739, thumbwidth: 320, thumbheight: 73,
...
describe('conservative Commons rights and media boundary', () => {
  it('selects the exact verified small original before fetching rather than guessing thumbnail bucket dimensions', () => {
    ...
    expect(commonsRasterPermission(smallOriginalFixture(), 'Example.png')).toMatchObject({
      selection: 'original', downloadUrl: 'https://upload.wikimedia.org/wikipedia/commons/a/aa/Example.png',
      width: 483, height: 110, mime: 'image/png', bytes: 70739,
    });
    expect(fetch).not.toHaveBeenCalled();
    expect(commonsRasterPermission(smallOriginalFixture({ width: 640, height: 640, size: 512 * 1024 }))).toMatchObject({ selection: 'original', bytes: 512 * 1024 });
  });
  it.each([
    { width: 641 }, { height: 641 }, { size: 512 * 1024 + 1 }, { size: undefined },
    { size: 0 }, { size: -1 }, { size: 1.5 }, { size: NaN }, { size: '70739' },
    { mime: 'image/svg+xml' }, { mime: 'image/gif' }, { mime: 'image/avif' }, { mime: 'image/tiff' },
    ...
    expect(commonsRasterPermission(smallOriginalFixture(patch))).toMatchObject({
      selection: 'thumbnail', width: 320, height: 73, mime: 'image/png', bytes: null,
    });
  });
  it.each([{ width: undefined }, { height: 0 }, { width: -1 }, { height: 1.2 }, { width: 6000, height: 6000 }])(
    'rejects unverified original geometry even when a thumbnail is supplied: %j', patch => {
  ...
  it('does not fetch an unbounded original when no permitted thumbnail is available', () => {
    ...
    expect(() => commonsRasterPermission(smallOriginalFixture({ size: 512 * 1024 + 1, thumburl: undefined }))).toThrow();
  });
  it('rejects an original byte mismatch and never retries or decodes an alternate thumbnail', async () => {
    const fetch = vi.fn().mockResolvedValue(new Response(new Uint8Array(png), { headers: { 'Content-Type': 'image/png' } }));
    vi.stubGlobal('fetch', fetch);
    const permission = commonsRasterPermission(smallOriginalFixture({ width: 1, height: 1, size: png.length + 1 }));
    await expect(fetchCommonsRaster(permission, new AbortController().signal, '2026-09-22T12:00:00.000Z')).rejects.toMatchObject({ code: 'invalid' });
  ...
  it('keeps exact selected original MIME and decoded dimensions instead of accepting another supported raster', async () => {
    const fetch = vi.fn().mockResolvedValue(new Response(new Uint8Array(png), { headers: { 'Content-Type': 'image/jpeg' } }));
    vi.stubGlobal('fetch', fetch);
  ...
  it('retains the decoded-dimension and animation gates on a selected small original without fallback attempts', async () => {
    ...
    const raster = await sharp({ create: { width: 1, height: 1, channels: 3, background: '#20231e' } }).png().toBuffer();
    const fetch = vi.fn().mockResolvedValue(new Response(new Uint8Array(raster), { headers: { 'Content-Type': 'image/png' } }));
    vi.stubGlobal('fetch', fetch);
    await expect(fetchCommonsRaster(commonsRasterPermission(smallOriginalFixture({ width: 2, height: 1, size: raster.length })),
      new AbortController().signal, '2026-09-22T12:00:00.000Z')).rejects.toMatchObject({ code: 'invalid' });
    expect(fetch).toHaveBeenCalledTimes(1);
    const chunk = Buffer.alloc(20);
    chunk.writeUInt32BE(8, 0); chunk.write('acTL', 4, 'ascii');
    ...
    await expect(fetchCommonsRaster(commonsRasterPermission(smallOriginalFixture({ width: 1, height: 1, size: animated.length })),
      new AbortController().signal, '2026-09-22T12:00:00.000Z')).rejects.toMatchObject({ code: 'invalid' });
  ...
  it('preserves complete attribution, source, original and exact license plus transform/trademark notices', () => {
    ...
    for (const part of ['Fixture artist', 'Original source', 'Full attribution', 'Resized and converted to WebP', 'Trademark rights']) expect(permission.credit).toContain(part);
  });
  ...
  it.each(['   ', '<span> </span>', '&nbsp;', '<span>&#160;</span>', '&#x20;'])('rejects a normalized-empty creator before authorizing a raster: %j', artist => {
    const fixture = permissionFixture();
  ...
  it.each(['https://localhost/private.png', 'https://upload.wikimedia.org.evil.test/file.png', 'https://upload.wikimedia.org/wikipedia/commons/a/a.svg', 'http://upload.wikimedia.org/wikipedia/commons/a/a.png'])('rejects arbitrary or nonraster image URL %s', url => {
    const fixture = permissionFixture();
  ...
  it.each(['0.34.5', '0.35.0', '0.35.3', '0.35.4-rc1', 'unknown'])('does not permit an unpatched or unverified decoder %s', version => {
  ...
  it('requires MIME and magic, excludes SVG/HTML/GIF/TIFF/AVIF and animated raster chunks before decoding', () => {
    ...
    for (const [text, mime] of [['<svg/>', 'image/png'], ['<html>', 'image/jpeg'], ['GIF89a', 'image/gif'], ['II*\0', 'image/tiff'], ['ftypavif', 'image/avif']]) {
    ...
    webp.write('RIFF', 0, 'ascii'); webp.writeUInt32LE(22, 4); webp.write('WEBPVP8X', 8, 'ascii'); webp.writeUInt32LE(10, 16); webp[20] = 2;
  ...
  it('does not decode MIME-spoofed active content with the patched decoder', async () => {
    const fetch = vi.fn().mockResolvedValue(new Response('<svg xmlns="http://www.w3.org/2000/svg"/>', { headers: { 'Content-Type': 'image/png' } }));
    vi.stubGlobal('fetch', fetch);
    await expect(fetchCommonsRaster(commonsRasterPermission(permissionFixture()), new AbortController().signal, '2026-09-22T12:00:00.000Z')).rejects.toMatchObject({ code: 'invalid' });
  });
  it('re-encodes one bounded verified raster, strips metadata and never enlarges it', async () => {
    ...
    const fixture = await sharp({ create: { width: 1, height: 1, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } }).png().toBuffer();
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(new Uint8Array(fixture), { headers: { 'Content-Type': 'image/png' } })));
    const artwork = await fetchCommonsRaster(commonsRasterPermission(permissionFixture()), new AbortController().signal, '2026-09-22T12:00:00.000Z');
