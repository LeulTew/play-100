...
import {
  ...
  steamRating,
  wikidataRatings,
} from '../../api/_lib/catalog-detail-data';
...
const date = (time: string, precision = 11) => ({
  time,
  precision,
...
describe('literal Wikidata game and review claims', () => {
  it('requires the exact nonredirected video-game entity', () => {
    ...
    expect(
      publicGameEntity({ entities: { Q15408545: entity({ P31: [claim({ id: 'Q5' })] }) } }, 'Q15408545'),
    ).toBeNull();
  ...
  it('preserves exact issuer, platform, scale, explicit method, date and reference without invented classification', () => {
    const review = {
      ...claim('8.5/10', {
        ...
        P400: [snak({ id: 'Q200' })],
        P459: [snak({ id: 'Q300' })],
        P585: [snak(date('+2024-02-29T00:00:00Z'))],
    ...
    expect(wikidataRatings(game, labels, now)[0]).toMatchObject({
      ...
      publisherId: 'Q100',
      score: { text: '8.5/10', value: 8.5, scale: 10 },
      method: 'User average',
      platforms: ['PC'],
      count: 25,
      asOf: '2024-02-29',
  ...
  it('keeps unspecified dates/platforms/type explicit and never treats zero as missing', () => {
    ...
    expect(wikidataRatings(game, {}, now)[0]).toMatchObject({
      publisher: 'Q100',
      score: { value: 0 },
      platforms: [],
      method: null,
  ...
  it.each([
    {
      ...
      referenceUrl: 'https://critic.example/review',
      referenceDate: null,
    },
    {
    ...
    },
    {
      ...
      references: [
        {
          snaks: {
            P854: [snak('https://critic.example/review')],
            P813: [snak(date('+2024-04-20T00:00:00Z')), snak(date('+2024-04-21T00:00:00Z'))],
          },
        },
      ],
      referenceUrl: 'https://critic.example/review',
  ...
  it('omits deprecated, ambiguous-issuer and malformed claims instead of inventing ratings', () => {
    ...
    const game = entity({
      P444: [
        claim('9/10', qualifiers, 'deprecated'),
        claim('Recommended', qualifiers),
        claim('9/10'),
...
describe('exact secondary-provider bridges', () => {
  ...
  it('uses one exact Commons filename and rejects ambiguous media', () => {
    expect(commonsFile(entity({ P154: [claim('Approved logo.svg')], P18: [claim('Screenshot.png')] }))).toBe(
      'Approved logo.svg',
    );
  ...
  it('keeps only Steam aggregate counts, not reviews or user identifiers', () => {
    const result = steamRating(
      {
        success: 1,
        query_summary: { total_positive: 91, total_negative: 9, total_reviews: 100 },
        reviews: [{ author: { steamid: 'private-user' }, review: 'Do not persist' }],
      },
      '379430',
      now,
    );
    expect(result).toMatchObject({
      ...
      count: 100,
      score: { value: 91, scale: 100, unit: 'percent' },
      asOf: null,
    });
  ...
  it('does not invent a score for zero or inconsistent Steam counts', () => {
    expect(
      steamRating(
        { success: 1, query_summary: { total_positive: 0, total_negative: 0, total_reviews: 0 } },
        '379430',
        now,
      ),
    ).toBeNull();
    expect(() =>
      steamRating(
        { success: 1, query_summary: { total_positive: 3, total_negative: 2, total_reviews: 8 } },
        '379430',
        now,
      ),
    ).toThrow();
