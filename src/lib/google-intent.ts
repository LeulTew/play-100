...
const storageError =
  'Google needs temporary storage in this tab to return safely. Use email or keep using this device; no library data was changed.';
...
export function googleReturnPath(value: string): string {
  ...
  if (
    !Object.values(PAGE_PATHS).includes(url.pathname) &&
    !/^\/u\/[a-z][a-z0-9_]{2,23}$/.test(url.pathname) &&
    !/^\/friends\/[A-Za-z0-9_-]{1,128}$/.test(url.pathname)
  )
    return '/account';
...
export function parseGoogleIntent(raw: string | null, now = Date.now()): GoogleRedirectIntent | null {
  ...
  try {
    ...
    throw new Error('The Google return request is unreadable. Start again from Account.');
  }
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('The Google return request is invalid. Start again from Account.');
  ...
  if (
    ...
    data.createdAt > now + 5000 ||
    now - data.createdAt > GOOGLE_INTENT_LIFETIME ||
    typeof data.returnPath !== 'string' ||
    googleReturnPath(data.returnPath) !== data.returnPath
  ) {
  ...
  if (typeof data.uid !== 'string' || !/^[A-Za-z0-9_-]{1,128}$/.test(data.uid))
    throw new Error('The Google account request is invalid. Nothing was changed.');
  if (data.kind === 'link') return { ...commonIntent, kind: 'link', uid: data.uid };
  if (
    data.kind === 'reauthenticate' &&
    (data.target === 'copy' || data.target === 'account') &&
    typeof data.epoch === 'number' &&
    Number.isSafeInteger(data.epoch) &&
    data.epoch >= 0
  )
    return { ...commonIntent, kind: 'reauthenticate', uid: data.uid, target: data.target, epoch: data.epoch };
...
export function readGoogleIntent(): { raw: string | null; intent: GoogleRedirectIntent | null; error: string } {
  let raw: string | null;
  try {
    raw = sessionStorage.getItem(GOOGLE_REDIRECT_KEY);
  } catch {
    return { raw: null, intent: null, error: storageError };
  }
  try {
    return { raw, intent: parseGoogleIntent(raw), error: '' };
  } catch (cause) {
    return {
      raw,
      intent: null,
      error: cause instanceof Error ? cause.message : 'The Google return request is invalid.',
    };
  }
}

export function writeGoogleIntent(
  request: GoogleRequest,
  returnPath: string,
): { intent: GoogleRedirectIntent; raw: string } {
  const bytes = crypto.getRandomValues(new Uint8Array(16));
  const intent: GoogleRedirectIntent = {
    ...request,
    version: 1,
    requestId: [...bytes].map((value) => value.toString(16).padStart(2, '0')).join(''),
    createdAt: Date.now(),
    returnPath: googleReturnPath(returnPath),
  };
...
export function clearGoogleIntent(raw: string | null): void {
  ...
  try {
    ...
    throw new Error(
      'Google returned, but this tab could not clear its temporary request. Reload Account before starting another Google action.',
    );
  }
}

export function validateGoogleReturn(
  intent: GoogleRedirectIntent,
  result: { uid: string; providerId: string | null; operationType: string },
  ...
  if (
    ...
    (intent.uid !== null && intent.uid !== result.uid)
  ) {
    throw new Error(
      'Google did not confirm the requested account and action. Nothing was deleted or copied. Review Account before trying again.',
    );
  }
}
