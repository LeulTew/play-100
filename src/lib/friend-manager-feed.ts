...
interface Page<Cursor> { items: FriendPair[]; cursor: Cursor | undefined }
export interface FriendManagerStore<Cursor> {
  ...
  watchRelations(uid: string, state: 'accepted' | 'pending', next: (page: Page<Cursor>) => void, error: (cause: Error) => void): () => void;
...
export class FriendManagerFeed<Cursor = FriendCursor> {
  private snapshot: FriendManagerSnapshot<Cursor> = { pairs: [], identities: {}, cursor: undefined, pages: 0, ready: false, loading: true, active: false, changed: false, error: null };
  ...
  private baseline = '';
  constructor(private store: FriendManagerStore<Cursor>, readonly uid: string, private kind: 'accepted' | 'pending', private isCurrent: () => boolean) {}
  getSnapshot = () => this.snapshot;
  subscribe = (listener: () => void) => { this.listeners.add(listener); return () => { this.listeners.delete(listener); }; };
  private current(version = this.version) { return this.snapshot.active && version === this.version && this.isCurrent(); }
  ...
  stop() {
    this.version += 1; this.loadVersion += 1;
    this.releaseHead?.(); this.releaseHead = undefined;
    this.rowWatches.forEach((release) => release()); this.rowWatches.clear();
    this.profileQueue = []; this.profileLeases.clear();
    this.update({ active: false, loading: false });
  }
  private bindHead() {
    const version = this.version;
    this.releaseHead?.();
    this.releaseHead = this.store.watchRelations(this.uid, this.kind, (page) => {
      if (!this.current(version)) return;
      if (!this.snapshot.ready) {
        this.baseline = friendPageSignature(page.items);
        this.update({ pairs: uniqueFriendPairs(page.items), cursor: page.cursor, pages: 1, ready: true, loading: false, error: null });
        this.bindRows();
  ...
  private forget(peer: string) {
    ...
    const identities = { ...this.snapshot.identities }; delete identities[peer];
    this.rowWatches.get(peer)?.(); this.rowWatches.delete(peer);
  ...
  private applyPair(peer: string, pair: FriendPair | null) {
    if (!pair || pair.state !== this.kind) { this.forget(peer); return; }
  ...
  private bindRows() {
    ...
    for (const [peer, release] of this.rowWatches) if (!peers.has(peer)) { release(); this.rowWatches.delete(peer); }
    for (const peer of peers) {
      if (this.rowWatches.has(peer)) continue;
      this.rowWatches.set(peer, this.store.watchPair(this.uid, peer, (pair) => {
  ...
  private pumpProfiles() {
    while (this.reads < 4 && this.profileQueue.length && this.current()) {
      ...
      void (publicOnly ? this.store.publicIdentity(job.peer) : this.store.identity(job.peer)).then((value) => {
        ...
        if (!valid()) return;
        const denied = cause && typeof cause === 'object' && 'code' in cause && cause.code === 'permission-denied';
        this.update({ identities: { ...this.snapshot.identities, [job.peer]: denied ? { status: 'unavailable', reason: 'denied' } : { status: 'error', cause } } });
      }).finally(() => { this.reads -= 1; this.pumpProfiles(); });
    }
  }
  async retryProfile(peer: string) {
    ...
    try {
      ...
      if (this.current(version)) this.update({ identities: { ...this.snapshot.identities, [peer]: { status: 'error', cause } } });
    }
  }
  async loadMore() { return this.load(false); }
  async refresh() { return this.load(true); }
  private async load(refresh: boolean): Promise<boolean> {
    if (!this.current() || this.snapshot.loading || !refresh && (!this.snapshot.cursor || this.snapshot.changed)) return false;
    const version = this.version;
    ...
    try {
      for (; pages < count; pages += 1) {
        ...
        rows.push(...result.items); cursor = result.cursor;
      ...
      this.update({ pairs: uniqueFriendPairs(refresh ? rows : [...this.snapshot.pairs, ...rows]),
        identities: refresh ? {} : this.snapshot.identities, cursor, pages: refresh ? pages : this.snapshot.pages + pages,
        ready: true, loading: false, changed: refresh ? false : this.snapshot.changed });
      this.bindRows();
