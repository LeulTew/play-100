...
describe('temporary library notices', () => {
  ...
  it('reports an identical failed migration and fallback only once', () => {
    const cause = 'Your previous device library could not be migrated. Unsupported format. The original data has not been changed.';
  ...
  it('retains distinct failures but only one safety assurance and Settings recovery instruction', () => {
    const legacy = 'Your previous device library could not be accessed. Allow device storage and retry, or reset device data in Settings. The original data has not been changed.';
