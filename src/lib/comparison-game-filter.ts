...
export interface ComparisonGameFilter { version: 1; scope: LibraryScope; records: LibraryRecord[] }

export function createComparisonGameFilter(scope: LibraryScope, records: readonly LibraryRecord[]): ComparisonGameFilter {
  scopeUid(scope);
  if (!Array.isArray(records) || records.length < 1 || records.length > 6) throw new Error('Choose between one and six games for comparison.');
  const selected: Record<string, unknown> = Object.create(null);
  const order: string[] = [];
  for (let index = 0; index < records.length; index += 1) {
    const record = records[index];
    if (!record || typeof record.id !== 'string' || Object.hasOwn(selected, record.id)) throw new Error('Choose distinct game identities for comparison.');
    selected[record.id] = record; order.push(record.id);
  }
...
export function parseComparisonGameFilter(value: unknown, scope: LibraryScope): ComparisonGameFilter | null {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('The saved comparison game filter is invalid.');
...
export function readComparisonGameFilter(scope: LibraryScope): { value: ComparisonGameFilter | null; warning: string | null } {
  if (scope === 'guest') return { value: null, warning: null };
  try {
    const stored = history.state?.play100ComparisonGames;
    if (stored && stored.scope === scope && stored.version === 1 && stored.cleared === true &&
      Object.keys(stored).sort().join() === 'cleared,scope,version') return { value: null, warning: null };
    ...
    return { value: null, warning: 'The comparison game filter could not be restored. Pin the games again; your library is unchanged.' };
  }
}
...
export function rememberComparisonGameFilter(filter: ComparisonGameFilter): string | null {
  ...
  try { sessionStorage.setItem(COMPARISON_GAMES_KEY, JSON.stringify(value)); }
  catch (cause) {
...
export function clearComparisonGameFilter(scope: LibraryScope): string | null {
  if (location.pathname === '/compare' || history.state?.play100ComparisonGames?.scope === scope) {
    history.replaceState({ ...history.state, play100ComparisonGames: { version: 1, scope, cleared: true } }, '', location.href);
  }
  let warning: string | null = null;
  try {
    const raw = sessionStorage.getItem(COMPARISON_GAMES_KEY);
    ...
    warning = 'This comparison is no longer filtered. Its old game filter may return in a new visit because tab storage could not be cleared.';
  }
  window.dispatchEvent(new Event(COMPARISON_GAMES_EVENT));
  return warning;
}
