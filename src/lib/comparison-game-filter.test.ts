...
import {
  clearComparisonGameFilter,
  COMPARISON_GAMES_KEY,
  createComparisonGameFilter,
  parseComparisonGameFilter,
  readComparisonGameFilter,
  rememberComparisonGameFilter,
} from './comparison-game-filter';
...
function browser() {
  ...
  const history = {
    ...
    replaceState: vi.fn((next: Record<string, unknown>) => {
      history.state = next;
    }),
  };
  const storage = {
...
describe('separate private game comparison filter', () => {
  it('stores only strict record metadata under the actual account scope, not scores or imagery', () => {
    ...
    expect(() =>
      parseComparisonGameFilter({ version: 1, scope, records: [{ ...record, note: 'private' }] }, scope),
    ).toThrow();
  ...
  it('keeps people history unchanged, survives refresh and can be cleared independently', () => {
    const b = browser();
    b.history.state = { play100Compare: { selected: ['owner', 'peer'] } };
  ...
  it('reports blocked or corrupt storage without losing valid history data or modifying a library', () => {
    ...
    expect(readComparisonGameFilter(scope).value).toEqual(filter);
    b.history.state = {};
    b.data.set(COMPARISON_GAMES_KEY, '{bad');
  ...
  it('a failed persistent clear still removes this view filter and explains that its old saved copy may return', () => {
    ...
    b.storage.removeItem.mockImplementation(() => {
      throw new DOMException('blocked', 'SecurityError');
    });
...
describe('game filters never fabricate participant entries', () => {
  ...
  const full = compareFriendRankings([
    {
      ...
      entries: [
        entry,
        {
          ...entry,
          id: different.id,
          sourceId: different.sourceId,
          sourceUrl: different.sourceUrl,
          score: 8,
          position: 2,
        },
      ],
  ...
  it('filters exact source IDs while retaining zero and unavailable cells', () => {
    ...
    expect(page.rows[0]?.cells).toMatchObject([
      { status: 'ranked', score: 0 },
      { status: 'unshared', score: null },
    ]);
  ...
  it('does not invent a row or title match for an unranked pinned game', () => {
    expect(
      getComparisonPage(full, { mode: 'all-shared', games: [{ ...record, id: 'wikidata:Q999', sourceId: 'Q999' }] })
        .rows,
    ).toEqual([]);
  });
