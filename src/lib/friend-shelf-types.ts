...
import { FriendStoreError, parseFriendSettings, parseFriendHead, friendSelection } from './friend-types';
...
export interface FriendShelfEntry {
  ...
  title: string;
  year: number | null;
  source: LibraryRecord['source'];
  sourceId: string;
  sourceUrl: string | null;
}
export interface FriendShelfConfig extends FriendSettings {
  consentSyncEpoch: number | null;
}
export type FriendShelfHead = FriendShareHead;
export interface FriendShelf {
  head: FriendShelfHead;
...
export class FriendShelfCommittedError extends FriendStoreError {
  ...
  constructor(
    readonly receipt: FriendShelfReceipt,
    readonly cause: unknown,
    readonly phase: 'refresh' | 'cleanup' = 'refresh',
  ) {
    super(
      'committed-refresh-failed',
      'Shared games were saved; refresh their details instead of repeating the change.',
    );
...
export function parseFriendShelfConfig(value: unknown): FriendShelfConfig {
  ...
  if (
    config.enabled
      ? typeof consentSyncEpoch !== 'number' || !Number.isSafeInteger(consentSyncEpoch) || consentSyncEpoch < 1
...
export function shelfSelection(value: unknown): string[] {
  try {
    return friendSelection(value);
  } catch {
    return invalid('Choose up to 200 distinct saved games.');
  }
}
export function parseFriendShelfEntry(value: unknown): FriendShelfEntry {
  ...
  if (
    Object.keys(row).sort().join() !== 'id,source,sourceId,sourceUrl,title,year' ||
    typeof row.title !== 'string' ||
    !row.title.trim() ||
    row.title.length > 200 ||
    [...row.title].some((char) => char.charCodeAt(0) < 32 || char.charCodeAt(0) === 127) ||
    (row.year !== null &&
      (typeof row.year !== 'number' || !Number.isInteger(row.year) || row.year < 1900 || row.year > 2100))
  )
    invalid();
  shelfSelection([row.id]);
  shelfSelection([row.sourceId]);
  ...
  const valid =
    source === 'collection'
      ? id === sourceId && url === null
      : source === 'wikidata'
        ? /^Q[1-9][0-9]*$/.test(sourceId) &&
          id === `wikidata:${sourceId}` &&
          url === `https://www.wikidata.org/wiki/${sourceId}`
        : source === 'steam'
          ? /^[1-9][0-9]*$/.test(sourceId) &&
            id === `steam:${sourceId}` &&
            url === `https://store.steampowered.com/app/${sourceId}/`
          : source === 'freetogame'
            ? /^[1-9][0-9]*$/.test(sourceId) &&
              id === `freetogame:${sourceId}` &&
              (url === null || (typeof url === 'string' && /^https:\/\/www\.freetogame\.com\/[a-z0-9/-]+$/.test(url)))
...
export function validateFriendShelfEntries(
  input: readonly FriendShelfEntry[],
  selectedIds: readonly string[],
): FriendShelfEntry[] {
  const ids = shelfSelection(selectedIds);
  const entries = input.map(parseFriendShelfEntry);
  if (entries.length !== ids.length || entries.some((entry, index) => entry.id !== ids[index]))
    invalid('The saved selection changed. Preview it again.');
  return entries;
}
export function parseFriendShelfChunk(value: unknown, index: number, count: number): FriendShelfEntry[] {
  if (
    ...
    index < 0 ||
    index >= FRIEND_SHELF_CHUNK_LIMIT ||
    !Number.isInteger(count) ||
    count < 1 ||
    count > FRIEND_SHELF_LIMIT
  )
    invalid();
  const row = value as Record<string, unknown>;
  if (
    Object.keys(row).sort().join() !== 'entries,ids,index' ||
    row.index !== index ||
    !Array.isArray(row.entries) ||
    row.entries.length !== Math.min(FRIEND_SHELF_CHUNK_SIZE, count - index * FRIEND_SHELF_CHUNK_SIZE) ||
    !row.entries.length
  )
    invalid();
  return validateFriendShelfEntries(row.entries, shelfSelection(row.ids));
}
export function projectFriendShelf(
  state: PersonalLibraryState,
  selectedIds: readonly string[],
  games: Game[],
  removed: ReadonlySet<string> = new Set(),
): { entries: FriendShelfEntry[]; selectedIds: string[] } {
  ...
  const entries = ids.map((id) => {
    ...
    if (saved.source === 'collection' && !trusted)
      invalid('An original game is missing from the public catalog. Reload it before sharing.');
...
export function recordFromFriendShelf(value: FriendShelfEntry, games: Game[]): LibraryRecord {
  ...
  if (entry.source === 'collection') {
    if (!canonical || entry.title !== canonical.title || entry.year !== canonical.year)
      invalid('The shared original game does not match the public catalog.');
