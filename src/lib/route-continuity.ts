...
export function isRouteArrival(previous: CommittedRouteState | null, current: CommittedRouteState): boolean {
  return (
    ...
    previous.scopeEpoch === current.scopeEpoch &&
    !current.blocked
  );
}

export function arrivalMotion(
  kind: 'route' | CommittedCue['kind'],
  coarse: boolean,
  direction: -1 | 1 = 1,
): {
  ...
  return {
    ...
    timing: {
      duration: kind === 'route' ? (coarse ? 120 : 160) : coarse ? 100 : 120,
...
export function playArrival(
  ...
  target: HTMLElement,
  motion: ReturnType<typeof arrivalMotion>,
): void {
