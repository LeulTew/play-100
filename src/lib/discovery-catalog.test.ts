...
const artwork: CatalogArtwork = {
  src: `/images/discovery/${hash}.webp`,
  width: 320,
  height: 180,
  alt: 'Game logo',
  sourceUrl: 'https://commons.wikimedia.org/wiki/File:Game.svg',
  credit: 'Creator; resized to WebP',
  license: 'CC BY 4.0',
...
describe('discovery catalog boundary', () => {
  it('retains precise diagnostics for timestamp and artwork validation', () => {
    ...
    expect(() => parseCatalogArtwork({ ...artwork, src: '/images/discovery/not-content-addressed.webp' })).toThrow(
      'artwork must use its content-addressed local WebP path.',
    );
  });
  ...
  it.each([
    ...
    [
      'wrong identity',
      (x: DiscoveryCatalog) => {
        x.items[0]!.record.id = 'wikidata:Q456';
      },
    ],
    [
      'wrong URL identity',
      (x: DiscoveryCatalog) => {
        x.items[0]!.record.sourceUrl = 'https://www.wikidata.org/wiki/Q456';
      },
    ],
    [
      'duplicate aliases',
      (x: DiscoveryCatalog) => {
        x.items[0]!.aliases = ['KCD', 'KCD'];
      },
    ],
    [
      'too many aliases',
      (x: DiscoveryCatalog) => {
        x.items[0]!.aliases = Array.from({ length: 31 }, (_, i) => String(i));
      },
    ],
    [
      'bad year',
      (x: DiscoveryCatalog) => {
        x.items[0]!.record.year = 1800;
      },
    ],
    [
      'ranked import',
      (x: DiscoveryCatalog) => {
        x.items[0]!.record.collectionRank = 1;
      },
    ],
    [
      'missing-art reason omitted',
      (x: DiscoveryCatalog) => {
        x.items[0]!.provenance.artworkMissingReason = null;
      },
    ],
    [
      'bad metadata license',
      (x: DiscoveryCatalog) => {
        x.items[0]!.provenance.metadataLicense = 'FreeToGame API terms';
      },
    ],
    [
      'bad timestamp',
      (x: DiscoveryCatalog) => {
        x.generatedAt = '2026-02-31T18:00:00.000Z';
      },
    ],
    [
      'future retrieval',
      (x: DiscoveryCatalog) => {
        x.generatedAt = '2025-09-17T18:00:00.000Z';
      },
    ],
    [
      'too many records',
  ...
  it.each([
    ...
    '/images/discovery/../file.webp',
    `/images/discovery/${hash}.svg`,
