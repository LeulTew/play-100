...
const entity = (id = 'Q90000001', extra = {}) => ({ id, claims: {
  P31: [claim({ id: 'Q7889' })],
...
describe('public-only catalog detail endpoint', () => {
  it.each(['', 'id=manual%3Asecret', 'id=Kingdom+Come', 'id=https%3A%2F%2Flocalhost', 'id=wikidata%3AQ1&id=wikidata%3AQ2', 'id=wikidata%3AQ1&title=private', 'id=wikidata%3AQ1&url=https%3A%2F%2Flocalhost', 'id=red-dead-redemption-2'])('rejects unsupported input before any upstream: %s', async query => {
  ...
  it('uses GET only and never invents FreeToGame review or image rights', async () => {
    ...
    expect(await response.json()).toMatchObject({ ratings: [], artwork: null, sources: [{ source: 'freetogame', status: 'unavailable', code: 'unsupported' }] });
    expect(upstream).not.toHaveBeenCalled();
  });
  it('returns source-labelled scores and never fetches an ambiguous Steam bridge', async () => {
    const upstream = vi.fn(async (input: URL, options: RequestInit) => {
      ...
      return input.searchParams.get('props') === 'claims'
        ...
        : json({ entities: { Q100: { labels: { en: { value: 'Example critic' } } }, Q200: { labels: { en: { value: 'PC' } } } } });
    });
    vi.stubGlobal('fetch', upstream);
    const response = await nativeFetch(`${base}/api/catalog-detail?id=wikidata%3AQ90000001`);
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data.ratings[0]).toMatchObject({ publisher: 'Example critic', score: { value: 8, scale: 10 }, platforms: ['PC'], asOf: null, referenceDate: null });
  ...
  it('keeps a Steam429 partial error and valid Wikidata scores with no success cache', async () => {
    ...
    const upstream = vi.fn(async (input: URL) => {
      if (input.hostname === 'store.steampowered.com') return new Response(new ReadableStream({ cancel }), { status: 429, headers: { 'Retry-After': '3' } });
      return input.searchParams.get('props') === 'claims' ? json({ entities: { Q90000001: entity('Q90000001', { P1733: [claim('379430')] }) } })
        : json({ entities: { Q100: { labels: { en: { value: 'Source' } } }, Q200: { labels: { en: { value: 'PC' } } } } });
    });
    vi.stubGlobal('fetch', upstream);
    const response = await nativeFetch(`${base}/api/catalog-detail?id=wikidata%3AQ90000001`);
    const data = await response.json();
    expect(data.ratings).toHaveLength(1);
    expect(data.sources).toContainEqual(expect.objectContaining({ source: 'steam', status: 'error', code: 'rate-limited', retryAfter: 3 }));
    expect(response.headers.get('cache-control')).toBe('no-store');
  ...
  it('does not request new Commons media when the exact ID already has bundled licensed art', async () => {
    const upstream = vi.fn(async (input: URL) => input.searchParams.get('props') === 'claims'
