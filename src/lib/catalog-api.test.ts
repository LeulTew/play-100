...
describe('same-origin catalog API boundary', () => {
  it.each([
    ...
    'q=A%00B',
    `q=${'x'.repeat(81)}`,
  ])('rejects invalid public request parameters: %s', async (query) => {
  ...
  it('echoes exact public query/source/offset, caches only success, identifies upstream and preserves literal query escaping', async () => {
    ...
    const response = await nativeFetch(
      `${base}/api/catalog?${new URLSearchParams({ source: 'wikidata', q, offset: '5' })}`,
    );
  ...
  it.each([400, 500, 503])('cancels the owned upstream HTTP%s error body without reading it', async (status) => {
    ...
    expect(await response.json()).toMatchObject({
      error: expect.stringContaining(`unavailable (${status})`),
      code: 'unavailable',
    });
    expect(cancel).toHaveBeenCalledOnce();
  });
  it.each([429, 503])(
    'preserves the original HTTP%s response when cancellation rejects and logs no upstream details',
    async (status) => {
      const cancel = vi.fn(async () => {
        throw new Error('private-query-token-must-not-be-logged');
      });
  ...
  it.each(['wikidata', 'freetogame'] as const)(
