...
describe('strict friend types and selected projection', () => {
  ...
  it('keeps head pointers during prune retries even after consent changes, without changing deletion cleanup', () => {
    ...
    const head: FriendShareHead = {
      ...
      current: { generation: current, count: 0, digest: '0'.repeat(64) },
      previous: { generation: previous, count: 0, digest: '0'.repeat(64) },
      updatedAt: 1000,
    };
    for (const settings of [
      control,
      { ...control, enabled: false },
      { ...control, epoch: 2, revision: 2 },
  ...
  it('keeps zero distinct from null and excludes all private fields', () => {
    ...
    expect(() =>
      validateFriendEntries([{ ...entry, sourceUrl: 'https://example.test/redirect' }], [entry.id]),
    ).toThrow();
  ...
  it('prunes removed games, allows an empty result, and never adds unselected games', () => {
    ...
    expect(projectFriendRanking(state, ['manual:removed', entry.id], [])).toEqual({
      entries: [entry],
  ...
  it('rejects arbitrary identity/control fields and malformed local avatars', () => {
    ...
    for (const data of [
      ...
      { ...identity, gameCount: 123 },
      { ...identity, avatar: { ...avatar, photoUrl: 'https://example.test/image' } },
    ]) {
  ...
  it('strictly validates chunk progress and protects invitation metadata', () => {
    ...
    for (const count of [1, 2, 3, 4, 5, 6, 7, 199, 200]) {
      ...
      for (let index = 0; index < uploaded; index += 1) {
        ...
        expect(parseFriendChunk({ index, entries: chunk, ids: chunk.map((row) => row.id) }, index, count)).toEqual(
          chunk,
        );
      }
      expect(
    ...
    expect(() =>
      parseFriendChunk({ index: 0, entries: Array(10).fill(entry), ids: Array(10).fill(entry.id) }, 0, 200),
    ).toThrow();
  });
  it('accepts a decreasing deleting countdown without weakening staging or ready progress validation', () => {
    const generation = {
      ...
      digest: 'a'.repeat(64),
      uploaded: 2,
      ids: ['manual:one', 'manual:two', 'manual:three'],
      status: 'deleting',
      createdAt: time,
    };
    for (const uploaded of [2, 1, 0])
      expect(parseFriendGeneration({ ...generation, uploaded }).uploaded).toBe(uploaded);
