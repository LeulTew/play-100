...
const profile = (uid: string): FriendIdentity => ({ format: 1, uid, displayName: `Player ${uid}`, avatar: { version: 1, seed: 'a'.repeat(32), palette: 'lime' }, revision: 1, updatedAt: 1 });
const pair = (index: number): FriendPair => ({
  format: 1, a: 'owner', b: `peer-${String(index).padStart(3, '0')}`, participants: ['owner', `peer-${String(index).padStart(3, '0')}`],
  from: 'owner', state: 'accepted', epoch: 1, inviteSlot: null, createdAt: 1, updatedAt: 1000 - index,
});
function deferred<T>() {
...
function fixture() {
  ...
  const store = {
    ...
    watchRelations: vi.fn<FriendManagerStore<string>['watchRelations']>((_uid, _state, next, error) => { head = next; headError = error; return releases; }),
...
describe('stable bounded manager feed', () => {
  it('retains loaded pages and cursor on live head changes, deduplicates boundaries and refreshes the same page window', async () => {
    ...
    f.feed.start(); f.head(first, 'first');
    ...
    f.store.listRelations.mockResolvedValueOnce({ items: [pair(100), ...first.slice(0, 19)], cursor: 'new-first' })
  ...
  it('removes a revoked tail row without clearing others or reviving its late profile', async () => {
    const f = fixture(); const waiting = deferred<FriendIdentity | null>();
    const first = Array.from({ length: 20 }, (_, index) => pair(index));
    f.feed.start(); f.head(first, 'first');
    f.store.listRelations.mockResolvedValueOnce({ items: [pair(20)], cursor: undefined });
    await f.feed.loadMore();
    f.store.identity.mockImplementation((uid) => uid === pair(20).b ? waiting.promise : Promise.resolve(profile(uid)));
  ...
  it('bounds identity reads to four, isolates missing/denied/error profiles and retries one row', async () => {
    const f = fixture(); const waits = Array.from({ length: 6 }, () => deferred<FriendIdentity | null>());
    f.store.identity.mockImplementation((uid) => waits[Number(uid.slice(-3))]!.promise);
    const people = waits.map((_, index) => pair(index));
    f.feed.start(); f.head(people);
    people.forEach((row) => f.rows.get(row.b)!.next(row));
    expect(f.store.identity).toHaveBeenCalledTimes(4);
    waits[0]!.resolve(profile(people[0]!.b)); waits[1]!.resolve(null);
    waits[2]!.reject(Object.assign(new Error('Denied'), { code: 'permission-denied' })); waits[3]!.reject(new Error('Temporary network fault'));
    await vi.waitFor(() => expect(f.store.identity).toHaveBeenCalledTimes(6));
    waits[4]!.resolve(profile(people[4]!.b)); waits[5]!.resolve(profile(people[5]!.b));
    ...
    f.store.pair.mockResolvedValue(people[3]!); f.store.identity.mockResolvedValue(profile(people[3]!.b));
  ...
  it('rejects late page/profile work after account or binding changes', async () => {
    const f = fixture(); const profileRead = deferred<FriendIdentity | null>(); const pageRead = deferred<{ items: FriendPair[]; cursor: string | undefined }>();
    f.feed.start(); f.head([pair(0)], 'first');
    f.store.identity.mockReturnValue(profileRead.promise); f.rows.get(pair(0).b)!.next(pair(0));
    f.store.listRelations.mockReturnValue(pageRead.promise);
    const loading = f.feed.loadMore();
    f.revokeScope(); f.feed.stop();
    profileRead.resolve(profile(pair(0).b)); pageRead.resolve({ items: [pair(1)], cursor: undefined });
  ...
  it('does not repopulate identities after a permission failure and supports an explicit fresh read', async () => {
    const f = fixture(); const waiting = deferred<FriendIdentity | null>();
    f.feed.start(); f.head([pair(0)], 'first');
    f.store.identity.mockReturnValue(waiting.promise); f.rows.get(pair(0).b)!.next(pair(0));
  ...
  it('preserves loaded rows and exposes a failed load-more error instead of claiming completion', async () => {
    const f = fixture(); f.feed.start(); f.head([pair(0)], 'first');
    const error = new Error('Offline'); f.store.listRelations.mockRejectedValue(error);
    expect(await f.feed.loadMore()).toBe(false);
  ...
  it('reattaches a failed tail relationship stream on profile retry so later revocation still removes the row', async () => {
    const f = fixture();
    f.feed.start(); f.head(Array.from({ length: 20 }, (_, index) => pair(index)), 'first');
    ...
    expect(recovered).toBeDefined(); expect(recovered).not.toBe(failed);
    await vi.waitFor(() => expect(f.feed.getSnapshot().identities[peer]?.status).toBe('ready'));
