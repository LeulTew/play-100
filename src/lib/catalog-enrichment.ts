...
export const ENRICHMENT_LIMITS = {
  ...
  imageBytes: 80 * 1024, imageEdge: 480, creditLength: 2000,
} as const;
...
export function parseReportedScore(text: string): ReportedScore | null {
  ...
  if (!Number.isFinite(value) || !Number.isFinite(scale) || scale <= 0 || scale > 1000 || value < 0 || value > scale) return null;
  return { text, value, scale, unit: match[3] ? 'percent' : 'points' };
}

function invalid(): never { throw new Error('The public game details returned an invalid response. Please retry.'); }
...
function text(value: unknown, max: number): string {
  return typeof value === 'string' && value.trim() && value.length <= max && !hasAsciiControl(value) ? value : invalid();
}
...
function day(value: unknown): string | null {
  ...
  if (!Number.isFinite(Date.parse(`${full}T00:00:00Z`)) || new Date(`${full}T00:00:00Z`).toISOString().slice(0, 10) !== full) return invalid();
  return result;
}
export function publicHttpsUrl(value: unknown): string | null {
  ...
  try {
    ...
    return url.protocol === 'https:' && !url.username && !url.password && !url.port &&
      ...
      !/^(?:localhost|127\.|0\.|10\.|169\.254\.|192\.168\.|172\.(?:1[6-9]|2\d|3[01])\.|\[)/.test(url.hostname) ? url.href : null;
  } catch { return null; }
}
function url(value: unknown): string { return publicHttpsUrl(value) ?? invalid(); }

function parseRating(value: unknown, expectedId: string): CatalogExternalRating {
  const row = object(value, ['id', 'source', 'kind', 'publisher', 'publisherId', 'score', 'platforms', 'method', 'count', 'asOf', 'referenceDate', 'retrievedAt', 'sourceUrl', 'referenceUrl']);
  ...
  if (row.source === 'wikidata' ? sourceUrl !== `https://www.wikidata.org/wiki/${expectedId.slice('wikidata:'.length)}#P444`
    : !/^https:\/\/store\.steampowered\.com\/app\/[1-9]\d*\/#app_reviews_hash$/.test(sourceUrl)) return invalid();
  ...
  return {
    id: text(row.id, 140), source: row.source, kind: row.kind, publisher: text(row.publisher, 200), publisherId, score,
    platforms: row.platforms.map(value => text(value, 200)), method: optionalText(row.method, 240),
    count: row.count === null ? null : integer(row.count, 0, 1_000_000_000), asOf: day(row.asOf), referenceDate: day(row.referenceDate),
    retrievedAt: date(row.retrievedAt), sourceUrl, referenceUrl: row.referenceUrl === null ? null : url(row.referenceUrl),
  };
}

export function parseExternalCatalogArtwork(value: unknown): ExternalCatalogArtwork | null {
  if (value === null) return null;
  const row = object(value, ['kind', 'src', 'width', 'height', 'alt', 'sourceUrl', 'originalUrl', 'license', 'licenseUrl', 'credit', 'retrievedAt']);
  ...
  if (new URL(sourceUrl).origin !== 'https://commons.wikimedia.org' || !new URL(sourceUrl).pathname.startsWith('/wiki/File:') ||
    new URL(originalUrl).origin !== 'https://upload.wikimedia.org' || !new URL(originalUrl).pathname.startsWith('/wikipedia/commons/') ||
    !expected || expected !== licenseUrl) return invalid();
  return {
    kind: 'commons-raster', src, width: integer(row.width, 1, ENRICHMENT_LIMITS.imageEdge), height: integer(row.height, 1, ENRICHMENT_LIMITS.imageEdge),
    alt: text(row.alt, 300), sourceUrl, originalUrl, license, licenseUrl, credit: text(row.credit, ENRICHMENT_LIMITS.creditLength), retrievedAt: date(row.retrievedAt),
  };
}
...
export function parseCatalogEnrichment(value: unknown, expectedId: string): CatalogEnrichment {
  ...
  if (row.schemaVersion !== 1 || row.id !== expectedId || !enrichmentIdentity(expectedId) ||
    !Array.isArray(row.ratings) || row.ratings.length > ENRICHMENT_LIMITS.ratings ||
    !Array.isArray(row.sources) || !row.sources.length || row.sources.length > 4) return invalid();
  ...
  const sources: EnrichmentSourceState[] = row.sources.map(value => {
    ...
    if (source.source !== 'wikidata' && source.source !== 'steam' && source.source !== 'commons' && source.source !== 'freetogame') return invalid();
    if (source.status !== 'ready' && source.status !== 'unavailable' && source.status !== 'error') return invalid();
    if (source.code !== null && source.code !== 'missing' && source.code !== 'unsupported' && source.code !== 'ambiguous' &&
      source.code !== 'rate-limited' && source.code !== 'timeout' && source.code !== 'invalid' && source.code !== 'unavailable') return invalid();
    if ((source.status === 'ready') !== (source.code === null)) return invalid();
    return { source: source.source, status: source.status, code: source.code, message: text(source.message, 500), retryAfter: integer(source.retryAfter, 0, 300) };
  });
  if (new Set(sources.map(source => source.source)).size !== sources.length) return invalid();
  return { schemaVersion: 1, id: expectedId, fetchedAt: date(row.fetchedAt), ratings, artwork: parseExternalCatalogArtwork(row.artwork), sources };
}
