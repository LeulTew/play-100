...
describe('public seed search independent of personal state', () => {
  it.each(['Kingdomcome', 'Kingdom Come', 'Kingdom Come: Deliverance', '  KINGDOM   CÔME ', 'KCD', 'KCD1'])(
    'finds the verified Kingdom Come identity for %s',
    (q) => {
      expect(
        searchDiscoveryItems([discoveryFixture], { ...defaultDiscoveryFilters, q }).map((item) => item.record.id),
      ).toEqual(['wikidata:Q15408545']);
    },
  );
  ...
  it('prioritizes exact title and alias relevance over imagery; only blank browsing is illustrated-first', () => {
    const illustrated = {
      ...discoveryFixture,
      record: { ...discoveryFixture.record, id: 'wikidata:Q1', title: 'Kingdom Come: Deliverance II' },
      aliases: [],
      artwork: artworkFixture,
    };
    expect(
      searchDiscoveryItems([illustrated, discoveryFixture], {
        ...defaultDiscoveryFilters,
        q: discoveryFixture.record.title,
      }),
    ).toEqual([discoveryFixture, illustrated]);
    expect(searchDiscoveryItems([discoveryFixture, illustrated], defaultDiscoveryFilters)).toEqual([
      illustrated,
      discoveryFixture,
    ]);
  ...
  it('matches metadata without mutating source records or adding personal fields', () => {
    ...
    expect(searchDiscoveryItems([discoveryFixture], { ...defaultDiscoveryFilters, q: 'Warhorse RPG 2018' })).toEqual([
      discoveryFixture,
    ]);
...
describe('stable, public discovery URLs', () => {
  it('keeps new family URLs separate from unchanged legacy exact genre matching', () => {
    ...
    expect(parseDiscoverySearch('?genre=shooter&genreFamily=Unknown')).toMatchObject({
      genre: 'shooter',
      genreFamily: '',
    });
  ...
  it('patches only Discover-owned query keys, retaining unrelated preview and repeated parameters through reset', () => {
    const original =
      '?genre=Action+RPG&offset=48&source=wikidata&view=list&catalogs=off&game=wikidata%3AQ1&campaign=a&campaign=b';
    const updated = patchDiscoverySearch(original, {
      genreFamily: 'role-playing',
      genre: '',
      offset: 0,
      online: 'auto',
    });
    expect(parseDiscoverySearch(updated)).toMatchObject({
      genreFamily: 'role-playing',
      genre: '',
      offset: 0,
      source: 'wikidata',
    ...
    expect(createDiscoverySearch(parseDiscoverySearch(`${updated}&uid=private&note=secret`))).not.toMatch(
      /uid|note|campaign|game=/,
    );
  });
...
describe('Discover selection key', () => {
  ...
  it('changes with the result page and every filter, from any URL origin', () => {
    ...
    for (const next of [
      ...
      `${base}&genre=RPG`,
      `${base}&genreFamily=role-playing`,
      `${base}&year=2010`,
      `${base}&online=on`,
      `${base}&progress=completed`,
      '?q=Mass%20Effect&catalogs=off',
      '?q=Mass%20Effect&include100=on',
    ])
