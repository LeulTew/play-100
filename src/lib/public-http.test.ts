...
describe('shared bounded public transport with S2 cleanup semantics', () => {
  it.each(['http://www.wikidata.org/w/api.php', 'https://www.wikidata.org.evil.test/', 'https://localhost/', 'https://127.0.0.1/', 'https://www.wikidata.org:8080/', 'https://user:secret@www.wikidata.org/'])('rejects unsafe upstream %s before fetch', async input => {
    const fetch = vi.fn();
  ...
  it.each([429, 400, 503])('cancels HTTP%s without leaking source payloads or masking the original status', async status => {
    const cancel = vi.fn(async () => { throw new Error('secret-provider-body'); });
    const warning = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(new ReadableStream({ cancel }), { status, headers: { 'Retry-After': '3' } })));
    await expect(upstreamJson(url, signal())).rejects.toMatchObject({ status: status === 429 ? 429 : 503, retryAfter: status === 429 ? 3 : 0 });
  ...
  it('enforces exact stream bounds and cancels one byte over', async () => {
    ...
    const fetch = vi.fn().mockResolvedValueOnce(new Response('{}  ')).mockResolvedValueOnce(new Response(new ReadableStream({
  ...
  it('rejects Content-Length and MIME before reading a large or active response', async () => {
    const cancel = vi.fn(async () => undefined);
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(new ReadableStream({ cancel }), { headers: { 'Content-Length': '100', 'Content-Type': 'text/html' } })));
    await expect(publicBytes(url, signal(), { maxBytes: 4, contentTypes: ['application/json'] })).rejects.toMatchObject({ code: 'invalid' });
    expect(cancel).toHaveBeenCalledOnce();
  });
  it('bounds a noncooperative fetch with a distinguishable timeout', async () => {
    vi.useFakeTimers();
    vi.stubGlobal('fetch', vi.fn(() => new Promise<Response>(() => undefined)));
    const result = expect(upstreamJson(url, signal(), { timeoutMs: 25 })).rejects.toMatchObject({ status: 504, code: 'timeout' });
