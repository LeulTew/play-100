...
const page = (source: CatalogSource = 'wikidata', query = 'KCD', offset = 0): CatalogPage => ({
  source, query, offset, items: source === 'wikidata' ? [discoveryFixture.record] : [], total: source === 'wikidata' ? 1 : 0, nextOffset: null, notices: [],
});
...
describe('per-query online request lifecycle', () => {
  it('rejects out-of-order old responses even when the upstream ignores abort', async () => {
    ...
    const fetcher = vi.fn().mockImplementationOnce(() => new Promise((resolve) => { old = resolve; })).mockResolvedValueOnce(page('wikidata', 'new'));
  ...
  it('lets one provider finish while the other is offline, rate limited, timed out or unavailable', async () => {
    for (const kind of ['offline', 'rate-limited', 'timeout', 'unavailable'] as const) {
      const fetcher = vi.fn().mockImplementation((source: CatalogSource) => source === 'wikidata'
        ? Promise.resolve(page()) : Promise.reject(new CatalogRequestError('Provider failed', kind)));
      const session = new CatalogSearchSession(fetcher);
  ...
  it('preserves good records after pagination failure and retries the failed offset', async () => {
    const fetcher = vi.fn().mockResolvedValueOnce({ ...page(), nextOffset: 5 })
      .mockRejectedValueOnce(new CatalogRequestError('Timed out', 'timeout')).mockResolvedValueOnce(page('wikidata', 'KCD', 5));
    const session = new CatalogSearchSession(fetcher);
    session.start('KCD', 'KCD', ['wikidata']);
    await flush(); session.more('wikidata'); await flush();
    expect(session.getSnapshot().sources[0]).toMatchObject({ status: 'error', records: [discoveryFixture.record], requestOffset: 5 });
    session.retry('wikidata'); await flush();
  ...
  it('honors a bounded rate-limit cooldown without blocking the other provider or looping', async () => {
    vi.useFakeTimers();
    const fetcher = vi.fn().mockRejectedValueOnce(new CatalogRequestError('Slow down', 'rate-limited', 1)).mockResolvedValue(page('freetogame'));
    const session = new CatalogSearchSession(fetcher);
    session.start('KCD', 'KCD', ['wikidata']); await flush();
    session.retry('wikidata'); await flush();
    expect(fetcher).toHaveBeenCalledTimes(1);
    session.retry('freetogame'); await flush();
