...
const browserEnvironment: MotionEnvironment = {
  ...
  subscribe(listener) {
    const hidden = () => {
      if (document.hidden) listener('hidden');
    };
...
const interactive =
  'a[href],button,input,textarea,select,summary,video,audio,iframe,[contenteditable]:not([contenteditable="false"])';
...
export function createMotionRuntime(
  ...
  const eligible = () =>
    mounted && read().policy.animate && !read().boundary.blocked && !environment.hidden() && environment.supported();
  const notify = (reason: MotionCancelReason) => {
    for (const listener of [...listeners]) {
      try {
        listener(reason);
      } catch {
        console.error('A motion interruption listener failed. Remaining cleanup will continue.');
      }
    }
  };
  const syncEvents = () => {
  ...
  const clearOrigin = (reason: MotionCancelReason) => {
    ...
    if (old) {
      ...
      try {
        ...
        console.error('A motion authority cleanup failed. Remaining cleanup will continue.');
      }
      old.sprite?.remove();
  ...
  const currentOrigin = (entry: OriginLease) =>
    origin === entry &&
    !entry.handle.signal.aborted &&
    eligible() &&
    sameBoundary(entry.hint.snapshot.boundary, read().boundary) &&
    entry.hint.snapshot.location.viewKey === read().location.viewKey &&
    entry.hint.snapshot.location.overlayKey === read().location.overlayKey &&
    guardCurrent(entry.guard);
  const canReturn = (entry: OriginLease) =>
    currentOrigin(entry) &&
    entry.phase !== 'pending' &&
    read().location.requestedDetailKey === null &&
    read().location.displayedDetailKey === null &&
    entry.openedNavigation !== null &&
    read().location.navigationGeneration === entry.openedNavigation + 1;
  const expectedOpen = (entry: OriginLease) =>
    currentOrigin(entry) &&
  ...
  const runtime: MotionController = {
    ...
    update() {
      ...
      if (
        prior.policy.animate !== next.policy.animate ||
        prior.policy.coarsePointer !== next.policy.coarsePointer ||
        prior.policy.reducedMotion !== next.policy.reducedMotion ||
        prior.policy.constrained !== next.policy.constrained
      ) {
      ...
      if (
        prior.location.navigationGeneration !== next.location.navigationGeneration ||
        prior.location.viewKey !== next.location.viewKey ||
        prior.location.overlayKey !== next.location.overlayKey ||
    ...
    originHint(input) {
      ...
      pendingHint = {
        handle,
        value: {
          source: new WeakRef(input.source),
          trigger: new WeakRef(input.trigger),
          visual,
          snapshot: read(),
          generation,
        },
    ...
    captureOrigin(hint, intent) {
      ...
      if (
        ...
        !eligible() ||
        hint.presentationId !== intent.displayedDetailKey ||
        ...
        !guardCurrent(intent.guard)
      )
        return null;
      const source = candidate.source.deref();
      const trigger = candidate.trigger.deref();
      if (
        !publicTarget(source ?? null) ||
        !source ||
        !trigger?.isConnected ||
        trigger.matches(':disabled') ||
        !imageReady(source, candidate.visual)
      )
        return null;
      ...
      const carry =
        origin?.phase === 'return' &&
        origin.displayed === intent.displayedDetailKey &&
        currentOrigin(origin) &&
        origin.sprite
          ? measure(origin.sprite)
          : null;
      runtime.cancel('superseded');
      const abort = new AbortController();
      const handle: MotionOriginLease = {
        [originLeaseBrand]: true,
      ...
      const entry: OriginLease = {
        ...
        abort,
        hint: candidate,
        requested: intent.requestedDetailKey,
        displayed: intent.displayedDetailKey,
        created: environment.now(),
        start: carry ?? fitMotionVisual(candidate.visual, sourceRect),
        phase: 'pending',
    ...
    startMotionSession({ channel, guard }) {
      if (!eligible() || (dialogs.size > 0 && channel !== 'dialog' && channel !== 'continuity') || !guardCurrent(guard))
        return null;
      ...
      const dispose = (reason: MotionCancelReason | 'finished') => {
        ...
        for (const cleanup of [...cleanups]) {
          try {
            ...
            console.error('A motion cleanup failed. Remaining cleanup will continue.');
          }
        }
        cleanups.clear();
      ...
      const session: MotionSession = {
        ...
        isCurrent: () =>
          ...
          snapshot.location.overlayKey === read().location.overlayKey &&
          guardCurrent(guard),
        addCleanup(cleanup) {
          ...
          try {
            animation = element.animate(
              frames.map((frame) => ({ ...frame })),
              {
                ...
                easing: timing.easing ?? MOTION_EASING,
                fill: 'both',
              },
            );
          } catch {
    ...
    openDialog(dialog, inner, slot, options) {
      ...
      const localArrival = () => {
        ...
        if (options.continuity) {
          ...
          entrance?.animate(
            ...
            {
              duration: read().policy.coarsePointer
                ? MOTION_TIMINGS.localArtwork.coarse
                : MOTION_TIMINGS.localArtwork.fine,
            },
          );
      ...
      if (
        entry &&
        expectedOpen(entry) &&
        read().location.displayedDetailKey === entry.displayed &&
        environment.now() - entry.created <= MOTION_ORIGIN_TTL &&
        publicTarget(target ?? null) &&
        target
      ) {
        entry.phase = 'open';
        entry.openedNavigation = read().location.navigationGeneration;
        nextFrame(() => {
          if (!dialog.open || !expectedOpen(entry)) {
            finishOrigin(entry);
            return;
          }
          const destination =
