...
export function useDiscoveryCatalog(enabled: boolean) {
  ...
  const [state, setState] = useState<{ status: 'idle' | 'loading' | 'ready' | 'error'; catalog: DiscoveryCatalog | null; error: string | null }>({
  ...
  useEffect(() => {
    ...
    void loadDiscoveryCatalog(controller.signal).then((catalog) => {
      ...
      if (!controller.signal.aborted) setState({ status: 'error', catalog: null, error: error instanceof Error ? error.message : 'The local catalog could not be loaded.' });
    });
