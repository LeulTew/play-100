...
export function useLibrary(canonicalRecords: LibraryRecord[], canonicalLoading: boolean) {
  ...
  useEffect(() => {
    ...
    const attempt = startupLoad.current ??
      takeGuestLibraryLoad() ?? { records: canonicalRecords, promise: loadPersonalLibrary(canonicalRecords) };
    ...
    void attempt.promise
      .catch((error: unknown) => {
        if (
          ...
          attempt.records !== canonicalRecords &&
          canonicalRecords.length > 0
        )
          return loadPersonalLibrary(canonicalRecords);
        throw error;
      })
      .then((result) => {
      ...
      void queue.current
        ...
        .catch((error: unknown) => {
          if (!canceled && sequence === loadSequence.current)
            publish({
              ...current.current,
              error: `Another-tab refresh failed. Your current view is retained. ${describeError(error)}`,
            });
        });
    };
    const visible = () => {
    ...
    (operation: () => Promise<void>): Promise<boolean> => {
      ...
      const task = queue.current.then(async () => {
        try {
          ...
          publish({ ...current.current, error: describeError(error) });
          return false;
        } finally {
          setPending((count) => count - 1);
        }
    ...
    (action: PersonalAction) =>
      enqueue(async () => {
        ...
        if (previous.status === 'loading')
          throw new Error('Your device library is still opening. Please try again in a moment.');
        const state =
