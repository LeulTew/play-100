...
export function useDiscoverSearch(
  ...
  const items = useMemo(
    () => (canonicalReady ? catalogSearchItems(games, seed.catalog?.items ?? []) : []),
    [canonicalReady, games, seed.catalog],
  );
  const scope = useMemo(
    () => discoveryScope(items, { q, source, genre, year, genreFamily, include100 }),
    [items, q, source, genre, year, genreFamily, include100],
  );
  const local = useMemo(
    () => scope.cards.filter((item) => matchesProgress(progress[item.record.id], progressView)),
    [scope.cards, progress, progressView],
  );
  const remoteEnabled = shouldSearchOnline(
    filters.q,
    canonicalReady &&
      source !== 'collection' &&
      filters.catalogs === 'on' &&
      progressView === 'all' &&
      (filters.online === 'on' || !scope.exactCollectionMatch),
    seed.status,
    local.length,
    filters.online === 'on',
  );
  const remote = useCatalogSearch(
    filters.q,
    remoteEnabled,
    source === 'collection' ? 'all' : source,
    filters.online === 'on' ? filters.offset : 0,
  );
  const resolvedRemote = (canonicalReady ? resolveCatalogRecords(remote.records, games) : []).filter(
    (record) =>
      matchesProgress(progress[record.id], progressView) &&
      matchesDiscoveryGenre(record.genre, genre, genreFamily) &&
      (!filters.year || record.year === Number(filters.year)),
  );
  const remoteRecords = scope.showCollection
    ? resolvedRemote
    : resolvedRemote.filter((record) => !collectionGameForId(games, record.id));
  const collectionMatches = [
    ...new Map(
      [
        ...scope.collectionMatches
          .filter((item) => matchesProgress(progress[item.record.id], progressView))
  ...
  return {
    ...
    localPage,
    collectionMatches,
    showCollection: scope.showCollection,
  };
}
