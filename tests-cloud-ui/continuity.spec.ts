...
test('own active saved copy restores into an empty local wrapper without uploading guest data or requiring another consent', async ({
  page,
  browser,
  request,
}) => {
  ...
  try {
    ...
    await peer.goto('/?game=red-dead-redemption-2');
    await seedGuestRating(peer, '3.5');
...
test('stopped cloud copies are not enabled by sign-in and the explicit consent action has one name editor', async ({
  page,
  browser,
  request,
}) => {
  ...
  try {
    const peer = await second.newPage();
    await signIn(peer, email);
    await expect(peer.locator('.sync-panel')).toContainText(
...
test('Data use opens separately without starting private storage or losing the current name draft', async ({
  page,
  context,
  request,
}) => {
  ...
  await page
    .locator('.consent-summary')
    .getByRole('link', { name: /Data use/ })
    .click();
  ...
  try {
    ...
    const result = await policy.evaluate(() => ({
      ...
      onlineResources: performance
        ...
        .map((row) => row.name)
        .filter((url) => /OnlineController|googleapis|:9199|:8188/.test(url)),
    }));
...
test('the owned IndexedDB boot marker restores identity on home when localStorage is blocked', async ({
  page,
  request,
}) => {
  ...
  await page.addInitScript(() => {
    Object.defineProperty(window, 'localStorage', {
      get() {
        throw new DOMException('Fixture denied localStorage', 'SecurityError');
      },
    });
  });
...
test('SDK localStorage persistence retains sign-in when only the Firebase auth IndexedDB is unavailable', async ({
  page,
  request,
}) => {
  await page.addInitScript(() => {
