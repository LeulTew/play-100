...
for (const policy of ['live-270f', 'candidate'] as const)
  test.describe(`profile read migration with ${policy} rules`, () => {
    ...
    if (policy === 'live-270f')
      test('a deployment-window LIST denial keeps payload intact and leaves Finish deleting visible after reload', async ({
        page,
        context,
        request,
        baseURL,
        ...
        if (!baseURL || !['127.0.0.1', 'localhost'].includes(new URL(baseURL).hostname))
          throw new Error('Use the owned emulator-bound UI.');
        ...
        await expect(page.locator('.sync-panel [role="alert"]')).toContainText('the online service needs an update');
        ...
        await expect(page.locator('.sync-panel [role="alert"]')).toContainText(
          'Once the service is updated, choose Finish deleting to continue.',
        );
        ...
        await expect(page.getByRole('region', { name: 'Deletion was requested', exact: true })).toContainText(
          'Removal of all online data could not be confirmed.',
        );
      });

    test('sent rows show the published source or an unavailable profile without substituting private identity', async ({
      page,
      context,
      request,
      baseURL,
      ...
      const documents: Record<string, Record<string, unknown>> = {
        ...
        [`friendIdentities/${peer}`]: {
          format: 1,
          uid: peer,
          displayName: 'Private name must not appear',
          avatar,
          revision: 1,
          updatedAt: new Date(now),
        },
      ...
      if (policy === 'live-270f')
        documents[`publicProfiles/${peer}`] = {
          uid: peer,
          handle: 'migration_published',
          displayName: 'Published migration name',
          avatar,
          title: 'Migration ranking',
          count: 1,
          preview: ['Fixture game'],
          generation,
          epoch: 1,
          published: true,
          listed: false,
