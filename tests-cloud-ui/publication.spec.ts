...
test('an explicitly published snapshot stays frozen, keeps private fields out, and can be safely saved by a guest', async ({ page, browser, request, isMobile, viewport }) => {
  ...
  await page.getByLabel(`Your note for ${title}`, { exact: true }).fill('PRIVATE ONLY: never publish this note or queue.');
  ...
  const visitor = await browser.newContext({ baseURL: 'http://127.0.0.1:4187', viewport, isMobile, hasTouch: isMobile });
  try {
...
test('a fresh unverified mistyped registration can be cancelled without touching its guest library', async ({ page, request }) => {
  ...
  const lookup = await request.post('http://127.0.0.1:9199/identitytoolkit.googleapis.com/v1/accounts:lookup?key=demo-play100-key', {
