...
declare global {
  interface Window { managerPairGate?: { waiting: boolean; finished: boolean; accepted: boolean; release: () => void } }
}
test.beforeEach(async ({ page }) => { page.setDefaultTimeout(20000); await page.emulateMedia({ reducedMotion: 'reduce' }); });

async function prepare(page: Page, request: Parameters<typeof seedManager>[0]) {
  const email = emailFor('manager');
  await page.goto('/?game=red-dead-redemption-2'); await seedGuestRating(page, '7.2');
  ...
  await page.evaluate(async (failingPeer) => {
    ...
    module.FriendStore.prototype.identity = function(uid) {
      reads.identities.push(uid); persist();
      if (uid === failingPeer && failOnce) { failOnce = false; return Promise.reject(Object.assign(new Error('Synthetic temporary profile failure'), { code: 'unavailable' })); }
      return original.call(this, uid);
    };
    module.FriendStore.prototype.ranking = function(uid) { reads.rankings += 1; persist(); return ranking.call(this, uid); };
  }, fixture.accepted[1]!);
...
async function checkManager(page: Page) {
  ...
  const small = await page.locator('.friends-page button:visible, .friends-page input:not([type="checkbox"]):visible, .friends-page select:visible, .friend-select').evaluateAll((elements) =>
    elements.filter((element) => { const box = element.getBoundingClientRect(); return box.width < 43.5 || box.height < 43.5; }).map((element) => element.textContent));
...
test('loaded pages survive live changes; directional scans, row recovery, private cohort and Back/reload remain correct', async ({ page, request }, testInfo) => {
  ...
  await writeManagerDocuments(request, { [pairPath(f.uid, f.accepted[19]!)]: {
    ...managerPair(f.uid, f.accepted[19]!, f.uid, 'removed', Date.now(), 2), createdAt: new Date(f.now - 119000),
  } });
  ...
  await page.evaluate(async (peer) => {
    ...
    module.FriendStore.prototype.watchPair = function(uid, other, next, error) {
      if (other === peer && failOnce) {
        failOnce = false;
        queueMicrotask(() => error(Object.assign(new Error('Synthetic selected stream interruption'), { code: 'unavailable' })));
        return () => {};
      }
  ...
  await page.evaluate(async (peer) => {
    ...
    const gate: NonNullable<Window['managerPairGate']> = { waiting: false, finished: false, accepted: false, release: () => { throw new Error('The comparison read has not started.'); } };
    ...
    module.FriendStore.prototype.pair = function(uid, other) {
      ...
      return new Promise<Awaited<ReturnType<typeof original>>>((resolve, reject) => {
        gate.release = () => {
          ...
          void original.call(this, uid, other).then((value) => {
            gate.accepted = value?.state === 'accepted'; gate.finished = true; resolve(value);
          }, (cause) => { gate.finished = true; reject(cause); });
        };
      });
    };
  }, f.accepted[30]!);
  ...
  await page.goBack(); await expect(page).toHaveURL(/view=incoming/);
  await page.goForward(); await expect(page).toHaveURL(origin);
  await page.evaluate(() => { if (!window.managerPairGate) throw new Error('Synthetic read gate is missing.'); window.managerPairGate.release(); });
...
test('menus confirm named actions, blocked profiles stay private, invite expiry is precise and account changes clear selections', async ({ page, request }, testInfo) => {
  test.setTimeout(150000);
  const f = await prepare(page, request);
  ...
  await more.click(); await page.getByRole('menuitem', { name: 'Block player', exact: true }).click();
  ...
  const readsBefore = await page.evaluate(() => JSON.parse(sessionStorage.getItem('qa:manager-reads') ?? '{}').identities.length);
  await page.getByRole('button', { name: 'Blocked', exact: true }).click();
  await expect(page.locator('.friend-list > li')).toHaveCount(2);
  const laterReads: string[] = await page.evaluate((start) => JSON.parse(sessionStorage.getItem('qa:manager-reads') ?? '{}').identities.slice(start), readsBefore);
  expect(laterReads).not.toContain(f.blocked);
  expect(laterReads).not.toContain(f.accepted[0]);
  await page.locator('.friend-list > li').filter({ hasText: f.accepted[0]!.slice(-6) }).getByRole('button', { name: 'Unblock', exact: true }).click();
  ...
  await expect(page.locator('.friend-list > li')).toHaveCount(5);
  for (const status of ['Active', 'Used', 'Expired', 'Revoked']) await expect(page.locator('.friend-invite-status').filter({ hasText: new RegExp(`^${status}$`) }).first()).toBeVisible();
  const inactive = page.locator('.friend-list > li').filter({ has: page.locator('.friend-invite-status').filter({ hasText: /^(Used|Expired|Revoked)$/ }) });
  expect(await inactive.getByRole('button').count()).toBe(0);
  const active = page.locator('.friend-list > li').filter({ has: page.locator('.friend-invite-status').filter({ hasText: /^Active$/ }) }).first();
  ...
  await createAccount(page, otherEmail); await verifyEmail(page, request, otherEmail); await enableSync(page);
  await page.goto('/friends');
