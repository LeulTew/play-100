...
export async function writeManagerDocuments(request: APIRequestContext, documents: Record<string, Record<string, unknown>>) {
  if (new URL(firestoreOrigin).hostname !== '127.0.0.1' || Object.keys(documents).length > 400) throw new Error('Only bounded local emulator fixtures may be written.');
  const response = await request.post(`${firestoreOrigin}/v1/${documentsRoot}:commit`, {
    headers: { Authorization: 'Bearer owner' },
    data: { writes: Object.entries(documents).map(([path, data]) => ({ update: { name: `${documentsRoot}/${path}`, fields: Object.fromEntries(Object.entries(data).map(([key, value]) => [key, field(value)])) } })) },
  });
...
export function managerPair(owner: string, peer: string, from: string, state: string, updatedAt: number, epoch = 1) {
  ...
  return { format: 1, a, b, participants: [a, b], from, state, epoch, inviteSlot: null, createdAt: new Date(updatedAt - 100000), updatedAt: new Date(updatedAt) };
}
export function pairPath(owner: string, peer: string) { return `friendPairs/${[owner, peer].sort().join('~')}`; }
export async function seedManager(request: APIRequestContext, uid: string) {
  ...
  const names = Array.from({ length: 45 }, (_, index) => index === 0 ? 'QA AlexandraTheExtraordinarilyLongSingleWordPlayerName000' : `QA Friend ${String(index).padStart(2, '0')}`);
  ...
  const data: Record<string, Record<string, unknown>> = {
    [`friendSettings/${uid}`]: { format: 1, enabled: false, deleted: false, selection: '', epoch: 1, revision: 1, updatedAt: new Date(now) },
  };
  for (const [index, peer] of [...accepted, ...sent, ...incoming].entries()) {
    ...
    data[`friendSettings/${peer}`] = { format: 1, enabled: false, deleted: false, selection: '', epoch: 1, revision: 1, updatedAt: new Date(now) };
    if (peer !== accepted[2]) data[`friendIdentities/${peer}`] = { format: 1, uid: peer, displayName: isAccepted ? names[index] : `${incoming.includes(peer) ? 'Incoming' : 'Sent'} ${peer.slice(-2)}`, avatar, revision: 1, updatedAt: new Date(now) };
  ...
  for (const [slot, token] of tokens.entries()) {
    data[`friendInvites/${token}`] = { format: 1, ownerUid: uid, slot, displayName: 'QA Manager', avatar, state: kinds[slot], createdAt: new Date(created[slot]!), acceptedBy: kinds[slot] === 'consumed' ? accepted[1] : null };
