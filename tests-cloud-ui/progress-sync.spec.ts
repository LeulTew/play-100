...
test('account sync keeps Played and Completed distinct and excludes both from All sharing', async ({
  page,
  request,
  browser,
  viewport,
  ...
  try {
    ...
    await expect(
      peerEditor.getByRole('button', { name: 'Completed: Sync progress fixture', exact: true }),
    ...
    await expect(editor.getByRole('button', { name: 'Completed: Sync progress fixture', exact: true })).toHaveAttribute(
      'aria-pressed',
      'false',
      { timeout: 30000 },
    );
    ...
    expect((await readAccount(page, uid)).state.progress[record.id]).toEqual({
      played: true,
      completed: false,
      later: false,
    });
