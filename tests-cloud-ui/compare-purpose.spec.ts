...
test('signed-out Compare explains friends rankings before authentication and preserves the device-only exit', async ({ page }) => {
  ...
  await expect(page.locator('.auth-purpose')).toContainText('Pins select games for comparison; they do not share your library.');
