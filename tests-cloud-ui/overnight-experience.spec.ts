...
async function saveCatalogGame(page: Page) {
  ...
  await card(page)
    .getByRole('button', { name: `Add to My games: ${title}`, exact: true })
    .click();
...
async function account(
  ...
  prefix: string,
  name: string,
  choice: 'guest' | 'empty' = 'empty',
) {
  const email = emailFor(prefix);
  await createAccount(page, email);
  await verifyEmail(page, request, email);
  await enableSync(page, choice);
...
async function shareShelf(owner: Page, names: string[]) {
  ...
  for (const name of names)
    await owner.locator('.friend-shelf-selection > li').filter({ hasText: name }).locator('input').check();
  ...
  await expect(owner.locator('.friend-shelf-editor .friend-shelf-heading [role="status"]')).toHaveText('saved', {
    timeout: 30000,
  });
}
...
test('the real cold catalog is image-led, finds compact aliases without providers, and retains public detail URLs', async ({
  page,
}, testInfo) => {
  const api: string[] = [];
  const images: string[] = [];
  ...
  await page.route('**/api/catalog?**', (route) =>
    route.fulfill({
      status: 503,
      json: { error: 'Synthetic provider outage.' },
  ...
  await expect(card(page).getByRole('link', { name: 'Image source', exact: true })).toHaveAttribute(
    'href',
    /^https:\/\/commons\.wikimedia\.org\/wiki\/File:/,
  );
  await expect(card(page).getByRole('link', { name: 'Public domain', exact: true })).toHaveAttribute(
    'href',
...
test('restricted storage and a failed online fallback never hide valid seeded matches', async ({
  page,
  browser,
  viewport,
  isMobile,
  ...
  try {
    await restricted.addInitScript(() => {
      ...
      IDBFactory.prototype.open = () => {
        throw new DOMException('Synthetic denied database', 'SecurityError');
      };
    });
    ...
    expect(
      await fresh.evaluate(() =>
        performance
          ...
          .some((entry) =>
            /identitytoolkit|firestore\.googleapis|:8188\/|OnlineController(?:-[^/]+\.js|\.tsx)/.test(entry.name),
          ),
      ),
    ).toBe(false);
  } finally {
  ...
  await page.route('**/api/catalog?**', (route) =>
    route.fulfill({
      ...
      json: { error: 'Synthetic temporary provider failure.' },
...
test('pinning and deliberate drag are UI-only, capped at six, persistent and safe above mobile navigation', async ({
  page,
  isMobile,
}, testInfo) => {
  await page.goto('/discover?catalogs=off');
...
test('My games keeps old links, unranked additions, manual drafts, valid exit saves and independent completed queue filtering', async ({
  page,
}, testInfo) => {
  ...
  await row(page)
    .getByRole('spinbutton', { name: `Your rating / 10 for ${title}`, exact: true })
    .fill('8.4');
  await tab(page, 'Library').click();
  ...
  await expect(
    page
      .getByRole('alert')
      .filter({ hasText: /edit has not saved|0.*10/ })
      .first(),
  ).toBeVisible();
...
test('an explicit unranked shelf stays independent, updates after removal, stops in an open viewer and hands pinned games to private comparisons', async ({
  page,
  browser,
  request,
  viewport,
  ...
  await saveCatalogGame(page);
  await card(page)
    .getByRole('button', { name: `Pin for comparison: ${title}`, exact: true })
    .click();
  ...
  try {
    ...
    await peer
      .locator('.friend-shelf-cards')
      .getByRole('button', { name: `Pin ${title}`, exact: true })
      .click();
    ...
    await expect(
      peer.locator('.friend-shelf-cards').getByRole('button', { name: 'Saved', exact: true }),
    ).toBeDisabled();
    const copied = (await readAccount(peer, viewer.uid)).state;
    expect(copied.records[kcd]?.title).toBe(title);
    expect(copied.ranking).toEqual([]);
    expect(copied.queueOrder).toEqual([]);
    expect(copied.progress).toEqual({});
    ...
    await page.evaluate(
      async ({ uid, id }) => {
        ...
        if (!record) throw new Error('Owned fixture record missing.');
    ...
    await expect(
      page.locator('.friend-shelf-selection > li').filter({ hasText: title }).locator('input'),
    ).not.toBeChecked();
...
test('account export and reversible/full deletion include the independent shelf without losing the device original', async ({
  page,
  request,
}) => {
...
test('revoked unsaved manual shelf previews close on Stop and block without deleting independent saved games', async ({
  page,
  browser,
  request,
  viewport,
  ...
  try {
    ...
    await peer
      .locator('.friend-shelf-cards')
      .getByRole('button', { name: 'QA transient manual preview', exact: true })
      .click();
    ...
    await expect(peer.getByRole('dialog')).toHaveCount(0);
    await expect(peer.locator('.friend-shelf-grid')).toHaveCount(0);
    expect(Object.values((await readAccount(peer, viewer.uid)).state.records).map((record) => record.title)).toEqual([
      'QA independent saved copy',
    ]);
...
test('tray Compare resets the mounted comparison filters and page without changing the people or sign-in runtime', async ({
  page,
  browser,
  request,
  viewport,
    ...
    await peer.evaluate(async (uid) => {
      ...
      for (const game of data.games.slice(0, 30))
        await library.commitScopedAction(`account:demo-play100:${uid}`, {
          type: 'rate-game',
          record: records.recordFromGame(game),
          score: 7,
        });
    }, viewer.uid);
    await peer.goto('/my-games');
    await workspace(peer)
      .getByRole('button', { name: 'Pin for comparison: Red Dead Redemption 2', exact: true })
      .click();
    await compareFromTray(peer);
...
test('initial shared-games consent stays checking or error until an enabled or absent configuration is actually confirmed', async ({
  page,
  browser,
  request,
  viewport,
  ...
  try {
    ...
    await returning.evaluate(async (uid) => {
      ...
      const gate: NonNullable<Window['shelfReadGate']> = {
        calls: 0,
        fail: () => {
          throw new Error('The initial watch is not attached yet.');
        },
      };
      ...
      source.FriendShelfStore.prototype.watchConfig = function (owner, next, error) {
        ...
        gate.fail = () => {
          gated = false;
          error(Object.assign(new Error('Synthetic initial consent failure'), { code: 'unavailable' }));
        };
        return () => {};
      };
    }, owner.uid);
    ...
    await returning.evaluate(() => {
      if (!window.shelfReadGate) throw new Error('Missing consent gate.');
      window.shelfReadGate.fail();
    });
