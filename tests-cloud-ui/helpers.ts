...
export function emailFor(prefix = 'qa') { return `${prefix}-${crypto.randomUUID()}@play100.test`; }
...
export async function uidFor(request: APIRequestContext, email: string): Promise<string> {
  const response = await request.post(`${authOrigin}/identitytoolkit.googleapis.com/v1/accounts:lookup?key=demo-play100-key`, {
...
export async function readAccount(page: Page, uid: string): Promise<ScopedLibrary> {
  return page.evaluate((key) => new Promise<ScopedLibrary>((resolve, reject) => {
    ...
    open.onsuccess = () => {
      ...
      request.onsuccess = () => { if (request.result) resolve(request.result); else reject(new Error('Account cache is missing.')); };
