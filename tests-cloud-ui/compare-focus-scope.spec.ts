...
test('a real emulator identity transition invalidates the guest Compare focus origin without losing guest pins', async ({ page, request, baseURL }) => {
  if (!baseURL || new URL(baseURL).hostname !== '127.0.0.1') throw new Error('Scope focus validation requires the owned local emulator app.');
  ...
  await page.evaluate(async credentials => {
    ...
    if (client.firebaseApp.options.projectId !== 'demo-play100') throw new Error('Never use a production identity for a scope fixture.');
    ...
    await scoped.connectScopedLibrary(scope, current.state, {
      format: 1, epoch: 1, revision: 0, enabled: true, deleted: false, current: null, previous: null, updatedAt: Date.now(),
    }, 'Paused local scope fixture', false, {
      localRevision: current.state.revision, epoch: current.sync.epoch, enabled: current.sync.enabled,
    });
    await scoped.pauseScopedLibrary(scope, 1);
    const sdkPath = performance.getEntriesByType('resource').map(item => item.name)
