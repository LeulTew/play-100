...
test('interrupted private deletion keeps Auth and resumes on the next sign-in before removing the account', async ({ page, context, request, baseURL }) => {
  ...
  await expect(page.locator('.sync-panel [role="alert"]')).toContainText('cleanup is incomplete');
  await expect(page.locator('.sync-panel [role="alert"]')).toContainText('20 chunk deletions');
  ...
  await expect(page.getByRole('button', { name: 'Finish deleting account', exact: true })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Finish deleting account', exact: true }).click();
  await page.getByLabel('Confirm your password', { exact: true }).fill(password);
  await page.getByRole('dialog').getByRole('button', { name: 'Confirm deletion', exact: true }).click();
  await expect(page.locator('.sync-panel [role="alert"]')).toContainText('Your sign-in still exists. Confirm deletion again');
  expect((await (await lookup()).json()).users).toHaveLength(1);
  await page.getByRole('dialog').getByRole('button', { name: 'Confirm deletion', exact: true }).click();
