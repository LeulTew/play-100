...
test('verified unused email registration can delete without first enabling sync or creating a profile', async ({
  page,
  request,
}) => {
...
test('corrupt account cache does not trap sign-out or prevent a network-only account export', async ({
  ...
  const exportFile = await backup.path();
  if (!exportFile) throw new Error('The account export was not produced.');
  ...
  expect(JSON.parse(exported)).toMatchObject({
    app: 'Play 100',
    formatVersion: 1,
    deviceCacheError: expect.any(String),
  });
  await expect(
    page
      .getByRole('status')
      .filter({ hasText: 'The unreadable copy on this device is marked unavailable in the export' }),
  ).toBeVisible();
  ...
  const guestFile = await guestBackup.path();
  if (!guestFile) throw new Error('The device-only backup was not produced.');
...
test('a guest inline rating keeps its original save target while another tab restores an account', async ({
  page,
  context,
  request,
}) => {
  ...
  await page
    .locator('.desktop-nav:visible, .mobile-nav:visible')
    .getByRole('link', { name: 'My games', exact: true })
    .click();
...
test('an interrupted upload recovers automatically after reconnect without a manual sync click', async ({
  page,
  context,
  request,
}) => {
  ...
  await page.route('**/documents:commit*', async (route) => {
    ...
    if (body.writes?.some((write) => write.update?.name?.endsWith(`/syncHeads/${uid}`))) {
      await route.fulfill({
        status: 503,
        json: { error: { code: 503, status: 'UNAVAILABLE', message: 'Isolated upload interruption fixture' } },
      });
    } else await route.continue();
  });
  await page.getByRole('spinbutton').fill('6.9');
...
test('the creator can inspect and hide a reported public profile even without a members document', async ({
  page,
  browser,
  request,
  isMobile,
  ...
  await createAccount(page, publisherEmail);
  await verifyEmail(page, request, publisherEmail);
  ...
  const removed = await request.delete(
    `http://127.0.0.1:8188/v1/projects/demo-play100/databases/(default)/documents/members/${publisherUid}`,
  ...
  const originalFields = originalOwner.ok()
    ? (((await originalOwner.json()) as { fields?: Record<string, unknown> }).fields ?? {})
    : null;
  try {
    const admin = await moderator.newPage();
    await createAccount(admin, creatorEmail);
    await verifyEmail(admin, request, creatorEmail);
    ...
    const bootstrapped = await request.patch(ownerConfig, {
      headers: { Authorization: 'Bearer owner' },
      data: { fields: { uid: { stringValue: creatorUid }, email: { stringValue: creatorEmail } } },
    });
    ...
    await expect(
      admin.getByRole('dialog').getByRole('heading', { name: 'Public-only publisher', exact: true }),
    ).toBeVisible();
...
test('a clean failed online check stays paused after a fresh unchanged head and a later edit until manual retry', async ({
  page,
  request,
  isMobile,
}) => {
  ...
  try {
    const invalid = await request.patch(url, {
      headers,
      data: { fields: { ...saved.fields, format: { integerValue: '99' } } },
    });
    ...
    await page
      .locator(isMobile ? '.mobile-nav' : '.desktop-nav')
      .getByRole('link', { name: 'My games', exact: true })
      .click();
    await page
      .getByRole('navigation', { name: 'My games views', exact: true })
      .getByRole('button', { name: /^Ranking\b/ })
      .click();
    await page.getByRole('spinbutton').fill('7.4');
    await page.getByRole('spinbutton').press('Tab');
    ...
    if (needsRestore) await request.patch(url, { headers, data: { fields: saved.fields } });
  }
});
