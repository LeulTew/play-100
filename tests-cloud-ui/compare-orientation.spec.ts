...
interface Actor { label: string; email: string; uid: string; displayName: string; verified: boolean }
interface FixtureLane { lane: string; ready: boolean; actors: Actor[]; routes: { compareTwo: string; compareSix: string } }
interface FixtureManifest { status: string; lanes: FixtureLane[] }
...
declare global { interface Window { compareOrientationProbe: CompareProbe } }
...
async function guard(context: BrowserContext) {
  ...
  await context.routeWebSocket('**/*', route => {
    ...
    else { blocked.push(url.origin); route.close(); }
  });
  await context.addInitScript(() => { Object.defineProperty(globalThis, 'RTCPeerConnection', { configurable: true, value: undefined }); });
  return blocked;
}
...
async function login(page: Page, actor = fixture().owner) {
  ...
  await expect(page.locator('.emulator-page-note')).toHaveText('Local emulator preview — no production account or cloud data connection.');
  const config = await page.evaluate(async () => {
    const path = '/src/cloud/firebase-client.ts';
    const client: typeof import('../src/cloud/firebase-client') = await import(path);
    return { project: client.firebaseApp.options.projectId, host: client.cloudAuth.emulatorConfig?.host, port: client.cloudAuth.emulatorConfig?.port };
  });
...
async function installProbe(page: Page) {
  await page.evaluate(async () => {
    ...
    const probe: CompareProbe = {
      counts, groups: [], rows: [],
      blockGroup: id => { let release = () => {}; const promise = new Promise<void>(resolve => { release = resolve; }); delays.set(id, { promise, release }); },
      releaseGroup: id => { delays.get(id)?.release(); delays.delete(id); },
      failRead: peer => { const head = heads.get(peer); if (!head?.value) throw new Error('No mounted ranking read to fail.'); failPeer = peer; head.next(head.value); },
      revoke: peer => { for (const next of [...pairs.get(peer) ?? []]) next(null); },
      rename: (peer, name) => { const person = people.get(peer); if (!person?.value) throw new Error('No mounted identity to rename.'); person.next({ ...person.value, displayName: name }); },
    };
    ...
    FriendStore.prototype.listGroups = async function (...args) { count('groupLists'); const value = await groups.apply(this, args); probe.groups = value.items; return value; };
    const relations = FriendStore.prototype.listRelations;
    FriendStore.prototype.listRelations = function (...args) { count('relationReads'); return relations.apply(this, args); };
    const save = FriendStore.prototype.saveGroup;
    FriendStore.prototype.saveGroup = function (...args) { count('groupWrites'); return save.apply(this, args); };
    ...
    FriendStore.prototype.watchPair = function (uid, peer, next, error) {
      ...
      callbacks.add(next); pairs.set(peer, callbacks);
      const release = pair.call(this, uid, peer, next, error);
      return () => { count('pairReleases'); callbacks.delete(next); release(); };
    };
    ...
    FriendStore.prototype.watchIdentity = function (peer, next, error) {
      ...
      const state = { value: null as Person | null, next }; people.set(peer, state);
      const release = identity.call(this, peer, value => { state.value = value; next(value); }, error);
      return () => { count('identityReleases'); release(); };
    };
    const settings = FriendStore.prototype.watchSettings;
    FriendStore.prototype.watchSettings = function (...args) { count('settingsSubscriptions'); const release = settings.apply(this, args); return () => { count('settingsReleases'); release(); }; };
    const legacyHead = FriendStore.prototype.watchShareHead;
    FriendStore.prototype.watchShareHead = function (...args) { count('legacySubscriptions'); const release = legacyHead.apply(this, args); return () => { count('legacyReleases'); release(); }; };
    const legacy = FriendStore.prototype.ranking;
    FriendStore.prototype.ranking = function (...args) { count('legacyReads'); return legacy.apply(this, args); };
    const shelf = FriendShelfStore.prototype.watchConfig;
    FriendShelfStore.prototype.watchConfig = function (...args) { count('shelfSubscriptions'); const release = shelf.apply(this, args); return () => { count('shelfReleases'); release(); }; };
    ...
    FriendAllStore.prototype.watchHead = function (peer, kind, next, error) {
      ...
      const release = watch.call(this, peer, kind, value => { state.value = value; next(value); }, error);
      return () => { count('allReleases'); release(); };
    };
    const page = FriendAllStore.prototype.page;
    FriendAllStore.prototype.page = function (peer, kind, ...args) {
      count('pageReads');
      if (peer === failPeer && kind === 'ranking') { failPeer = null; return Promise.reject(new Error('Synthetic Compare read failure. Refresh this player.')); }
      return page.call(this, peer, kind, ...args);
    };
    const exact = FriendAllStore.prototype.exact;
    FriendAllStore.prototype.exact = function (...args) { count('exactReads'); counts.maxExact = Math.max(counts.maxExact ?? 0, args[2].length); return exact.apply(this, args); };
  });
}
...
async function settleScroll(region: Locator) {
  await region.evaluate(element => new Promise<void>((resolve, reject) => {
...
test('restored two/six-person cohorts retain rows, zero/unrated, group draft and mounted loader budget across disclosures', async ({ page, context }, info) => {
  ...
  await page.evaluate(() => { window.compareOrientationProbe.rows = [...document.querySelectorAll('.compare-freshness > li')]; });
  const counts = await page.evaluate(() => ({ ...window.compareOrientationProbe.counts }));
  for (let index = 0; index < 3; index += 1) {
    await toggle(page, 'people', true); await toggle(page, 'coverage', true);
    await toggle(page, 'people', false); await toggle(page, 'coverage', false);
  }
  const afterToggles = await page.evaluate(() => ({ ...window.compareOrientationProbe.counts }));
  expect(afterToggles).toEqual(counts);
  expect(await page.evaluate(() => window.compareOrientationProbe.rows.every((row, index) => row === document.querySelectorAll('.compare-freshness > li')[index]))).toBe(true);
  expect(await page.locator('.friend-matrix').textContent()).toBe(before);
  await writeFile(info.outputPath('disclosure-budget.json'), JSON.stringify({ before: counts, after: afterToggles, nodesRetained: true, rowTextUnchanged: true, blockedExternalOrigins: blocked }, null, 2));
...
test('one-person chooser stays open through second-to-sixth checks and below-two revocation reopens it', async ({ page, context }) => {
  const blocked = await guard(context);
  await login(page); await installProbe(page);
  ...
  await toggle(page, 'people', false); await toggle(page, 'people', true);
  await expect(page.locator('.compare-people input:checked')).toHaveCount(6);
  ...
  await expect(page.locator('.compare-people input:checked')).toHaveCount(1);
  await expect(people(page)).toHaveAttribute('open', '');
  await expect(page.getByText('A player is no longer a friend and was removed from this comparison.', { exact: true })).toBeVisible();
...
test('late groups respect early disclosure intent and cannot replace a newer chosen group or name draft', async ({ page, context }) => {
  const blocked = await guard(context);
  await login(page); await installProbe(page);
...
test('a failed mounted read opens coverage, stays visibly named when closed, and recovers without remounting peers', async ({ page, context }) => {
  const blocked = await guard(context);
  await login(page); await installProbe(page);
  await navigate(page, fixture().routes.compareSix); await sixReady(page);
  await toggle(page, 'people', false); await toggle(page, 'coverage', false);
...
test('exact-six game filters keep bounded exact reads and unknown whole-cohort totals', async ({ page, context }, info) => {
  const blocked = await guard(context);
  await login(page); await installProbe(page);
  await navigate(page, fixture().routes.compareSix); await sixReady(page);
  await toggle(page, 'people', true);
  ...
  await page.evaluate(async uid => {
    ...
    const games = collection.parseCollection(await (await fetch('/data/collection.json')).json()).games.slice(0, 6).map(records.recordFromGame);
  ...
  await expect(page.locator('.compare-coverage-summary')).toHaveText('Only the chosen games are checked. Overall totals are unknown.');
  await expect(people(page)).toHaveAttribute('open', '');
  ...
  await writeFile(info.outputPath('exact-read-budget.json'), JSON.stringify({ before, after, exactGames: 6, rows: 6, blockedExternalOrigins: blocked }, null, 2));
  expectGuarded(blocked);
});

test('the same saved six-person table fits normal banners at desktop390/320 and keeps both sticky bearings', async ({ browser, isMobile }, info) => {
  ...
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }, { width: 320, height: 740 }]) {
    ...
    const context = await browser.newContext({ ...(touch ? devices['Pixel 7'] : devices['Desktop Chrome']), baseURL: origin, viewport, deviceScaleFactor: 1, reducedMotion: 'reduce', serviceWorkers: 'block' });
    try {
      const blocked = await guard(context);
      ...
      const before = await page.evaluate(() => {
        ...
        return { width: innerWidth, height: innerHeight, coarse: matchMedia('(pointer: coarse)').matches, touchPoints: navigator.maxTouchPoints, scrollY, documentWidth: document.documentElement.scrollWidth,
          navbar: box('.mobile-nav'), banner: box('.emulator-page-note'), table: box('.comparison-scroll'), header: box('.friend-matrix thead'), firstGame: box('.friend-matrix tbody th button'), firstRow: box('.friend-matrix tbody th'),
          people: document.querySelector('.compare-chosen-people')!.textContent, rows: [...document.querySelectorAll('.friend-matrix tbody tr')].map(row => row.textContent),
      ...
      expect(before.width).toBe(viewport.width); expect(before.coarse).toBe(touch);
      ...
      if (touch) {
        ...
        try {
          ...
          for (const distance of [20, 40, 60, 80]) await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x: x - distance, y }] });
          await cdp.send('Input.dispatchTouchEvent', { type: 'touchEnd', touchPoints: [] });
          await expect.poll(() => region.evaluate(element => element.scrollLeft)).toBeGreaterThan(prior);
          await settleScroll(region);
        } finally { await cdp.detach(); }
      }
      await region.evaluate(element => { element.scrollLeft = 520; element.scrollTop = 250; });
      const scrolled = await page.evaluate(() => {
        ...
        return { region: region.getBoundingClientRect().toJSON(), corner: corner.getBoundingClientRect().toJSON(), header: header.getBoundingClientRect().toJSON(), row: row.getBoundingClientRect().toJSON(), scrollTop: region.scrollTop, scrollLeft: region.scrollLeft };
      });
      ...
      await page.evaluate(({ uid, name }) => window.compareOrientationProbe.rename(uid, name), { uid: peer.uid, name: longName });
