...
interface InviteTiming {
  ...
  dialog: number | null;
  link: number | null;
  calls: Array<{ method: string; elapsed: number }>;
}
declare global {
  interface Window {
...
test('measures first and warm invitation feedback and confirmed links with a 300ms request delay', async ({
  ...
  await page.evaluate(async () => {
    ...
    const tracked = async <T>(method: string, operation: () => Promise<T>) => {
      ...
      const timing = window.inviteTiming;
      try {
        return await operation();
      } finally {
        if (timing) timing.calls.push({ method, elapsed: performance.now() - start });
      }
    };
    source.FriendStore.prototype.initialize = function (uid) {
      return tracked('initialize', () => originalInitialize.call(this, uid));
    };
    source.FriendStore.prototype.identity = function (uid) {
      return tracked('identity', () => originalIdentity.call(this, uid));
    };
    source.FriendStore.prototype.settings = function (uid) {
      return tracked('settings', () => originalSettings.call(this, uid));
    };
    source.FriendStore.prototype.createInvite = function (uid) {
      return tracked('createInvite', () => originalCreate.call(this, uid));
    };
  });
  ...
  page.on('request', (request) => {
    ...
    const kind = url.pathname.includes(':batchGet')
      ? 'transaction-read'
  ...
  page.on('response', (response) => {
    ...
    if (body?.includes('/documents/friendInvites/') && body.includes('"active"'))
      mutation.acknowledged = Date.now() - started;
  });
  for (const state of ['first', 'warm'] as const) {
    requests = {};
    mutation.acknowledged = null;
    await page.evaluate(() => {
      ...
      const button = [...document.querySelectorAll<HTMLButtonElement>('button')].find((button) =>
        button.textContent?.includes('Invite someone'),
      );
      ...
      const observer = new MutationObserver(() => {
        ...
        if (timing.dialog === null && document.querySelector('#invite-link-title'))
          timing.dialog = performance.now() - timing.start;
        if (document.querySelector('#friend-invite-link')) {
          timing.link = performance.now() - timing.start;
          observer.disconnect();
        }
      });
    ...
    results.push({
      state,
      injectedDelayPerFirestoreHttpRequest: 300,
      feedbackMs: Math.round(timing!.disabled!),
      creatingDialogMs: Math.round(timing!.dialog!),
      linkMs: Math.round(timing!.link!),
      mutationAckMs: mutation.acknowledged,
      requests: { ...requests },
      calls: timing!.calls.map((call) => ({ method: call.method, elapsedMs: Math.round(call.elapsed) })),
    });
    measuring = false;
