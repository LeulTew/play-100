...
test('fresh mounted account defaults All without a sharing click or guest adoption, then follows new saved games and Stop', async ({
  page,
  request,
}, info) => {
  const errors: string[] = [];
  ...
  await expect(
    card.getByRole('button', { name: 'In My games: Kingdom Come: Deliverance', exact: true }),
  ).toBeDisabled();
  await page.goto('/my-games');
  await expect(page.locator('.friend-sharing-summary')).toContainText('Up to date', { timeout: 30000 });
  expect(await sdk(page, 'heads')).toMatchObject({
    games: { status: 'ready', count: 1 },
    ranking: { status: 'ready', count: 0 },
  });
...
test('legacy off stays off until one inline Share all action and old-client Stop stays authoritative', async ({
  page,
  request,
}) => {
  ...
  await expect(page.locator('.friend-sharing-summary')).toContainText('Up to date', { timeout: 30000 });
  expect(await sdk(page, 'controls')).toMatchObject({
    policy: { enabled: true, origin: 'explicit' },
...
test('quota progress survives reload across both scopes without claiming all sharing is Saved', async ({
  page,
  request,
}) => {
  ...
  await page.evaluate(async (uid) => {
    ...
    module.FriendAllStore.prototype.publish = function (owner, kind, input, policy, source, isCurrent, progress) {
      return original.call(this, owner, kind, input, policy, source, isCurrent, (value) => {
        ...
        if (kind === 'ranking' && fail && value.applied === 50 && !value.ready) {
          fail = false;
          throw Object.assign(new Error('Synthetic quota after50 confirmed changes'), { code: 'resource-exhausted' });
        }
      });
    };
    ...
    await source.commitScopedAction(`account:demo-play100:${uid}`, {
      ...
      records: Array.from({ length: 52 }, (_, index) => ({
        id: `manual:quota-${index + 1}`,
        title: `Quota fixture ${index + 1}`,
        source: 'manual' as const,
        sourceId: `quota-${index + 1}`,
        sourceUrl: null,
        year: null,
        studio: null,
        genre: null,
  ...
  expect(await sdk(page, 'heads')).toMatchObject({
    games: { status: 'ready', count: 52 },
    ranking: { status: 'ready', count: 52 },
  });
  expect((await readAccount(page, uid)).state.ranking).toHaveLength(52);
});

test('friends read paginated All data, compare exact tray games, receive score updates and revoke an unsaved preview on old-client Stop', async ({
  page,
  request,
  browser,
  viewport,
  ...
  await verifyEmail(page, request, ownerEmail);
  await enableSync(page, 'empty');
  ...
  await page.evaluate(async (uid) => {
    ...
    await source.commitScopedAction(scope, {
      type: 'edit-ranking',
      id: records[0]!.id,
      score: 8.2,
      note: 'Owner-only note must never be shared.',
    });
  }, ownerUid);
  await expect(page.locator('.friend-sharing-summary')).toContainText('Up to date', { timeout: 30000 });
  ...
  try {
    ...
    await createAccount(friend, friendEmail);
    await verifyEmail(friend, request, friendEmail);
    await enableSync(friend, 'empty');
    ...
    await friend.evaluate(async () => {
      ...
      const page = module.FriendAllStore.prototype.page;
      const full = legacy.FriendStore.prototype.ranking;
      module.FriendAllStore.prototype.exact = async function (uid, kind, ids) {
        counts.exact += 1;
        counts.maxExact = Math.max(counts.maxExact, ids.length);
        return exact.call(this, uid, kind, ids);
      };
      module.FriendAllStore.prototype.page = async function (uid, kind, cursor, revision) {
        counts.pages += 1;
        const result = await page.call(this, uid, kind, cursor, revision);
        counts.maxPage = Math.max(counts.maxPage, result.entries.length);
        return result;
      };
      legacy.FriendStore.prototype.ranking = async function (uid) {
        counts.legacyFull += 1;
    ...
    expect(counts?.maxExact).toBe(1);
    expect(counts?.legacyFull).toBe(0);
    ...
    await friend.goto(`/friends/${ownerUid}`);
    await friend
      .locator('.friend-shelf-grid > li')
      .filter({ hasText: 'All fixture 002' })
      .getByRole('button', { name: 'All fixture 002', exact: true })
      .click();
...
test('an old private write is denied atomically while durable local edits survive and the refreshed current client resumes', async ({
  page,
  request,
}) => {
  ...
  await page.route('**/*:commit?*', async (route) => {
    ...
    if (!value || typeof value !== 'object' || !('writes' in value) || !Array.isArray(value.writes)) {
    ...
    const documentName = (write: unknown) => {
      if (
        ...
        !('update' in write) ||
        !write.update ||
        typeof write.update !== 'object' ||
        !('name' in write.update)
      )
        return '';
      return typeof write.update.name === 'string' ? write.update.name : '';
    };
    if (
      value.writes.some((write) => documentName(write).includes('/syncHeads/')) &&
      value.writes.some((write) => documentName(write).includes('/friendAllHeads/'))
    ) {
      stripped += 1;
      await route.continue({
        postData: JSON.stringify({
          ...value,
          writes: value.writes.filter((write) => !documentName(write).includes('/friendAllHeads/')),
        }),
      });
  ...
  await page.evaluate(async (uid) => {
    ...
    await source.commitScopedAction(`account:demo-play100:${uid}`, {
      type: 'rate-game',
      record: {
        id: 'manual:pending-old',
        title: 'Durable old-writer edit',
        year: null,
        source: 'manual',
        sourceId: 'pending-old',
        sourceUrl: null,
        genre: null,
        studio: null,
        collectionRank: null,
...
test('All export and reversible then full deletion retain the device copy and require fresh Auth before re-enabling', async ({
  page,
  request,
}) => {
  test.setTimeout(120000);
  ...
  await page.evaluate(async (uid) => {
    ...
    await source.commitScopedAction(`account:demo-play100:${uid}`, {
      type: 'rate-game',
      record: {
        id: 'manual:all-delete',
        title: 'Retained account copy',
        year: null,
        source: 'manual',
        sourceId: 'all-delete',
        sourceUrl: null,
  ...
  expect(
    exported.friends.automaticSharing.views.every(
      (view: { head: { count: number; status: string } }) => view.head.count === 1 && view.head.status === 'ready',
    ),
  ).toBe(true);
  await page.evaluate(async () => {
    ...
    window.allDeleteCalls = counts;
    friends.FriendStore.prototype.saveSettings = function (...args) {
      counts.ranking += 1;
      return rankSave.apply(this, args);
    };
    shelf.FriendShelfStore.prototype.saveConfig = function (...args) {
      counts.shelf += 1;
      return shelfSave.apply(this, args);
    };
  });
...
test('interrupted online-copy cleanup resumes after private deletion without dropping the retained account copy', async ({
  page,
  request,
}) => {
  ...
  await page.evaluate(async (uid) => {
    ...
    await source.commitScopedAction(`account:demo-play100:${uid}`, {
      ...
      record: {
        id: 'manual:cleanup',
        title: 'Cleanup recovery fixture',
        year: null,
        source: 'manual',
        sourceId: 'cleanup',
        sourceUrl: null,
  ...
  await page.evaluate(async () => {
    ...
    all.FriendAllStore.prototype.cleanupPage = function (...args) {
      if (fail) {
        fail = false;
        return Promise.reject(new Error('Synthetic resumable All cleanup interruption'));
      }
