...
for (const policy of ['candidate', 'live-270f'] as const)
  test.describe(`cancelled recovery with ${policy} rules`, () => {
    ...
    test('a verified cancelled registration offers removal without bootstrapping content', async ({
      page,
      context,
      request,
      baseURL,
      ...
      if (!baseURL || !['localhost', '127.0.0.1'].includes(new URL(baseURL).hostname))
        throw new Error('Use only the owned emulator-bound app.');
      ...
      if (
        ...
        typeof actor !== 'object' ||
        !('localId' in actor) ||
        typeof actor.localId !== 'string' ||
        !('idToken' in actor) ||
        typeof actor.idToken !== 'string'
      )
        throw new Error('The local cancellation actor was not created.');
      ...
      try {
        const cancelled = await request.patch(document, {
          ...
          data: { fields: { state: { stringValue: 'cancelled' } } },
        });
