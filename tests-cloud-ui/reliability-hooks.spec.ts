...
declare global {
  interface Window {
    syncReliability: {
      ...
      inspect: () => { sameLifetime: boolean; oldCurrent: boolean; restores: number; publishes: number; error: string; stateRevision: number };
    };
    sharingReliability: {
...
async function mountSync(page: Page) {
  ...
  await page.evaluate(async () => {
    ...
    const restore = (_store: import('../src/cloud/cloud-store').CloudStore, current: () => boolean) => {
      ...
      const done = pending.then(() => { if (current()) publishes += 1; });
    ...
    function Harness() {
      const [config, setConfig] = React.useState({
        ...
        snapshot: initial, verified: true, initialProbe: true, authGeneration: 1,
      });
      const api = hook.useCloudSync(config.scope, config.snapshot, config.verified, config.initialProbe ? restore : undefined, config.authGeneration);
      window.syncReliability = {
        capture: () => { captured = api.reportProfileError; },
        unmount: () => root.unmount(),
        change: key => setConfig(old => {
          switch (key) {
            case 'scope': return { ...old, scope: null };
            case 'enabled': return { ...old, snapshot: { ...old.snapshot, sync: { ...old.snapshot.sync, enabled: true } } };
            case 'epoch': return { ...old, snapshot: { ...old.snapshot, sync: { ...old.snapshot.sync, epoch: 1 } } };
            case 'verified': return { ...old, verified: false };
            case 'initialProbe': return { ...old, initialProbe: false };
            case 'authGeneration': return { ...old, authGeneration: old.authGeneration + 1 };
            default: return { ...old, snapshot: { ...old.snapshot, state: { ...old.snapshot.state, revision: old.snapshot.state.revision + 1 } } };
          }
        }),
        finishOld: async () => {
        ...
        inspect: () => ({
          sameLifetime: captured === api.reportProfileError, oldCurrent: restores[0]?.current() ?? false,
          restores: restores.length, publishes, error: api.error, stateRevision: config.snapshot.state.revision,
        }),
      };
...
test('sync lifetime retains ordinary edits and invalidates delayed work at every ownership or consent boundary', async ({ page }) => {
  ...
  for (const key of ['scope', 'enabled', 'epoch', 'verified', 'initialProbe', 'authGeneration'] as const) {
    await test.step(key, async () => {
      ...
      await page.evaluate(() => { window.syncReliability.capture(); window.syncReliability.change(); });
      await expect.poll(() => page.evaluate(() => window.syncReliability.inspect())).toMatchObject({ sameLifetime: true, oldCurrent: true, restores: 1, stateRevision: before + 1 });
      await page.evaluate(changed => window.syncReliability.change(changed), key);
      await expect.poll(() => page.evaluate(() => window.syncReliability.inspect())).toMatchObject({ sameLifetime: false, oldCurrent: false });
  ...
  await mountSync(page);
  await page.evaluate(() => { window.syncReliability.capture(); window.syncReliability.unmount(); });
...
async function mountSharing(page: Page) {
  ...
  await page.evaluate(async () => {
    ...
    const record: import('../src/lib/personal-types').LibraryRecord = {
      id: 'manual:reliability', source: 'manual', sourceId: 'reliability', sourceUrl: null,
      title: 'Selection fixture', year: null, collectionRank: null, studio: null, genre: null,
    };
    ...
    const settings: import('../src/lib/friend-types').FriendSettings = {
      format: 1, enabled: false, deleted: false, selectedIds: [record.id], epoch: 1, revision: 1, updatedAt: 1,
    };
    ...
    function Harness() {
      ...
      window.sharingReliability = {
        snapshot: (present, unrelatedRevision = false) => {
          if (unrelatedRevision) current = { ...current, state: { ...current.state, revision: current.state.revision + 1 } };
        ...
        inspect: async () => {
          ...
          return { writes, cache: value ?? null, removed: [...await cache.pendingFriendRemovals(scope)], error: api.error };
        },
      };
...
test('sharing cache initializes current settings when a snapshot arrives without tracking unrelated state revisions', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.sharingReliability.inspect())).toMatchObject({ writes: 1, cache: { revision: 2, selected: ['manual:latest'] } });
  await page.evaluate(() => window.sharingReliability.snapshot(true, true));
  expect(await page.evaluate(() => window.sharingReliability.inspect())).toMatchObject({ writes: 1, cache: { revision: 2 } });
  ...
  await page.evaluate(() => window.sharingReliability.snapshot(true));
  await expect.poll(() => page.evaluate(() => window.sharingReliability.inspect())).toMatchObject({ writes: 2, cache: { revision: 2 }, error: '' });
});

test('sharing settings revision and selected IDs refresh without clearing private removal markers', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.sharingReliability.inspect())).toMatchObject({ writes: 2, removed: ['manual:reliability'] });
  await page.evaluate(() => window.sharingReliability.settings(2, ['manual:reliability']));
  await expect.poll(() => page.evaluate(() => window.sharingReliability.inspect())).toMatchObject({ writes: 4, cache: { revision: 2 }, removed: ['manual:reliability'] });
  await page.evaluate(() => window.sharingReliability.settings(2, ['manual:reliability', 'manual:second']));
  await expect.poll(() => page.evaluate(() => window.sharingReliability.inspect())).toMatchObject({
    writes: 6, cache: { revision: 2, selected: ['manual:reliability', 'manual:second'] }, removed: ['manual:reliability'], error: '',
  });
});
