...
test('an invitation resumes after email sign-in, sharing stays opt-in, selected scores compare, groups persist, and unfriend revokes the open comparison', async ({
  page,
  browser,
  request,
  viewport,
  ...
  await seedGuestRating(page, '8.8');
  await createAccount(page, senderEmail);
  await verifyEmail(page, request, senderEmail);
  ...
  try {
    ...
    await other.goto('/?game=red-dead-redemption-2');
    await seedGuestRating(other, '6.8');
    await createAccount(other, receiverEmail);
    await verifyEmail(other, request, receiverEmail);
    ...
    expect(
      await receiving.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth),
    ).toBe(true);
    await receiving.evaluate(() => {
      if (document.activeElement instanceof HTMLElement) document.activeElement.blur();
      window.scrollTo({ top: 0, behavior: 'instant' });
    });
...
test('revoked invites show no inviter snapshot and a cancelled sharing preview does not enable sharing', async ({
  page,
  browser,
  request,
}) => {
