...
export default defineConfig(({ mode }) => {
  ...
  if (mode !== 'cloud-test' && (online.error || (environment.VITE_FIREBASE_REQUIRED === 'true' && !online.config))) {
    throw new Error(
      online.error ??
        'This release requires a complete public Firebase configuration. Build stopped before publication.',
    );
  }
  ...
  if (appCheck.config) {
    ...
    if (problems.length)
      throw new Error(`App Check is enabled but vercel.json CSP is not ready: ${problems.join(' ')}`);
  }
  return {
    define: {
    ...
    plugins: [
      ...
      clientEnvGuard(),
      {
        ...
        configureServer(server) {
          server.middlewares.use('/api/catalog-detail', (request, response) => {
            void catalogDetailHandler(request, response);
          });
          server.middlewares.use('/api/catalog', (request, response) => {
            void catalogHandler(request, response);
          });
        },
