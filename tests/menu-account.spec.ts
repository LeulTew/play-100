...
test('Menu keeps guest, Alpha and Beta drafts in their own scopes through real emulator sign-in and sign-out', async ({ page, context, baseURL }) => {
  test.setTimeout(90_000);
  test.skip(!alpha.email || !alpha.uid || !beta.email || !beta.uid, 'Requires two dedicated Menu emulator actors; run this file with one worker.');
  ...
  const emulator = await page.evaluate(async () => {
    ...
    const source: { EMULATOR_MODE: boolean; firebaseConfiguration: () => { projectId: string } | null } = await import(path);
  ...
  await expect(rating(page)).toHaveValue(String(beforeAlpha.state.ranking.find(entry => entry.id === game.id)?.score));
