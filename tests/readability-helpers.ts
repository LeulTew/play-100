...
export async function expectReadableSurface(page: Page, surface: string, checkClipping = false) {
  ...
  const report = await page.evaluate(({ checkClipping }) => {
    const label = (element: Element) => {
      const parts: string[] = [];
      for (let current: Element | null = element; current && parts.length < 4; current = current.parentElement) {
        if (current.id) { parts.unshift(`#${CSS.escape(current.id)}`); break; }
        parts.unshift(current.tagName.toLowerCase() + Array.from(current.classList).map(name => `.${CSS.escape(name)}`).join(''));
      }
      return parts.join(' > ');
    };
    const visible = (element: Element) => {
      if (element.closest('script, style, noscript, template, [hidden], [inert], dialog:not([open]), .sr-only')) return false;
    ...
    while (walker.nextNode()) {
      ...
      for (let ancestor: Element | null = element; ancestor && ancestor !== root.parentElement; ancestor = ancestor.parentElement) {
        ...
        if (rects.some(rect => clipsX && (rect.left < bounds.left - 1 || rect.right > bounds.right + 1) ||
          ...
          clipped.push({ selector: label(element), text: node.textContent.trim().slice(0, 140), container: label(ancestor) });
          break;
        }
      }
    }
    for (const element of root.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>('input, select, textarea')) {
      if (visible(element)) checkSize(element, element.value || element.getAttribute('placeholder') || element.getAttribute('aria-label') || element.tagName);
    }
    const targets: { selector: string; text: string; width: number; height: number; minimum: number }[] = [];
    for (const element of root.querySelectorAll<HTMLElement>('button, a[href], input:not([type="hidden"]), select, textarea, summary')) {
      ...
      const target = element instanceof HTMLInputElement && ['radio', 'checkbox'].includes(element.type)
        ? Array.from(element.labels ?? []).find(visible) ?? element : element;
      const bounds = target.getBoundingClientRect();
      const minimum = element.matches('.icon-button, .button, .mobile-nav > *') ? 44 : 24;
      if (bounds.width < minimum - .5 || bounds.height < minimum - .5) {
        targets.push({ selector: label(element), text: element.getAttribute('aria-label') ?? element.textContent?.trim().slice(0, 100) ?? '', width: bounds.width, height: bounds.height, minimum });
      }
    }
    ...
    const horizontalScroll = [root, ...root.querySelectorAll('*')].filter(element => {
      if (!visible(element) || element.closest('.ratings-scroll') || element.matches('input, select, textarea')) return false;
      return ['auto', 'scroll'].includes(getComputedStyle(element).overflowX) && element.scrollWidth > element.clientWidth + 1;
    }).map(label);
    return { checkedTextNodes, small, clipped, targets, horizontalScroll, documentWidth: document.documentElement.scrollWidth, viewportWidth: innerWidth, overflow };
  }, { checkClipping });
  ...
  expect.soft(report.documentWidth, `${surface}: document overflow ${JSON.stringify(report.overflow)}`).toBeLessThanOrEqual(report.viewportWidth);
  expect.soft(report.horizontalScroll, `${surface}: non-table horizontal scroll regions`).toEqual([]);
  expect.soft(report.targets, `${surface}: undersized effective targets\n${JSON.stringify(report.targets, null, 2)}`).toEqual([]);
  if (checkClipping) expect.soft(report.clipped, `${surface}: clipped text\n${JSON.stringify(report.clipped, null, 2)}`).toEqual([]);
}
