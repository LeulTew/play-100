...
test('bulk Discover and main100 actions keep canonical IDs for MassEffect2 and GTAIV without false completion', async ({ page }) => {
  ...
  await page.getByRole('region', { name: 'Bulk game actions' }).getByRole('button', { name: 'Add to my ranking', exact: true }).click();
  await expect.poll(async () => (await readLibrary(page)).ranking.map(entry => entry.id)).toEqual(['grand-theft-auto-iv']);
  ...
  await page.getByRole('button', { name: 'Mark played', exact: true }).click();
  await expect.poll(async () => (await readLibrary(page)).progress['grand-theft-auto-iv']).toEqual({ played: true, completed: false, later: false });
...
test('a legacy-only saved copy stays Saved and owns every implicit create path, including canonical detail and bulk', async ({ page }, info) => {
  ...
  await expect(played).toBeFocused(); expect(await readLibrary(page)).toEqual(beforeConfirmation);
  await played.click(); await page.keyboard.press('Escape');
  await expect(confirmation).toHaveCount(0); await expect(detailFor(page)).toBeVisible();
  await expect(played).toBeFocused(); expect(await readLibrary(page)).toEqual(beforeConfirmation);
  await played.click(); await confirmation.getByRole('button', { name: 'Mark not played', exact: true }).click();
  await expect.poll(async () => (await readLibrary(page)).progress[provider.id]).toEqual({ played: false, completed: false, later: true });
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('play100:compare-tray:v1:guest') ?? '{}').items.map((item: LibraryRecord) => item.id))).toEqual([provider.id]);
  await page.goto('/my-games?tab=ranking');
  await expect(page.locator('.my-games-editor:visible [data-record-id]')).toHaveCount(1);
  await expect(page.locator('.my-games-editor:visible [data-record-id]')).toHaveAttribute('data-record-id', provider.id);
...
test('both owned copies keep conflicting opinions and manual names remain separate', async ({ page }) => {
  const manual: LibraryRecord = { ...provider, id: 'manual:own-version', source: 'manual', sourceId: 'own-version', sourceUrl: null };
  let state = applyPersonalAction(emptyPersonalLibrary(), { type: 'add-ranking', records: [canonical, provider, manual] });
  state = applyPersonalAction(state, { type: 'edit-ranking', id: canonical.id, score: 9.1, note: 'Original-ID opinion.' });
  ...
  const actual = await readLibrary(page);
  expect(actual.records).toEqual(state.records);
  expect(actual.ranking.find(entry => entry.id === provider.id)).toEqual(state.ranking.find(entry => entry.id === provider.id));
...
test('provider echoes resolve before filtering and cannot reappear from another local page', async ({ page }) => {
  ...
  const outside: LibraryRecord = { ...provider, id: 'wikidata:Q90000001', sourceId: 'Q90000001', sourceUrl: null, title: 'Bounded provider-only fixture' };
...
test('late canonical data gates duplicate actions, error is recoverable, and all100 remain available without the seed', async ({ page }) => {
  let release = () => {}; const waiting = new Promise<void>(resolve => { release = resolve; });
  ...
  const nextIds = await page.locator('[data-catalog-id]').evaluateAll(cards => cards.map(card => card.getAttribute('data-catalog-id')));
...
test('verified unsaved legacy links open canonical details while genuinely different remake identities stay separate', async ({ page }, info) => {
...
test('a superseded provider response cannot restore an old query or duplicate a newly resolved canonical result', async ({ page }) => {
  await page.unroute('**/api/catalog?**');
  ...
  await page.route('**/api/catalog?**', async route => {
    ...
    const source = params.get('source') ?? 'wikidata'; const query = params.get('q') ?? '';
    if (source === 'wikidata' && query === 'RDR2') { oldRequested = true; await waiting; }
    const record = query === 'RDR2' ? provider : seed.items.find(item => item.record.id === 'wikidata:Q725057')!.record;
