...
test('local page 1, 2 and last preserve identity, view, history and truthful 24-item bearings', async ({ page }) => {
  ...
  page.on('request', (request) => {
    if (request.url().includes('/api/catalog?')) queries.push(request.url());
  });
  await page.goto('/discover?include100=on');
  await expect(pager(page)).toContainText('1–24 of 845 catalog games');
  await expect(cards(page)).toHaveCount(24);
  const firstIds = await cards(page).evaluateAll((elements) =>
    elements.map((element) => element.getAttribute('data-catalog-id')),
  );
  ...
  const secondIds = await cards(page).evaluateAll((elements) =>
    elements.map((element) => element.getAttribute('data-catalog-id')),
  );
  ...
  await expect(cards(page)).toHaveCount(24);
  expect(
    await cards(page).evaluateAll((elements) => elements.map((element) => element.getAttribute('data-catalog-id'))),
  ).toEqual(secondIds);
...
test('filters reset and bound local pages without stealing focus while typing or changing selection scope', async ({
  page,
}) => {
...
test('a cold late seed does not clamp a valid last-page URL to the temporary canonical-only count', async ({
  page,
}) => {
  let release = () => {};
  ...
  await page.route('**/data/discovery/catalog.v1.json', async (route) => {
    await waiting;
    await route.fulfill({ json: rawSeed });
  });
...
test('provider offsets stay separate from known-local pages and local navigation never requests provider offsets', async ({
  page,
}) => {
  ...
  await page.route('**/api/catalog?**', (route) => {
    ...
    return route.fulfill({
      json: {
        ...
        items: [],
        total: 11,
        nextOffset: offset === 5 ? 10 : null,
        notices: ['Bounded provider fixture; provider count is separate from local coverage.'],
      },
    });
  });
...
test('invalid and failed ratings stay with their original card; successful paging waits for persistence', async ({
  page,
}) => {
  ...
  await page.evaluate(() => {
    ...
    IDBObjectStore.prototype.put = function (...args) {
      if (this.name === 'library' && window.browsingDenyWrites)
        throw new DOMException('Synthetic public-browsing write failure.', 'QuotaExceededError');
  ...
  await page.evaluate(() => {
    window.browsingDenyWrites = false;
  });
...
test('a pending page intent cannot replace a newer Back navigation after its delayed flush', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    window.releaseBrowsingNavigation?.();
    await window.removeBrowsingEditor?.();
  });
  await expect(pager(page).getByRole('combobox')).toHaveValue('1');
  await expect(page).not.toHaveURL(/offset=/);
});

test('short targets and horizontal table scrolling retain a compact visible original identity', async ({
  page,
  isMobile,
}) => {
  ...
  const targets = await table.locator('.table-sort').evaluateAll((elements) =>
  ...
  await expect(table.getByRole('link', { name: 'Red Dead Redemption 2', exact: true })).toHaveAttribute(
    'href',
...
test('the tray preserves complete art provenance behind a labelled disclosure and names friends rankings', async ({
  page,
}) => {
...
test('focused mobile recovery clears the fixed bar and retries through ordinary input', async ({ page, isMobile }) => {
  ...
  await page.route('**/data/collection.json', (route) =>
    route.fulfill({ status: 503, body: 'Synthetic public collection failure' }),
  );
  await page.goto('/');
  ...
  const focus = await retry.evaluate((element) => {
    ...
    return {
      ...
      bottom: rect.bottom,
      navTop: nav.top,
      centerHitsButton: element.contains(document.elementFromPoint(rect.x + rect.width / 2, rect.y + rect.height / 2)),
    };
  });
...
test('a pinned tray leaves the first explored game identity unobscured at 320px without changing target sizes or saved data', async ({
  page,
  isMobile,
}, info) => {
    ...
    page
      ...
      .evaluate((element) => {
        ...
        const hits = [0.15, 0.5, 0.85].map((fraction) => {
          ...
          return {
            intendedGame: Boolean(hit && (element.contains(hit) || link?.contains(hit))),
            tag: hit?.tagName,
        ...
        for (const obstacle of document.querySelectorAll('.site-header, .compare-tray-dock, .mobile-nav')) {
          ...
          visible = visible
            .flatMap((rect) => {
              const top = Math.max(rect.top, cover.top),
                bottom = Math.min(rect.bottom, cover.bottom);
              const left = Math.max(rect.left, cover.left),
                right = Math.min(rect.right, cover.right);
              if (top >= bottom || left >= right) return [rect];
              return [
                { ...rect, bottom: top },
                { ...rect, top: bottom },
                { top, bottom, left: rect.left, right: left },
                { top, bottom, left: right, right: rect.right },
              ];
            })
  ...
  expect(empty.coarse).toBe(true);
  expect(empty.touch).toBeGreaterThan(0);
  ...
  await writeFile(
    info.outputPath('pinned-explore-geometry.json'),
    JSON.stringify({ empty, pinned, targetsBefore, pinnedGame: pinnedGame.id }, null, 2),
  );
...
test('batched desktop and mobile pixels keep games before secondary filters and bound native chevrons', async ({
  browser,
  isMobile,
  baseURL,
}, info) => {
  ...
  for (const viewport of [
    ...
    try {
      ...
      await openBrowsingFilters(page);
      const controls = await page.locator('.filter-select').evaluateAll((elements) =>
        elements.map((element) => {
          ...
          return {
            ...
            contained: arrow.left >= select.left && arrow.right <= select.right,
            centered: Math.abs(select.y + select.height / 2 - arrow.y - arrow.height / 2),
          };
        }),
      );
