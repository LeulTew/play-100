...
test.beforeEach(async ({ page, baseURL }) => {
  if (!baseURL || !['127.0.0.1', 'localhost'].includes(new URL(baseURL).hostname))
    throw new Error('Use an owned loopback server for private source fixtures.');
...
test('private Compare bindings fail closed without a gate and preserve exact title and nested-control actions', async ({
  page,
}) => {
  ...
  await played.click();
  const markedPlayed = applyPersonalAction(before, {
    type: 'set-progress',
    records: [record],
    key: 'played',
    value: true,
  ...
  const completedPlayed = workspace
    .locator(`ul.personal-records > [data-record-id="${completedRecord.id}"]`)
  ...
  const clearedPlayed = applyPersonalAction(markedPlayed, {
    type: 'set-progress',
