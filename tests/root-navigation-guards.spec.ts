...
const rankedList = (page: Page) =>
  page.getByRole('list', { name: 'Your ranked games', exact: true, includeHidden: true });
const rating = (page: Page) =>
  rankedList(page).getByRole('spinbutton', { name: `Your rating / 10 for ${first.title}`, exact: true });
const primary = (page: Page, mobile: boolean) =>
  page.getByRole('navigation', { name: mobile ? 'Mobile navigation' : 'Main navigation', exact: true });
...
async function releaseSave(page: Page) {
  await page.evaluate(() => {
    if (document.documentElement.dataset.rootSavePhase !== 'held')
      throw new Error('Release requires an observed held save.');
...
async function rejectWrites(page: Page) {
  await page.evaluate(() => {
    ...
    IDBObjectStore.prototype.put = function (...args: Parameters<IDBObjectStore['put']>) {
      if (this.transaction.db.name === 'play100-personal' && this.name === 'library' && args[1] === 'state') {
        document.documentElement.dataset.rootRejectedWrites = String(
          Number(document.documentElement.dataset.rootRejectedWrites) + 1,
        );
...
test.beforeEach(async ({ context, page, baseURL }) => {
  ...
  test.skip(
    (await document.text()).includes('site-header-online'),
  ...
  await context.route('**/*', (route) => {
    ...
    if (url.pathname === '/api/catalog-detail')
      return route.fulfill({ status: 503, json: { error: 'Synthetic detail provider failure.' } });
      ...
      new Promise<void>((resolve, reject) => {
        ...
        request.onsuccess = () => {
          ...
          transaction.oncomplete = function (event) {
            ...
            root.dataset.rootProbeReceiver =
              this === transaction && event.target === transaction && event.type === 'complete'
                ? 'original'
                : 'unexpected';
            database.close();
          };
...
test('primary links keep an invalid rating and wait for one real note save before leaving', async ({
  page,
  isMobile,
}) => {
  await prepareRanking(page);
  ...
  await expect(primary(page, isMobile).getByRole('link', { name: 'Discover', exact: true })).toHaveAttribute(
    'aria-current',
    'page',
  );
  await expectOneWrite(page, before.revision);
  const after = await readLibrary(page);
  expect(after.ranking.find((entry) => entry.id === first.id)?.note).toBe(
    'Primary navigation must await this original record.',
  );
...
test('primary modified and nonprimary clicks retain native defaults and the current draft', async ({
  page,
  context,
  isMobile,
}) => {
...
for (const roundtrip of [false, true]) {
  test(`a held primary-link save cannot override native ${roundtrip ? 'Back/Forward A-B-A' : 'Back'}`, async ({
    page,
    isMobile,
  }) => {
    await prepareRanking(page);
    ...
    expect((await readLibrary(page)).ranking.find((entry) => entry.id === first.id)?.note).toBe(
      'Persist once, but do not resurrect the old route intent.',
    );
    await primary(page, isMobile).getByRole('link', { name: 'Discover', exact: true }).click();
...
test('same-page mobile Browse scrolls and focuses the collection without changing history or data', async ({
  page,
}) => {
  ...
  await expect
    .poll(() => page.evaluate(() => document.getElementById('collection')?.contains(document.activeElement)))
    .toBe(true);
...
test('the My games host exit waits for one save, and Menu Settings still opens and closes natively', async ({
  page,
}) => {
  await prepareRanking(page);
  const before = await readLibrary(page);
  ...
  await expectOneWrite(page, before.revision);
  expect((await readLibrary(page)).ranking.find((entry) => entry.id === first.id)?.note).toBe(
    'The host exit keeps this record and saves only once.',
  );
...
test('public enable blocks invalid edits, then awaits persistence and preserves detail/query/history', async ({
  ...
  page.on('request', (request) => {
    if (new URL(request.url()).pathname === '/api/catalog-detail') requests.push(new URL(request.url()));
  });
  const dialog = await publicDetail(page);
...
for (const roundtrip of [false, true]) {
  test(`public-enable consent cannot survive native ${roundtrip ? 'Back/Forward A-B-A' : 'Back'} during its save`, async ({
    page,
  }) => {
    const requests: string[] = [];
    page.on('request', (request) => {
      if (new URL(request.url()).pathname === '/api/catalog-detail') requests.push(request.url());
    });
    const dialog = await publicDetail(page);
    const original = page.url();
    const before = await readLibrary(page);
