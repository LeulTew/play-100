...
for (const view of ['grid', 'list'] as const) {
  test(`${view}: scrolled native Menu and detail preserve background geometry and exact focus return`, async ({
    page,
    isMobile,
  }) => {
...
test('offscreen content remains findable, focusable and printable without containing modal or motion hosts', async ({
  page,
}) => {
  ...
  await page
    .getByRole('button', { name: /^\S.* \d+:[0-5]\d · Watch film$/ })
    .first()
    .click();
  const film = page.locator('.film-dialog');
  await expect(film).toBeVisible();
  expect(await film.evaluate((node) => getComputedStyle(node.closest('.collection-films')!).contentVisibility)).toBe(
    'visible',
  );
  await page.keyboard.press('Escape');
