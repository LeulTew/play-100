...
const provider = catalog.items.find(item => item.artwork && item.artwork.credit.length > 200 && canonicalCatalogId(item.record.id) === item.record.id);
if (!provider) throw new Error('Readability coverage requires the existing long-credit, non-canonical catalog fixture.');
...
async function surfaces(page: Page, spacing = false) {
  ...
  for (const name of ['Queue', 'Library', 'Ranking']) {
    await page.getByRole('navigation', { name: 'My games views' }).getByRole('button', { name: new RegExp(`^${name}`) }).click();
    await expect(page.getByRole('navigation', { name: 'My games views' }).getByRole('button', { name: new RegExp(`^${name}`) })).toHaveAttribute('aria-pressed', 'true');
  ...
  if (await page.evaluate(() => matchMedia('(forced-colors: active)').matches)) {
    const borders = await page.locator('.compare-tray-dock, .compare-tray-action, .search-field, .game-cover').evaluateAll(elements =>
      elements.map(element => ({ selector: element.className, style: getComputedStyle(element).borderTopStyle, width: parseFloat(getComputedStyle(element).borderTopWidth) })));
    expect(borders.every(border => border.style === 'solid' && border.width >= 1), JSON.stringify(borders)).toBe(true);
    expect(await page.locator('.compare-tray-action svg').evaluate(element => getComputedStyle(element).stroke)).not.toBe('none');
  }
  ...
  await closeDialog(page);
  await openMenu(page);
  await page.getByRole('dialog', { name: 'Menu', exact: true }).getByRole('button', { name: 'About & credits', exact: true }).click();
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/catalog?**', route => route.fulfill({ status: 503, json: { error: 'Readability offline-provider fixture.' } }));
});
...
test('forced colors preserves native focus, selected navigation and comparison controls', async ({ page }) => {
  ...
  const appearance = await selected.evaluate(element => {
    ...
    return { outline: style.outlineStyle, width: parseFloat(style.outlineWidth), border: style.borderBottomStyle, borderWidth: parseFloat(style.borderBottomWidth), decoration: style.textDecorationLine };
  });
...
test('empty and failed local views retain readable recovery at 320px with text spacing', async ({ page }) => {
  ...
  await page.route('**/data/collection.json', route => route.fulfill({ status: 503, body: 'Synthetic readability collection failure' }));
  await page.goto('/?catalogs=off');
  await page.addStyleTag({ content: textSpacingCSS });
