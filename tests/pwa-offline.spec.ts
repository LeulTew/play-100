...
test('explicit offline preparation preserves guest data and serves fresh local routes without caching APIs or auth', async ({
  ...
  expect(core?.urls.map((url) => new URL(url).pathname)).toEqual(
  ...
  expect(workerRequests.every((request) => request.method === 'GET' && new URL(request.url).origin === origin)).toBe(
    true,
  );
  await page.keyboard.press('Escape');
  ...
  try {
    ...
    for (const [tab, label] of [
      ...
      await expect(
        page.getByRole('navigation', { name: 'My games views' }).getByRole('button', { name: new RegExp(`^${label}`) }),
      ).toHaveAttribute('aria-current', 'page');
      await expect(page.locator('.my-games-editor:visible')).toContainText(title);
    }
    ...
    await info.attach('pwa-offline-cache-boundary', {
      ...
      body: JSON.stringify({
        workerRequests,
        caches: cached.map((cache) => ({ name: cache.name, paths: cache.urls.map((url) => new URL(url).pathname) })),
      }),
    });
  } finally {
