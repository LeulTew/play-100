...
export const libraryRecords = [...collection.games.map(recordFromGame), ...catalog.items.map(item => item.record)].slice(0, 500);
...
export function libraryFixture(total = 500): PersonalLibraryState {
  ...
  const state = {
    ...emptyPersonalLibrary(), revision: 1, motion: 'lite' as const,
    records: Object.fromEntries(records.map(record => [record.id, record])),
    progress: Object.fromEntries(ranked.map((record, index) => [record.id, {
      later: true, played: index !== 2, completed: index === 1,
    }])),
    queueOrder: ranked.map(record => record.id),
    ranking: ranked.map((record, index) => ({
      id: record.id, score: index === 0 ? 8.5 : index === 1 ? 7 : null,
      note: `Synthetic Library paging opinion ${index + 1}.`, manualPosition: index + 1,
    })),
  };
...
export async function installGuestLibrary(page: Page, state = libraryFixture(), url = '/my-games?catalogs=off') {
  ...
  await page.evaluate(({ name, version, store, key, state }) => new Promise<void>((resolve, reject) => {
    if (!['127.0.0.1', 'localhost'].includes(location.hostname)) throw new Error('Synthetic Library fixtures require a loopback origin.');
...
export async function libraryGeometry(page: Page) {
  return page.evaluate(() => ({
    ...
    touchPoints: navigator.maxTouchPoints, documentHeight: document.documentElement.scrollHeight,
    documentWidth: document.documentElement.scrollWidth, domNodes: document.querySelectorAll('*').length,
...
export async function loadedExitSaveModule(page: Page): Promise<string> {
  return page.evaluate(() => {
    const module = performance.getEntriesByType('resource').findLast(entry =>
      new URL(entry.name).pathname === '/src/hooks/useExitSave.ts');
    if (!module) throw new Error('The running app has not loaded the exit-save registry module. Use the Vite development server for this fixture.');
    return module.name;
  });
}
