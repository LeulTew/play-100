...
const additions = catalog.items
  .map((item) => item.record)
  .filter((record) => canonicalCatalogId(record.id) === record.id && record.collectionRank === null);
const modes = [
  ...
  const bounds = await identity.evaluate((element) => {
    ...
    return {
      ...
      hits: [0.15, 0.5, 0.85].map((fraction) =>
        target.contains(document.elementFromPoint(rect.left + rect.width * fraction, rect.bottom - 2)),
      ),
...
for (const width of [320, 393]) {
  for (const total of [100, 117, 500]) {
    for (const mode of modes) {
      test(`Explore clearance ${width}px ${total} games ${mode.name} uses the committed grid, list and table`, async ({
        page,
        isMobile,
      }) => {
        ...
        await page
          .getByRole('searchbox', { name: 'Search games, studios or genres', exact: true })
          .fill('Mass Effect 2');
...
for (const mode of modes) {
  for (const width of [320, 393]) {
    test(`live toast clearance ${width}px ${mode.name} keeps Compare and dismissal hittable`, async ({
      page,
      isMobile,
    }) => {
      ...
      const live = await page.locator('.compare-tray-action').evaluate((element) => {
        ...
        return {
          ...
          announcement: notice.getAttribute('aria-live'),
...
test('an empty transient native drag does not reflow collection controls or reserve page space', async ({
  page,
  isMobile,
}) => {
  ...
  for (const card of await cards.all()) {
    ...
    await settleLayout();
    await card.locator('img').evaluateAll((images) =>
      Promise.all(
        images.map((image) => {
    ...
    page.evaluate(() => ({
      controls: [
        ...document.querySelectorAll(
          '.collection-title-line, .collection-search, .collection-utilities, .collection-extra-actions',
        ),
      ].map((element) => {
        const rect = element.getBoundingClientRect();
...
test('operational status uses the utility scale and the header fits desktop and 200 percent reflow', async ({
  page,
  isMobile,
}) => {
  ...
  expect(
    await page.locator('.artifact-status').evaluate((element) => parseFloat(getComputedStyle(element).fontSize)),
