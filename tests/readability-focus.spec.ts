  ...
  for (const width of [320, 393]) {
    test(`text spacing keeps live toast, dock and native focus clear at ${width}px ${mode}`, async ({ page }) => {
      ...
      const normal = await page.locator('.mobile-nav').evaluate(element => ({
        ...
        gap: element.getBoundingClientRect().top - document.querySelector('.compare-tray-dock')!.getBoundingClientRect().bottom,
      }));
      ...
      const live = await page.evaluate(() => {
        ...
        return {
          ...
          compareHit: hit(action), dismissHit: hit(dismiss),
      ...
      expect(await page.evaluate(() => parseFloat(getComputedStyle(document.documentElement).scrollPaddingBottom)))
        .toBeGreaterThanOrEqual(await page.locator('.mobile-nav').evaluate(element => element.getBoundingClientRect().height));
    });
  }
}
