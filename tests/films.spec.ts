...
function filmName(title: string, { narrow, posterFailed = false }: { narrow: boolean; posterFailed?: boolean }) {
  ...
  return [
    posterFailed && 'Poster unavailable',
    film.title,
    !narrow && film.description,
    `${filmDuration(film.durationSeconds)} · Watch film`,
  ]
    .filter(Boolean)
    .join(' ');
}

test('the public film fragment lands at the section after delayed collection loading without requesting movies', async ({
  page,
}) => {
...
test('optional films stay unloaded until Watch, play and seek natively, switch without overlap and restore focus', async ({
  page,
  isMobile,
}, info) => {
  ...
  for (const [index, film] of collectionFilms.entries()) {
    ...
    if (index)
      expect(
        await page.evaluate(() => ({
          paused: window.previousFilm?.paused,
          src: window.previousFilm?.getAttribute('src'),
        })),
  ...
  ).toEqual({ paused: true, src: null, overflow: '' });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  expect(errors).toEqual([]);
});

test('poster and media failures keep Watch, retry, text alternatives and downloads usable', async ({
  page,
  isMobile,
}) => {
  ...
  await page
    .getByRole('button', { name: filmName('The 100', { narrow: isMobile, posterFailed: true }), exact: true })
    .click();
  ...
  await expect
    .poll(() => page.locator('video').evaluate((player: HTMLVideoElement) => player.readyState))
