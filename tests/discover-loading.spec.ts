...
for (const view of ['grid', 'list']) {
  test(`Discover ${view} loads without a false count, blank anatomy or shifted first artwork`, async ({ page }) => {
    ...
    try {
      ...
      await expect(
        page.locator('.discovery-skeleton button, .discovery-skeleton a, .discovery-skeleton input'),
      ).toHaveCount(0);
      ...
      expect(
        await page
          .locator('.discovery-skeleton')
          .evaluate((element) => element.getAnimations({ subtree: true }).length),
      ).toBe(0);
      await page.evaluate(() => {
        if (!PerformanceObserver.supportedEntryTypes.includes('layout-shift'))
          throw new Error('This check needs native layout-shift observations.');
        let total = 0;
      ...
      for (const dimension of ['x', 'y', 'width', 'height'] as const) {
        expect(Math.abs(after[dimension] - before[dimension]), `${view} artwork ${dimension}`).toBeLessThanOrEqual(0.5);
      }
      await page.evaluate(
...
test('a failed original catalog is an explicit recovery state, not an endless skeleton or zero count', async ({
  page,
}) => {
  await page.route('**/data/collection.json', (route) =>
    route.fulfill({ status: 503, body: 'Controlled collection load failure.' }),
  );
...
for (const destination of ['/discover?q=Kingdomcome&catalogs=off', '/?q=Kingdomcome']) {
  for (const diagnostic of [
    ...
    test(`catalog validation on ${destination} keeps ${diagnostic} in diagnostics, not visible copy`, async ({
      page,
    }) => {
      ...
      let fail = true;
      await page.route('**/data/discovery/catalog.v1.json', (route) =>
        fail ? route.fulfill({ json: broken }) : route.fulfill({ path: catalogFile, contentType: 'application/json' }),
      );
      ...
      await expect(notice).toContainText(
        destination.startsWith('/discover') ? 'The 100 remains searchable.' : 'Saved games remain available.',
      );
