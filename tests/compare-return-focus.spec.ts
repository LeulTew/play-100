...
const comparePurpose = (page: Page) =>
  signIn(page).getByRole('region', { name: "Compare friends' rankings", exact: true });
...
test.beforeEach(async ({ page, baseURL }) => {
  if (!baseURL || !['127.0.0.1', 'localhost'].includes(new URL(baseURL).hostname))
    throw new Error('Compare focus fixtures require the owned local app.');
...
async function returnSnapshot(page: Page) {
  return compare(page).evaluate((node) => {
    ...
    return {
      ...
      compareHit: Boolean(hit && node.contains(hit)),
      hitTag: hit?.tagName,
      hitClass: hit?.getAttribute('class'),
      toastActive: Boolean(document.querySelector('.toast.toast-visible')),
...
test('ready Compare keyboard device exit restores the remounted action while its toast is active, then ordinary Account returns normally', async ({
  page,
}, info) => {
  await fullDock(page);
  ...
  await info.attach('compare-keyboard-return', {
    contentType: 'application/json',
    body: JSON.stringify({ elapsedMs: Date.now() - began, ...actual }),
  });
...
async function holdAccountModule(page: Page) {
  let release: () => void = () => {
    throw new Error('Module hold was not initialized.');
  };
  let requested: () => void = () => {
    throw new Error('Module signal was not initialized.');
  };
  ...
  await page.route(
    /\/(?:assets\/OnlineController-[^/]+\.js|src\/cloud\/OnlineController\.tsx)(?:\?|$)/,
    async (route) => {
      requested();
      await gate;
...
test('native Back invalidates a Compare return ticket even when that action exists on the new page', async ({
  page,
  isMobile,
}) => {
  await readyAccount(page);
...
test('a failed rating edit keeps its exact field and never captures or opens a Compare sign-in origin', async ({
  page,
}) => {
  await readyAccount(page);
  await page
