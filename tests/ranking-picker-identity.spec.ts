...
const seedCatalog = parseDiscoveryCatalog(
...
const manual: LibraryRecord = {
  ...provider,
  id: 'manual:same-title-picker',
  source: 'manual',
  sourceId: 'same-title-picker',
  sourceUrl: null,
};
...
test.beforeEach(async ({ context, page, baseURL }) => {
  ...
  await context.route('**/*', (route) => {
    ...
    if (url.pathname === '/api/catalog')
      return route.fulfill({ status: 503, json: { error: 'Provider lookup is outside this guest identity fixture.' } });
...
function legacyState(ranked: boolean, record = provider): PersonalLibraryState {
  let state = applyPersonalAction(emptyPersonalLibrary(), {
    type: ranked ? 'add-ranking' : 'add-records',
    records: [record],
  });
  ...
  if (ranked)
    state = applyPersonalAction(state, {
      ...
      id: record.id,
      score: 8.5,
      note: 'Keep the historical provider opinion.',
    });
  return state;
}
...
test('fresh private picker adds one canonical record without inferring play history', async ({ page }) => {
  ...
  await picker(page)
    .getByRole('button', { name: `Add ${canonical.title} to ranking`, exact: true })
    .click();
  await expect(
    picker(page).getByRole('button', { name: `Already in ranking: ${canonical.title}`, exact: true }),
  ).toBeDisabled();
...
test('an already-ranked unique provider copy has one honest disabled target and no implicit canonical copy', async ({
  page,
}, info) => {
...
test('ranking an unranked owned alias reuses only its existing metadata and preserves Queue and Played', async ({
  page,
}) => {
  ...
  await expect(choices(page)).toHaveAttribute('data-picker-id', provider.id);
  await picker(page)
    .getByRole('button', { name: `Add ${provider.title} to ranking`, exact: true })
    .click();
  await expect(
    picker(page).getByRole('button', { name: `Already in ranking: ${provider.title}`, exact: true }),
  ).toBeDisabled();
...
test('both owned opinions and an unknown same-title manual game stay independently accessible', async ({ page }) => {
  ...
  before = applyPersonalAction(before, {
    type: 'edit-ranking',
    id: canonical.id,
    score: 9.3,
    note: 'Separate canonical opinion.',
  });
  ...
  await picker(page)
    .locator(`[data-picker-id="${manual.id}"]`)
    .getByRole('button', { name: `Add ${manual.title} to ranking`, exact: true })
    .click();
  await expect.poll(async () => (await readLibrary(page)).ranking.length).toBe(3);
  const after = await readLibrary(page);
  for (const id of [canonical.id, provider.id])
    expect(after.ranking.find((entry) => entry.id === id)).toEqual(before.ranking.find((entry) => entry.id === id));
  expect(after.records).toEqual(before.records);
  expect(after.progress).toEqual(before.progress);
  expect(after.queueOrder).toEqual(before.queueOrder);
  await page.getByRole('button', { name: 'Close game picker', exact: true }).click();
  await expect(
    page.getByRole('list', { name: 'Your ranked games', exact: true }).locator('[data-record-id]'),
  ).toHaveCount(3);
});

test('saved and canonical titles both find the one existing add target without overwriting its metadata', async ({
  page,
}) => {
  ...
  await picker(page)
    .getByRole('button', { name: `Add ${saved.title} to ranking`, exact: true })
    .click();
...
test('late canonical props retain the already-owned candidate and its searchable saved title', async ({ page }) => {
  ...
  await page.evaluate(
    async ({ saved, canonical }) => {
      ...
      const domPath = resources.findLast(
      ...
      function Fixture() {
        ...
        return React.createElement(
          ...
          React.createElement(Panel, {
            records: available,
            ownedRecords: { [saved.id]: saved },
            existingIds: new Set([saved.id]),
            busy: false,
            onDiscover: () => {},
            onAdd: async () => {
              document.documentElement.dataset.latePickerUnexpectedAdd = 'yes';
              return true;
            },
          }),
        );
