...
test.beforeEach(async ({ page, baseURL }) => {
  if (!baseURL || !['localhost', '127.0.0.1'].includes(new URL(baseURL).hostname))
    throw new Error('Feature loading proof requires the owned local preview.');
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.route('**/api/**', (route) =>
    route.fulfill({ status: 503, json: { error: 'Synthetic unavailable provider.' } }),
  );
});

test('cold Settings opens and closes natively without loading update execution or preparing offline data', async ({
  page,
}) => {
