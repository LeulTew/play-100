...
declare global {
  interface Window {
    guestStartupReads: number;
  }
}

test('dense guest data loads independently of canonical metadata and is not reread when it arrives', async ({
  page,
}) => {
  ...
  for (let index = 0; index < 500; index += 1) {
    ...
    state.records[id] = {
      id,
      title: `Startup ${String(index).padStart(3, '0')}`,
      year: null,
      studio: null,
      genre: null,
      source: 'manual',
      sourceId: `startup-${index}`,
      sourceUrl: null,
      collectionRank: null,
    };
    if (index < 3) {
  ...
  try {
    ...
    expect(await rows.evaluateAll((nodes) => nodes.map((node) => node.getAttribute('data-record-id')))).toEqual(
      state.queueOrder,
    );
