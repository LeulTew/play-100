...
const member: Member = {
  uid: identity.uid,
  displayName: 'Different private member',
  avatar,
  createdAt: 1,
  updatedAt: 1,
  consentVersion: 1,
...
const profile: PublicProfile = {
  uid: identity.uid,
  displayName: 'Published persona',
  handle: 'published_persona',
  title: 'Stored public title',
  avatar,
  count: 2,
  preview: games.map((game) => game.title),
  generation: 'original',
  epoch: 3,
  published: true,
  listed: true,
...
const initial = applyPersonalAction(
  applyPersonalAction(emptyPersonalLibrary(), { type: 'rate-game', record: records[0]!, score: 8.5 }),
  { type: 'rate-game', record: records[1]!, score: 7 },
);
...
interface Probe {
  ...
  calls: {
    controls: string[];
    saves: number;
    publications: { uid: string; input: PublishInput; control: PublicControl }[];
  };
}
declare global {
  interface Window {
    publicationDraftProbe: Probe;
  }
}
...
async function mount(page: Page, options: { existing?: boolean; holdControl?: boolean } = {}) {
  ...
  await page.evaluate(
    async ({ identity, member, profile, avatar, state, games, options }) => {
      ...
      let release: (control: PublicControl) => void = () => {
        throw new Error('The held control was not initialized.');
      };
      const pending = new Promise<PublicControl>((resolve) => {
        release = resolve;
      });
      const social: PanelProps['social'] = {
        control: async (uid) => {
          calls.controls.push(uid);
          return options.holdControl && uid === identity.uid ? pending : { epoch: 3, hidden: false, deleted: false };
        },
        saveMember: async () => {
          calls.saves += 1;
        },
        publish: async (uid, input, control) => {
          calls.publications.push({ uid, input, control });
          return { ...profile, uid, ...input, generation: 'mock-confirmed', updatedAt: 2 };
        },
        unpublish: async () => {
          throw new Error('This controlled fixture must not unpublish.');
        },
      };
      let props: PanelProps = {
        social,
        identity,
        member: options.existing ? member : null,
        existing: options.existing ? profile : null,
        avatar,
        state,
        games,
        isCreator: false,
      ...
      window.publicationDraftProbe = {
        ...
        releaseControl: async () => {
          release({ epoch: 99, hidden: true, deleted: false });
          await pending;
...
test('a legacy reserved handle explains its rejection before preview and permits a compliant rename', async ({
  page,
}) => {
...
test('late owned publication hydrates untouched public fields and listing, not the private member identity', async ({
  page,
}) => {
  await mount(page);
...
test('late props update untouched inputs but never alter the frozen preview or its explicitly submitted mock payload', async ({
  page,
}) => {
  await mount(page, { existing: true });
  await panel(page).getByRole('button', { name: 'Preview public snapshot', exact: true }).click();
  ...
  const changed = applyPersonalAction(initial, {
    type: 'edit-ranking',
    id: records[0]!.id,
    score: 9.25,
    note: 'Still private',
  ...
  await patch(page, {
    existing: { ...profile, displayName: 'Later public identity', title: 'Later title', listed: false, updatedAt: 2 },
    state: changed,
  });
  ...
  expect(published.input).toEqual({
    ...
    listed: true,
    avatar,
    entries: projectPublicRanking(initial, new Set(initial.ranking.map((entry) => entry.id)), games),
    control: { epoch: 3, hidden: false, deleted: false },
    creator: false,
  });
  expect(published.control).toEqual({ epoch: 3, hidden: false, deleted: false });
});

test('an identity boundary drops prior edits and rejects foreign props and the previous pending control read', async ({
  page,
}) => {
