...
async function expectCssMotionOff(page: Page) {
  const moving = await page
    .locator(
      'button, a, .game-cover img, .game-copy h3 > svg, .dialog-inner, .toast, .magnet > div, .personal-row, .avatar-picker__candidate, .avatar-picker__palette',
    )
    .evaluateAll((nodes) =>
      nodes
        .filter((node) => {
