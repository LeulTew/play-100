  ...
  await page.route(
    ...
    async (route) => {
      ...
      const headers = Object.fromEntries(
        Object.entries(response.headers()).filter(([name]) => !['content-encoding', 'content-length'].includes(name)),
      );
...
function differences(before: Box[], after: Box[], tolerance: number, started = false): string[] {
  ...
  return keys.flatMap((key) => {
    ...
    const changes = (['x', 'y', 'width', 'height'] as const)
      .filter((side) => Math.abs(from[side] - to[side]) > tolerance)
  ...
  test(`first paint equals React's first commit: ${scenario.name}`, async ({ page, isMobile }) => {
    ...
    try {
      ...
      expect(
        ...
        'the boot script accepts the shell (it runs only under its CSP hash and with the metric-matched local fonts)',
      ).toEqual(['landing', scenario.art(isMobile)]);
      await expect(page.locator('head > style')).toHaveCount(1);
      await expect(page.locator('head > script:not([src])')).toHaveCount(1);
      test.info().annotations.push({
        type: 'shell variant',
        description: await page.evaluate(() =>
          document.querySelector('.first-paint-shell .site-header-online') ? 'online' : 'offline',
        ),
      });
      await frames(page);
      ...
      await page.waitForFunction(
        (href) =>
          Array.from(document.styleSheets).some(
            (sheet) => sheet.href !== null && new URL(sheet.href).pathname === href,
          ),
        entryStylesheet,
      );
      await frames(page);
      ...
      expect(
        differences(critical, shell, 0.01),
        'the inline critical CSS renders the shell exactly like the full stylesheet',
      ).toEqual([]);
      ...
      expect(
        requests.first,
        "nothing the app needs is requested before the shell's first contentful paint",
      ).toBeGreaterThanOrEqual(requests.paint);
      ...
      if (!isMobile && (scenario.hint === null || scenario.name === 'saved Full')) {
        ...
        await expect(page.locator('.collection-artifact')).toHaveAttribute('data-scene-status', 'ready', {
          timeout: 0,
        });
        await expect(page.locator('.artifact-control')).toBeVisible();
        await frames(page);
        expect(
          await page.locator('.artifact-footer').evaluate((node) => node.getBoundingClientRect().height),
          'automatic scene startup and its fan control do not grow the initial footer',
        ).toBe(initialFooter.height);
      }
...
function startupStylesheets(html: string): string[] {
  const paths = [...html.replace(/<noscript>[\s\S]*?<\/noscript>/g, '').matchAll(/<link\b[^>]*>/g)]
    .map((match) => match[0])
 ...
 */
async function expectStartupStylesheetsFirst(
  page: Page,
  startup: readonly string[],
  when: string,
  chunks: 'some' | 'any' = 'some',
) {
  const { links, inlineStyleFirst } = await page.evaluate(() => {
    ...
    return {
      ...
      inlineStyleFirst:
        !style || !first || Boolean(style.compareDocumentPosition(first) & Node.DOCUMENT_POSITION_FOLLOWING),
    };
  });
  expect(
    links.filter((link) => !link.head).map((link) => link.path),
    `${when}: stylesheets outside <head>`,
  ).toEqual([]);
  expect(inlineStyleFirst, `${when}: the inline critical style precedes every stylesheet`).toBe(true);
  expect(
    links.slice(0, startup.length).map((link) => link.path),
    `${when}: the entry stylesheet precedes every lazy chunk stylesheet`,
  ).toEqual(startup);
  const lazy = links.slice(startup.length).map((link) => link.path);
  expect(
    lazy.filter((path) => startup.includes(path)),
    `${when}: a startup stylesheet linked again`,
  ).toEqual([]);
...
for (const [path, heading] of [
  ['/my-games', '#my-games-title'],
  ['/discover?catalogs=off', '#discover-title'],
] as const) {
...
test('lazy chunk stylesheets follow the entry stylesheet after navigating from the landing shell', async ({
  page,
  isMobile,
}) => {
  const errors: string[] = [];
...
test("the account page's stylesheets follow the entry stylesheet on a first load", async ({ page }) => {
  ...
  test.skip(
    !html.includes('site-header-online'),
    'Only builds with online tools load the account page from a lazy chunk.',
  );
