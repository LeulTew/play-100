...
export async function mountMotionFixture(page: Page, workspace = false) {
  ...
  await page.evaluate(
    async ({ fixture, workspace }) => {
      ...
      Element.prototype.animate = function (...args: Parameters<Element['animate']>) {
        ...
        if (this.closest('#route-motion-fixture')) {
          ...
          const entry: ArrivalEvent = {
            target: this.matches('h1')
              ? 'heading'
              : this.matches('.my-games-tab-marker')
                ? 'tab'
                : this.matches('.local-pager > p')
                  ? 'range'
                  : 'other',
            ...
            transforms:
              effect instanceof KeyframeEffect
                ? effect
                    .getKeyframes()
                    .map((frame) => (typeof frame.transform === 'string' ? frame.transform : undefined))
                : [],
            ended: false,
          };
      ...
      const patch = (value: Partial<MotionHarnessControls>) => {
        ...
        return React.createElement(
          ...
          workspace
            ? React.createElement(MyGames, {
                ...
                onFilters: (next) =>
                  patch({
                    filters: { ...controls.filters, ...next },
                    route: { ...controls.route, navigationEpoch: controls.route.navigationEpoch + 1 },
                  }),
              })
            : React.createElement(
                'section',
                { className: controls.loading ? 'page-loading' : 'app-page' },
                React.createElement(
                  'h1',
                  { 'data-page-heading': '', tabIndex: -1 },
                  controls.loading ? 'Loading...' : controls.route.family,
                ),
                React.createElement(
                  'label',
                  null,
                  'Uncontrolled draft',
                  React.createElement('input', { 'aria-label': 'Uncontrolled draft', defaultValue: '' }),
                ),
              ),
        );
      }
      function Fixture() {
        ...
        return React.createElement(MotionProvider, {
          ...
          location: {
            ...
            navigationGeneration: controls.route.navigationEpoch,
            overlayKey: null,
          },
      ...
      window.routeArrivalHarness = {
        ...
        requestRoute: async (family) => {
          ...
          if (
            ...
            controls.route.scopeEpoch !== before.scopeEpoch ||
            controls.route.navigationEpoch !== before.navigationEpoch
          )
            return false;
  ...
  if (workspace)
    await expect(page.locator('#route-motion-fixture ul.personal-records > .personal-row-static')).toHaveCount(25);
}

export async function arrivalEvents(page: Page, target?: ArrivalEvent['target']) {
  return page.evaluate(
    (target) => window.routeArrivalLog.events.filter((event) => !target || event.target === target),
    target,
  );
}
