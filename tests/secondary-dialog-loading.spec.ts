...
test('an 800ms Settings chunk retains the Menu and its focus, then opens the real native dialog', async ({ page }) => {
  ...
  const notice = await menu.evaluateHandle((dialog) => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (
        !status?.textContent?.includes('Opening Settings…') ||
        !dialog.matches('[open]') ||
        status.closest('[inert], [hidden]') ||
      ...
      observation.focused =
        dialog.contains(document.activeElement) && document.activeElement?.textContent?.trim() === 'Settings & backups';
  ...
  try {
    ...
    await expect
      .poll(() => notice.evaluate((probe) => probe.observation))
      .toEqual({
        visible: true,
        focused: true,
        pageToast: false,
...
test('toast recovery can be dismissed by touch and a later explicit request resurfaces the terminal failure', async ({
  page,
}) => {
  const asset = await dialogAsset('src/components/AboutDialog.tsx');
  let attempts = 0;
